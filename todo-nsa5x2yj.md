...
- [ ] Add and update localization, accessibility, persistence, cache, and routing regressions; run focused tests, the complete suite, TypeScript, production audit/build, and desktop/mobile multi-language verification; then checkpoint, auto-publish, and synchronize the exact validated tree through a history-preserving pull request into protected GitHub `main`.
...
- [ ] Validate the first-visit language select value with `isSupportedLanguage` before changing locale, add a focused regression for the guard, rerun release gates, publish a replacement checkpoint, update pull request 30, and resolve both review conversations without bypassing requirements.
