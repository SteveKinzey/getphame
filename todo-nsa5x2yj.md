...
- [x] Prove the current managed tree, deployed checkpoint, GitHub protected `main`, and latest release tree are synchronized, or document and remediate any exact-tree drift.
- [x] Audit the active session ledger and every other project `todo*.md` file read-only for unchecked, interrupted, duplicated, or stale work; do not edit another session’s tracker without explicit takeover evidence.
- [x] Audit all open pull requests and remote branches, classify every non-main tip by ancestry and unique work, and identify interrupted work without deleting branches or bypassing protection.
- [x] Build an evidence matrix for every suspected unfinished task, classifying it as completed, stale bookkeeping, preserved unique work, blocked, or confirmed gap with explicit acceptance evidence.
- [x] Complete every confirmed gap that belongs to this task, add focused regressions where application behavior changes, and preserve unrelated concurrent work.
- [x] Run the complete release gates and responsive/runtime verification on any corrected application tree; if no application source changes, prove why a no-op validation path is sufficient.
- [x] Save and auto-publish any required corrective or administrative checkpoint, synchronize its exact tree through protected GitHub `main`, and deliver the completion audit with no active-session unchecked tasks.
- [x] Package the missing reusable administrator CSV-preview workflow as an installed skill covering selectable columns, sanitized preview, date-range filenames, clipboard copy, download, and regression contracts.
- [x] Add send-time subject and body editing to the individual Send Request flow with localized UI, recipient-specific variable rendering, compliance safeguards, a truthful final preview, backend validation, and focused tests.
- [x] Mount the Help Assistant across public and authenticated Get Phame pages with anonymous-safe behavior, persisted minimization, the branded question-mark launcher, support escalation, localization, accessibility, and focused regressions.
- [x] Add Mailjet as a complete SMTP provider preset with correct credential fields, host/port/security mapping, inline localized setup guidance, provider detection, validation, and focused regression coverage.
- [x] Rebuild the Get Phame WordPress connector against the consent-first `/api/v1/contacts` Sources contract with current key handling, authorized source ID, affirmative consent, stable idempotency, provenance, preview/approval, paid-access guidance, safe retries, migrations, tests, documentation, and a release package.
- [x] Reconcile stale pull request 33 and the preserved unique branches from evidence without force-push or deleting unique work; close only objectively superseded review bookkeeping.
- [x] Fix the authenticated mobile Help Assistant launcher so it clears the full fixed BottomNav and safe area while public pages retain a low-edge position; add focused regression coverage and verify the rendered 390 px geometry has no overlap.
- [x] Eliminate the quota integration test’s consecutive-run teardown timeout by replacing the disposable database lifecycle with a session-scoped temporary table; verify the test passes repeatedly and the full production build gate completes.
- [x] Package the interrupted-task audit, completion, validation, checkpoint, protected-main synchronization, and exact-tree verification workflow as a reusable skill using the skill-creator process.
- [x] Audit the current Get Phame data surfaces, search patterns, loading states, export utilities, authorization boundaries, localization contracts, and reusable UI components before designing the new features.
- [x] Implement a tenant-scoped natural-language search experience that converts conversational queries into safe structured filters, returns explainable links to matching Get Phame content, and never exposes unauthorized customer data.
- [x] Add accessible loading skeletons and reduced-motion-safe transition feedback to the new search and export flows, reusing established Get Phame visual tokens and interaction patterns.
...
- [x] Synchronize the exact released tree through protected GitHub `main` using a fresh history-preserving release branch and normal pull request, supersede PR #40 without force-push, prove final tree parity, and leave connector PR #2 unmerged until PHP CI passes.
...
- [x] Audit the current application pairing contracts, Connector PR #2 head and protected checks, reusable release skill, production origins, and available real WordPress test path without exposing credentials or customer data.
- [x] Update and validate the reusable `security-remediation-release-loop` skill with repeatable real-pairing smoke tests, cross-repository contract automation, Connector merge gates, and exact-tree release evidence.
- [x] Run a safe real WordPress pairing smoke test that verifies localized generic-404 recovery and the Connector’s distinct per-IP rate-limit warning without creating customer-facing review activity or exposing secrets.
...
- [x] Audit the authoritative Get Phame `main`, PR #47, and manual Quality Gate runs to distinguish concurrency cancellation from test failure and capture the exact workflow refs and conclusions.
- [x] Correct any confirmed Quality Gate concurrency-key defect so unrelated branch and `main` validations do not cancel one another while duplicate runs on the same ref remain deduplicated; the reported group was explicitly `refs/heads/main`, confirming expected same-ref deduplication, so the correct ref-scoped policy was preserved and regression-tested.
- [x] Upgrade GitHub Actions that still target the deprecated Node.js 20 action runtime to supported releases without changing the application quality commands.
- [x] Validate the CI-only remediation, save and auto-publish checkpoint `a2c2e9a9`, synchronize exact tree `e10af3fa0d8dacc11094c6dc2777b172a316743c` through protected Get Phame PR #48, resolve both Copilot conversations, verify the authoritative quality run is green, merge normally as `241495e95c47e29a56c454fead81647ff43a9ea4`, and prove protected `main` tree parity.
- [x] Address PR #49 Copilot feedback by distinguishing the evidence-only closeout checkpoint from PR #48’s implementation checkpoint and scoping the runtime statement to `.github/workflows/quality.yml`, then revalidate the corrected exact tree for protected synchronization.
- [x] Create and validate the reusable `github-actions-quality-release` skill with GitHub Actions diagnosis, Node action-runtime remediation, preventive CI templates, exact-tree checkpoint synchronization, Copilot review resolution, post-review triggering, protected merge, and executable parity verification with tested pass/fail paths.
- [x] Add a dedicated, read-only, ref-deduplicated GitHub Actions workflow that runs pinned actionlint 1.7.12 against every repository workflow using Node 24-native `actions/checkout@v7` and a five-minute timeout.
- [x] Extend the existing Dependabot configuration with one controlled GitHub Actions entry that runs weekly on Monday at 09:00 Pacific, groups updates, limits open pull requests to five, applies CI labels, and requests owner review.
- [x] Add a dynamic protected-`main` Quality Gate workflow status badge to the repository README using the selected `SteveKinzey/getphame` repository.
- [x] Add focused regression coverage for actionlint triggers, permissions, concurrency, version pinning, Dependabot cadence and limits, and the exact README badge contract; all four focused workflow tests pass.
- [x] Run actionlint 1.7.12, skill package validation, four focused workflow tests, strict TypeScript, all 136 Vitest files with 800 passing tests and 6 skipped, production dependency audit with no known vulnerabilities, production build, Drizzle consistency, semantic three-way pairing-contract parity at SHA-256 `52bb7d6ddd84083d36c31324b4763449d03e348a4258cab35792240a83dd73d6`, Prettier, whitespace, and added-line credential scanning; responsive verification is not applicable because no application UI changed.
- [x] Save and auto-publish reconciled checkpoint `b48adf19`, synchronize exact tree `40ba2a5da8325efed4ef67a0e8db37094155e63c` through history-preserving Get Phame PR #50, resolve all Copilot conversations, obtain a clean protected state, merge normally as `dbfebc6717aa1ea11df4e3531988e430f57ed252`, and prove protected `main` tree parity.
- [x] Address PR #50’s valid maintainability feedback by consolidating the duplicated repository-file reader into one shared test helper and rerunning actionlint, four focused tests, strict TypeScript, all 136 Vitest files with 800 passing tests and 6 skipped, audit, build, migration, three-way pairing parity, formatting, whitespace, and credential checks; the stale workflow-presence comment will be resolved with branch evidence during protected synchronization.
- [x] Fix the CI-only actionlint failure caused by ShellCheck SC2034 in `.github/workflows/api-recovery-e2e.yml`, align its bounded-loop contract test, update the reusable skill to reject disabled shellcheck or pyflakes integrations, and pass the complete release gate with both official-image integrations enabled without suppressing the rule globally.
- [x] Reconcile concurrent protected-main commits `ac75198`, `a04254c`, and `09a8000` into the managed project and local PR #50 history without overwriting unrelated work; preserve the ShellCheck-safe recovery loop, normalize the incoming YAML indentation, update the Node 24 contract, retain the 5-minute/20-start pairing limit, and pass the complete integrated release gate.
- [x] Inspect the repository’s current Dependabot configuration, workflows, protection, auto-merge, Actions permissions, CODEOWNERS state, and reusable skill: auto-merge is off; strict protected `main` requires the Quality Gate and resolved conversations; the default workflow token is read-only; no merge queue or CODEOWNERS exists; code-owner review enforcement is off; `@SteveKinzey` is the sole write collaborator; and Node 24-native `dependabot/fetch-metadata` v3.1.0 resolves to immutable commit `25dd0e34f4fe68f24cc83900b1fe3fe149efef98`.
- [x] Extend and validate the reusable `github-actions-quality-release` skill with actor/ecosystem/patch gating, least-privilege Dependabot auto-merge, monthly workflow-drift auditing, CODEOWNERS routing modes, platform prerequisites, failure handling, immutable Node 24 metadata-action guidance, and copy-ready workflow, script, and ownership templates; the package validator, full actionlint integrations, and temporary-repository drift fixture pass.
- [x] Add a least-privilege `pull_request` workflow that validates both Dependabot actor fields, same-repository head, default base, non-draft state, `github_actions` ecosystem, patch-only group, semver-patch type, and no maintainer changes before enabling normal protected auto-merge with immutable Node 24 metadata action v3.1.0.
- [x] Add a monthly first-day 09:17 UTC and manually dispatchable read-only workflow-drift audit with static concurrency, an eight-minute timeout, repository auto-merge verification, deterministic Node 24/action/permission/timeout/Dependabot/CODEOWNERS checks, and full actionlint shellcheck and pyflakes integrations.
- [x] Add `.github/CODEOWNERS` rules assigning workflows, Dependabot configuration, the drift script, and the ownership file itself to `@SteveKinzey` for automatic review routing while intentionally leaving mandatory code-owner approval disabled for hands-off patch auto-merge.
- [x] Add four focused automation contracts plus updated API recovery and Dependabot contracts covering trusted identity, ecosystem and semver gating, least privilege, immutable Node 24 metadata, monthly schedule, manual dispatch, auto-merge drift, CODEOWNERS scope, current action majors, and removal of deprecated pnpm setup; all 14 focused tests pass.
- [x] Run full actionlint with shellcheck and pyflakes, the deterministic drift audit, reusable-skill validation, 14 focused automation tests, strict TypeScript, all 137 Vitest files with 805 passing tests and 6 skipped, production audit with no known vulnerabilities, production build, Drizzle consistency, three-way semantic pairing-contract parity at SHA-256 `0e7868e8bdf44c1fca9fb2cd0dacd550dfd9b480dc03366c8956e34498cc7006`, formatting, whitespace, CODEOWNERS contracts, and corrected added-line credential scanning; responsive verification is not applicable because no application UI changed.
- [x] Save and auto-publish checkpoint `9436f817`, enable repository auto-merge, synchronize exact tree `d88005da376d299e2e8dba300e6874b2baa57cbf` through history-preserving Get Phame PR #52, obtain successful actionlint, Quality Gate, API recovery, and Copilot review results with no unresolved threads, merge normally as `4f35c4787b9f17d489e798d1cd9e440f51d7c4f0`, and prove protected `main` tree parity with auto-merge still enabled.
- [ ] Diagnose and remediate failed live workflow-drift audit run `30479608074`, add regression coverage for the production-only failure mode, rerun complete validation, save a replacement checkpoint, synchronize it through protected `main`, and prove a subsequent manual drift-audit run succeeds.
- [ ] Preserve concurrent managed checkpoint `1112566b` without overwriting its AI email tone, privacy-safe CSV import, Activity Trend export, localization, and regression changes; validate the complete integrated tree and reconcile it safely with the latest protected GitHub `main` before release.
- [x] Remove the two extra end-of-file blank lines discovered in concurrent regression files so the exact integrated candidate passes `git diff --check` without altering behavior.
