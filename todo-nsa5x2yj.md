...
- [ ] Complete every confirmed gap that belongs to this task, add focused regressions where application behavior changes, and preserve unrelated concurrent work.
- [ ] Run the complete release gates and responsive/runtime verification on any corrected application tree; if no application source changes, prove why a no-op validation path is sufficient.
- [ ] Save and auto-publish any required corrective or administrative checkpoint, synchronize its exact tree through protected GitHub `main`, and deliver the completion audit with no active-session unchecked tasks.
...
- [ ] Rebuild the Get Phame WordPress connector against the consent-first `/api/v1/contacts` Sources contract with current key handling, authorized source ID, affirmative consent, stable idempotency, provenance, preview/approval, paid-access guidance, safe retries, migrations, tests, documentation, and a release package.
- [ ] Reconcile stale pull request 33 and the preserved unique branches from evidence without force-push or deleting unique work; close only objectively superseded review bookkeeping.
