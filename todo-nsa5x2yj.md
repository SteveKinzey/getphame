...
- [ ] Run a safe real WordPress pairing smoke test that verifies localized generic-404 recovery and the Connector’s distinct per-IP rate-limit warning without creating customer-facing review activity or exposing secrets.
...
- [ ] Implement the selected hybrid model: run a bounded synthetic production pairing smoke during release and enforce deterministic application/Connector pairing contracts on every repository change without persistent production polling.
