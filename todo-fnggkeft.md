...
- [ ] Add adaptive provider-aware daily and hourly sending limits with reputation ramping, 70% and 85% warnings, a hard safety ceiling, and a clear Bulk Sender upgrade path that cannot bypass anti-abuse protections
- [ ] Require a registered authenticated Get Phame account for API-key issuance, record API Terms and acceptable-use acceptance, default new keys to import-only access, and gate higher-risk sending scope behind business-use details and risk controls
