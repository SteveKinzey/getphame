...
## Free-Tier Rolling Quota System
- [x] DB schema: lifetimeSendCount, quotaWindowStart, windowSendCount columns on business_profiles
- [x] Server: checkQuota, consumeQuota, getQuotaSummary helpers in db.ts
- [x] Server: enforceFreeLimit replaced with rolling quota engine in routers.ts
- [x] Server: consumeQuota wired into single send, resend, and bulk-resend paths
- [x] Server: profile.get now returns quota summary object instead of totalSent
- [x] Client: SendRequest quota banner shows phase-aware remaining count + next window date
- [x] Client: Home.tsx quota stat tile uses quota.remaining / quota.phase
- [x] Landing: Hero CTA updated to "First 10 Sends on Us" + trust line "10 free sends · then 5/30 days free"
- [x] Tests: 13 pure-logic quota unit tests all passing (server/quota.test.ts)

## Stripe Paywall System
- [x] Add paidProcedure middleware to server/_core/trpc.ts
- [x] Gate woo.sync, woo.bulkSend, contacts.bulkSend, reminders.scheduleFollowUp, reminders.sendNow behind paidProcedure
- [x] Build PaywallModal component with USD/THB plan comparison and Stripe checkout
- [x] Wire PaywallModal into SavedContacts.tsx (bulk send + woo sync)
- [x] Wire PaywallModal into Reminders.tsx (sendNow)
- [x] Wire PaywallModal into SendRequest.tsx (scheduleFollowUp)
- [x] Update Upgrade.tsx comparison table to reflect new gated features and rolling quota model
- [x] Add Pro crown badge to Reminders page header for free users
- [x] Add Pro crown badge to WooCommerce sync button and bulk send button for free users

## Backlog Items (July 11 batch)
- [x] Add language picker to landing page Navbar (all 6 locales: EN, TH, CN, FR, ES, IT)
- [x] Update Upgrade page copy to explain rolling quota model (10 free, then 5/30 days)
- [x] Move contacts CSV export to server paidProcedure and gate it on client
- [x] Add visual quota tracker tile to Home dashboard for free users
- [x] Create reusable paywall-pattern skill via /skill-creator

## Backlog batch 3 (Jul 11)
- [x] Landing page language picker (all 6 locales: en, th, zh-CN, fr, es, it)
- [x] Upgrade page rolling quota explanation (10 free → 5/30d window, FAQ updated)
- [x] Contacts CSV export moved to server paidProcedure (exportCSV), Export CSV button in SavedContacts header with paywall gate for free users
- [x] Visual quota tracker bar on Home dashboard (onboarding phase: /10, rolling phase: /5 with reset date)

## Batch 4 — Language persistence, display name, quota tracker, skill
- [x] Persist language selection to localStorage and restore on page load (landing page + app)
- [x] Add display-name override field to Settings so Home greeting uses it with graceful fallback
- [x] Build visual quota countdown tracker on Home with reset timer (days/hours until next batch)
- [x] Create landing-page-language-picker skill via /skill-creator
## Landing Page i18n Wiring (Jul 11)
- [x] Wire useTranslation() into Hero.tsx
- [x] Wire useTranslation() into Features.tsx
- [x] Wire useTranslation() into HowItWorks.tsx
- [x] Wire useTranslation() into Pricing.tsx
- [x] Wire useTranslation() into FAQ.tsx
- [x] Wire useTranslation() into Testimonials.tsx
- [x] Wire useTranslation() into TrustBar.tsx
- [x] Wire useTranslation() into FinalCTA.tsx
- [x] Wire useTranslation() into Footer.tsx
- [x] Wire useTranslation() into Comparison.tsx
- [x] Generate all 5 non-English locale translation files (TH, ZH-CN, FR, ES, IT)
## i18n Translation Completion (Jul 11)
- [x] Generate 102 landing page translation keys for TH locale
- [x] Generate 102 landing page translation keys for ZH-CN locale
- [x] Generate 102 landing page translation keys for FR locale
- [x] Generate 102 landing page translation keys for ES locale
- [x] Generate 102 landing page translation keys for IT locale
- [x] Confirm i18n localStorage persistence already wired (rr-lang + rr-lang-chosen)
- [x] Confirm i18n localStorage persistence already wired (rr-lang + rr-lang-chosen)

## In-App i18n + Mobile Language Selector (Jul 11)
- [x] Add LanguageFlyout to BottomNav as 6th column (mobile language selector)
- [x] Add LanguageFlyout to AppLayout sidebar (tablet/desktop in-app language selector)
- [x] Wire VideoDemo, Stats, ProductShowcase landing components with useTranslation()
- [x] Generate translations for videoDemo, stats, productShowcase keys (TH, ZH-CN, FR, ES, IT)
- [x] Translate OnboardingWizard step4Connector keys (18 keys) into all 5 non-English locales
- [x] Translate settings hints (fromNameHint, replyToDescription, googleWorkspaceTitle) into all 5 locales
- [x] Translate theme toggle, tabs.account, profile.displayName keys into all 5 locales
- [x] Fix LanguageFlyout panel position on mobile — opens upward when near BottomNav
- [x] Bump i18n cache-buster to phame6
- [x] Bump i18n cache-buster to phame6

## Referral Card i18n + Locale-aware Formatting (Jul 11)
- [x] Wire useTranslation into ReferralRewardsCard (12 keys: title, subtitle, joined, converted, freeMonths, whatsapp, copied, copyLink, shareHint, rewardProcessing, copySuccessToast, copyErrorToast)
- [x] Generate referralRewardsCard translations for TH, ZH-CN, FR, ES, IT
- [x] Replace date-fns format() with Intl.DateTimeFormat in Dashboard activity feed (locale-aware timestamps)
- [x] Replace date-fns format() with Intl.DateTimeFormat for weekly chart day labels and trend chart date labels
- [x] Bump i18n cache-buster to phame7
- [x] Bump i18n cache-buster to phame7
## PWA Force-Reload + Version Badge (Jul 11)
- [x] Add system.appVersion publicProcedure to systemRouter returning current build version
- [x] Update sw.js to poll /__manus__/version.json every 60s and post RELOAD_REQUIRED message when version changes
- [x] Add useAppVersionCheck hook that listens for SW RELOAD_REQUIRED message and shows "New version available" toast
- [x] Add version badge to Settings page (admin-visible: shows current version + force update button)
- [x] Bump i18n cache-buster to phame10
- [x] Bump i18n cache-buster to phame10

## i18n Completion + Skill (Jul 11)
- [x] Translate ShareAndEarnCard strings into all 6 locales (title, subtitle, stat labels, copy button, upgrade tip)
- [x] Translate Dashboard bulk-action toast messages into all 6 locales (bulkRestartPartial, bulkRestartSuccess, updateFailed, updatedSuccess)
- [x] Add slide-in animation to "New version available" toast (versionToastIn keyframe + .version-toast CSS class)
- [x] Create pwa-force-reload reusable skill via /skill-creator
- [x] Create pwa-force-reload reusable skill via /skill-creator

## Admin Lock + Lifetime Access + i18n + Changelog (Jul 11)
- [x] Lock admin role to steve@sk-america.com in DB (role='admin')
- [x] Add adminManagement router with promoteToAdmin and grantLifetime procedures (admin-only)
- [x] Auto-grant lifetime tier to admin accounts on profile upsert
- [x] Translate Activity Trend heading and empty state in Dashboard (dashboard.activityTrend.* keys)
- [x] Add What's New changelog modal (shown once per version via localStorage, dismissible)
- [x] Wire activityTrend + whatsNew locale keys into all 6 locale JSON files
- [x] shareAndEarn.* locale keys were already wired in all 6 locales (confirmed)
- [x] shareAndEarn.* locale keys were already wired in all 6 locales (confirmed)

## Install Guide + Share Button + Admin Panel (Jul 11)
- [x] Add install guide as final step in OnboardingGuide wizard (step 7: Install the App)
- [x] Add Share with a friend button (navigator.share) to InstallGuideCard in Settings
- [x] Build Promote User section in AdminDashboard (promoteToAdmin + grantLifetime)
- [x] Build Promote User section in AdminDashboard (promoteToAdmin + grantLifetime)

## PWAInstallPrompt + Install Step i18n (Jul 11)
- [x] Link PWAInstallPrompt bottom sheet to OnboardingGuide install step (step 6)
- [x] Wire t() i18n keys into StepInstallApp with defaultValue fallbacks
- [x] Generate and inject install step translations into all 6 locale JSON files

## Admin Subscription Management + Sticky CTA + WhatsNew (Jul 25)
- [x] Create admin-user-management reusable skill via /skill-creator
- [x] Update WhatsNewModal with real release notes from recent sessions (7 entries)
- [x] Add sticky mobile CTA bar to landing page (fixed bottom, lg:hidden, gold button)
- [x] Add grantSubscription tRPC procedure (flexible: days/months/lifetime, extends from current expiry)
- [x] Add Privileged Users table to AdminDashboard (admin/lifetime/pro/annual filter, role + tier + expiry columns)
- [x] Add Grant Subscription section to AdminDashboard (email + duration type selector + amount input)

## Admin Coupon Codes + Revoke Access + Skill (Jul 25 cont.)
- [x] Add grantDurationType + grantAmount columns to access_codes schema (migration)
- [x] Update redeemAccessCode helper to apply days/months/lifetime duration on redemption
- [x] Add revokeAccess tRPC procedure to adminManagement router (sets tier=free, planExpiresAt=null)
- [x] Add Revoke Access button with confirm dialog to Privileged Users table in AdminDashboard
- [x] Add Coupon Code Generation form to AdminDashboard (code prefix, duration type, amount, max uses, expiry)
...
- [x] ⚠️ STRIPE DONE: Updated lifetime price ID to price_1TxKeNLryXlEZmjyp1pNaHGO ($349 one-time) in server/stripe.ts
