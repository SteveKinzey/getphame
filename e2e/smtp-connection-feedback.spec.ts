...
test("exercises the Settings SMTP feedback controls through test, save, success, and provider-help states", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("smtp-candidate-test-result")).toContainText(
  ...
  await expect(page.getByTestId("smtp-saved-notice")).toContainText(
    "ready for customer outreach"
  );
  await expect(page.getByTestId("bulk-saved-notice")).toContainText(
    "bulk mail server is connected and ready to use"
  );
