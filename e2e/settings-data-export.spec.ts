...
test("Settings export history applies and clears a date filter", async ({ page }) => {
  await page.goto("/__test/settings-data-export");

  await page.getByTestId("settings-profile-data-export-date-start").fill("2026-08-01");
  await page.getByTestId("settings-profile-data-export-date-end").fill("2026-08-31");
...
test("Settings export receipt downloads metadata without profile values", async ({ page }, testInfo) => {
  await page.goto("/__test/settings-data-export");
  ...
  expect(download.suggestedFilename()).toBe("get-phame-export-receipt-2026-08-12.txt");
...
test("Settings deletion preview is aggregate-only and remains confirmation-gated", async ({ page }) => {
  ...
  await expect(preview).toContainText("19 records are currently scheduled for permanent deletion.");
