...
test("keeps login and SMTP identity fields browser-autofill ready with immediate format feedback", async ({ page }) => {
  ...
  await expect(page.getByTestId("magic-link-confirmation")).toContainText("Check your inbox");
  await expect(page.locator("[data-sonner-toast]")).toContainText("Check your inbox");
  ...
  await expect(smtpPassword).toHaveAttribute("autocomplete", "current-password");
  ...
  await expect(page.getByTestId("autofill-smtp-username-feedback")).toContainText("Looks good.");
  ...
  await expect(page.getByTestId("autofill-smtp-password-toggle")).toHaveAttribute("aria-pressed", "true");
  await smtpPassword.fill("app-password");
  await expect(smtpPassword).toHaveAttribute("aria-invalid", "false");
  await expect(page.getByTestId("autofill-smtp-password-feedback")).toContainText("Password entered.");
});
