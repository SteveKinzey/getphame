...
test("shows the actual dashboard loading state and a sanitized API recovery toast", async ({
  page,
}) => {
  await page.goto("/__test/dashboard-feedback?from_webdev=1");

  await expect(page.getByTestId("dashboard-loading")).toHaveAttribute(
    "role",
    "status"
  );
  await expect(page.getByTestId("dashboard-loading")).toHaveAccessibleName(
  ...
  await expect(
    page.getByTestId("dashboard-feedback-preview-retried")
  ).toHaveText("Retry requested");
});
