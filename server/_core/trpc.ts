...
async function ensureAccountIsActive(userId: number) {
  ...
  if (account?.suspendedUntil && account.suspendedUntil > Date.now()) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message:
        "This account is temporarily suspended. Contact Get Phame support if you need assistance.",
    });
  }
}
  ...
  protectedProcedure.use(
    t.middleware(async ({ ctx, input, next }) => {
      ...
      const result = await authorizeRequest(
        ctx,
        permission,
        resolveOptions?.(input, ctx) ?? {}
      );
      if (result.mode === "enforce" && !result.decision.allowed) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: `Access denied (${result.decision.reasonCode})`,
        });
      }
