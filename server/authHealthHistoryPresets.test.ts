...
describe("auth health history preset persistence", () => {
  ...
  it("accepts only a complete duplicate-free permutation of the owner's preset IDs", () => {
    expect(validateCompleteAuthHealthHistoryPresetOrder([3, 1, 2], [1, 2, 3])).toEqual([3, 1, 2]);
    expect(() => validateCompleteAuthHealthHistoryPresetOrder([1, 1, 2], [1, 2, 3])).toThrow("duplicate IDs");
    expect(() => validateCompleteAuthHealthHistoryPresetOrder([1, 2], [1, 2, 3])).toThrow("every saved preset");
    expect(() => validateCompleteAuthHealthHistoryPresetOrder([1, 2, 4], [1, 2, 3])).toThrow("current owner's preset IDs");
  });
