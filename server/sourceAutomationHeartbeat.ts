...
export const SOURCE_AUTOMATION_HEARTBEAT_NAME =
  "get-phame-source-automation-v1";
export const SOURCE_AUTOMATION_CALLBACK_PATH =
...
function desiredUpdate(): HeartbeatJobUpdate {
  return {
    ...
    payload: {},
    description:
      "Process due Get Phame source-bound review-request automation events.",
    enable: true,
  };
}
  ...
  const persisted = persistedTaskUid
    ? jobs.find(
        job =>
          ...
          job.name === SOURCE_AUTOMATION_HEARTBEAT_NAME
      )
    : undefined;
  return (
    persisted ??
    jobs.find(job => job.name === SOURCE_AUTOMATION_HEARTBEAT_NAME) ??
    null
  );
}
