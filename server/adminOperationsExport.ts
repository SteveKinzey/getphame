...
export function serializeAdminOperationsCsv(
  rows: AdminOperationsCsvRow[]
): string {
  const header = ["section", "metric", "period", "value", "unit", "details"];
  const body = rows.map(row =>
    [row.section, row.metric, row.period, row.value, row.unit, row.details]
      .map(escapeAdminOperationsCsvCell)
      .join(",")
  );
...
export async function buildAdminOperationsAnalyticsExport(nowMs = Date.now()) {
  ...
  const [profiles, subscriptions, surveys, reminderPerformance] =
    await Promise.all([
  ...
  for (const profile of profiles) {
    if (profile.tier in tierCounts)
      tierCounts[profile.tier as keyof typeof tierCounts] += 1;
  }
  ...
  const rows: AdminOperationsCsvRow[] = [
    {
      ...
      unit: "iso_8601",
      details: "Get Phame administrator operations analytics export",
    },
    {
      section: "revenue",
      ...
      value: mrrCents / 100,
      unit: "USD",
      details: "Monthly plans plus annual plans at monthly equivalent",
    },
    {
      section: "revenue",
      metric: "annual_recurring_revenue",
      period: "annualized",
      value: (mrrCents * 12) / 100,
      unit: "USD",
      details: "Current MRR multiplied by 12",
    },
    {
      section: "revenue",
      metric: "lifetime_revenue",
      period: "all_time",
      value: (tierCounts.lifetime * USD_PRICE_CENTS.lifetime) / 100,
      unit: "USD",
      details:
        "Lifetime-tier purchases represented by current lifetime accounts",
    },
    {
      section: "revenue",
      ...
      unit: "accounts",
      details: "Subscriptions with active status",
    },
    ...
    })),
    {
      ...
      unit: "percent",
      details:
        "Recent canceled subscriptions divided by current active subscriptions",
    },
    {
      section: "churn",
      metric: "recent_cancellations",
      period: "last_30_days",
      value: recentCancellations,
      unit: "subscriptions",
      details: "Subscriptions marked canceled during the rolling 30-day window",
    },
    {
      section: "churn",
      ...
      unit: "responses",
      details: "No respondent identity or free-text comment is included",
    },
  ];

  const churnReasonCounts = new Map<string, number>();
  for (const survey of surveys) {
    churnReasonCounts.set(
      survey.reason,
      (churnReasonCounts.get(survey.reason) ?? 0) + 1
    );
  }
  for (const [reason, count] of Array.from(churnReasonCounts.entries()).sort(
    ([a], [b]) => a.localeCompare(b)
  )) {
    rows.push({
      section: "churn",
      metric: "survey_reason",
      period: "all_time",
      value: count,
      unit: "responses",
  ...
  for (const reminder of reminderPerformance) {
    ...
    rows.push(
      ...
      },
      {
        ...
        metric: "success_rate",
        period: "all_time_attributed",
        value: reminder.successRate ?? "unavailable",
        unit: "percent",
        details: configuration,
      }
    );
