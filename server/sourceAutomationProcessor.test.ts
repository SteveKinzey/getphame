...
function context(overrides: Record<string, unknown> = {}) {
  return {
    event: event(),
    contact: { id: 9, name: "Customer", email: "customer@example.com", optedOut: 0 },
    source: {
...
describe("source automation scheduled processor", () => {
  ...
  beforeEach(() => {
    deps = createDeps();
    vi.mocked(deps.claimDue).mockResolvedValueOnce(event()).mockResolvedValue(null);
    vi.mocked(deps.getContext).mockResolvedValue(context());
    vi.mocked(deps.complete).mockResolvedValue(undefined);
    vi.mocked(deps.retryOrFail).mockResolvedValue({ failed: false, scheduledAt: 301_000 });
  ...
  it("delivers one claimed event and records the immutable delivery snapshots", async () => {
    await expect(processDueSourceAutomationEvents(1_000, deps)).resolves.toEqual({
      checked: 1,
      sent: 1,
      queued: 0,
      suppressed: 0,
  ...
  it("suppresses an opted-out contact without calling delivery", async () => {
    vi.mocked(deps.getContext).mockResolvedValue(context({
      contact: { id: 9, name: "Customer", email: "customer@example.com", optedOut: 1 },
    }));
  ...
  it("honors a source dry run without sending", async () => {
    vi.mocked(deps.getContext).mockResolvedValue(context({
      source: { automationEnabled: true, automationMode: "review_request", pausedAt: null, dryRun: true },
    }));
    const result = await processDueSourceAutomationEvents(1_000, deps);
    expect(result.dryRun).toBe(1);
    expect(deps.deliver).not.toHaveBeenCalled();
    expect(deps.complete).toHaveBeenCalledWith(expect.objectContaining({ status: "dry_run" }));
  });

  it("retries a transient delivery failure with a stable error code", async () => {
    vi.mocked(deps.deliver).mockRejectedValue(new Error("SMTP not configured. Connect email."));
    const result = await processDueSourceAutomationEvents(1_000, deps);
    expect(result.retried).toBe(1);
    expect(deps.retryOrFail).toHaveBeenCalledWith(expect.objectContaining({
