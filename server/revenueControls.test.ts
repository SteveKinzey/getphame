...
describe("complimentary access", () => {
  it("uses exact UTC days and clamps calendar months at month end", () => {
    ...
    expect(calculateComplimentaryExpiry(startsAt, 1, "day")).toBe(
      startsAt + 24 * 60 * 60 * 1000
    );
    expect(calculateComplimentaryExpiry(startsAt, 1, "month")).toBe(
  ...
  it("clamps leap-day annual grants and enforces bounded durations", () => {
    ...
    expect(calculateComplimentaryExpiry(leapDay, 1, "year")).toBe(
      Date.UTC(2025, 1, 28, 8, 30)
    );
    expect(() => calculateComplimentaryExpiry(leapDay, 0, "day")).toThrow(
      /between 1 and 365/i
    );
    expect(() => calculateComplimentaryExpiry(leapDay, 25, "month")).toThrow(
      /between 1 and 24/i
    );
    expect(() => calculateComplimentaryExpiry(leapDay, 6, "year")).toThrow(
      /between 1 and 5/i
    );
  });
  ...
  it("gives revocation precedence over expiry and treats the exact expiry as ended", () => {
    ...
    expect(
      getComplimentaryAccessStatus({ expiresAt: now + 1, revokedAt: null }, now)
    ).toBe("active");
    expect(
      getComplimentaryAccessStatus({ expiresAt: now, revokedAt: null }, now)
    ).toBe("expired");
    expect(
      getComplimentaryAccessStatus(
        { expiresAt: now + 1000, revokedAt: now - 1 },
        now
      )
    ).toBe("revoked");
  });
  ...
  it("normalizes email identity into a deterministic one-way fingerprint", () => {
    ...
    try {
      const fingerprint = complimentaryEmailFingerprint(
        "  Customer@Example.COM  "
      );

      expect(fingerprint).toBe(
        complimentaryEmailFingerprint("customer@example.com")
      );
...
describe("paid entitlement precedence", () => {
  ...
  it("allows administrators, lifetime plans, active subscriptions, and active complimentary grants", () => {
    expect(hasPaidOrAdminAccess({ role: "admin", tier: "free" }, now)).toBe(
      true
    );
    expect(hasPaidOrAdminAccess({ role: "user", tier: "lifetime" }, now)).toBe(
      true
    );
    expect(
      hasPaidOrAdminAccess(
        { role: "user", tier: "annual", planExpiresAt: now },
        now
      )
    ).toBe(true);
    expect(
  ...
  it("rejects expired subscriptions and expired complimentary grants", () => {
    expect(
      hasPaidOrAdminAccess(
        { role: "user", tier: "pro", planExpiresAt: now - 1 },
        now
      )
    ).toBe(false);
    expect(
...
describe("free quota integration", () => {
  ...
  it("bypasses free limits for active complimentary access without loading quota", async () => {
    ...
    await expect(
      evaluateFreeQuotaAccess(42, "free", dataSource)
    ).resolves.toEqual({
      allowed: true,
      bypassed: true,
      quota: null,
