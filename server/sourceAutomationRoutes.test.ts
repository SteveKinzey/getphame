...
vi.mock("./sourceAutomation", () => ({
  ...
  getSourceAutomationSchedulerByTaskUid: mocks.getSourceAutomationSchedulerByTaskUid,
...
function buildApp() {
  ...
  app.post("/api/scheduled/source-automation", sourceAutomationHeartbeatHandler);
  return app;
}
...
describe("source automation scheduled callback", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.authenticateRequest.mockResolvedValue({ isCron: true, taskUid: "task_source_automation_1" });
  ...
  it("rejects non-cron callers", async () => {
    ...
    const response = await request(buildApp()).post("/api/scheduled/source-automation").send({});
  ...
  it("records a successful bounded run", async () => {
    ...
    expect(response.body).toMatchObject({
      ok: true,
      summary: { checked: 0, sent: 0, queued: 0, suppressed: 0, dryRun: 0, retried: 0, failed: 0 },
    });
  ...
  it("returns a generic 500 and persists only a stable error code", async () => {
    mocks.claimDueSourceAutomationEvent.mockRejectedValue(new Error("private-api-key-should-never-leak"));
    const response = await request(buildApp()).post("/api/scheduled/source-automation").send({});
    expect(response.status).toBe(500);
    expect(response.body.error).toBe("SOURCE_AUTOMATION_PROCESSING_FAILED");
    expect(JSON.stringify(response.body)).not.toContain("private-api-key-should-never-leak");
    expect(mocks.recordSourceAutomationSchedulerRun).toHaveBeenCalledWith({
      taskUid: "task_source_automation_1",
      status: "failed",
