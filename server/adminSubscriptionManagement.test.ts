...
describe("admin subscription management", () => {
  ...
  it("preserves legacy codes and resolves day, month, and lifetime grants", () => {
    ...
    expect(
      resolveAccessCodeGrant(
        { grantDurationValue: null, grantDurationUnit: null },
        startsAt
      )
    ).toEqual({
      tier: "pro",
    ...
    expect(
      resolveAccessCodeGrant(
        { grantDurationValue: 14, grantDurationUnit: "day" },
        startsAt
      )
    ).toEqual({
      tier: "pro",
    ...
    expect(
      resolveAccessCodeGrant(
        { grantDurationValue: 1, grantDurationUnit: "month" },
        startsAt
      )
    ).toEqual({
      tier: "pro",
    ...
    expect(
      resolveAccessCodeGrant(
        { grantDurationValue: null, grantDurationUnit: "lifetime" },
        startsAt
      )
    ).toEqual({
      tier: "lifetime",
    ...
    expect(() =>
      resolveAccessCodeGrant(
        { grantDurationValue: null, grantDurationUnit: "month" },
        startsAt
      )
    ).toThrow("A duration value is required");
  });

  it("wires duration metadata through schema, routers, and both admin surfaces", () => {
    expect(schemaSource).toContain(
      'grantDurationValue: integer("grant_duration_value")'
    );
    expect(schemaSource).toContain(
    ...
    expect(dashboardSource).toContain(
      "trpc.admin.grantSubscription.useMutation"
    );
    expect(dashboardSource).toContain(
      "trpc.admin.revokeSubscription.useMutation"
    );
