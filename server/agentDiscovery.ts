    ...
    components: {
      securitySchemes: {
        developerApiKey: {
          ...
          description:
            "Owner-provisioned gp_live_ key. Not an OAuth access token.",
        },
      },
    },
    paths: {
      ...
      "/api/v1/contacts": {
        post: {
          ...
          description:
            "Requires contacts:write and account-owner authorization.",
          responses: {
            "200": { description: "Contact imported or deduplicated" },
            "401": { description: "Missing, revoked, expired, or invalid key" },
            "403": {
              description: "Insufficient scope or source authorization",
            },
      ...
      "/api/v1/source-events/review-request/validate": {
        post: {
          summary:
            "Validate an eligible source-bound review request without delivery",
      ...
      "/api/v1/source-events/review-request": {
        post: {
          ...
          description:
            "Consequential. Requires owner authorization, source ownership, consent evidence, and configured account safeguards.",
          responses: {
            "200": { description: "Delivered or queued event" },
            "202": { description: "Scheduled or suppressed event" },
          },
        },
      },
    },
...
export function registerAgentDiscoveryRoutes(app: Express): void {
  app.get("/", (req, res, next) =>
    acceptsMarkdown(req) ? sendMarkdown(res, HOMEPAGE_MARKDOWN) : next()
  );
  ...
  app.post("/mcp", (req, res) => {
    ...
    if (body.method === "tools/list")
      return mcpSuccess(res, id, { tools: [publicMcpTool()] });
    if (body.method === "tools/call") {
      ...
      return mcpSuccess(res, id, {
        content: [
          { type: "text", text: JSON.stringify(publicProductInformation()) },
        ],
        isError: false,
      });
    }
  ...
  app.get(
    "/.well-known/agent-skills/get-phame-public-api/SKILL.md",
    (_req, res) => sendMarkdown(res, AGENT_SKILL)
  );
      ...
      skills: [
        {
          ...
          type: "skill-md",
          description:
            "Discover Get Phame public integration resources and safely prepare an owner-authorized developer API integration.",
  ...
  app.get("/api/agent/authorize", (_req, res) =>
    res.status(400).json({
      ...
      error_description:
        "Get Phame does not provide automated OAuth authorization. Use account-owner-provisioned developer API keys as documented in /auth.md.",
    })
  );
  app.post("/api/agent/token", (_req, res) =>
    res.status(400).json({
      ...
      error_description:
        "Get Phame does not mint OAuth access tokens. Use an owner-provisioned developer API key as documented in /auth.md.",
    })
  );
}
