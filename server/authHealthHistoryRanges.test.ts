...
describe("auth health history relative ranges", () => {
  it("returns inclusive local-calendar dates for the last 7 and 30 days", () => {
    ...
    expect(getRelativeAuthHealthHistoryDateInputs(7, now)).toEqual({
      from: "2026-07-15",
      to: "2026-07-21",
    });
    expect(getRelativeAuthHealthHistoryDateInputs(30, now)).toEqual({
      from: "2026-06-22",
      to: "2026-07-21",
    });
  });
  ...
  it("removes one filter at a time and resets pagination", () => {
    ...
    expect(clearAuthHealthHistoryFilter(state, "status")).toEqual({
      ...state,
      status: "all",
      page: 1,
    });
    expect(clearAuthHealthHistoryFilter(state, "triggerSource")).toEqual({
      ...state,
      triggerSource: "all",
      page: 1,
    });
    expect(clearAuthHealthHistoryFilter(state, "from")).toEqual({
      ...state,
      from: "",
      page: 1,
    });
    expect(clearAuthHealthHistoryFilter(state, "to")).toEqual({
      ...state,
      to: "",
      page: 1,
    });
  ...
  it("clears every active filter and returns to page one", () => {
    expect(clearAllAuthHealthHistoryFilters()).toEqual({
      status: "all",
      triggerSource: "all",
      from: "",
      to: "",
  ...
  it("matches Alt+Shift+C only outside editable targets and ignores repeats", () => {
    ...
    expect(
      shouldClearAuthHealthHistoryFiltersFromShortcut({ ...base, repeat: true })
    ).toBe(false);
    expect(
      shouldClearAuthHealthHistoryFiltersFromShortcut({
        ...base,
  ...
  it("matches Ctrl/Cmd+Z only while reorder Undo is available and never steals editing history", () => {
    ...
    expect(
      shouldUndoAuthHealthHistoryPresetReorderFromShortcut(base, true)
    ).toBe(true);
    expect(
      shouldUndoAuthHealthHistoryPresetReorderFromShortcut(
    ...
    expect(
      shouldUndoAuthHealthHistoryPresetReorderFromShortcut(base, false)
    ).toBe(false);
    expect(
      shouldUndoAuthHealthHistoryPresetReorderFromShortcut(
        { ...base, repeat: true },
    ...
    expect(
      shouldUndoAuthHealthHistoryPresetReorderFromShortcut(
        { ...base, target: { tagName: "TEXTAREA" } },
        true
      )
    ).toBe(false);
    expect(
      shouldUndoAuthHealthHistoryPresetReorderFromShortcut(
        { ...base, target: { tagName: "DIV", isContentEditable: true } },
        true
      )
    ).toBe(false);
  });
  ...
  it("keeps stored CSV columns owner-scoped, versioned, allowlisted, and in server order", () => {
    ...
    expect(getAuthHealthHistoryCsvColumnsStorageKey(42)).toBe(
      "getphame:admin-auth-health-history-csv-columns:v1:42"
    );
    expect(parseStoredAuthHealthHistoryCsvColumns(null, available)).toEqual(
      available
    );
    expect(
      parseStoredAuthHealthHistoryCsvColumns("not-json", available)
    ).toEqual(available);
    expect(
      parseStoredAuthHealthHistoryCsvColumns(
        JSON.stringify({ version: 0, columns: ["status"] }),
        available
      )
    ).toEqual(available);
    expect(
      parseStoredAuthHealthHistoryCsvColumns(
        JSON.stringify({ version: 1, columns: ["unknown", "status", "ranAt"] }),
        available
      )
    ).toEqual(["ranAt", "status"]);
    expect(
      parseStoredAuthHealthHistoryCsvColumns(
        JSON.stringify({ version: 1, columns: ["unknown"] }),
        available
      )
    ).toEqual(available);
    expect(
