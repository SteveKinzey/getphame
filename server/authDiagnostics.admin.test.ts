...
describe("admin authentication diagnostics", () => {
  beforeEach(() => {
    ...
    mocks.getAuthDiagnosticSummary.mockResolvedValue({
      total: 0,
      ok: 0,
      fail: 0,
    });
    mocks.listAuthHealthChecks.mockResolvedValue([]);
    mocks.listAuthHealthChecksPage.mockResolvedValue({
      rows: [],
      page: 1,
      pageSize: 20,
      total: 0,
    ...
    mocks.getAuthHealthUptimeSummary.mockResolvedValue({
      runCount: 0,
      uptimePercent: null,
    });
    mocks.listAuthHealthHistoryPresets.mockResolvedValue([]);
    mocks.saveAuthHealthHistoryPreset.mockResolvedValue({
      outcome: "saved",
      id: 5,
      created: true,
    });
    mocks.deleteAuthHealthHistoryPreset.mockResolvedValue(true);
    mocks.duplicateAuthHealthHistoryPreset.mockResolvedValue({
      outcome: "duplicated",
      preset: {
        id: 6,
        name: "Manual failures copy",
        status: "fail",
    ...
    mocks.reorderAuthHealthHistoryPresets.mockResolvedValue({
      outcome: "reordered",
      orderedIds: [6, 5],
    });
  });
  ...
  it("rejects non-admin accounts", async () => {
    ...
    await expect(caller.authDiagnostics.dashboard()).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
  });
  ...
  it("allows admins and converts an exact email filter to a one-way fingerprint", async () => {
    ...
    await caller.authDiagnostics.dashboard({
    ...
    expect(
      JSON.stringify(mocks.listAuthDiagnosticEvents.mock.calls[0]?.[0])
  ...
  it("passes bounded status, source, and page filters to the protected health-history query", async () => {
    ...
    const userCaller = appRouter.createCaller(context("user"));
    await expect(
      userCaller.authDiagnostics.healthHistory()
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("rejects inverted or excessively wide date ranges before querying history", async () => {
    const caller = appRouter.createCaller(context("admin"));
    await expect(
      caller.authDiagnostics.healthHistory({
        fromMs: 999,
        toMs: 100,
        page: 1,
        pageSize: 20,
  ...
  it("exports only administrators' sanitized filtered health history", async () => {
    ...
    expect(mocks.listAuthHealthChecksForExport).toHaveBeenCalledWith({
      status: "fail",
      triggerSource: "manual",
      fromMs: 100,
      toMs: 900,
    ...
    expect(result.csv).toContain(
      "record_id,provider_name,failure_detail_sanitized"
    );
    ...
    const userCaller = appRouter.createCaller(context("user"));
    await expect(
      userCaller.authDiagnostics.exportHealthHistoryCsv({
        columns: ["recordId"],
      })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    ...
    await expect(
      adminCaller.authDiagnostics.exportHealthHistoryCsv({ columns: [] })
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
    await expect(
      adminCaller.authDiagnostics.exportHealthHistoryCsv({
        columns: ["recordId", "privateColumn"],
      } as never)
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  ...
  it("keeps saved health-history presets administrator-only and owner-scoped", async () => {
    ...
    expect(mocks.saveAuthHealthHistoryPreset).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        name: "Manual failures",
        status: "fail",
    ...
    await adminCaller.authDiagnostics.reorderHealthHistoryPresets({
      orderedIds: [6, 5],
    });
    expect(mocks.reorderAuthHealthHistoryPresets).toHaveBeenCalledWith(1, {
      orderedIds: [6, 5],
    });
    ...
    expect(mocks.reorderAuthHealthHistoryPresets).toHaveBeenLastCalledWith(1, {
      orderedIds: [5, 6],
    });

    const userCaller = appRouter.createCaller(context("user"));
    await expect(
      userCaller.authDiagnostics.healthHistoryPresets()
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      userCaller.authDiagnostics.saveHealthHistoryPreset({ name: "Denied" })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      userCaller.authDiagnostics.deleteHealthHistoryPreset({ id: 5 })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      userCaller.authDiagnostics.duplicateHealthHistoryPreset({ id: 5 })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      userCaller.authDiagnostics.reorderHealthHistoryPresets({
        orderedIds: [6, 5],
      })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
