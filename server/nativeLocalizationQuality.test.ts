...
const completenessRoot = path.join(
  projectRoot,
  "client/src/lib/i18nCompleteness"
);
...
const auditedPaths = {
  es: [
    "bulkSendDialog.dailyLimitStatus",
    "bulkSendDialog.sendButton",
    "callToAction.appleSignInButton",
    "callToAction.googleSignInButton",
    "cancelConfirm.cancelReminder",
    "columnLabels.phoneOptional",
    "comparison.sectionSubtitle",
    "comparisonTable.price",
    "comparisonTable.savedContacts",
    "inlineFromNameEdit.senderName",
    "inlineFromNameEdit.successToast",
    "mainForm.pageTitle",
    "profile.addBusinessNameAndLink",
    "smtp.emailNotConnectedDesc",
    "smtp.testConnection",
    "successScreen.requestSent",
    "comparisonTable.csvImport",
    "comparisonTable.woocommerceSync",
    "contactList.importCsv",
    "contactList.importPrompt",
    "header.backToContacts",
    "mainForm.emailTemplateLabel",
    "sendDialog.send",
    "sendHistoryDrawer.sendReminderNow",
    "successScreen.requestsSentMilestone",
  ],
  fr: [
    ...
    "cancelConfirm.cancelReminder",
    "comparisonTable.savedContacts",
    "defaultTemplate.body.0",
    "defaultTemplate.body.1",
    "defaultTemplate.body.2",
    "defaultTemplate.body.4",
    "defaultTemplate.body.5",
    "defaultTemplate.subject",
    "inlineFromNameEdit.senderName",
    "inlineFromNameEdit.successToast",
    "page.freeLimitReachedDesc",
    "paidUser.enjoyFeatures",
    "profile.reviewLink",
    "smtp.emailNotConnectedDesc",
    "bulkSendDialog.yelpWarning",
    "comparison.sectionSubtitle",
    "contactList.importPrompt",
    "dashboard.bulkActions.markReviewed",
    "dashboard.bulkActions.markSent",
    "dashboard.bulkActions.selectedCount",
    "infoBanner.description",
    "onboardingWizard.header.title",
    "onboardingWizard.navigation.nextStep",
    "onboardingWizard.stepContent.step1.title",
    "onboardingWizard.stepContent.step2.description",
    "onboardingWizard.stepContent.step3.description",
    "profile.saveProfile",
    "smtp.description",
    "smtp.disconnectSuccess",
    "wooCommerce.description",
    "bulkSendDialog.complianceChecklistTitle",
    "bulkSendDialog.defaultPlatformWithName",
    "callToAction.appleSignInButton",
    "callToAction.googleSignInButton",
    "columnLabels.phoneOptional",
    "comparisonTable.price",
    "dashboard.emailPerformance.title",
    "paidUser.lifetimeLicense",
    "smtp.passwordPlaceholderGmail",
    "tools.savedContacts",
  ],
  it: [
    "deleteAccount.title",
    ...
    "page.title",
    "trust.dataPrivacy",
    "trust.emailProviders",
    "trust.encryptedPassword",
    "trust.unsubscribe",
    "faq.q10",
    "faq.q11",
    "faq.q5",
...
function readManifest(locale: LocaleCode): AuditManifest {
  return JSON.parse(
    fs.readFileSync(
      path.join(completenessRoot, `${locale}.exceptions.json`),
      "utf8"
    )
  ) as AuditManifest;
}
...
function flattenStrings(value: unknown, prefix = ""): Map<string, string> {
  ...
  if (Array.isArray(value)) {
    value.forEach((child, index) => {
      ...
      for (const [key, text] of flattenStrings(child, childPrefix))
        rows.set(key, text);
    });
  } else if (value && typeof value === "object") {
    for (const [key, child] of Object.entries(value as JsonRecord)) {
      const childPrefix = prefix ? `${prefix}.${key}` : key;
      for (const [childKey, text] of flattenStrings(child, childPrefix))
        rows.set(childKey, text);
    }
...
function protectedTokens(value: unknown): string[] {
  ...
  return [
    ...new Set(
      value.match(
        /{{\s*[^{}]+\s*}}|\*\*|https?:\/\/\S+|\b(?:Get Phame|GET PHAME|SMTP|WooCommerce|Stripe|Google|Yelp|TripAdvisor|Facebook|Bing|Gmail|Outlook|Yahoo|CSV|API)\b/g
      ) ?? []
    ),
  ].sort();
}
...
describe("native-quality localization audit", () => {
  ...
  const fallback = readJson(
    path.join(projectRoot, "client/src/lib/i18nCompleteFallbackResources.json")
  );
  ...
  it("keeps all three-stage-approved native-quality corrections synchronized with runtime fallbacks", () => {
    ...
    for (const [locale, keyPaths] of Object.entries(auditedPaths)) {
      ...
      for (const keyPath of keyPaths) {
        ...
        expect(getPath(localeFallback, keyPath), `${locale}:${keyPath}`).toBe(
          maintainedValue
        );
        expect(
          protectedTokens(maintainedValue),
          `${locale}:${keyPath} protected tokens`
        ).toEqual(protectedTokens(getPath(englishTranslation, keyPath)));
        verified += 1;
      }
    }
  ...
  it("accounts for all 587 independently reviewed English-identical candidates", () => {
    ...
    for (const locale of localeCodes) {
      ...
      expect(manifest.audit).toBe(
        "Exhaustive English-identical customer-copy review"
      );
      ...
      expect(manifest.sourceCandidateCount).toBe(
        manifest.translatedCount + manifest.exceptionCount
      );
      ...
      expect(
        [...translatedSet].filter(entry => exceptionSet.has(entry))
      ).toEqual([]);
      ...
      for (const namespace of namespaces) {
        ...
        for (const [key, maintainedValue] of maintainedRows) {
          ...
          if (
            sourceValue &&
            sourceValue === maintainedValue &&
            hasEnglishWords(sourceValue)
          ) {
      ...
      for (const entry of manifest.translatedEntries) {
        ...
        expect(
          maintainedValue,
          `${locale}:${entry} must be translated`
        ).not.toBe(sourceValue);
        expect(fallbackValue, `${locale}:${entry} fallback parity`).toBe(
          maintainedValue
        );
        expect(
          protectedTokens(maintainedValue),
          `${locale}:${entry} protected tokens`
        ).toEqual(protectedTokens(sourceValue));
      }
      ...
      for (const entry of manifest.exceptions) {
        ...
        expect(
          manifest.categoryDefinitions[category],
          `${locale}:${identity} category`
        ).toBeTruthy();
        expect(
          maintainedValue,
          `${locale}:${identity} documented retention`
        ).toBe(sourceValue);
        expect(
          protectedTokens(maintainedValue),
          `${locale}:${identity} protected tokens`
        ).toEqual(protectedTokens(sourceValue));
      }
    ...
    expect(totalCandidates).toBe(587);
    expect(totalTranslations).toBe(349);
    expect(totalExceptions).toBe(238);
  });
  ...
  it("preserves natural Thai grammar across the partner-priority FAQ block", () => {
    ...
    for (const keyPath of [
      ...
      expect(getPath(thaiFallback, keyPath), `${keyPath} fallback parity`).toBe(
        value
      );
    }
  ...
  it("uses the verified lifetime-access meaning in every supported non-English locale", () => {
    ...
    for (const locale of localeCodes) {
      expect(
        getPath(catalog(locale, "translation"), "adminUsers.life"),
        locale
      ).toBe(expected[locale]);
      expect(
        getPath(fallback[locale], "adminUsers.life"),
        `${locale} fallback`
      ).toBe(expected[locale]);
    }
  });
  ...
  it("uses Taiwan-appropriate deletion terminology without changing zh-CN", () => {
    ...
    expect(getPath(simplified, "deleteAccount.buttonText")).not.toBe(
      "刪除帳戶"
    );
  });
  ...
  it("advances the HTTP and offline locale caches for the exhaustive audit", () => {
    const i18nSource = fs.readFileSync(
      path.join(projectRoot, "client/src/lib/i18n.ts"),
      "utf8"
    );
    ...
    expect(i18nSource).toContain("{{ns}}.json?v=phame63");
