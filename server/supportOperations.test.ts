...
describe("support operations saved views, SLA exports, and breach alerts", () => {
  it("keeps saved queue views owner-controlled while safely exposing explicit team-visible views", () => {
    ...
    const savedViewsBlock = router.slice(
      router.indexOf("savedViews: adminProcedure"),
      router.indexOf("deleteView: adminProcedure")
    );
    const deleteViewBlock = router.slice(
      router.indexOf("deleteView: adminProcedure"),
      router.indexOf("checkSlaBreach: adminProcedure")
    );

    expect(savedViewsBlock).toContain("savedViews: adminProcedure");
    expect(savedViewsBlock).toContain("saveView: adminProcedure");
    expect(savedViewsBlock).toContain(
    ...
    expect(savedViewsBlock).toMatch(
      /value\.assigneeScope\s*===\s*"specific"\s*&&\s*value\.assigneeUserId\s*===\s*null/
    );
    expect(savedViewsBlock).toContain('eq(users.role, "admin")');
    expect(deleteViewBlock).toContain(
      "eq(supportSavedQueueViews.ownerUserId, ctx.user.id)"
    );
  });
  ...
  it("exports only aggregate SLA metrics in a spreadsheet-safe CSV artifact across presets or validated custom ranges", () => {
    const router = readProjectFile("./routers.ts");
    const exportBlock = router.slice(
      router.indexOf("exportMetricsCsv: adminProcedure"),
      router.indexOf("savedViews: adminProcedure")
    );
    const csv = serializeAdminOperationsCsv([
      ...
      },
      {
        ...
        value: "=unavailable",
        unit: "milliseconds",
        details: "No customer fields included",
      },
    ]);
    ...
    expect(router).toContain(
      "Choose a range of ${MAX_SUPPORT_EXPORT_RANGE_DAYS} days or fewer."
    );
    ...
    expect(
      csv.startsWith("\uFEFFsection,metric,period,value,unit,details\r\n")
    ).toBe(true);
  ...
  it("creates policy-routed urgent breach alerts from persisted SLA timestamps and suppresses repeats per target window", () => {
    const router = readProjectFile("./routers.ts");
    const breachBlock = router.slice(
      router.indexOf("checkSlaBreach: adminProcedure"),
      router.indexOf("adminAssignees: adminProcedure")
    );
  ...
  it("protects escalation-policy updates with bounded thresholds, valid administrators, and a required recipient route", () => {
    const router = readProjectFile("./routers.ts");
    const policyBlock = router.slice(
      router.indexOf("updateEscalationPolicy: adminProcedure"),
      router.indexOf("adminAssignees: adminProcedure")
    );
