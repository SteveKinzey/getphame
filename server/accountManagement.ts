...
export function buildMergedProfileValues(source: MergeProfileInput, target: MergeProfileInput) {
  ...
  const strongerTier = tierRank[sourceTier] > tierRank[targetTier] ? sourceTier : targetTier;

  return {
    businessName: target.businessName || source.businessName,
    reviewLink: target.reviewLink || source.reviewLink,
    tier: strongerTier,
    planExpiresAt: strongerTier === "lifetime"
      ? null
      : Math.max(source.planExpiresAt ?? 0, target.planExpiresAt ?? 0) || null,
    monthlyCount: target.monthlyCount + source.monthlyCount,
    onboardingDismissed: Math.max(target.onboardingDismissed, source.onboardingDismissed),
...
export function buildMergedUserValues(source: MergeUserInput, target: MergeUserInput) {
  return {
    role: source.role === "admin" || target.role === "admin" ? "admin" as const : "user" as const,
...
async function deleteOwnedData(tx: any, userId: number, email: string | null) {
  await tx.delete(userIdentityAliases).where(eq(userIdentityAliases.userId, userId));
  ...
  await tx.delete(wooPendingImports).where(eq(wooPendingImports.userId, userId));
  ...
  await tx.delete(referrals).where(or(eq(referrals.referrerUserId, userId), eq(referrals.referredUserId, userId)));
  await tx.delete(accessCodeRedemptions).where(eq(accessCodeRedemptions.userId, userId));
  await tx.delete(notificationPrefs).where(eq(notificationPrefs.userId, userId));
  await tx.delete(bulkSenderCredentials).where(eq(bulkSenderCredentials.userId, userId));
  ...
  if (email) {
    ...
    await tx.delete(magicLinkTokens).where(eq(magicLinkTokens.email, normalized));
    await tx.delete(magicLinks).where(eq(magicLinks.email, normalized));
    await tx.delete(leads).where(eq(leads.email, normalized));
  }
}

export async function deleteAccountAsAdmin(actorId: number, targetUserId: number) {
  if (actorId === targetUserId) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "You cannot delete your own administrator account." });
  }
  ...
  return db.transaction(async (tx) => {
    ...
    if (target.role === "admin") {
      const [row] = await tx.select({ count: sql<number>`count(*)` }).from(users).where(eq(users.role, "admin"));
      if (Number(row?.count ?? 0) <= 1) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "The final administrator account cannot be deleted." });
      }
    }
...
async function assertNoSingletonConflict(tx: any, table: any, sourceId: number, targetId: number, label: string) {
  const rows = await tx.select({ userId: table.userId }).from(table).where(or(eq(table.userId, sourceId), eq(table.userId, targetId)));
...
export async function combineAccountsAsAdmin(actorId: number, sourceUserId: number, targetUserId: number) {
  ...
  if (actorId === sourceUserId) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "Your signed-in administrator account must be the account that remains." });
  }
  const db = await getDb();
  if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });

  return db.transaction(async (tx) => {
    const accounts = await tx.select().from(users).where(or(eq(users.id, sourceUserId), eq(users.id, targetUserId)));
    ...
    const profiles = await tx
      ...
      .where(or(eq(businessProfiles.userId, sourceUserId), eq(businessProfiles.userId, targetUserId)));
    const sourceProfile = profiles.find((profile) => profile.userId === sourceUserId);
    const targetProfile = profiles.find((profile) => profile.userId === targetUserId);
    if (sourceProfile?.stripeCustomerId && targetProfile?.stripeCustomerId && sourceProfile.stripeCustomerId !== targetProfile.stripeCustomerId) {
      throw new TRPCError({
        code: "CONFLICT",
        message: "Both accounts are linked to different Stripe customers. Resolve billing ownership before combining them.",
      });
    }

    if (sourceProfile && targetProfile) {
      await tx.update(businessProfiles).set({
        ...buildMergedProfileValues(sourceProfile, targetProfile),
        updatedAt: new Date(),
      }).where(eq(businessProfiles.userId, targetUserId));
      await tx.delete(businessProfiles).where(eq(businessProfiles.userId, sourceUserId));
    } else if (sourceProfile) {
      await tx.update(businessProfiles).set({ userId: targetUserId, updatedAt: new Date() }).where(eq(businessProfiles.userId, sourceUserId));
    }
    ...
    await tx.update(customerRequests).set({ userId: targetUserId }).where(eq(customerRequests.userId, sourceUserId));
    await tx.update(savedContacts).set({ userId: targetUserId }).where(eq(savedContacts.userId, sourceUserId));
    await tx.update(emailTemplates).set({ userId: targetUserId }).where(eq(emailTemplates.userId, sourceUserId));
    await tx.update(reviewPlatforms).set({ userId: targetUserId }).where(eq(reviewPlatforms.userId, sourceUserId));
    await tx.update(followUpReminders).set({ userId: targetUserId }).where(eq(followUpReminders.userId, sourceUserId));
    await tx.update(emailEvents).set({ userId: targetUserId }).where(eq(emailEvents.userId, sourceUserId));
    await tx.update(wooCustomers).set({ userId: targetUserId }).where(eq(wooCustomers.userId, sourceUserId));
    await tx.update(wooSyncLogs).set({ userId: targetUserId }).where(eq(wooSyncLogs.userId, sourceUserId));
    await tx.update(wooPendingImports).set({ userId: targetUserId }).where(eq(wooPendingImports.userId, sourceUserId));
    await tx.update(apiKeys).set({ userId: targetUserId }).where(eq(apiKeys.userId, sourceUserId));
    await tx.update(apiImportEvents).set({ userId: targetUserId }).where(eq(apiImportEvents.userId, sourceUserId));
    await tx.update(webhookConfigs).set({ userId: targetUserId }).where(eq(webhookConfigs.userId, sourceUserId));
    await tx.update(webhookDeliveryLogs).set({ userId: targetUserId }).where(eq(webhookDeliveryLogs.userId, sourceUserId));
    ...
    const targetPrefs = await tx.select().from(notificationPrefs).where(eq(notificationPrefs.userId, targetUserId)).limit(1);
    if (targetPrefs.length) await tx.delete(notificationPrefs).where(eq(notificationPrefs.userId, sourceUserId));
    else await tx.update(notificationPrefs).set({ userId: targetUserId }).where(eq(notificationPrefs.userId, sourceUserId));

    const targetRedemption = await tx.select().from(accessCodeRedemptions).where(eq(accessCodeRedemptions.userId, targetUserId)).limit(1);
    if (targetRedemption.length) await tx.delete(accessCodeRedemptions).where(eq(accessCodeRedemptions.userId, sourceUserId));
    else await tx.update(accessCodeRedemptions).set({ userId: targetUserId }).where(eq(accessCodeRedemptions.userId, sourceUserId));
    ...
    await tx.delete(referrals).where(and(
      or(eq(referrals.referrerUserId, sourceUserId), eq(referrals.referrerUserId, targetUserId)),
      or(eq(referrals.referredUserId, sourceUserId), eq(referrals.referredUserId, targetUserId)),
    ));
    const targetReferral = await tx.select().from(referrals).where(eq(referrals.referredUserId, targetUserId)).limit(1);
    if (targetReferral.length) await tx.delete(referrals).where(eq(referrals.referredUserId, sourceUserId));
    else await tx.update(referrals).set({ referredUserId: targetUserId }).where(eq(referrals.referredUserId, sourceUserId));
    await tx.update(referrals).set({ referrerUserId: targetUserId }).where(eq(referrals.referrerUserId, sourceUserId));
    ...
    await tx.update(userIdentityAliases).set({ userId: targetUserId }).where(eq(userIdentityAliases.userId, sourceUserId));
    ...
    return {
      ...
      target: { id: target.id, name: target.name ?? source.name, email: target.email ?? source.email },
    };
  });
}
