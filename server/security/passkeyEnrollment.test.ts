...
vi.mock("./webauthnEnvironment", () => ({
  resolveWebauthnEnvironment: mocks.resolveWebauthnEnvironment,
}));
vi.mock("./passkeySessions", () => ({
  hashSecurityValue: (value: string) => `hash:${value}`,
  issuePasskeySession: vi.fn(),
}));
...
describe("guided passkey authentication", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.resolveWebauthnEnvironment.mockReturnValue({
      rpID: "getphame.com",
      rpName: "Get Phame",
      origin: "https://getphame.com",
    });
  ...
  it("returns the same neutral state for an unknown account", async () => {
    ...
    await expect(
      beginPasskeyAuthentication("unknown@example.test", {} as never)
    ).resolves.toEqual({ state: "enrollment_required" });
  ...
  it("returns the same neutral state for an existing account without an active credential", async () => {
    ...
    mocks.getDb.mockResolvedValue(db);
    await expect(
      beginPasskeyAuthentication("Member@Example.Test", {} as never)
    ).resolves.toEqual({ state: "enrollment_required" });
    expect(mocks.generateAuthenticationOptions).not.toHaveBeenCalled();
  });
  ...
  it("starts the unchanged WebAuthn ceremony when an active credential exists", async () => {
    ...
    const db = {
      select: vi
        ...
        .mockReturnValueOnce(
          directResult([
            { credentialId: "credential-1", transportsJson: '["internal"]' },
          ])
        ),
    ...
    expect(insertValues).toHaveBeenCalledWith(
      expect.objectContaining({ userId: 42, purpose: "authentication" })
    );
  });
});
