...
const scopeSet = new Set<SecurityScope>([
  "platform",
  "organization",
  "self",
  "ticket",
...
const isSecurityRole = (value: string): value is SecurityRole =>
  roleSet.has(value);
const isSecurityScope = (value: string): value is SecurityScope =>
  scopeSet.has(value as SecurityScope);
...
async function writeDecisionAudit(input: {
  ...
  try {
    ...
    console.error("[SecurityAudit] Authorization decision persistence failed", {
      correlationId: input.correlationId,
...
export async function authorizeRequest(
  ...
  const organizationId =
    ...
    (actorUserId !== null && policy.scope === "organization"
      ? await resolveDefaultOrganizationId(actorUserId)
      : null);
  const targetUserId =
    options.targetUserId ?? (policy.scope === "self" ? actorUserId : null);
  const evidence: AuthorizationEvidence = {
    authenticated: actorUserId !== null,
    actorUserId,
    targetUserId,
    organizationId,
    assurance: (ctx.securitySession?.assurance ??
      (actorUserId !== null ? "a1" : "a0")) as SecurityAssurance,
    recentAuthenticationAt: ctx.securitySession?.recentAuthenticationAt ?? null,
    roleGrants: [],
    permissionOverrides: [],
    independentApproverUserIds: [],
    customerConsent: options.customerConsent === true,
    ticketGrantActive: options.ticketGrantActive === true,
    now,
  };
  const db = await getDb();
  if (db && actorUserId !== null) {
    const [grants, overrides] = await Promise.all([
      db
        .select({
          ...
          scope: securityRoleGrants.scopeType,
          organizationId: securityRoleGrants.organizationId,
        })
        .from(securityRoleGrants)
        .where(
          and(
            eq(securityRoleGrants.userId, actorUserId),
      ...
      db
        .select({
          permission: securityPermissionOverrides.permission,
          effect: securityPermissionOverrides.effect,
          scope: securityPermissionOverrides.scopeType,
          organizationId: securityPermissionOverrides.organizationId,
        })
        .from(securityPermissionOverrides)
        .where(
          and(
            eq(securityPermissionOverrides.userId, actorUserId),
            eq(securityPermissionOverrides.permission, permission),
            isNull(securityPermissionOverrides.revokedAt),
            or(
              isNull(securityPermissionOverrides.expiresAt),
              gt(securityPermissionOverrides.expiresAt, now)
            )
          )
        ),
    ]);
    evidence.roleGrants = grants.flatMap(grant =>
      isSecurityRole(grant.role) && isSecurityScope(grant.scope)
        ? [
            {
              role: grant.role,
              scope: grant.scope,
    ...
    evidence.permissionOverrides = overrides.flatMap(override =>
      isSecurityScope(override.scope) &&
    ...
    if (options.actionKey && options.actionId) {
      const approvals = await db
        .select({ approverUserId: securityActionApprovals.approverUserId })
        .from(securityActionApprovals)
        .where(
          and(
            eq(securityActionApprovals.actionKey, options.actionKey),
            eq(securityActionApprovals.resourceId, options.actionId),
            eq(securityActionApprovals.status, "approved")
          )
        );
      evidence.independentApproverUserIds = approvals.flatMap(row =>
        row.approverUserId === null ? [] : [row.approverUserId]
      );
    }
  }
  ...
  await writeDecisionAudit({
    ...
    organizationId,
    permission,
    eventType: options.eventType ?? "authorization.decision",
    decision,
    mode,
    correlationId,
