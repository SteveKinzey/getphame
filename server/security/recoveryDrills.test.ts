...
describe("staging recovery runtime", () => {
  it("defaults to disabled and denies non-allowlisted hosts", () => {
    expect(evaluateRecoveryRuntime({ requestHost: stagingHost })).toMatchObject({ enabled: false, reason: "mode_disabled" });
    expect(evaluateRecoveryRuntime({ mode: "staging", requestHost: stagingHost })).toMatchObject({ enabled: false, reason: "host_not_allowed" });
  });

  it("enables only an exact explicitly allowlisted staging host", () => {
    expect(evaluateRecoveryRuntime({ mode: "staging", allowedHosts: stagingHost, requestHost: `${stagingHost}:443` }))
      .toEqual({ enabled: true, reason: "enabled", host: stagingHost });
  });

  it.each(["getphame.app", "www.getphame.app", "getphame.manus.space", "revrocket-j5ynazte.manus.space"])(
    "blocks the published production host %s even if it is allowlisted",
    host => {
      expect(evaluateRecoveryRuntime({ mode: "staging", allowedHosts: host, requestHost: host }))
...
describe("staging recovery assurance and duties", () => {
  ...
  const session = (overrides: Partial<SecuritySessionContext> = {}): SecuritySessionContext => ({
    id: "session-1",
    method: "passkey",
    assurance: "a2",
  ...
  it("requires a recent A2 passkey session for every sensitive recovery mutation", () => {
    ...
    expect(hasRecentPasskeyA2(session({ method: "magic_link" }), now)).toBe(false);
    expect(hasRecentPasskeyA2(session({ assurance: "a1" }), now)).toBe(false);
    expect(hasRecentPasskeyA2(session({ recentAuthenticationAt: now - RECENT_STEP_UP_MS - 1 }), now)).toBe(false);
  ...
  it("rejects the same user as custodian and approver", () => {
    ...
    expect(() => assertSeparatedRecoveryRoles(7, 7)).toThrowError(RecoveryDrillError);
  });

  it("keeps the approver grant narrow and time-bounded", () => {
    expect(RECOVERY_DRILL_APPROVER_PERMISSIONS).toEqual(["recovery.drill.view", "recovery.drill.approve"]);
...
describe("staging recovery lifecycle and evidence", () => {
  it("permits only the explicit lifecycle transitions", () => {
    ...
    expect(() => assertRecoveryTransition("ready", "in_progress")).not.toThrow();
    expect(() => assertRecoveryTransition("in_progress", "paused")).not.toThrow();
    expect(() => assertRecoveryTransition("in_progress", "completed")).not.toThrow();
    expect(() => assertRecoveryTransition("ready", "completed")).toThrowError(RecoveryDrillError);
    expect(() => assertRecoveryTransition("completed", "in_progress")).toThrowError(RecoveryDrillError);
  });

  it("accepts redacted references and rejects likely secrets or personal identifiers", () => {
    expect(assertRedactedEvidenceText("audit:event-7f2b", "Evidence reference")).toBe("audit:event-7f2b");
    expect(() => assertRedactedEvidenceText("owner@example.com", "Evidence reference")).toThrowError(RecoveryDrillError);
    expect(() => assertRedactedEvidenceText("token=abc123", "Evidence reference")).toThrowError(RecoveryDrillError);
