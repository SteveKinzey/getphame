...
function parseTransports(
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter(
          (item): item is AuthenticatorTransport => typeof item === "string"
        )
      : undefined;
  } catch {
    return undefined;
...
async function createCeremony(
  ...
  await db.insert(webauthnCeremonies).values({
    ...
    userId,
    purpose,
    challengeHash: hashSecurityValue(challenge),
    rpId: environment.rpID,
    expectedOrigin: environment.origin,
...
async function consumeCeremony(
  ...
  const now = Date.now();
  const conditions = [
    eq(webauthnCeremonies.id, id),
    eq(webauthnCeremonies.purpose, purpose),
    isNull(webauthnCeremonies.consumedAt),
    gt(webauthnCeremonies.expiresAt, now),
  ];
  if (userId !== undefined)
    conditions.push(eq(webauthnCeremonies.userId, userId));
  ...
  if (!ceremony)
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Passkey request expired or already used",
    });
  ...
  if (affectedRows !== 1)
    throw new TRPCError({
      code: "CONFLICT",
      message: "Passkey request was already used",
    });
  return ceremony;
}
...
export async function beginPasskeyRegistration(userId: number, req: Request) {
  ...
  if (!user?.email)
    throw new TRPCError({
      ...
      message: "Add an email address before creating a passkey",
    });
...
export async function finishPasskeyRegistration(
  ...
  const verification = await verifyRegistrationResponse({
    ...
    expectedChallenge: challenge =>
      hashSecurityValue(challenge) === ceremony.challengeHash,
  ...
  const [existing] = await db
    ...
    .from(webauthnCredentials)
    .where(eq(webauthnCredentials.credentialIdHash, credentialIdHash))
    .limit(1);
  if (existing)
    throw new TRPCError({
      code: "CONFLICT",
  ...
  await db.insert(webauthnCredentials).values({
    ...
    aaguid: null,
    displayName: displayName.trim().slice(0, 80) || "Passkey",
    status: "active",
    createdAt: Date.now(),
    lastUsedAt: null,
    revokedAt: null,
...
export async function beginPasskeyAuthentication(email: string, req: Request) {
  ...
  if (!user || credentials.length === 0)
    return { state: "enrollment_required" as const };
  const environment = resolveWebauthnEnvironment(req);
  ...
  return {
    state: "authentication_ready" as const,
  ...
  const [credential] = await db
    ...
    .where(
      and(
        ...
        eq(
          webauthnCredentials.credentialIdHash,
          hashSecurityValue(response.id)
        ),
        eq(webauthnCredentials.status, "active")
      )
    )
...
export async function renamePasskey(
  ...
  await db
    .update(webauthnCredentials)
    .set({ displayName: displayName.trim().slice(0, 80) })
    .where(
      and(
...
export async function revokePasskey(userId: number, credentialId: number) {
  ...
  await db
    .update(webauthnCredentials)
    .set({ status: "revoked", revokedAt: Date.now(), revokedByUserId: userId })
    .where(
      and(
        eq(webauthnCredentials.id, credentialId),
        eq(webauthnCredentials.userId, userId),
