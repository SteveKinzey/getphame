...
const baseEvidence = (overrides: Partial<AuthorizationEvidence> = {}): AuthorizationEvidence => ({
  authenticated: true,
  actorUserId: 7,
  targetUserId: 7,
  organizationId: 42,
  ...
  roleGrants: [{ role: "organization_owner", scope: "organization", organizationId: 42 }],
  permissionOverrides: [],
...
describe("evaluateAuthorization", () => {
  it("denies unknown permissions", () => {
    expect(evaluateAuthorization("invented.permission", baseEvidence())).toEqual({
      allowed: false,
  ...
  it("denies unauthenticated requests", () => {
    expect(evaluateAuthorization("profile.self.view", baseEvidence({ authenticated: false, actorUserId: null }))).toMatchObject({
      allowed: false,
  ...
  it("lets an explicit denial override an otherwise valid role grant", () => {
    expect(evaluateAuthorization("contacts.view", baseEvidence({
      permissionOverrides: [{ permission: "contacts.view", effect: "deny", scope: "organization", organizationId: 42 }],
    }))).toMatchObject({ allowed: false, reasonCode: "explicit_deny" });
  });

  it("rejects a role grant from a different organization", () => {
    expect(evaluateAuthorization("contacts.view", baseEvidence({
      roleGrants: [{ role: "organization_owner", scope: "organization", organizationId: 99 }],
    }))).toMatchObject({ allowed: false, reasonCode: "scope_mismatch" });
  });

  it("requires recent A2 authentication for sensitive actions", () => {
    expect(evaluateAuthorization("contacts.delete", baseEvidence({
  ...
  it("requires an independent approver for dual-control actions", () => {
    ...
    expect(evaluateAuthorization("org.delete", baseEvidence({ independentApproverUserIds: [11] }))).toMatchObject({
      allowed: true,
      reasonCode: "allowed",
    });
  });
  ...
  it("requires both customer consent and an active ticket grant for support access", () => {
    const supportEvidence = baseEvidence({
      roleGrants: [{ role: "support_agent", scope: "platform", organizationId: null }],
    });
    expect(evaluateAuthorization("support.grant.use", supportEvidence)).toMatchObject({
      allowed: false,
      reasonCode: "missing_customer_consent",
    });
    expect(evaluateAuthorization("support.grant.use", {
      ...supportEvidence,
