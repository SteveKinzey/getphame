...
function isSecuritySessionAuthMethod(value: string): value is SecuritySessionAuthMethod {
  return value === "oauth" || value === "magic_link" || value === "passkey";
}

function isSecuritySessionAssurance(value: string): value is SecuritySessionAssurance {
...
export function isSecuritySessionToken(value: string | undefined): value is string {
  return typeof value === "string" && /^(?:oa|ml|pk)\.[0-9a-f-]{36}\.[A-Za-z0-9_-]{40,}$/.test(value);
}
...
export async function issueSecuritySession({
  ...
  return { id, token, maxAge: SECURITY_SESSION_LIFETIME_MS, expiresAt: now + SECURITY_SESSION_LIFETIME_MS };
}

export async function issuePasskeySession(userId: number, req: Request, res: Response) {
  return issueSecuritySession({ userId, authMethod: "passkey", assurance: "a2", req, res });
}
...
export async function revokeSecuritySessionFromRequest(req: Request, reason = "user_logout") {
  const token = getRequestSessionToken(req);
