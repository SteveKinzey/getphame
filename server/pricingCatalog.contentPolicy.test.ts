...
describe("customer-facing social-proof policy", () => {
  it("contains no fabricated testimonial payloads, unsupported outcomes, or adoption counts", () => {
    ...
    const prohibitedPatterns: Array<[string, RegExp]> = [
      ...
      ["unsupported response-rate claim", /42\s*%[^\n]{0,80}(?:response|respuesta|réponse|risposta|ตอบกลับ|回复|回覆)/iu],
      ["unsupported review-lift claim", /(?:10|3)\s*[×x][^\n]{0,100}(?:reviews?|reseñas?|avis|recensioni|รีวิว|评价|評論)/iu],
      ["unsupported setup-time claim", /<\s*2\s*(?:min|minutes?)/i],
      ["unsupported batch-volume claim", /(?:500\+|one click sends to hundreds)/i],
      ["guaranteed free-review claim", /(?:first 10 reviews free|primeras 10 reseñas gratis|10 premières évaluations gratuites|prime 10 recensioni gratis|10 รีวิวแรก[^\n]{0,20}ฟรี|前 10 条评价[^\n]{0,20}免费|前 10 則評論[^\n]{0,20}免費)/iu],
      ["unsupported English setup-duration promise", /(?:under|less than|in)\s*(?:2\s*minutes?|60\s*seconds?)/i],
      ["unsupported localized setup-duration promise", /(?:2\s*(?:minutos|minutes|minuti|นาที|分钟|分鐘)|60\s*(?:segundos|secondes|secondi|วินาที|秒))/iu],
      ["five-star outcome claim", /(?:5[- ]star reviews?|5\s*estrellas|5\s*étoiles|5\s*stelle|5\s*ดาว|五星|五顆星)/iu],
