...
const legacyEvidence = readFileSync(
  resolve(projectRoot, "drizzle/legacyLiveTables.ts"),
  "utf8"
);
const additiveMigration = readFileSync(
  resolve(
    projectRoot,
    "drizzle/manual-pending/20260812_add_runtime_schema_contracts.sql"
  ),
  "utf8"
);
...
describe("A1b application-first schema reconciliation", () => {
  it("preserves the fourteen live-only tables as read-only schema evidence", () => {
    ...
    expect(legacyEvidence).toContain(
      "excluded from the active generator schema"
    );
