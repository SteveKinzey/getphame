...
import {
  deleteAccountOwnedData,
  getAccountDeletionImpact,
} from "./accountDeletionImpact";
...
import {
  ...
  supportEscalationPolicies,
  supportEscalationPolicyRecipients,
  supportInternalNoteMentions,
...
import {
  ONBOARDING_CHECKLIST_EVENT_NAMES,
  ONBOARDING_CHECKLIST_EVENT_SOURCE,
...
import {
  buildAdminOperationsAnalyticsExport,
...
const validateSmtpAuditDateRange = (value: {
  dateFrom?: number;
  dateTo?: number;
}) =>
...
const releaseHistoryExportInput = releaseHistoryInput
  ...
  .extend({
    selectedColumns: z
      .array(z.enum(RELEASE_HISTORY_EXPORT_COLUMN_KEYS))
      .min(1)
      .max(RELEASE_HISTORY_EXPORT_COLUMN_KEYS.length)
      .optional(),
  });
...
function releaseHistoryWhere(
  input: ReleaseHistoryFilters,
  snapshotToMs?: number
) {
  const conditions = [];
  if (input.status === "matched")
    conditions.push(eq(releaseParityRecords.parityStatus, "matched"));
  if (input.status === "needs_review")
    conditions.push(ne(releaseParityRecords.parityStatus, "matched"));
  if (snapshotToMs !== undefined)
    conditions.push(lte(releaseParityRecords.recordedAt, snapshotToMs));
...
function releaseHistoryOrder(input: ReleaseHistoryFilters) {
  if (input.sortBy === "checkpointId") {
    return input.sortDirection === "asc"
      ...
      : [
          desc(releaseParityRecords.checkpointId),
...
async function pruneAuditHistory(
  ...
  const [policy] = await db
    ...
    .limit(1);
  const routeRetentionDays =
    policy?.routeAuditRetentionDays ?? AUDIT_RETENTION_DEFAULT_DAYS;
  const rendererRetentionDays =
    policy?.rendererErrorRetentionDays ?? AUDIT_RETENTION_DEFAULT_DAYS;
  const now = Date.now();
  await Promise.all([
    db
      .delete(routeAuditRuns)
      .where(
        lt(routeAuditRuns.auditedAt, now - routeRetentionDays * 86_400_000)
      ),
    db
      .delete(emailPreviewRendererErrors)
      .where(
        lt(
          emailPreviewRendererErrors.occurredAt,
          now - rendererRetentionDays * 86_400_000
        )
      ),
  ]);
}
...
let routeAuditInFlight: Promise<
  Awaited<ReturnType<typeof runProductionRouteAudit>>
> | null = null;
...
function getUnsubscribeSigningSecret(): string {
  ...
  if (!secret || secret.length < 32) {
    throw new Error(
      "UNSUBSCRIBE_SIGNING_SECRET must contain at least 32 characters"
    );
  }
  return secret;
}
...
export function buildUnsubToken(
...
export function verifyUnsubToken(
  token: string
): { contactType: "contact" | "woo"; id: number; userId: number } | null {
  try {
    ...
    const primaryExpected = unsubSignature(
      payload,
...
async function enforceFreeLimit(userId: number, tier: string) {
  ...
  if (!decision.allowed) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: formatFreeQuotaBlockedMessage(
        decision.quota,
        FREE_LIMIT_ERR_MSG
      ),
    });
...
function isValidAvatarSignature(
  data: Buffer,
  mimeType: (typeof avatarMimeTypes)[number]
) {
...
async function queueSupportTicketAlerts(
  ...
  const createdAfter = new Date(
    Date.now() - SUPPORT_TICKET_ALERT_DEDUP_WINDOW_MS
  );
  const recipientsWithoutRecentUnreadAlert: number[] = [];

  for (const recipientUserId of recipientUserIds) {
    const [existingAlert] = await db
      .select({ id: supportTicketAlerts.id })
      ...
      .where(
        input.type === "sla_breach"
          ? and(
              ...
              gte(
                supportTicketAlerts.createdAt,
                input.dedupSince ?? createdAfter
              )
            )
          : and(
              eq(supportTicketAlerts.ticketId, input.ticketId),
      ...
      .limit(1);
    if (!existingAlert)
      recipientsWithoutRecentUnreadAlert.push(recipientUserId);
  }
...
const supportMetricsInputSchema = z
  .object({
    ...
    endDate: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/, "Use a valid end date.")
      .optional(),
  })
  .superRefine((value, issueContext) => {
    ...
    if (hasCustomRange && value.periodDays) {
      issueContext.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["periodDays"],
        message: "Choose either a preset period or a custom date range.",
      });
    }
  });
...
function parseSupportReportDay(value: string, field: "start" | "end"): Date {
  ...
  if (
    ...
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: `Choose a valid ${field} date for the support report.`,
    });
  }
  return parsed;
}
...
function resolveSupportMetricsRange(input?: SupportMetricsInput) {
  ...
  if (input?.startDate && input.endDate) {
    ...
    if (periodEnd.getTime() < periodStart.getTime()) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "The report end date must be on or after its start date.",
      });
    }
    const periodDays =
      Math.floor(
    ...
    if (periodDays > MAX_SUPPORT_EXPORT_RANGE_DAYS) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `Choose a range of ${MAX_SUPPORT_EXPORT_RANGE_DAYS} days or fewer.`,
      });
    }
    return {
      generatedAt,
  ...
  return {
    ...
    periodStart: new Date(
      generatedAt.getTime() - periodDays * 24 * 60 * 60 * 1000
    ),
...
const onboardingChecklistFunnelInputSchema = z
  .object({
    periodDays: z.enum(["7", "30", "90"]).optional(),
    startDate: z
      .string()
  ...
  .superRefine((value, issueContext) => {
    ...
    if (hasCustomRange && (!value.startDate || !value.endDate)) {
      issueContext.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["startDate"],
        message: "Choose both a start and end date for the onboarding report.",
      });
    }
    if (hasCustomRange && value.periodDays) {
      issueContext.addIssue({
...
function parseOnboardingFunnelDay(value: string, field: "start" | "end"): Date {
  ...
  if (
    ...
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: `Choose a valid ${field} date for the onboarding report.`,
    });
  }
  return parsed;
}

function resolveOnboardingChecklistFunnelRange(
  ...
  if (input?.startDate && input.endDate) {
    ...
    if (periodEnd.getTime() < periodStart.getTime()) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message:
          "The onboarding report end date must be on or after its start date.",
      });
    }
    const periodDays =
      Math.floor(
    ...
    if (periodDays > MAX_ONBOARDING_FUNNEL_RANGE_DAYS) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `Choose a range of ${MAX_ONBOARDING_FUNNEL_RANGE_DAYS} days or fewer.`,
      });
    }
    return {
      generatedAt,
...
async function getOnboardingChecklistFunnelSnapshot(
  ...
  return {
    ...summarizeOnboardingChecklistEvents(rows, range.generatedAt.getTime()),
    range,
  };
}
...
async function getOnboardingChecklistFunnelComparison(
  ...
  const periodDurationMs =
    range.periodEnd.getTime() - range.periodStart.getTime() + 1;
  ...
  const currentRows = rows.filter(
    row => row.createdAt.getTime() >= range.periodStart.getTime()
  );
  const previousRows = rows.filter(
    row => row.createdAt.getTime() <= previousPeriodEnd.getTime()
  );
  const current = summarizeOnboardingChecklistEvents(
    currentRows,
    range.generatedAt.getTime()
  );
  const previous = summarizeOnboardingChecklistEvents(
    previousRows,
...
async function getSupportMetricsSnapshot(
  ...
  const {
    ...
    periodLabel,
    isCustomRange,
  } = resolveSupportMetricsRange(input);
  const [metrics] = await db
    .select({
      ...
      avgFirstResponseMs: sql<
        number | null
      >`AVG(CASE WHEN ${supportSubmissions.firstRespondedAt} IS NOT NULL THEN TIMESTAMPDIFF(SECOND, ${supportSubmissions.createdAt}, ${supportSubmissions.firstRespondedAt}) * 1000 ELSE NULL END)`,
      avgResolutionMs: sql<
        number | null
      >`AVG(CASE WHEN ${supportSubmissions.resolvedAt} IS NOT NULL THEN TIMESTAMPDIFF(SECOND, ${supportSubmissions.createdAt}, ${supportSubmissions.resolvedAt}) * 1000 ELSE NULL END)`,
    ...
    .where(
      and(
        gte(supportSubmissions.createdAt, periodStart),
        lte(supportSubmissions.createdAt, periodEnd)
      )
    );
...
async function getSupportEscalationPolicySettings(
  ...
  const [policy] = await db
    .select({
      ...
      includeAllAdminsWhenUnassigned:
        supportEscalationPolicies.includeAllAdminsWhenUnassigned,
    })
    .from(supportEscalationPolicies)
    .where(
      eq(supportEscalationPolicies.policyKey, SUPPORT_ESCALATION_POLICY_KEY)
    )
    .limit(1);
  ...
  const recipients = await db
    .select({
      recipientUserId: supportEscalationPolicyRecipients.recipientUserId,
    })
...
export const appRouter = router({
  ...
  koalendar: router({
    status: paidProcedure.query(async ({ ctx }) =>
      getKoalendarConnectionStatus(ctx.user.id)
    ),
    connect: paidProcedure.mutation(async ({ ctx }) =>
      connectKoalendar(ctx.user.id)
    ),
    rotateWebhook: paidProcedure.mutation(async ({ ctx }) =>
      rotateKoalendarWebhook(ctx.user.id)
    ),
    disconnect: paidProcedure.mutation(async ({ ctx }) =>
      disconnectKoalendar(ctx.user.id)
    ),
  }),
  ...
  accountProfile: router({
    get: protectedProcedure.query(async ({ ctx }) => {
      ...
      const avatar = profile.avatarKey
        ? await storageGet(profile.avatarKey)
        : null;
      return {
        name: profile.name,
        email: profile.email,
    ...
    uploadAvatar: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          ...
          !isValidAvatarSignature(data, input.mimeType)
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Upload a valid JPG, PNG, or WebP image up to 3 MB.",
          });
        }
    ...
    recordExport: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await recordProfilePreferenceExport({
          userId: ctx.user.id,
          format: input.format,
        });
        return { success: true as const };
  ...
  smtp: router({
    ...
    detect: protectedProcedure
      .input(
        z.object({ email: z.string().email(), host: z.string().optional() })
      )
      .query(({ input }) => {
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!test.ok) {
          throw new TRPCError({
            ...
            message:
              test.error ??
              "Could not connect to email server. Check your credentials and try again.",
          });
        }
    ...
    updateReplyTo: protectedProcedure
      .input(
        z.object({ replyTo: z.string().email().optional().or(z.literal("")) })
      )
      .mutation(async ({ ctx, input }) => {
        const creds = await getSmtpCredentials(ctx.user.id);
        if (!creds)
    ...
    sendTestEmail: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          console.warn(
            "[smtp.sendTestEmail] Unable to save sanitized diagnostic history",
            historyError
          );
        }
        if (!result.ok) {
          throw new TRPCError({
            ...
            message:
              result.error ??
              "The test email could not be sent. Please re-test your mail server.",
          });
        }
    ...
    pausedAutomationQueue: protectedProcedure.query(async ({ ctx }) => {
      ...
      if (channel)
        return { paused: false as const, total: 0, items: [] as Array<never> };

      const [[quietCount], [reminderCount], quietRows, reminderRows] =
        await Promise.all([
          db
            .select({ value: count() })
          ...
          db
            ...
            .where(
              and(
                eq(followUpReminders.userId, ctx.user.id),
                eq(followUpReminders.status, "pending")
              )
            ),
          db
            .select({
              id: quietHoursQueuedSends.id,
              scheduledAt: quietHoursQueuedSends.scheduledAt,
              customerName: customerRequests.customerName,
            })
            .from(quietHoursQueuedSends)
            .leftJoin(
              customerRequests,
              eq(quietHoursQueuedSends.customerRequestId, customerRequests.id)
            )
            .where(
              and(
                eq(quietHoursQueuedSends.userId, ctx.user.id),
          ...
          db
            .select({
              ...
              scheduledAt: followUpReminders.scheduledAt,
              customerName: followUpReminders.customerName,
            })
            .from(followUpReminders)
            .where(
              and(
      ...
      const items = [
        ...quietRows.map(row => ({
          ...
          label: row.customerName || "Scheduled review request",
          scheduledAt: row.scheduledAt,
        })),
        ...reminderRows.map(row => ({
          id: `reminder:${row.id}`,
          type: "follow_up" as const,
          label: row.customerName || "Scheduled follow-up",
          scheduledAt: row.scheduledAt,
        })),
      ]
        ...
        .slice(0, 12);
      const total =
        Number(quietCount?.value ?? 0) + Number(reminderCount?.value ?? 0);
    ...
    previewEmail: protectedProcedure.query(async ({ ctx }) => {
      ...
      if (db) {
        ...
        const [defaultPlatform] = await db
          ...
          .where(
            and(
              eqOp(reviewPlatforms.userId, ctx.user.id),
              eqOp(reviewPlatforms.isDefault, 1)
            )
          )
          .limit(1);
  ...
  profile: router({
    get: protectedProcedure.query(async ({ ctx }) => {
      ...
      const complimentaryAccess =
        ctx.user.role === "admin"
          ? null
          : await findActiveComplimentaryAccess({
              userId: ctx.user.id,
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!existing) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message:
              "Complete your business profile before configuring quiet hours.",
          });
        }
        ...
        const resolved = await resolveBusinessAddressTimeZone(
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!profile?.physicalAddress || !profile.businessTimeZone) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Save and verify your physical business address before requesting a quiet-hours exception.",
          });
        }
        ...
        if (duration >= QUIET_HOURS_MINIMUM_DURATION_MINUTES) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "This requested window already meets the 12-hour minimum and does not need an exception.",
          });
        }
        ...
        if (!ctx.user.email) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Add an email address to your account before requesting a quiet-hours exception.",
          });
        }
        const requesterName = ctx.user.name ?? profile.businessName;
        const start =
          String(Math.floor(input.requestedStartMinutes / 60)).padStart(
            2,
            "0"
          ) +
          ":" +
          String(input.requestedStartMinutes % 60).padStart(2, "0");
        const end =
          String(Math.floor(input.requestedEndMinutes / 60)).padStart(2, "0") +
          ":" +
          String(input.requestedEndMinutes % 60).padStart(2, "0");
        ...
        if (!submissionId)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Could not record your request.",
          });
  ...
  stripe: router({
    ...
    promotionMonitor: adminProcedure.query(async () => {
      try {
        ...
        throw new TRPCError({
          code: "BAD_GATEWAY",
          message:
            "Stripe promotion data is temporarily unavailable. Please refresh in a moment.",
        });
      }
    }),
    ...
    createPromotionCode: adminProcedure
      .input(
        z.object({
          ...
          applicablePlans: z
            .array(z.enum(["monthly", "annual", "lifetime"]))
            .min(1)
            .max(3),
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          ...
          if (safeInputError) {
            throw new TRPCError({
              code: message.includes("already exists")
                ? "CONFLICT"
                : "BAD_REQUEST",
              message,
            });
          ...
          console.error(
            "[Stripe] Administrator promotion-code creation failed",
            error
          );
          throw new TRPCError({
            code: "BAD_GATEWAY",
            message:
              "Stripe could not create this promotion code. Please retry or review Stripe configuration.",
          });
        }
      }),
    ...
    subscriptionStatus: protectedProcedure.query(async ({ ctx }) => {
      ...
      try {
        ...
        console.warn(
          "[Stripe] Failed to refresh subscription status; using stored status",
          error
        );
        const profile = await getBusinessProfile(ctx.user.id);
        return {
    ...
    cancelRenewal: protectedProcedure
      ...
      .mutation(async ({ ctx }) => {
        ...
        try {
          return await cancelSubscriptionRenewal(
            stripeCustomerId,
            stripeSubscriptionId
          );
        } catch (error) {
  ...
  woo: router({
    ...
    bulkSetStatus: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await bulkSetWooCustomerStatus(
          ctx.user.id,
          input.customerIds,
          input.sent
        );
    ...
    bulkSend: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (eligibleCustomers.length === 0)
          throw new Error(
            "No eligible customers found (all may have unsubscribed)."
          );
        ...
        if (input.platformId) {
          ...
          const wooDefaultPlatform = await getDefaultReviewPlatform(
            ctx.user.id
          );
    ...
    sendHistory: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const rows = await db
          ...
          .where(
            andW(
              eqW(wooCustomers.userId, ctx.user.id),
              eqW(wooCustomers.id, input.customerId)
            )
          );
        ...
        const history = await db
          ...
          .where(
            andW(
              eqW(customerRequests.userId, ctx.user.id),
              eqW(customerRequests.customerEmail, email)
            )
          )
        ...
        return history.map(r => ({
          ...r,
          platformLabel: r.platformId
            ? (platformMap.get(r.platformId)?.label ??
              platformMap.get(r.platformId)?.platform ??
              null)
            : null,
          sentAt:
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        await createSavedContact(ctx.user.id, {
          ...
          consentSource: input.consentGiven
            ? (input.consentSource ?? "manual_add_contact_form")
            : undefined,
        });
        return { ok: true };
      }),
    ...
    bulkSend: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.platformId) {
          ...
          if (chosen) {
            reviewUrl = chosen.url;
            isYelpPlatform = chosen.platform === "yelp";
          }
        } else {
          const defaultPlatform = await getDefaultReviewPlatform(ctx.user.id);
          if (defaultPlatform) {
            reviewUrl = defaultPlatform.url;
            isYelpPlatform = defaultPlatform.platform === "yelp";
          }
        }
        ...
        for (const contact of toSend) {
          try {
            ...
            const bulkToken = encodeTrackingToken(
              bulkRequestId,
              ctx.user.id,
            ...
            const trackedBulkHtml = isYelpPlatform
              ...
              : (() => {
                  const trackedBulkUrl = wrapClickUrl(
                    reviewUrl,
                    bulkToken,
                    bulkBase
                  );
        ...
        if (resolvedTemplate && sent + queued > 0) {
          ...
          if (dbT) {
            ...
            await dbT
              .update(etTable)
              .set({
                usageCount: sqlT`${etTable.usageCount} + ${sent + queued}`,
              })
    ...
    syncFromStripe: protectedProcedure.mutation(async ({ ctx }) => {
      ...
      while (hasMore) {
        ...
        for (const customer of page.data) {
          ...
          if (metaUserId && String(metaUserId) !== String(ctx.user.id))
            continue;
    ...
    sendHistory: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const contacts = await db
          ...
          .where(
            andH(
              eqH(savedContacts.userId, ctx.user.id),
              eqH(savedContacts.id, input.contactId)
            )
          );
        ...
        const rows = await db
          ...
          .where(
            andH(
              eqH(customerRequests.userId, ctx.user.id),
              eqH(customerRequests.customerEmail, contact.email)
            )
          )
    ...
    bulkConsentRequest: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const targets = input.contactIds
          .map(id => contactMap.get(id))
          .filter(
            c => c && !c.optedOut && c.consentBasis !== "explicit_opt_in"
          ) as typeof allContacts;
        if (targets.length === 0)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "No eligible contacts (all have consent or are opted out).",
          });
        const businessName = String(
          (profile as any).consentLabelName || profile.businessName || ""
        );
    ...
    sendConsentTestEmail: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!toAddress) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "A test recipient email address is required.",
          });
        }
        ...
        if (!result)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message:
              "Test email could not be delivered. Check your SMTP settings.",
          });
      ...
      .query(async ({ ctx, input }) => {
        ...
        return db
          ...
          .where(
            andR(
              eq(followUpReminders.userId, ctx.user.id),
              eqR(followUpReminders.customerRequestId, input.requestId)
            )
          )
          .orderBy(asc(followUpReminders.scheduledAt));
      }),
          ...
          .superRefine((value, ctx) => {
            ...
            if (hasEditedSubject && value.complianceConfirmed !== true) {
              ctx.addIssue({
                ...
                message:
                  "Confirm the compliance checklist before sending edited copy",
              });
            }
          })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.platformId) {
          ...
          if (chosen) {
            ...
            reviewUrl = isYelpSingle
              ? buildYelpSearchInstruction(profile.businessName)
              : chosen.url;
          }
        } else {
          // Use default platform if available
          const defaultPlatform =
            userPlatforms.find(p => p.isDefault === 1) ??
            userPlatforms[0] ??
            null;
          if (defaultPlatform) {
        ...
        const resolvedTemplate = input.templateId
          ? (allTemplates.find(t => t.id === input.templateId) ?? null)
          : await getDefaultTemplate(ctx.user.id);
        const platformLinksList = buildSafePlatformLinks(
          userPlatforms,
          profile.businessName,
          reviewUrl
        );
        const renderDraft = (value: string) =>
          renderReviewRequestDraft(value, {
        ...
        if (
          input.editedSubject !== undefined &&
          input.editedBody !== undefined
        ) {
          subject = renderDraft(input.editedSubject).replace(
            /\s*[\r\n]+\s*/g,
            " "
          );
          htmlBody = wrapPlainTextReviewRequestHtml(
            renderDraft(input.editedBody)
          );
        } else if (resolvedTemplate) {
          subject = renderDraft(resolvedTemplate.subject).replace(
            /\s*[\r\n]+\s*/g,
            " "
          );
          htmlBody = wrapPlainTextReviewRequestHtml(
            renderDraft(resolvedTemplate.body)
          );
        } else {
          subject = `${profile.businessName} would love your feedback!`;
          htmlBody = buildReviewRequestEmail({
        ...
        const [row] = await db
          ...
          .where(
            and(
              ...
              eqOp(customerRequests.id, input.id)
            )
          )
          .limit(1);
    ...
    resend: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!original.customerEmail)
          throw new Error("No email address on file for this customer");
        ...
        const openPixel = buildOpenPixel(trackingToken, baseUrl);
        const trackedHtml = input.emailBody.replace(
          /<\/div>\s*$/,
          `${openPixel}</div>`
        );
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        for (const id of input.ids) {
          try {
            ...
            const openPixel = buildOpenPixel(trackingToken, baseUrl);
            const trackedHtml = original.emailBody.replace(
              /<\/div>\s*$/,
              `${openPixel}</div>`
            );
    ...
    stats: protectedProcedure.query(async ({ ctx }) => {
      ...
      if (db) {
        const allPlatforms = await db
          .select()
          .from(reviewPlatforms)
          .where(eq(reviewPlatforms.userId, ctx.user.id));
  ...
  tracking: router({
    ...
    requestStats: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        return Array.from(map.entries()).map(([requestId, stats]) => ({
          requestId,
          ...stats,
        }));
      }),
    ...
    templateStats: protectedProcedure.query(async ({ ctx }) => {
      ...
      return Array.from(map.entries()).map(([templateId, stats]) => ({
        templateId,
        ...stats,
      }));
    }),
    ...
    recentEvents: protectedProcedure.query(async ({ ctx }) => {
      const db = await getDb();
      if (!db) return [];
      const { emailEvents, customerRequests } = await import(
        "../drizzle/schema"
      );
      ...
      const rows = await db
        ...
        .leftJoin(
          customerRequests,
          eqOp(emailEvents.requestId, customerRequests.id)
        )
        .where(
          and(
            eqOp(emailEvents.userId, ctx.user.id),
    ...
    /** Returns tenant-scoped daily activity for a preset or custom range. */
    dailyTrend: protectedProcedure
      .input(
        z
          .object({
            ...
            startTimezoneOffsetMinutes: z
              .number()
              .int()
              .min(-840)
              .max(840)
              .optional(),
            endTimezoneOffsetMinutes: z
              .number()
              .int()
              .min(-840)
              .max(840)
          ...
          .superRefine((value, issueContext) => {
            ...
            if (hasCustomRange && (!value.startDate || !value.endDate)) {
              issueContext.addIssue({
                code: z.ZodIssueCode.custom,
                message: "Choose both a start and end date for Activity Trend.",
              });
            }
            if (hasCustomRange && value.days !== undefined) {
              issueContext.addIssue({
                code: z.ZodIssueCode.custom,
                message:
      ...
      .query(async ({ ctx, input }) => {
        const db = await getDb();
        if (!db) return [];
        const { emailEvents: evTable, customerRequests: crTable } =
        ...
        let range;
        try {
          range = resolveDailyTrendRange(input, nowMs);
        } catch (error) {
          if (error instanceof DailyTrendRangeError) {
            throw new TRPCError({
              code: "BAD_REQUEST",
        ...
        const sendRows = await db
          ...
          .where(
            andOp(
              eqOp(crTable.userId, ctx.user.id),
              gte(crTable.sentAt, range.startAt),
              lt(crTable.sentAt, range.endExclusive)
            )
          );
        ...
        const eventRows = await db
          ...
          .where(
            andOp(
              ...
              gte(evTable.createdAt, range.startAt),
              lt(evTable.createdAt, range.endExclusive)
            )
          );
    ...
    overallStats: protectedProcedure.query(async ({ ctx }) => {
      ...
      const openRows = await db
        .select({
          count: sqlOp<number>`count(distinct ${emailEvents.requestId})`,
        })
        .from(emailEvents)
        .where(
      ...
      const openRate =
        totalSent > 0 ? Math.round((uniqueOpens / totalSent) * 100) : 0;
      const clickRate =
        totalSent > 0 ? Math.round((uniqueClicks / totalSent) * 100) : 0;
  ...
  reviewPlatforms: router({
    ...
    restore: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const restored = await addReviewPlatform(
          ctx.user.id,
          input.platform,
          input.url,
          input.label
  ...
  onboarding: router({
    ...
    status: protectedProcedure.query(async ({ ctx }) => {
      ...
      const allDone =
        smtpConnected && hasPlatform && hasContacts && hasSentRequest;
  ...
  account: router({
    previewDeletion: protectedProcedure.query(async ({ ctx }) => {
      try {
        ...
        console.warn(
          "[account.previewDeletion] Could not calculate deletion impact:",
          error
        );
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
    ...
    delete: protectedProcedure.mutation(async ({ ctx }) => {
      ...
      try {
        ...
        console.warn(
          "[account.delete] Confirmation email failed (non-fatal):",
          emailErr
        );
      }
      return { ok: true };
  ...
  admin: router({
    ...
    listReleaseParityRecords: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (!db)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Release history storage is unavailable.",
          });
    ...
    prepareReleaseParityExport: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        return buildReleaseHistoryCsvExport({
          ...
          selectedColumns: parsed.selectedColumns as
    ...
    getAuditRetentionPolicy: adminProcedure.query(async () => {
      ...
      if (!db)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Audit retention storage is unavailable.",
        });
      const [policy] = await db
        .select()
        .from(auditRetentionPolicies)
      ...
      return policy
        ? {
            routeAuditRetentionDays: policy.routeAuditRetentionDays,
            rendererErrorRetentionDays: policy.rendererErrorRetentionDays,
            updatedAt: policy.updatedAt,
          }
        : {
            routeAuditRetentionDays: AUDIT_RETENTION_DEFAULT_DAYS,
            rendererErrorRetentionDays: AUDIT_RETENTION_DEFAULT_DAYS,
            updatedAt: null,
          };
    }),
    ...
    updateAuditRetentionPolicy: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [existing] = await db
          ...
          .where(eq(auditRetentionPolicies.policyKey, "global"))
          .limit(1);
        const previousRouteAuditRetentionDays =
          existing?.routeAuditRetentionDays ?? AUDIT_RETENTION_DEFAULT_DAYS;
        const previousRendererErrorRetentionDays =
          existing?.rendererErrorRetentionDays ?? AUDIT_RETENTION_DEFAULT_DAYS;
        if (existing) {
          await db
            .update(auditRetentionPolicies)
            .set({ ...input, updatedByUserId: ctx.user.id, updatedAt: now })
      ...
      .query(async ({ input }) => {
        ...
        return db
          .select({
            ...
            previousRouteAuditRetentionDays:
              auditRetentionPolicyChanges.previousRouteAuditRetentionDays,
            previousRendererErrorRetentionDays:
              auditRetentionPolicyChanges.previousRendererErrorRetentionDays,
            routeAuditRetentionDays:
              auditRetentionPolicyChanges.routeAuditRetentionDays,
            rendererErrorRetentionDays:
              auditRetentionPolicyChanges.rendererErrorRetentionDays,
            changedAt: auditRetentionPolicyChanges.changedAt,
          })
          .from(auditRetentionPolicyChanges)
          .orderBy(
            desc(auditRetentionPolicyChanges.changedAt),
            desc(auditRetentionPolicyChanges.id)
          )
          .limit(input?.limit ?? 20);
      }),

    getReleaseHistoryExportSchedule: adminProcedure.query(() =>
      getReleaseHistoryExportSchedule()
    ),
    ...
    listReleaseHistoryExportRuns: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        return db
          .select()
          .from(releaseHistoryExportRuns)
          .orderBy(
            desc(releaseHistoryExportRuns.generatedAt),
            desc(releaseHistoryExportRuns.id)
          )
    ...
    rendererFailureTrend: adminProcedure
      .input(
        z
          .object({
            windowHours: z.number().int().min(24).max(720).default(168),
      ...
      .query(async ({ input }) => {
        ...
        const rows = await db
          .select({
            templateKey: emailPreviewRendererErrors.templateKey,
            viewportMode: emailPreviewRendererErrors.viewportMode,
            darkMode: emailPreviewRendererErrors.darkMode,
            errorCode: emailPreviewRendererErrors.errorCode,
            occurredAt: emailPreviewRendererErrors.occurredAt,
          })
          .from(emailPreviewRendererErrors)
        ...
        const acknowledgements = await db
          .select()
          .from(rendererFailureAlertAcknowledgements)
          .limit(100);
        const acknowledgedBySignature = new Map(
          acknowledgements.map(entry => [entry.signature, entry])
        );
        const signals = repeatSignals.map(signal => {
          const signature = [
            signal.templateKey,
            signal.viewportMode,
            signal.darkMode ? "dark" : "light",
            signal.errorCode,
          ].join("|");
          ...
          return {
            ...
            acknowledged: Boolean(
              acknowledgement &&
                acknowledgement.acknowledgedLatestOccurredAt >=
                  signal.latestOccurredAt
            ),
    ...
    acknowledgeRendererFailureAlert: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [evidence] = await db
          .select({ occurredAt: emailPreviewRendererErrors.occurredAt })
          .from(emailPreviewRendererErrors)
          .where(
            and(
              eq(emailPreviewRendererErrors.templateKey, input.templateKey),
              eq(emailPreviewRendererErrors.viewportMode, input.viewportMode),
              eq(emailPreviewRendererErrors.darkMode, input.darkMode),
              eq(emailPreviewRendererErrors.errorCode, input.errorCode),
              eq(emailPreviewRendererErrors.occurredAt, input.latestOccurredAt)
            )
          )
          .limit(1);
        if (!evidence)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "The alert evidence is no longer current.",
          });
        const signature = [
          input.templateKey,
          input.viewportMode,
          input.darkMode ? "dark" : "light",
          input.errorCode,
        ].join("|");
        const acknowledgedAt = Date.now();
        await db
          .insert(rendererFailureAlertAcknowledgements)
          .values({
            signature,
            ...input,
            acknowledgedLatestOccurredAt: input.latestOccurredAt,
    ...
    complimentaryAccess: adminProcedure
      ...
      .query(({ input }) =>
        listComplimentaryAccessGrants({ status: input?.status ?? "all" })
      ),

    complimentaryAccessLookup: adminProcedure
      .input(z.object({ email: z.string().trim().email().max(320) }))
      .query(({ input }) =>
        lookupComplimentaryAccessByEmail({ email: input.email })
      ),
    ...
    grantComplimentaryAccess: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          return await createComplimentaryAccessGrant({
            ...input,
            createdByUserId: ctx.user.id,
          });
        } catch (error) {
    ...
    revokeComplimentaryAccess: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          ...
          console.error(
            "[Admin] Complimentary-access revocation failed",
            error
          );
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
    ...
    stats: adminProcedure.query(async () => {
      ...
      const sendsLast30 = recentSends.filter(
        r => r.sentAt && r.sentAt > thirtyDaysAgo
      ).length;
    ...
    exportRelayOutageCsv: adminProcedure.mutation(async () => {
      const [{ getRelayOutageExportSnapshot }, { buildRelayOutageCsvExport }] =
        await Promise.all([
          import("./relayHealth"),
    ...
    smtpStats: adminProcedure.query(async () => {
      ...
      const totals = {
        ...
        lastRunAt:
          rows.reduce((max, r) => Math.max(max, r.lastHealthCheck ?? 0), 0) ||
          null,
      };
      ...
      .query(async ({ input }) => {
        ...
        if (userIds.length > 0) {
          const churnRows = await db
            .select({
              userId: churnSurveys.userId,
              reason: churnSurveys.reason,
            })
            .from(churnSurveys)
    ...
    listUsers: adminProcedure
      .input(
        z.object({
          ...
          smtpStatus: z
            .enum(["all", "verified", "unverified", "failing", "unconnected"])
            .default("all"),
          page: z.number().int().min(1).default(1),
      ...
      .query(async ({ input }) => {
        ...
        const searchClause = search
          ...
          : undefined;
        const smtpClause =
          input.smtpStatus === "verified"
            ? and(
                isNotNull(smtpCredentials.id),
                eq(smtpCredentials.verified, 1)
              )
            : input.smtpStatus === "unverified"
              ? and(
                  isNotNull(smtpCredentials.id),
                  eq(smtpCredentials.verified, 0)
                )
              : input.smtpStatus === "failing"
                ? and(
                    isNotNull(smtpCredentials.id),
                    eq(smtpCredentials.lastHealthStatus, "fail")
                  )
        ...
        return {
          users: rows.map(row => ({
            ...
            isSuspended:
              typeof row.suspendedUntil === "number" &&
              row.suspendedUntil > Date.now(),
          })),
          page: input.page,
    ...
    retestUserSmtp: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!credential)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "No SMTP credentials are connected for this user.",
          });
        ...
        try {
          ...
          result = {
            ...
            error:
              error instanceof Error
                ? error.message
                : "Unable to decrypt or test the stored credentials.",
          };
        }
        ...
        const errorMessage = result.ok
          ? null
          : (result.error ?? "SMTP verification failed.").slice(0, 500);
        await db
          .update(smtpCredentials)
          .set({
          ...
          .where(eq(smtpCredentials.userId, input.userId));
        console.log(
          `[Admin] SMTP credentials for user ${input.userId} re-tested by admin ${ctx.user.id}: ${result.ok ? "ok" : "fail"}`
        );
    ...
    listSmtpAuditLogs: adminProcedure
      .input(
        z
          ...
          .refine(validateSmtpAuditDateRange, {
            message: "The audit start date must be before the end date.",
            path: ["dateFrom"],
          })
      )
      .query(async ({ input }) => {
    ...
    exportSmtpAuditLogs: adminProcedure
      .input(
        z.object(smtpAuditFilterShape).refine(validateSmtpAuditDateRange, {
          message: "The audit start date must be before the end date.",
          path: ["dateFrom"],
        })
      )
    ...
    setUserRole: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        console.log(
          `[Admin] User ${input.userId} role set to ${input.role} by admin ${ctx.user.id}`
        );
        return { ok: true };
      }),
    ...
    setLifeAccess: adminProcedure
      .input(
        z.object({ userId: z.number().int().positive(), enabled: z.boolean() })
      )
      .mutation(async ({ ctx, input }) => {
        const db = await getDb();
        if (!db)
        ...
        if (!input.enabled && target.role === "admin") {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Administrators always have Life access. Change the role first.",
          });
        }
        ...
        await db
          ...
          .values({
            userId: input.userId,
            businessName:
              target.name?.trim() || target.email?.trim() || "Get Phame User",
            reviewLink: "",
    ...
    createUser: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [created] = await db
          ...
          .$returningId();
        await db.insert(adminUserLifecycleAuditLogs).values({
    ...
    grantFlexibleAccess: adminProcedure
      .input(
        z
          ...
          .superRefine((value, context) => {
            if (value.kind !== "lifetime" && !value.quantity)
              context.addIssue({
                code: z.ZodIssueCode.custom,
            ...
            if (
              value.kind === "months" &&
              (value.quantity ?? 0) > MAX_ADMIN_GRANT_MONTHS
            )
              context.addIssue({
                code: z.ZodIssueCode.custom,
                message: `Months may not exceed ${MAX_ADMIN_GRANT_MONTHS}.`,
                path: ["quantity"],
              });
            if (
              value.kind === "years" &&
              (value.quantity ?? 0) > MAX_ADMIN_GRANT_YEARS
            )
              context.addIssue({
                code: z.ZodIssueCode.custom,
                message: `Years may not exceed ${MAX_ADMIN_GRANT_YEARS}.`,
                path: ["quantity"],
              });
          })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        if (target.role === "admin" && input.kind !== "lifetime")
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Administrators already have Life access.",
          });
        const yearMonth = new Date().toISOString().slice(0, 7);
        const planExpiresAt =
          input.kind === "lifetime"
            ? null
            : resolveFlexibleAccessExpiry(target.planExpiresAt, {
                kind: input.kind,
        ...
        await db
          ...
          .values({
            userId: target.id,
            businessName:
              target.name?.trim() || target.email || "Get Phame User",
            reviewLink: "",
            tier,
            monthlyCount: 0,
            monthlyResetDate: yearMonth,
          ...
          .onDuplicateKeyUpdate({
            set: { tier, planExpiresAt, updatedAt: new Date() },
          });
        await db.insert(adminUserLifecycleAuditLogs).values({
          actorUserId: ctx.user.id,
    ...
    suspendUser: adminProcedure
      .input(
        z.object({
          userId: z.number().int().positive(),
          days: z.number().int().min(1).max(MAX_ADMIN_SUSPENSION_DAYS),
    ...
    sendUserEmail: adminProcedure
      .input(
        z.object({
          userId: z.number().int().positive(),
          template: z.enum(["smtp_onboarding", "custom"]),
          subject: z.string().max(180).optional(),
          bodyText: z.string().max(12_000).optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const db = await getDb();
        ...
        if (!target?.email)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "The selected user has no deliverable email address.",
          });
        const preset =
          input.template === "smtp_onboarding"
            ? buildSmtpOnboardingTemplate(target.name)
            : null;
        const message = sanitizeAdminMessage({
          subject: input.subject ?? preset?.subject ?? "",
          bodyText: input.bodyText ?? preset?.bodyText ?? "",
        });
        const delivered = await sendAdminPlatformEmail({
          to: target.email,
          subject: message.subject,
          bodyText: message.bodyText,
        });
        const now = Date.now();
        await db.insert(adminPlatformEmailMessages).values({
          actorUserId: ctx.user.id,
          recipientUserId: target.id,
          recipientEmail: target.email,
          fromEmail: "hello@getphame.app",
          template: input.template as AdminPlatformEmailTemplate,
          subject: message.subject,
          bodyText: message.bodyText,
          status: delivered.sent ? "sent" : "failed",
          providerMessageId: delivered.providerMessageId,
          failureCode: delivered.failureCode,
          createdAt: now,
          sentAt: delivered.sent ? now : null,
          expiresAt: now + ADMIN_MESSAGE_RETENTION_MS,
        });
    ...
    listUserEmailOutbox: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        const [totalRow] = await db
          .select({ value: count() })
          .from(adminPlatformEmailMessages)
          .where(outboxVisibilityPredicate);
        const entries = await db
          .select()
          .from(adminPlatformEmailMessages)
          .where(outboxVisibilityPredicate)
          .orderBy(desc(adminPlatformEmailMessages.createdAt))
          .limit(input.pageSize)
          .offset((input.page - 1) * input.pageSize);
    ...
    removeUserSmtp: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const removed = await db.transaction(async tx => {
          ...
          if (
            !target.email ||
            input.confirmationEmail.toLowerCase() !==
              target.email.trim().toLowerCase()
          ) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Type the user's exact email address to remove SMTP credentials.",
            });
          }
        ...
        console.log(
          `[Admin] SMTP credentials for user ${input.userId} ${removed ? "removed" : "were already absent"} by admin ${ctx.user.id}`
        );
    ...
    deleteUser: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        console.log(
          `[Admin] Account ${deleted.id} deleted by admin ${ctx.user.id}`
        );
    ...
    listKoalendarFailures: adminProcedure
      ...
      .query(async ({ input }) =>
        listFailedKoalendarBookings(input.page, input.pageSize)
      ),
    ...
    retryKoalendarImport: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (result.outcome === "not_found") {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Koalendar import was not found.",
          });
        }
        if (result.outcome === "not_eligible") {
          throw new TRPCError({
            code: "CONFLICT",
            message:
              "This Koalendar import is no longer eligible for a manual retry. Refresh the queue to see its current state.",
          });
        }
        return result;
      }),
    ...
    combineAccounts: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const result = await combineAccountsAsAdmin(
          ctx.user.id,
          input.sourceUserId,
          input.targetUserId
        );
        console.log(
          `[Admin] Account ${input.sourceUserId} combined into ${input.targetUserId} by admin ${ctx.user.id}`
        );
    ...
    setTier: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        console.log(
          `[Admin] User ${input.userId} tier set to ${input.tier} by admin ${ctx.user.id}`
        );
        return { ok: true };
      }),
    ...
    grantSubscription: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const granted = await grantSubscriptionByEmail(
            input.email,
            input.plan
          );
          if (!granted) {
            throw new TRPCError({
              code: "NOT_FOUND",
              message: "No account matches that email address.",
            });
          }
          console.log(
            `[Admin] ${input.plan} access granted to user ${granted.userId} by admin ${ctx.user.id}`
          );
          return granted;
        } catch (error) {
    ...
    revokeSubscription: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!revoked) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "No subscription profile matches that email address.",
          });
        }
        console.log(
          `[Admin] Paid access revoked for user ${revoked.userId} by admin ${ctx.user.id}`
        );
        return revoked;
      }),
    ...
    manualSearchInsights: adminProcedure
      .input(
        z
          .object({
            periodDays: z
              .enum(
                MANUAL_SEARCH_REPORTING_PERIODS.map(String) as [
                  "30",
                  "90",
                  "365",
                ]
      ...
      .query(async ({ input }) => {
        ...
        return getManualSearchInsights({
          ...
          periodDays: Number(
            input.periodDays
          ) as (typeof MANUAL_SEARCH_REPORTING_PERIODS)[number],
          limit: input.limit,
        });
      }),

    /** Aggregate checklist setup funnel. It intentionally returns no raw event or identity data. */
    onboardingChecklistFunnel: adminProcedure
      .input(onboardingChecklistFunnelInputSchema.optional())
      .query(async ({ input }) => {
        const db = await getDb();
        if (!db)
          throw new TRPCError({
        ...
        const rows: AdminOperationsCsvRow[] = [
          {
            ...
            metric: "generated_at",
            period,
            value: funnel.range.generatedAt.toISOString(),
            unit: "iso_8601",
            details: "Get Phame aggregate onboarding funnel export",
          },
          {
            section: "metadata",
            metric: "period_start",
            period,
            value: funnel.range.periodStart.toISOString(),
            unit: "iso_8601",
            details: "Inclusive UTC reporting boundary",
          },
          {
            section: "metadata",
            metric: "period_end",
            period,
            value: funnel.range.periodEnd.toISOString(),
            unit: "iso_8601",
            details: "Inclusive UTC reporting boundary",
          },
          {
            section: "onboarding_funnel",
            metric: "checklist_views",
            period,
            value: funnel.allTime.checklist_viewed,
            unit: "unique_accounts",
            details: "Unique accounts that viewed the setup checklist",
          },
          {
            section: "onboarding_funnel",
            metric: "checklist_completed",
            period,
            value: funnel.allTime.checklist_completed,
            unit: "unique_accounts",
            details: "Unique accounts that completed all setup steps",
          },
          {
            section: "onboarding_funnel",
            metric: "completion_rate",
            period,
            value: funnel.rates.completion,
            unit: "percent",
            details: "Completed accounts divided by checklist viewers",
          },
            ...
            return [
              {
                ...
                value: metric.shown,
                unit: "unique_accounts",
                details: "Unique accounts that viewed this step",
              },
              {
                section: "onboarding_funnel",
                metric: `${label}_continued`,
                period,
                value: metric.actioned,
                unit: "unique_accounts",
                details: "Unique accounts that used this step action",
              },
              {
                section: "onboarding_funnel",
                metric: `${label}_drop_off`,
                period,
                value: metric.dropOff,
                unit: "unique_accounts",
                details: "Viewed this step without using its action",
              },
              {
                section: "onboarding_funnel",
                metric: `${label}_continuation_rate`,
                period,
                value: metric.continuationRate,
                unit: "percent",
                details: "Accounts that continued after viewing this step",
              },
    ...
    revenue: adminProcedure.query(async () => {
      ...
      const platformOpenRows = await db
        .select({
          count: sqlRev<number>`count(distinct ${emailEvents.requestId})`,
        })
        .from(emailEvents)
      ...
      const platformOpenRate =
        platformTotalSent > 0
          ? Math.round((platformUniqueOpens / platformTotalSent) * 100 * 10) /
            10
          : 0;
      const platformClickRate =
        platformTotalSent > 0
          ? Math.round((platformUniqueClicks / platformTotalSent) * 100 * 10) /
            10
          : 0;
      ...
      const growthChart = Object.entries(monthlyGrowth).map(
        ([month, newSubs]) => ({ month, newSubs })
      );
      ...
      const promptpayRevealLast30 = Number(
        promptpayRevealLast30Rows[0]?.count ?? 0
      );
      ...
      const promptpayConversionRate =
        promptpayRevealTotal > 0
          ? Math.round(
              ((tierCounts.pro + tierCounts.annual + tierCounts.lifetime) /
      ...
      const recentCancels = allSubRows.filter(
        s =>
          s.status === "canceled" &&
          s.createdAt &&
      ...
      const churnRate =
        activeSubs > 0
          ? Math.round((recentCancels / activeSubs) * 100 * 10) / 10
          : 0;
    ...
    sendTestEmail: adminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const TEST_PREVIEW_LINK =
          "https://getphame.app/login?from=test-email-preview";
        ...
        switch (input.template) {
          ...
          case "welcome": {
            ...
            const stepsHtml = steps
              .map(
                ([t, n]) =>
                  `<p style="margin:0 0 12px;font-size:14px;color:#1a2744;line-height:1.6;"><span style="display:inline-block;background:#C9A84C;color:#0F1B2D;font-weight:800;font-size:12px;border-radius:50%;width:22px;height:22px;text-align:center;line-height:22px;margin-right:8px;">${n}</span>${t}</p>`
              )
              .join("");
          ...
          case "upgrade-receipt-lifetime": {
            ...
            const perksHtml = perks
              .map(
                p =>
                  `<li style="margin:0 0 10px;font-size:14px;color:#1a2744;line-height:1.6;"><span style="display:inline-block;background:#C9A84C;color:#0F1B2D;font-weight:800;font-size:11px;border-radius:50%;width:20px;height:20px;text-align:center;line-height:20px;margin-right:8px;">✓</span>${p}</li>`
              )
              .join("");
    ...
    stripeStatus: adminProcedure.query(async () => {
      ...
      try {
        ...
        const webhookData = await new Promise<
          { url: string; status: string; enabled_events: string[] }[]
        >((resolve, reject) => {
          const req = https.get(
            "https://api.stripe.com/v1/webhook_endpoints?limit=5",
            {
    ...
    emailPreview: publicProcedure
      ...
      .query(({ input }) => {
        return {
          html: buildAdminEmailPreviewTemplate({ template: input.template }),
        };
        ...
        const body = `<p style="margin:0 0 16px;font-size:17px;font-weight:700;color:#0F1B2D;">Hi ${SAMPLE_NAME},</p><p style="margin:0 0 16px;font-size:15px;color:#555;line-height:1.7;">Your Get Phame account and all associated data have been permanently deleted as requested.</p><p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.7;">If you change your mind, you're always welcome to create a new account at <a href="https://getphame.app" style="color:#C9A84C;">getphame.app</a>.</p>`;
        return {
          html: wrapEmail("Your account has been deleted", body, footer()),
        };
      }),
  }),
  ...
  adminReferrals: router({
    ...
    listDeferred: adminProcedure.query(async () => {
      ...
      const rows = await db
        ...
        .from(referrals)
        .where(
          and(isNotNull(referrals.convertedAt), isNull(referrals.rewardedAt))
        )
      ...
      return rows.map(row => {
        ...
        const isNowEligible = ELIGIBLE_TIERS.includes(
    ...
    processAll: adminProcedure.mutation(async () => {
      ...
      console.log(
        `[AdminReferrals] processAll: ${processed} rewarded, ${skipped} skipped (not on paid plan)`
      );
  ...
  wordpressPairing: router({
    get: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (!pairing)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "This WordPress connection request was not found.",
          });
        return pairing;
      }),
    approve: paidProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          return await approveWordPressPairing({
            userId: ctx.user.id,
  ...
  apiKey: router({
    ...
    acceptTerms: protectedProcedure
      ...
      .mutation(async ({ ctx }) => {
        ...
        return acceptDeveloperApiTerms({
          userId: ctx.user.id,
          acceptanceFingerprint: fingerprintAuthValue(
            `developer-api-enrollment:${clientIp}:${userAgent}`
          ),
        });
      }),
        ...
        try {
          ...
          throw new TRPCError({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Unable to request sending access.",
          });
        }
      }),
        ...
        try {
          ...
          throw new TRPCError({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Unable to review sending access.",
          });
        }
      }),
    ...
    generate: protectedProcedure
      .input(
        z.object({
          ...
          scopes: z
            .array(z.enum(DEVELOPER_API_SCOPES))
            .min(1)
            .max(DEVELOPER_API_SCOPES.length)
            .default(["contacts:write"]),
          expiresInDays: z
            .number()
            .int()
            .min(1)
    ...
    rotate: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          return await rotateDeveloperApiKey({
            userId: ctx.user.id,
            keyId: input.id,
            label: input.label,
          });
  ...
  webhook: router({
    ...
    test: protectedProcedure
      .input(
        z.object({ id: z.number().int().positive(), url: z.string().url() })
      )
      .mutation(async ({ ctx, input }) => {
  ...
  analytics: router({
    ...
    trackManualZeroResultSearch: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.manualRole !== sessionRole) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Manual scope does not match the authenticated account.",
          });
        }
    ...
    trackCaptionLanguage: publicProcedure
      .input(
        z.object({ language: z.enum(CAPTION_LANGUAGE_ANALYTICS_LANGUAGES) })
      )
      .mutation(async ({ input }) => {
        const db = await getDb();
        if (!db) return { ok: true };
    ...
    trackPageView: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.insert(pageEvents).values({
          ...
          userAgent:
            (ctx as any).req?.headers?.["user-agent"]?.slice(0, 512) ?? null,
        });
        return { ok: true };
      }),
  }),
  ...
  referral: router({
    ...
    getStats: protectedProcedure.query(async ({ ctx }) => {
      const db = await getDb();
      if (!db)
        return { totalReferrals: 0, convertedReferrals: 0, monthsEarned: 0 };
      const rows = await db
        .select()
        .from(referrals)
        .where(eq(referrals.referrerUserId, ctx.user.id));
      const totalReferrals = rows.length;
      const convertedReferrals = rows.filter(
        r => r.convertedAt !== null
      ).length;
    ...
    claimReferral: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!referrerUserId || referrerUserId === ctx.user.id)
          return { ok: false };
  ...
  support: router({
    approveQuietHoursShortening: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!submission || submission.topic !== "quiet_hours_exception") {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Quiet-hours exception request not found.",
          });
        }
        ...
        if (!requester) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "The requesting user account no longer exists.",
          });
        }
        await db
          .update(businessProfiles)
          .set({
            ...
            quietHoursShorteningApprovedByUserId: input.approved
              ? ctx.user.id
              : null,
            updatedAt: new Date(),
          })
    ...
    uploadScreenshot: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          data.length === 0 ||
          data.length > MAX_SUPPORT_ATTACHMENT_BYTES ||
          !isValidSupportScreenshot(data, input.mimeType)
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
          ...
          attachment: z
            .object({
              ...
              size: z
                ...
                .int()
                .positive()
                .max(MAX_SUPPORT_ATTACHMENT_BYTES),
            })
            .optional(),
          website: z.string().max(250).optional(),
        })
    ...
    adminList: adminProcedure
      .input(
        z
          .object({
            ...
            assigneeUserId: z
              .union([z.literal("unassigned"), z.number().int().positive()])
              .optional(),
      ...
      .query(async ({ input }) => {
        ...
        if (!db)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Support inbox is unavailable.",
          });
        ...
        const selectedSlaWindow =
          ...
          (input?.slaDeadline === "overdue"
            ...
            : input?.slaDeadline === "next_24h"
              ...
              : undefined);
        if (
          selectedAssigneeScope === "specific" &&
          typeof input?.assigneeUserId !== "number"
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Choose an administrator for a specific-assignee queue filter.",
          });
        }
        const orderBy =
          selectedSort === "oldest"
            ...
            : selectedSort === "priority"
              ? [
                  desc(
                    sql`CASE ${supportSubmissions.priority} WHEN 'urgent' THEN 4 WHEN 'high' THEN 3 WHEN 'normal' THEN 2 ELSE 1 END`
                  ),
                  desc(supportSubmissions.createdAt),
                ]
              : selectedSort === "assignee"
                ? [
                    asc(
                      sql`CASE WHEN ${supportSubmissions.assigneeUserId} IS NULL THEN 1 ELSE 0 END`
                    ),
                    asc(users.name),
                    desc(supportSubmissions.createdAt),
                  ]
                : selectedSort === "sla_soonest"
                  ? [
                      asc(
                        sql`CASE WHEN ${supportSubmissions.slaTargetAt} IS NULL THEN 1 ELSE 0 END`
                      ),
                      asc(supportSubmissions.slaTargetAt),
                      desc(supportSubmissions.createdAt),
                    ]
                  : selectedSort === "due_soonest"
                    ? [
                        asc(
                          sql`CASE WHEN ${supportSubmissions.dueAt} IS NULL THEN 1 ELSE 0 END`
                        ),
                        asc(supportSubmissions.dueAt),
                        desc(supportSubmissions.createdAt),
                      ]
                    : [
                        desc(supportSubmissions.createdAt),
                        desc(supportSubmissions.id),
                      ];
        ...
        const rows = await db
          ...
          .where(
            and(
              input?.status
                ? eq(supportSubmissions.status, input.status)
                : sql`1 = 1`,
              input?.topic
                ? eq(supportSubmissions.topic, input.topic)
                : sql`1 = 1`,
              input?.priority
                ? eq(supportSubmissions.priority, input.priority)
                : sql`1 = 1`,
        ...
        return Promise.all(
          rows.map(async row => ({
            ...
            attachmentUrl: row.attachmentKey
              ? await storageGet(row.attachmentKey)
                  .then(attachment => attachment.url)
                  .catch(() => null)
              : null,
          }))
        );
    ...
    adminMetrics: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (!db)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Support reporting is unavailable.",
          });
        ...
        const rows = [
          {
            ...
            metric: "generated_at",
            period,
            value: metrics.generatedAt.toISOString(),
            unit: "iso_8601",
            details: "Get Phame support SLA performance export",
          },
          {
            section: "support_sla",
            ...
            unit: "tickets",
            details: "Tickets created during the selected period",
          },
          {
            section: "support_sla",
            ...
            unit: "tickets",
            details:
              "Tickets with a recorded resolution during the selected period",
          },
          {
            section: "support_sla",
            metric: "tickets_open",
            ...
            unit: "tickets",
            details: "Created-period tickets not currently resolved",
          },
          {
            section: "support_sla",
            metric: "first_response_samples",
            period,
            value: metrics.firstResponseCount,
            unit: "tickets",
            details: "Tickets with a recorded first response",
          },
          {
            section: "support_sla",
            metric: "average_first_response",
            period,
            value: metrics.avgFirstResponseMs ?? "unavailable",
            unit: "milliseconds",
            details: "Average elapsed time from creation to first response",
          },
          {
            section: "support_sla",
            metric: "average_resolution",
            period,
            value: metrics.avgResolutionMs ?? "unavailable",
            unit: "milliseconds",
            details: "Average elapsed time from creation to resolution",
          },
          {
            section: "support_sla",
            ...
            unit: "tickets",
            details: "Open tickets with an SLA target before export generation",
          },
        ];
    ...
    savedViews: adminProcedure.query(async ({ ctx }) => {
      ...
      if (!db)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Saved support queue views are unavailable.",
        });
      ...
      return db
        ...
        .orderBy(
          desc(supportSavedQueueViews.updatedAt),
          desc(supportSavedQueueViews.id)
        )
    ...
    saveView: adminProcedure
        ...
        z
          .object({
            name: z
              ...
              .trim()
              .min(1)
              .max(MAX_SUPPORT_SAVED_QUEUE_VIEW_NAME_CHARS),
            ...
            visibility: z
              .enum(SUPPORT_QUEUE_VIEW_VISIBILITIES)
              .default("private"),
          })
          .superRefine((value, issueContext) => {
            if (
              value.assigneeScope === "specific" &&
              ...
              issueContext.addIssue({
                ...
                path: ["assigneeUserId"],
                message:
                  "Choose an administrator for a specific-assignee view.",
              });
            }
          })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        if (!normalizedName)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Enter a name for this queue view.",
          });

        const assigneeUserId =
          input.assigneeScope === "specific"
        ...
        if (assigneeUserId !== null) {
          ...
          if (!assignee)
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "Choose an active administrator for this queue view.",
            });
        }

        const [existing] = await db
          .select({ id: supportSavedQueueViews.id })
          .from(supportSavedQueueViews)
          .where(
            and(
              eq(supportSavedQueueViews.ownerUserId, ctx.user.id),
        ...
        if (existing) {
          await db
            .update(supportSavedQueueViews)
            .set(viewFields)
            .where(
              and(
                eq(supportSavedQueueViews.id, existing.id),
                eq(supportSavedQueueViews.ownerUserId, ctx.user.id)
              )
            );
        ...
        const [ownedCount] = await db
          .select({ value: count(supportSavedQueueViews.id) })
          .from(supportSavedQueueViews)
          .where(eq(supportSavedQueueViews.ownerUserId, ctx.user.id));
        if (Number(ownedCount?.value ?? 0) >= MAX_SUPPORT_SAVED_QUEUE_VIEWS) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: `You can save up to ${MAX_SUPPORT_SAVED_QUEUE_VIEWS} support queue views.`,
          });
        }
        const inserted = await db
          .insert(supportSavedQueueViews)
          .values({
        ...
        if (!id)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Could not save this queue view.",
          });
    ...
    deleteView: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db
          .delete(supportSavedQueueViews)
          .where(
            and(
              eq(supportSavedQueueViews.id, input.id),
              eq(supportSavedQueueViews.ownerUserId, ctx.user.id)
            )
          );
        return { ok: true as const };
    ...
    checkSlaBreach: adminProcedure.mutation(async ({ ctx }) => {
      ...
      if (!db)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Support alerts are unavailable.",
        });

      const now = new Date();
      const policy = await getSupportEscalationPolicySettings(db);
      const breachCutoff = new Date(
        now.getTime() - policy.breachThresholdMinutes * 60 * 1000
      );
      ...
      for (const ticket of breachedTickets) {
        const recipientUserIds = Array.from(
          new Set([
            ...
            ...(policy.includeAssignee && ticket.assigneeUserId
              ? [ticket.assigneeUserId]
              : []),
            ...(!ticket.assigneeUserId && policy.includeAllAdminsWhenUnassigned
      ...
      return {
        checkedTicketCount: breachedTickets.length,
        alertedTicketCount,
        breachThresholdMinutes: policy.breachThresholdMinutes,
      };
    }),
    escalationPolicy: adminProcedure.query(async () => {
      ...
      if (!db)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Support escalation policy is unavailable.",
        });
    ...
    updateEscalationPolicy: adminProcedure
      .input(
        z
          .object({
            breachThresholdMinutes: z
              .number()
              .int()
              .min(0)
              .max(MAX_SUPPORT_ESCALATION_THRESHOLD_MINUTES),
          ...
          .superRefine((value, issueContext) => {
            if (
              ...
              !value.includeAllAdminsWhenUnassigned &&
              ...
              issueContext.addIssue({
                ...
                message:
                  "Keep at least one escalation recipient route enabled.",
              });
            }
          })
      )
      .mutation(async ({ input }) => {
        ...
        if (recipientUserIds.length > 0) {
          const validAdmins = await db
            ...
            .from(users)
            .where(
              and(eq(users.role, "admin"), inArray(users.id, recipientUserIds))
            );
          if (validAdmins.length !== recipientUserIds.length) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Choose only active administrators as escalation recipients.",
            });
          }
        }
        ...
        const updatedAt = new Date();
        await db
          .insert(supportEscalationPolicies)
          .values({
            ...
            includeAllAdminsWhenUnassigned:
              input.includeAllAdminsWhenUnassigned,
            updatedAt,
          })
          .onDuplicateKeyUpdate({
            set: {
        ...
        const [policy] = await db
          .select({ id: supportEscalationPolicies.id })
          .from(supportEscalationPolicies)
          .where(
            eq(
              supportEscalationPolicies.policyKey,
              SUPPORT_ESCALATION_POLICY_KEY
            )
          )
          .limit(1);
        if (!policy)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Could not save the support escalation policy.",
          });

        await db
          .delete(supportEscalationPolicyRecipients)
          .where(eq(supportEscalationPolicyRecipients.policyId, policy.id));
        if (recipientUserIds.length > 0) {
          await db.insert(supportEscalationPolicyRecipients).values(
    ...
    updateStatus: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.status !== "open")
          await recordSupportFirstResponse(db, input.id);
        return { ok: true as const };
      }),
    updatePriority: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const escalated = isSupportEscalation(
        ...
        await db
          ...
          .set({
            ...
            ...(escalated
              ? { slaTargetAt: getSupportSlaTargetAt(input.priority) }
              : {}),
          })
          .where(eq(supportSubmissions.id, input.id));
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.assigneeUserId !== null) {
          const [assignee] = await db
            ...
            .from(users)
            .where(
              and(eq(users.id, input.assigneeUserId), eq(users.role, "admin"))
            )
            .limit(1);
          if (!assignee) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Choose an active administrator as the support ticket assignee.",
            });
          }
        }
    ...
    updateDueAt: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (
          dueAt &&
          dueAt.getTime() >
            Date.now() + MAX_SUPPORT_DUE_DATE_FUTURE_DAYS * 24 * 60 * 60 * 1000
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
    ...
    internalNotes: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        const mentions =
          noteIds.length === 0
            ...
            : await db
                ...
                .leftJoin(
                  users,
                  eq(supportInternalNoteMentions.mentionedUserId, users.id)
                )
        ...
        return notes.map(note => ({
          ...
          bodyHtml:
            note.bodyPlainText !== null
              ? note.body
              : renderSupportInternalNoteHtml(note.body),
          displayText:
            note.bodyPlainText ?? getSupportInternalNotePlainText(note.body),
          mentions: mentions
    ...
    addInternalNote: adminProcedure
      .input(
        z.object({
          ...
          mentionUserIds: z
            .array(z.number().int().positive())
            .max(MAX_SUPPORT_INTERNAL_NOTE_MENTIONS)
            .default([]),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        if (mentionUserIds.length > 0) {
          const mentionableAdmins = await db
            .select({ id: users.id })
            .from(users)
            .where(
              and(inArray(users.id, mentionUserIds), eq(users.role, "admin"))
            );
          if (mentionableAdmins.length !== mentionUserIds.length) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "Mention only active administrators.",
            });
          }
        }
        ...
        if (!noteId)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Could not save internal note.",
          });
        ...
        if (mentionUserIds.length > 0) {
          ...
          await queueSupportTicketAlerts(db, {
            ...
            recipientUserIds: mentionUserIds.filter(
              mentionedUserId => mentionedUserId !== ctx.user.id
            ),
            actorUserId: ctx.user.id,
            type: "mention",
          });
    ...
    myTicketAlerts: adminProcedure.query(async ({ ctx }) => {
      ...
      return db
        ...
        .leftJoin(
          supportSubmissions,
          eq(supportTicketAlerts.ticketId, supportSubmissions.id)
        )
    ...
    getPreferences: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        const rows = await db
          .select()
          .from(leads)
          .where(eqP(leads.email, input.email.toLowerCase()))
          .limit(1);
      ...
      .mutation(async ({ input }) => {
        ...
        await db
          ...
          .set({
            ...
            unsubscribeReason: input.unsubscribe
              ? (input.reason ?? null)
              : null,
          })
          .where(eqP(leads.email, normalizedEmail));
