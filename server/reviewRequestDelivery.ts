...
export async function validateReviewRequestDelivery(
  ...
  if (!name || !email)
    throw new Error("A customer name and email address are required.");
  ...
  if (profile.tier === "free") {
    ...
    if (!decision.allowed)
      throw new Error(
        formatFreeQuotaBlockedMessage(decision.quota, FREE_LIMIT_ERR_MSG)
      );
  }
  ...
  if (params.platformId && !chosenPlatform)
    throw new Error("The selected review platform was not found.");
  const reviewValue = getReviewPlatformValue(
    chosenPlatform,
  ...
  if (!reviewValue)
    throw new Error(
      "A review destination must be configured before sending requests."
    );
  ...
  if (params.templateId && !selectedTemplate)
    throw new Error("The selected email template was not found.");
  const revision = selectedTemplate
    ? await getLatestApprovedTemplateRevision({
  ...
  if (locale !== "en" && !revision) {
    throw new Error(
      `An approved ${locale} template is required before sending in that language.`
    );
  }
...
export async function deliverReviewRequest(
  ...
  if (params.sourceConnectionId && params.sourceEventId) {
    ...
    if (existingRequest) {
      if (
        ...
        existingRequest.status === "followed_up"
      ) {
        return {
          ...
          preferredLocale: outreachLocaleSchema.parse(
            existingRequest.preferredLocale
          ),
      ...
      if (
        existingQueueItem &&
        ["pending", "sending"].includes(existingQueueItem.status)
      ) {
        return {
          requestId: existingRequest.id,
  ...
  const platformLinks = buildSafePlatformLinks(
    allPlatforms,
    profile.businessName,
    reviewValue
  );
  ...
  const htmlBody = wrapPlainTextReviewRequestHtml(
    renderReviewRequestDraft(copy.body, context)
  );
  ...
  const trackedHtml = isTrackableHttpUrl(reviewValue)
    ? htmlBody.replace(
        new RegExp(reviewValue.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"),
        trackedReviewValue
      )
    : htmlBody;
  const trackedHtmlBody = trackedHtml.replace(
    /<\/div>\s*$/,
    `${openPixel}</div>`
  );
