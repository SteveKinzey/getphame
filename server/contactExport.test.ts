...
describe("contact CSV and PDF export snapshot", () => {
  ...
  it("exports only requested contacts present in the authenticated user's supplied dataset", () => {
    ...
    expect(snapshot.filename).toBe(
      "get-phame-contacts-2026-01-05_to_2026-01-05.csv"
    );
  ...
  it("caps PDF snapshots and never duplicates CSV content in the PDF response", () => {
    const contacts = Array.from(
      { length: CONTACT_PDF_EXPORT_LIMIT + 1 },
  ...
  it("keeps both protected procedures behind the authenticated user's contact list", () => {
    ...
    const contactBlock = routers.slice(
      routers.indexOf("contacts: router({"),
      routers.indexOf("getDailyStatus:", routers.indexOf("contacts: router({"))
    );
    ...
    expect(
      contactBlock.match(/listSavedContacts\(ctx\.user\.id\)/g)
    ).toHaveLength(3);
    expect(contactBlock).toMatch(
      /contactIds:\s*z\s*\.array\(\s*z\.number\(\)\.int\(\)\.positive\(\),?\s*\)\s*\.min\(1\)\s*\.max\(CONTACT_CSV_EXPORT_LIMIT\)/
    );
  });
});
