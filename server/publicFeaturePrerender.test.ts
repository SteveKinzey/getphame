...
import {
  renderPublicFeatureHtml,
  shouldPrerenderUserAgent,
} from "./publicFeaturePrerender";
...
describe("public feature prerender", () => {
  it("injects route-specific metadata, visible FAQ schema, comparison content, and related feature links into the root shell", () => {
    ...
    expect(html).toContain(
      "<title>Review Request Software for Local Businesses | Get Phame</title>"
    );
    expect(html).toContain(
      'property="og:url" content="https://getphame.app/review-requests"'
    );
    expect(html).toContain(
      'property="og:image" content="https://files.manuscdn.com/user_upload_by_module/session_file/310519663507659115/UhORQpjNTcwKpeBp.webp"'
    );
