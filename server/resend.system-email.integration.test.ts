...
const hasSystemEmailConfig = requiredEnv.every(key =>
  Boolean(process.env[key])
);
const shouldVerifySystemEmailIntegration =
  process.env.RUN_SYSTEM_EMAIL_INTEGRATION === "true";

describe.runIf(shouldVerifySystemEmailIntegration && hasSystemEmailConfig)(
  "Resend system-email integration",
  () => {
    it("authenticates the supplied API key and verifies the SMTP transport", async () => {
      ...
      if (domainsResponse.ok) {
        ...
        expect(
          domain,
          "getphame.app must exist in Resend Domains"
        ).toBeDefined();
        expect(
          domain?.status,
          "getphame.app must be verified before sending"
        ).toBe("verified");
      }
...
describe("Resend system-email integration configuration", () => {
  it.skipIf(!shouldVerifySystemEmailIntegration)(
    ...
    () => {
      ...
      expect(
        missing,
        `Missing system email variables: ${missing.join(", ")}`
      ).toEqual([]);
    }
  );
});
