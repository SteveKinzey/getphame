...
describe("WordPress self-service pairing", () => {
  ...
  it("fails closed when the stored digest is malformed", async () => {
    const state: State = {
      pairing: pendingPairing({ secretHash: "not-hex" }),
      inserted: null,
    };
    mocks.getDb.mockResolvedValue(createDb(state));
  ...
  it("preserves explicit lifecycle errors after a valid constant-time secret check", async () => {
    const expired = pendingPairing({ expiresAt: Date.now() - 1 });
