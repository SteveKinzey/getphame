...
export function buildReviewRequestEmail(opts: ReviewEmailOptions): string {
  const { customerName, businessName, reviewUrl, bodyHtml, productName } = opts;
...
export function buildReviewRequestText(opts: ReviewEmailOptions): string {
  const { customerName, businessName, reviewUrl, productName } = opts;
  ...
  return `Hi ${customerName}!\n\n${context}\n\nCould you take 30 seconds to leave us a quick review?\n\n${reviewUrl}\n\nThank you so much!\nThe ${businessName} team\n\n---\nYou received this email because you are a customer of ${businessName}. To unsubscribe, reply with "unsubscribe".`;
}
