...
import {
  buildDeveloperApiAbuseRules,
  DEVELOPER_API_ABUSE_SUSPENSION_MS,
...
describe("developer API abuse safeguards", () => {
  it("layers key, account, IP, daily, and recipient controls around review-request sending", () => {
    ...
    expect(
      rules.find(rule => rule.dimension === "recipient_daily")
    ).toMatchObject({ limit: 2, suspendOnBreach: false });
    expect(
      rules.find(rule => rule.dimension === "account_daily")
    ).toMatchObject({ limit: 100, suspendOnBreach: true });
  ...
  it("uses separate import limits and never persists raw IP or recipient dimensions", () => {
    ...
    expect(
      rules.find(rule => rule.dimension === "recipient_daily")
    ).toMatchObject({ limit: 8, suspendOnBreach: false });

    const source = fs.readFileSync(
      path.resolve(process.cwd(), "server/developerApiAbuse.ts"),
      "utf8"
    );
    const insertBlock = source.slice(
      source.indexOf("db.insert(apiAbuseLimitWindows)"),
      source.indexOf(
        "return { allowed: true",
        source.indexOf("db.insert(apiAbuseLimitWindows)")
      )
  ...
  it("requires abuse checks before contact persistence and email delivery, with successful-use tracking after side effects", () => {
    const source = fs.readFileSync(
      path.resolve(process.cwd(), "server/publicApi.ts"),
      "utf8"
    );
    const importAbuseIndex = source.indexOf('action: "contact_import"');
    const contactWriteIndex = source.indexOf(
      "upsertApiContact",
      importAbuseIndex
    );
    const sendAbuseIndex = source.indexOf('action: "review_request_send"');
    const emailSendIndex = source.indexOf(
      "deliverReviewEmailOrQueue",
      sendAbuseIndex
    );
    const successUseIndex = source.lastIndexOf(
