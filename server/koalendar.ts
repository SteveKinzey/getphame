...
const koalendarPayloadSchema = z.object({
  ...
  type: z.enum(["event.created", "event.tentative", "event.rescheduled", "event.canceled"]),
  invitee: z.object({
...
function parseIsoTimestamp(value: string, field: string): number {
  ...
  if (!Number.isFinite(timestamp)) throw new Error(`${field} must be a valid ISO date-time.`);
  return timestamp;
}
...
export async function getKoalendarConnectionStatus(userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const [connection] = await db.select().from(koalendarConnections)
    .where(eq(koalendarConnections.userId, userId)).limit(1);
...
export async function connectKoalendar(userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const [existing] = await db.select().from(koalendarConnections)
    .where(eq(koalendarConnections.userId, userId)).limit(1);
  if (existing) {
    await db.update(koalendarConnections).set({ enabled: true, updatedAt: Date.now() })
      .where(eq(koalendarConnections.id, existing.id));
    return { webhookUrl: buildKoalendarWebhookUrl(existing.webhookToken) };
  }
  const webhookToken = randomBytes(24).toString("hex");
  await db.insert(koalendarConnections).values({ userId, webhookToken, enabled: true });
...
export async function rotateKoalendarWebhook(userId: number) {
  ...
  if (!db) throw new Error("Database not available");
  const webhookToken = randomBytes(24).toString("hex");
  const [result] = await db.update(koalendarConnections)
...
export async function disconnectKoalendar(userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(koalendarConnections).set({ enabled: false, updatedAt: Date.now() })
    .where(eq(koalendarConnections.userId, userId));
  return { success: true };
}
...
export async function ingestKoalendarPayload(token: string, rawPayload: unknown) {
  ...
  if (!db) throw new Error("Database not available");
  const [connection] = await db.select().from(koalendarConnections)
    .where(and(eq(koalendarConnections.webhookToken, token), eq(koalendarConnections.enabled, true))).limit(1);
  if (!connection) return { accepted: false as const, statusCode: 404, error: "Koalendar connection not found." };
  ...
  try {
    ...
    if (endsAt <= startsAt) throw new Error("end_at must be later than start_at.");
    canceledAt = payload.type === "event.canceled" || Boolean(payload.canceled_at)
      ? payload.canceled_at ? parseIsoTimestamp(payload.canceled_at, "canceled_at") : Date.now()
      : null;
  } catch (error) {
    return { accepted: false as const, statusCode: 400, error: error instanceof Error ? error.message : "Invalid scheduling data." };
  }

  const externalBookingId = bookingKey(payload);
  const now = Date.now();
  const [existing] = await db.select().from(koalendarBookings)
    .where(and(eq(koalendarBookings.userId, connection.userId), eq(koalendarBookings.externalBookingId, externalBookingId)))
    .limit(1);
  ...
  if (existing) {
    await db.update(koalendarBookings).set(values).where(eq(koalendarBookings.id, existing.id));
  } else {
    await db.insert(koalendarBookings).values({ userId: connection.userId, externalBookingId, ...values });
  }
  await db.update(koalendarConnections).set({ lastEventAt: now, updatedAt: now })
...
export async function processDueKoalendarBookings(limit = 100): Promise<number> {
  const db = await getDb();
  if (!db) return 0;
  const due = await db.select().from(koalendarBookings)
    .where(and(eq(koalendarBookings.status, "pending"), lte(koalendarBookings.nextAttemptAt, Date.now())))
    .orderBy(koalendarBookings.nextAttemptAt).limit(limit);
  let processed = 0;
  for (const booking of due) {
    const [claim] = await db.update(koalendarBookings).set({ status: "processing", updatedAt: Date.now() })
...
export function registerKoalendarRoutes(app: Express) {
  app.post("/api/integrations/koalendar/:token", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!result.accepted) return res.status(result.statusCode).json({ error: result.error });
