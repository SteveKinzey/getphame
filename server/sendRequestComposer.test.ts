...
describe("individual Send Request composer", () => {
  it("keeps send-time edits local, renders recipient variables, and sends only after final review", () => {
    ...
    expect(source).toContain(
      "renderReviewRequestDraft(draftSubject, draftContext)"
    );
    expect(source).toContain(
      "renderReviewRequestDraft(draftBody, draftContext)"
    );
    expect(source).toContain(
      "containsDirectYelpLink(`${draftSubject}\\n${draftBody}`)"
    );
  ...
  it("validates paired edited copy and confirmation on the server, then stores the exact rendered message", () => {
    ...
    expect(source).toMatch(/editedSubject:\s*z\s*\.string\(\)/);
    expect(source).toContain(".max(MAX_REVIEW_REQUEST_SUBJECT_CHARS)");
    expect(source).toMatch(
      /\.max\(MAX_REVIEW_REQUEST_BODY_CHARS\)\s*\.optional\(\)/
    );
    expect(source).toContain(
      "Edited subject and body must be submitted together"
    );
    expect(source).toContain(
      "Confirm the compliance checklist before sending edited copy"
    );
    expect(source).toMatch(/subject\s*=\s*renderDraft\(input\.editedSubject\)/);
    expect(source).toMatch(
      /htmlBody\s*=\s*wrapPlainTextReviewRequestHtml\(\s*renderDraft\(input\.editedBody\)\s*\)/
    );
  ...
  it.each(locales)(
    "provides every composer key in the %s catalog and runtime fallback",
    locale => {
      const catalog = JSON.parse(
        projectFile(`client/public/locales/${locale}/translation.json`)
      ) as Record<string, unknown>;
      const fallback = directKeyFallbackResources[locale] as Record<
        string,
        unknown
      >;
      ...
      for (const key of requiredComposerKeys) {
        ...
        expect(fallbackValue, `fallback:${locale}:${key}`).toBe(
          maintainedValue
        );
      }
    }
