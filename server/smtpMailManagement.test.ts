...
describe("saved SMTP mail-management safeguards", () => {
  it("validates and rate-limits a chosen recipient before delivering through saved tenant SMTP", () => {
    const procedure = routerSource.slice(
      routerSource.indexOf("sendTestEmail: protectedProcedure"),
      routerSource.indexOf("previewEmail: protectedProcedure")
    );
    expect(procedure).toContain(
      "z.object({ to: z.string().trim().email().max(320) })"
    );
  ...
  it("retains a confirmed disconnect path that deletes credentials and clears personal delivery selection", () => {
    ...
    expect(smtpSource).toContain(
      "clearPersonalDeliveryChannel ?? clearOutboundDeliveryChannel"
    );
