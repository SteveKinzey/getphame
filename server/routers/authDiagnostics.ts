...
function validateHealthHistoryRange(value: { fromMs?: number | null; toMs?: number | null }, ctx: z.RefinementCtx) {
  if (value.fromMs !== undefined && value.fromMs !== null && value.toMs !== undefined && value.toMs !== null) {
    if (value.fromMs > value.toMs) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["toMs"], message: "The end date must not be before the start date." });
    if (value.toMs - value.fromMs > MAX_AUTH_HEALTH_HISTORY_RANGE_MS) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["toMs"], message: "Choose a date range of 366 days or less." });
  }
}
...
const healthHistoryPresetSchema = z.object({
  id: z.number().int().positive().optional(),
  name: z.string().trim().min(1).max(MAX_AUTH_HEALTH_HISTORY_PRESET_NAME_CHARS),
  status: z.enum(["ok", "fail"] as const).nullable().optional(),
  triggerSource: z.enum(["scheduled", "manual"] as const).nullable().optional(),
...
export const authDiagnosticsRouter = router({
  ...
  saveHealthHistoryPreset: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (result.outcome === "name_conflict") throw new TRPCError({ code: "CONFLICT", message: "You already have a preset with this name." });
