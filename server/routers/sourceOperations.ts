...
function deliveryReadinessCode(error: unknown) {
  ...
  if (message.includes("Business profile"))
    return "PROFILE_NOT_CONFIGURED" as const;
  if (message.includes("SMTP")) return "SMTP_NOT_CONFIGURED" as const;
  if (message.includes("quota") || message.includes("limit"))
    return "QUOTA_LIMITED" as const;
  if (message.includes("platform") || message.includes("destination"))
    return "PLATFORM_NOT_READY" as const;
  if (message.includes("template") || message.includes("approved"))
    return "TEMPLATE_NOT_READY" as const;
...
const FIELD_MAPPING = [
  {
    field: "name",
    required: true,
    description: "Customer name from the trigger record.",
  },
  {
    field: "email",
    required: true,
    description: "Customer email from the trigger record.",
  },
  {
    field: "phone",
    required: false,
    description: "Customer phone, when available.",
  },
  {
    field: "externalId",
    required: false,
    description: "Stable order, form, or customer ID for traceability.",
  },
  {
    field: "notes",
    required: false,
    description:
      "Short operational context; do not include secrets or payment data.",
  },
  {
    field: "tags",
    required: false,
    description: "Up to 20 source or workflow tags.",
  },
] as const;
...
export const sourceOperationsRouter = router({
  setupManifest: protectedProcedure.query(async ({ ctx }) => {
    ...
    return {
      ...
      apiKeys: keys
        .filter(
          key =>
            key.status === "active" && key.scopes.includes("contacts:write")
        )
        .map(key => ({
          id: key.id,
          label: key.label,
  ...
  list: protectedProcedure.query(async ({ ctx }) =>
    listSourceConnectionsForUser(ctx.user.id)
  ),
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!source) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Choose an active API key with the contacts:write scope.",
        });
      }
      return source;
    }),
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        ...
        input.automationMode === "review_request"
      ) {
        const key = (await listDeveloperApiKeys(ctx.user.id)).find(
          candidate =>
            candidate.id === existing.apiKeyId && candidate.status === "active"
        );
        if (!key?.scopes.includes("review_requests:send")) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Choose a source API key with the review_requests:send scope before enabling automation.",
          });
        }
      }
      if (input.dryRun === false && !existing.dryRunCompletedAt) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "Complete one safe provider dry run before enabling live delivery.",
        });
      }
      const source = await updateSourceConnection({
        userId: ctx.user.id,
        ...input,
      });
      if (!source)
  ...
  refreshHealth: protectedProcedure
    .input(sourceIdInput)
    .mutation(async ({ ctx, input }) => {
      ...
      if (!source.monitoringEnabled) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Enable monitoring before refreshing health.",
        });
      }
  ...
  preflight: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const key =
        keys.find(
          (candidate: { id: number; status: string; scopes: string[] }) =>
      ...
      let delivery: Awaited<
        ReturnType<typeof validateReviewRequestDelivery>
      > | null = null;
      ...
      try {
        delivery = await validateReviewRequestDelivery({
          ...
          preferredLocale: outreachLocaleSchema.parse(
            source.preferredLocale ?? "en"
          ),
