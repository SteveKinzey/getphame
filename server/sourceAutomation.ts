...
import { consentTextHash, type ReviewOutreachConsent } from "./integrationExpansion";
...
export async function claimSourceAutomationEvent(params: {
  ...
  const lookup = async () => {
    const [event] = await db.select().from(sourceAutomationEvents).where(and(
      eq(sourceAutomationEvents.userId, params.userId),
      eq(sourceAutomationEvents.sourceConnectionId, params.sourceConnectionId),
  ...
  if (existing) {
    if (existing.requestHash !== params.requestHash) return { kind: "conflict" as const, event: existing };
    return { kind: "replay" as const, event: existing };
  }
  try {
    const now = Date.now();
    const [result] = await db.insert(sourceAutomationEvents).values({
    ...
    if (raced.requestHash !== params.requestHash) return { kind: "conflict" as const, event: raced };
...
export async function completeSourceAutomationEvent(params: {
  ...
  if (!db) throw new Error("Database not available");
  const now = Date.now();
  await db.update(sourceAutomationEvents).set({
    ...
    claimExpiresAt: null,
    ...(params.status === "scheduled" ? { attemptCount: 0, lastAttemptAt: null } : {}),
...
export async function getSourceAutomationScheduler() {
  const db = await getDb();
  if (!db) return null;
  const [row] = await db.select().from(sourceAutomationSchedulers)
    .where(eq(sourceAutomationSchedulers.scheduleKey, SOURCE_AUTOMATION_SCHEDULE_KEY))
    .limit(1);
  return row ?? null;
}
...
export async function saveSourceAutomationSchedulerTaskUid(taskUid: string, now = Date.now()) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.insert(sourceAutomationSchedulers).values({
...
export async function claimSourceAutomationSchedulerRun(
  ...
  const db = await getDb();
  if (!db) return false;
  const result = await db.update(sourceAutomationSchedulers).set({
    lastRunAt: now,
...
export async function recordSourceAutomationSchedulerRun(params: {
  ...
  await db.update(sourceAutomationSchedulers).set({
    lastRunAt: now,
...
export async function claimDueSourceAutomationEvent(now = Date.now()) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const [candidate] = await db.select().from(sourceAutomationEvents).where(or(
    and(
      eq(sourceAutomationEvents.status, "scheduled"),
      lte(sourceAutomationEvents.scheduledAt, now),
      or(isNull(sourceAutomationEvents.claimExpiresAt), lte(sourceAutomationEvents.claimExpiresAt, now)),
    ),
    and(
      eq(sourceAutomationEvents.status, "processing"),
      lte(sourceAutomationEvents.claimExpiresAt, now),
    ),
  )).orderBy(asc(sourceAutomationEvents.scheduledAt), asc(sourceAutomationEvents.id)).limit(1);
  if (!candidate) return null;
  const claimExpiresAt = now + SOURCE_AUTOMATION_CLAIM_MS;
  const claimResult = await db.update(sourceAutomationEvents).set({
    status: "processing",
    lastAttemptAt: now,
    claimExpiresAt,
  ...
  const [claimed] = await db.select().from(sourceAutomationEvents).where(and(
    eq(sourceAutomationEvents.id, candidate.id),
    eq(sourceAutomationEvents.status, "processing"),
...
export async function getSourceAutomationDeliveryContext(eventId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const [event] = await db.select().from(sourceAutomationEvents).where(eq(sourceAutomationEvents.id, eventId)).limit(1);
  if (!event || !event.contactId) return null;
  const [contact] = await db.select().from(savedContacts).where(and(
    eq(savedContacts.id, event.contactId),
    eq(savedContacts.userId, event.userId),
  )).limit(1);
  const [source] = await db.select().from(sourceConnections).where(and(
...
export async function retryOrFailSourceAutomationEvent(params: {
  ...
  const scheduledAt = failed
    ? null
    : now + Math.min(60, 5 * (2 ** Math.max(0, params.attemptCount - 1))) * 60_000;
  await db.update(sourceAutomationEvents).set({
...
export async function recordConsentEvidence(params: {
  ...
  const [existing] = await db.select().from(contactConsentEvidence).where(and(
    eq(contactConsentEvidence.userId, params.userId),
    eq(contactConsentEvidence.sourceSubmissionId, params.sourceSubmissionId),
    eq(contactConsentEvidence.purpose, params.consent.purpose),
  )).limit(1);
  if (existing) {
    const same = existing.consentTextHash === hash
      && existing.consentVersion === params.consent.version
      && existing.contactId === params.contactId;
    if (!same) throw new Error("Consent evidence conflicts with the existing source submission.");
    return existing;
  }
  const [result] = await db.insert(contactConsentEvidence).values({
