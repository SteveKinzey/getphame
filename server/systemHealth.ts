...
import { authHealthChecks, smtpHealthSnapshots, type InsertSmtpHealthSnapshot } from "../drizzle/schema";
...
export async function createSmtpHealthSnapshot(input: InsertSmtpHealthSnapshot) {
...
export async function getRecentSmtpSnapshotForTask(taskUid: string, sinceMs: number) {
  const db = await getDb();
  if (!db) return null;
  const rows = await db
    .select()
...
export async function getSystemHealthTrend(hours = 24) {
  const db = await getDb();
  if (!db) return { windowHours: hours, since: Date.now() - hours * 60 * 60 * 1000, smtp: [], authentication: [] };
  const since = Date.now() - hours * 60 * 60 * 1000;
  const [smtpRows, authRows] = await Promise.all([
    db.select().from(smtpHealthSnapshots)
  ...
  return {
    ...
    smtp: smtpRows.map((row) => ({
      ...
      successRate: row.totalAccounts > 0
...
export function evaluateOperationsAlertState(
  ...
  const reminderSent = reminderRows.reduce((total, row) => total + row.sentCount, 0);
  const reminderSuccesses = reminderRows.reduce((total, row) => total + row.successCount, 0);
  const reminderSuccessRate = reminderSent > 0
    ? Math.round((reminderSuccesses / reminderSent) * 1000) / 10
    : null;

  return {
    smtp: {
      status: smtpSuccessRate !== null && smtpSuccessRate < SMTP_HEALTH_MIN_PERCENT ? "alert" : "ok",
    ...
    reminders: {
      status: reminderSent >= REMINDER_PERFORMANCE_MIN_SAMPLE
