...
export async function checkDeveloperApiRateLimit(
  ...
  const windowStartedAt =
    Math.floor(now / DEVELOPER_API_RATE_WINDOW_MS) *
    DEVELOPER_API_RATE_WINDOW_MS;
  await db
  ...
  if (!existing) {
    ...
    return {
      allowed: true,
      remaining: DEVELOPER_API_RATE_LIMIT - 1,
      retryAfterSeconds: 0,
    };
  }
  ...
  const [updated] = await db
    .select({
      requestCount: apiRateLimitWindows.requestCount,
  ...
  return {
    ...
    retryAfterSeconds: allowed
      ...
      : Math.max(
          1,
          Math.ceil(((updated?.expiresAt ?? existing.expiresAt) - now) / 1000)
        ),
  };
}
...
export async function getDeveloperApiIdempotency(params: {
  ...
  const keyHash = fingerprintAuthValue(
    `developer-api-idempotency:${params.principal.apiKeyId}:${params.idempotencyKey}`
  );
  const [row] = await db
    .select()
  ...
  if (row.expiresAt <= now) {
    await db
      ...
      .where(eq(apiIdempotencyRecords.id, row.id));
    return { kind: "missing" as const, keyHash };
  }
  if (row.payloadHash !== params.requestHash)
    return { kind: "conflict" as const, keyHash };
  try {
    return {
...
export async function saveDeveloperApiIdempotency(params: {
  ...
  try {
    await db.insert(apiIdempotencyRecords).values({
      ...
      contactId:
        typeof params.response.contactId === "number"
          ? params.response.contactId
          : null,
    ...
    const [existing] = await db
      ...
      .limit(1);
    if (!existing || existing.payloadHash !== params.requestHash)
