...
const smtpSource = readFileSync(
  resolve(process.cwd(), "server/smtp.ts"),
  "utf8"
);
...
describe("customer outreach sender routing", () => {
  ...
  it.each([
    ...
    "rejects a Get Phame platform sender for customer outreach: %s",
    sender => {
      expect(() => assertCustomerOutreachSender(sender)).toThrow(
        /connected personal or business email address/i
      );
    }
  );
  ...
  it("validates public SMTP destinations and returns sanitized recovery messages", () => {
    ...
    expect(smtpSource).toContain(
      "Private, local, or reserved SMTP destinations are not allowed."
    );
    expect(smtpSource).toContain(
      "Authentication failed. Check the provider username and app password or SMTP password."
    );
    expect(testConnectionBlock).not.toContain(
