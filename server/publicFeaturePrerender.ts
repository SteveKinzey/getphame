...
const CRAWLER_USER_AGENT =
  /googlebot|bingbot|yandexbot|baiduspider|duckduckbot|facebookexternalhit|twitterbot|linkedinbot|slackbot|discordbot|whatsapp|pinterestbot|applebot|semrushbot/i;
  ...
  "/email-campaigns": {
    ...
    comparisonRows: [
      ...
      },
      {
        ...
        commonApproach:
          "Combine signals from an email provider and manual notes",
      },
      {
    ...
    comparisonRows: [
      {
        ...
        getPhame:
          "Keep outreach, destinations, and engagement in one workspace",
        commonApproach:
...
function absoluteUrl(value: string) {
  return /^https?:\/\//i.test(value)
    ? value
    : `${PUBLIC_SITE_ORIGIN}${value.startsWith("/") ? value : `/${value}`}`;
}
...
function comparisonMarkup(feature: PrerenderFeature) {
  const rows = feature.comparisonRows
    .map(
      row =>
        `<tr><th scope="row">${escapeHtml(row.consideration)}</th><td>${escapeHtml(row.getPhame)}</td><td>${escapeHtml(row.commonApproach)}</td></tr>`
    )
    .join("");
...
function relatedFeatureMarkup(feature: PrerenderFeature) {
  const links = Object.values(PUBLIC_FEATURES)
    ...
    .map(
      candidate =>
        `<li><a href="${escapeHtml(candidate.route)}"><strong>${escapeHtml(candidate.label)}</strong><span>${escapeHtml(candidate.introduction)}</span></a></li>`
    )
    .join("");
...
export function renderPublicFeatureHtml(
  ...
  html = html.replace(
    /<script id="getphame-feature-faq-jsonld"[\s\S]*?<\/script>\s*/gi,
    ""
  );
  const schema = JSON.stringify(faqSchema(feature)).replace(/</g, "\\u003c");
  html = html.replace(
    "</head>",
    `  <script id="getphame-feature-faq-jsonld" type="application/ld+json">${schema}</script>\n</head>`
  );
  ...
  app.get(routes, async (req, res, next) => {
    ...
    try {
      ...
      res
        ...
        .send(renderPublicFeatureHtml(template, feature));
    } catch (error) {
      next(error);
    }
  });
