...
vi.mock("./disposableDomains", () => ({
  ...
  getDisposableDomainSchedulerByTaskUid:
    mocks.getDisposableDomainSchedulerByTaskUid,
...
describe("scheduled disposable-domain callback", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getDisposableDomainSchedulerByTaskUid.mockResolvedValue({
      scheduleCronTaskUid: "disposable-task-1",
    });
    ...
    mocks.syncDisposableEmailDomains.mockResolvedValue({
      processed: 3,
      deactivated: 0,
    });
  ...
  it("skips ordinary executions outside the Pacific 2 AM window", async () => {
    ...
    mocks.getPacificScheduleDecision.mockReturnValue({
      ...NORMAL_SCHEDULE,
      localHour: 3,
      shouldRun: false,
    });
  ...
  it("uses the persisted claim to deduplicate platform retries", async () => {
    ...
    expect(mocks.claimDisposableDomainSchedulerRun).toHaveBeenCalledWith(
      "disposable-task-1"
    );
    expect(mocks.syncDisposableEmailDomains).not.toHaveBeenCalled();
  });
  ...
  it("records a generic failure without exposing upstream details", async () => {
    ...
    mocks.syncDisposableEmailDomains.mockRejectedValue(
      new Error("upstream credential secret@example.test")
    );
    const response = await request(buildApp()).post(
      "/api/scheduled/disposable-domains"
    );
