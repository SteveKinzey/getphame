...
export type RetrievedHelpSource = Pick<
  HelpSource,
...
const HELP_SOURCES: HelpSource[] = [
  {
    ...
    href: "/",
    content:
      "Set up Get Phame in three stages: connect and test a sending email, add at least one review destination in Settings, then open Send Request to contact an eligible customer. The onboarding guide can be reopened from the app if a setup step was skipped.",
    keywords: [
      "start",
      "setup",
      "onboarding",
  ...
  },
  {
    ...
    href: "/settings",
    content:
      "Get Phame sends through the business email account the account owner connects. Settings provides guided provider detection, provider-specific credential guidance, a connection test, and a Custom SMTP fallback. Many providers require an app password instead of the normal mailbox password. Credentials are handled by the secure connection form; never paste a password, API key, or token into the help assistant.",
    keywords: [
      "email",
      "smtp",
      "gmail",
  ...
  },
  {
    ...
    href: "/reminders",
    content:
      "The Reminders workspace shows scheduled follow-ups and provides cancellation controls. Keep follow-ups measured and relevant. Future reminders must stop when the requested customer action completes, and a user can cancel a scheduled reminder from the Reminders workspace.",
    keywords: [
      "reminder",
      "follow-up",
      "followup",
  ...
  },
  {
    ...
    href: "/developer",
    content:
      "Developer Integrations provides scoped API-key creation, one-time secret display, key rotation and revocation, the versioned customer-import endpoint, consent-aware field mappings, privacy-safe recent-import history, and setup guides for supported form builders. Store API secrets only in a server-side secret field; never put them in browser JavaScript, page markup, screenshots, or this assistant.",
    keywords: [
      "api",
      "developer",
      "integration",
...
export function tokenizeHelpQuery(value: string): string[] {
  const localizedTokenPattern = new RegExp(
...
export function retrieveHelpSources(
  question: string,
  limit = 4
): RetrievedHelpSource[] {
...
export function redactHelpQuestion(question: string): {
  text: string;
  redacted: boolean;
} {
  let text = question;
