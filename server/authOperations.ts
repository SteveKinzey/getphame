...
export function redactAuthDiagnosticDetail(error: unknown): string {
  ...
  return raw
    ...
    .replace(
      /[?&](token|code|key|secret|password|pass)=([^&\s]+)/gi,
      "$1=[redacted]"
    )
...
export async function recordAuthLifecycleEvent(input: {
  ...
  try {
    ...
    console.warn(
      "[AuthDiagnostics] Event persistence failed:",
...
export async function findAuthRequestByToken(token: string) {
  try {
    return await getLatestAuthDiagnosticByTokenFingerprint(
      fingerprintAuthValue(`token:${token}`)
    );
  } catch (error) {
    console.warn(
      "[AuthDiagnostics] Correlation lookup failed:",
      redactAuthDiagnosticDetail(error)
    );
    return null;
  }
      ...
      new Promise<{ ok: false; error: string }>(resolve => {
        timeout = setTimeout(
          () =>
            resolve({
              ok: false,
              error: "Email provider verification timed out",
            }),
...
export async function runAuthHealthCheck(options: AuthHealthRunOptions) {
  ...
  const missingConfig = requiredConfig
    .filter(([, value]) => !value)
    .map(([name]) => name);
  const configStatus: "ok" | "fail" =
    missingConfig.length === 0 ? "ok" : "fail";
  ...
  if (!database) {
    ...
    try {
      ...
      failures.push({
        ...
        detail: redactAuthDiagnosticDetail(error),
      });
    }
    try {
      await database
      ...
      failures.push({
        code: "magic_link_schema_invalid",
        detail: redactAuthDiagnosticDetail(error),
      });
    }
  }
  ...
  try {
    sessionStatus =
      fingerprintAuthValue("getphame-auth-health-session-signing").length === 64
        ? "ok"
        : "fail";
  } catch {
    sessionStatus = "fail";
  ...
  if (
    sessionStatus === "fail" &&
    !failures.some(failure => failure.code === "configuration_invalid")
  ) {
    failures.push({
      code: "session_signing_invalid",
      detail: "Session signing configuration is unavailable",
    });
  }
  ...
  const result: InsertAuthHealthCheck = {
    ...
    failureDetail:
      failures
        ...
        .join(" | ")
        .slice(0, SAFE_DETAIL_LIMIT) || null,
  ...
  try {
    ...
    console.warn(
      "[AuthHealth] Result persistence failed:",
      redactAuthDiagnosticDetail(error)
    );
  }
