...
describe("rendered Free-plan quota states", () => {
  it("renders the initial allowance state", () => {
    const html = render({ phase: "initial", limit: 10, used: 9, remaining: 1, totalSent: 9, blocked: false, nextAvailableAt: null });
  ...
  it("renders the rolling allowance state", () => {
    const html = render({ phase: "rolling", limit: 5, used: 1, remaining: 4, totalSent: 11, blocked: false, nextAvailableAt: null });
  ...
  it("renders the blocked state and next available date", () => {
    const html = render({ phase: "rolling", limit: 5, used: 5, remaining: 0, totalSent: 15, blocked: true, nextAvailableAt: Date.UTC(2026, 6, 20) });
