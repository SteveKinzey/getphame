...
import { normalizeContactImportPayload, registerPublicApiRoutes } from "./publicApi";

describe("developer API key security helpers", () => {
  it("parses only supported scopes and removes duplicates", () => {
    expect(parseDeveloperApiScopes('["contacts:write","contacts:write","unknown"]')).toEqual(["contacts:write"]);
  ...
  it("enforces exact scopes from the authenticated principal", () => {
    ...
    expect(developerApiKeyHasScope(principal, "review_requests:send")).toBe(false);
  });
  ...
  it("expires keys after twelve months without a successful use and resets from the last successful use", () => {
    ...
    expect(getDeveloperApiKeyInactivityExpiresAt({ createdAt, lastUsedAt: null })).toBe(createdAt + DEVELOPER_API_KEY_INACTIVITY_MS);
    expect(getDeveloperApiKeyInactivityExpiresAt({ createdAt, lastUsedAt })).toBe(lastUsedAt + DEVELOPER_API_KEY_INACTIVITY_MS);
    ...
    expect(expired).toMatchObject({ status: "expired", statusReason: "inactivity" });
  });
  ...
  it("surfaces thirty-day and seven-day warnings plus temporary abuse suspensions", () => {
    ...
    const warning = describeDeveloperApiKeyLifecycle({ createdAt, lastUsedAt: null, revokedAt: null, expiresAt: null }, inactivityExpiresAt - DEVELOPER_API_KEY_WARNING_MS);
    const urgent = describeDeveloperApiKeyLifecycle({ createdAt, lastUsedAt: null, revokedAt: null, expiresAt: null }, inactivityExpiresAt - DEVELOPER_API_KEY_URGENT_WARNING_MS);
    ...
    expect(suspended).toMatchObject({ status: "suspended", statusReason: "send_account_burst" });
  });
});

describe("developer API import primitives", () => {
  it("hashes identical normalized payloads consistently", () => {
    const payload = JSON.stringify({ email: "person@example.com", sourceApp: "gravity-forms" });
    expect(hashDeveloperApiRequest(payload)).toHaveLength(64);
    expect(hashDeveloperApiRequest(payload)).toBe(hashDeveloperApiRequest(payload));
    expect(hashDeveloperApiRequest(payload)).not.toBe(hashDeveloperApiRequest(`${payload} `));
  });
  ...
  it("normalizes flat form-builder consent aliases without weakening affirmative consent", () => {
    expect(normalizeContactImportPayload({
      name: "Jordan Lee",
    ...
    expect(normalizeContactImportPayload({ consentConfirmed: "false" })).toMatchObject({
