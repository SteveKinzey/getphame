...
describe("grounded help assistant", () => {
  it("retrieves approved email connection guidance for provider questions", () => {
    const sources = retrieveHelpSources(
      "How do I connect Gmail with an app password?"
    );
  ...
  it("retrieves high-risk Yelp compliance guidance without review gating", () => {
    const sources = retrieveHelpSources(
      "Can I ask only happy customers for a Yelp review?"
    );
    const compliance = sources.find(
      source => source.id === "platform-compliance"
    );
    expect(compliance?.content).toContain("does not support review gating");
    expect(compliance?.content).toContain(
  ...
  it("retrieves the developer workspace for webhook and API questions", () => {
    const sources = retrieveHelpSources(
      "Where do I rotate my webhook API key?"
    );
    expect(sources.some(source => source.id === "developer-integrations")).toBe(
      true
    );
  });
  ...
  it("supports Unicode search terms used by localized quick questions", () => {
    expect(
      tokenizeHelpQuery("ฉันจะเชื่อมต่ออีเมลได้อย่างไร").join(" ")
    ).toContain("เชื่อมต่อ");
    expect(
      retrieveHelpSources("ฉันจะเชื่อมต่ออีเมลได้อย่างไร").some(
        source => source.id === "email-connection"
      )
    ).toBe(true);
    expect(
      retrieveHelpSources("如何连接我的邮箱？").some(
        source => source.id === "email-connection"
      )
    ).toBe(true);
  });
  ...
  it("redacts secrets, email addresses, and phone-like private values before model processing", () => {
    const result = redactHelpQuestion(
      "password: super-secret-123 email me at owner@example.com or +1 (555) 123-4567"
    );
    ...
    expect(
      result.text.match(/\[private value removed\]/g)?.length
  ...
  it("does not call the model when approved sources cannot answer and escalates safely", async () => {
    const result = await answerHelpQuestion({
      question: "quantum banana astrophysics",
      language: "en",
    });
  ...
  it("uses public server access and exposes one globally mounted assistant with citations, privacy, and secure escalation", () => {
    const routerSource = readFileSync(
      resolve(projectRoot, "server/helpAssistant.ts"),
      "utf8"
    );
    const appRouterSource = readFileSync(
      resolve(projectRoot, "server/routers.ts"),
      "utf8"
    );
    const componentSource = readFileSync(
      resolve(projectRoot, "client/src/components/HelpAssistant.tsx"),
      "utf8"
    );
    ...
    const layoutSource = readFileSync(
      resolve(projectRoot, "client/src/components/AppLayout.tsx"),
      "utf8"
    );
    const supportSource = readFileSync(
      resolve(projectRoot, "client/src/components/landing/SupportDialog.tsx"),
      "utf8"
    );
    ...
    expect(componentSource).toContain(
      "Questions are not saved as a durable chat transcript"
    );
    expect(componentSource).toContain(
      'URLSearchParams(window.location.search).has("help")'
    );
    expect(componentSource).toMatch(/<SupportDialog\s+open=\{supportOpen\}/);
