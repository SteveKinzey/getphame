...
describe("passkey provider verification state", () => {
  beforeEach(() => {
    process.env.JWT_SECRET =
      "passkey-provider-test-secret-with-sufficient-entropy";
  });
  ...
  it("signs, verifies, and email-binds a bounded enrollment intent", () => {
    ...
    expect(verifyProviderOAuthState(state)).toEqual(
  ...
  it("matches normalized provider email without exposing it in state", () => {
    ...
    expect(
      providerEmailMatches(" member@example.test ", expectedEmailHash)
    ).toBe(true);
    expect(
      providerEmailMatches("different@example.test", expectedEmailHash)
    ).toBe(false);
  });
  ...
  it("fails closed without throwing for malformed provider email hashes", () => {
    ...
    expect(
      malformedHashes.map(hash =>
        providerEmailMatches("member@example.test", hash)
      )
  ...
  it("accepts only exact cookie-bound historical Google state without downgrading signed state", () => {
    ...
    expect(verifyProviderOAuthCallbackState(legacyState, legacyState)).toEqual({
      kind: "legacy",
    });
    expect(
      verifyProviderOAuthCallbackState(legacyState, "cd".repeat(16))
    ).toBeNull();
    expect(
      verifyProviderOAuthCallbackState("a".repeat(31), "a".repeat(31))
    ).toBeNull();
    expect(
      verifyProviderOAuthCallbackState("G".repeat(32), "G".repeat(32))
    ).toBeNull();

    const expectedEmailHash = crypto
      .createHash("sha256")
      .update("member@example.test")
      .digest("hex");
    const signedState = createProviderOAuthState({
      intent: PASSKEY_ENROLLMENT_INTENT,
      expectedEmailHash,
    });
    ...
    expect(
      verifyProviderOAuthCallbackState(tamperedSignedState, tamperedSignedState)
    ).toBeNull();
  });

  it("wires both providers to canonical verified-email linking and fixed recovery paths", () => {
    const google = readFileSync(
      path.resolve(process.cwd(), "server/auth-google.ts"),
      "utf8"
    );
    const apple = readFileSync(
      path.resolve(process.cwd(), "server/appleAuth.ts"),
      "utf8"
    );
    expect(google).toMatch(
      /verifyProviderOAuthCallbackState\(\s*storedState,\s*state\s*\)/
    );
