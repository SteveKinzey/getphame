...
export type WordPressPairingState =
  | "pending"
  | "approving"
  | "approved"
  | "claimed"
...
function stateFor(
  pairing: { status: string; expiresAt: number },
  now = Date.now()
): WordPressPairingState {
...
function publicStatus(
  pairing: typeof wordpressPairings.$inferSelect,
  now = Date.now()
) {
  return {
...
async function loadPairing(pairingId: string) {
  ...
  const [pairing] = await db
    ...
    .where(eq(wordpressPairings.publicId, pairingId))
    .limit(1);
  return pairing ?? null;
}
 ...
 */
export async function initiateWordPressPairing(params: {
  siteUrl: string;
 ...
 */
export async function approveWordPressPairing(params: {
  userId: number;
  pairingId: string;
}) {
  const db = await getDb();
  ...
  if (!pairing)
    throw new WordPressPairingError(
      "NOT_FOUND",
  ...
  if (status === "expired") {
    ...
    throw new WordPressPairingError(
      "EXPIRED",
      "This WordPress connection request has expired. Return to WordPress and start a new connection."
    );
  }
  if (pairing.userId && pairing.userId !== params.userId) {
    throw new WordPressPairingError(
      "ALREADY_APPROVED",
      "This WordPress connection is already authorized for another Get Phame account."
    );
  }
  ...
  if (pairing.status === "approving") {
    throw new WordPressPairingError(
      "NOT_READY",
      "This connection is already being authorized. Return to WordPress in a moment."
    );
  }
  ...
  if (!enrollment.termsAccepted) {
    throw new WordPressPairingError(
      "TERMS_REQUIRED",
      "Accept the Developer API Terms before connecting WordPress."
    );
  }
  ...
  if (
    ...
    throw new WordPressPairingError(
      "NOT_READY",
      "This connection request changed before it could be authorized. Refresh and try again."
    );
  }
  ...
  try {
    ...
    if (!source)
      throw new Error("Could not create an authorized WooCommerce Source.");
    sourceId = source.id;
    ...
    if (sourceId)
      await archiveSourceConnection(params.userId, sourceId, now).catch(
        () => undefined
      );
    if (apiKeyId)
      await revokeDeveloperApiKey(params.userId, apiKeyId).catch(
        () => undefined
      );
    await db
      .update(wordpressPairings)
 ...
 */
export async function claimWordPressPairing(params: {
  pairingId: string;
  ...
  if (
    !pairing ||
    !pairingSecretMatches(pairing.secretHash, params.pairingSecret)
  ) {
    throw new WordPressPairingError(
      "NOT_FOUND",
      "This WordPress connection request was not found."
  ...
  if (status === "expired")
    throw new WordPressPairingError(
      "EXPIRED",
      "This WordPress connection request has expired. Start a new connection."
    );
  if (status === "claimed")
    throw new WordPressPairingError(
      "ALREADY_CLAIMED",
      "This WordPress connection has already been completed."
    );
  if (
    status !== "approved" ||
    !pairing.encryptedApiKey ||
    !pairing.sourceConnectionId
  ) {
    throw new WordPressPairingError(
      "NOT_READY",
      "Waiting for Get Phame account approval."
    );
  }
  ...
  const [claimed] = await db
    ...
    .where(
      and(
        eq(wordpressPairings.id, pairing.id),
        eq(wordpressPairings.status, "approved")
      )
    );
  if (
    Number(
      (claimed as unknown as { affectedRows?: number }).affectedRows ?? 0
    ) !== 1
  ) {
    throw new WordPressPairingError(
      "ALREADY_CLAIMED",
  ...
  const [source] = await db
    .select({ publicId: sourceConnections.publicId })
    .from(sourceConnections)
    .where(eq(sourceConnections.id, pairing.sourceConnectionId))
    .limit(1);
  if (!source)
    throw new Error("The authorized WooCommerce Source could not be found.");
...
export function getWordPressPairingStatusCode(error: unknown) {
  ...
  if (error.code === "ALREADY_CLAIMED" || error.code === "ALREADY_APPROVED")
    return 409;
  return 410;
}
