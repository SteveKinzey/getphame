...
function pageDatabase(total: number, rows: ReturnType<typeof healthRow>[]) {
  ...
  const rowsLimit = vi.fn(() => Object.assign(Promise.resolve(rows), { offset: rowsOffset }));
...
function collectPrimitiveValues(value: unknown, seen = new Set<object>()): string[] {
  if (value === null || value === undefined) return [];
  if (["string", "number", "boolean"].includes(typeof value)) return [String(value)];
  if (typeof value !== "object" || seen.has(value)) return [];
  seen.add(value);
  return Object.values(value).flatMap((entry) => collectPrimitiveValues(entry, seen));
}
...
describe("auth health history data helpers", () => {
  ...
  it("returns bounded page metadata and uses stable checked-at plus id ordering", async () => {
    ...
    expect(result).toEqual({ rows, page: 3, pageSize: 20, total: 43, pageCount: 3 });
    ...
    expect(collectPrimitiveValues(pageWhere)).toEqual(expect.arrayContaining(["fail", "manual"]));
  ...
  it("applies the same requested filters to the count and filtered export row query", async () => {
    ...
    expect(collectPrimitiveValues(exportWhere)).toEqual(expect.arrayContaining(["fail", "manual"]));
    expect(query.rowsOrderBy.mock.calls[0]).toHaveLength(2);
