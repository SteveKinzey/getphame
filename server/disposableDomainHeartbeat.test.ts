...
function createDeps() {
  return { list: vi.fn(), create: vi.fn(), update: vi.fn(), getScheduler: vi.fn(), saveTaskUid: vi.fn() };
}
...
describe("disposable-domain Heartbeat reconciliation", () => {
  ...
  it("skips all scheduler calls when reconciliation is disabled", async () => {
    await expect(reconcileDisposableDomainHeartbeat({ enabled: false, deps })).resolves.toEqual({ status: "skipped" });
  ...
  it("creates and persists the project-owner nightly job when absent", async () => {
    await expect(reconcileDisposableDomainHeartbeat({ enabled: true, deps })).resolves.toEqual({ status: "created" });
  ...
  it("repairs schedule drift and adopts the existing owned job", async () => {
    deps.getScheduler.mockResolvedValue({ scheduleCronTaskUid: "task_disposable_domains_1" });
    deps.list.mockResolvedValue({ total: 1, actorUserId: "owner", jobs: [heartbeatJob()] });
    await expect(reconcileDisposableDomainHeartbeat({ enabled: true, deps })).resolves.toEqual({ status: "reconciled" });
    expect(deps.create).not.toHaveBeenCalled();
    expect(deps.update).toHaveBeenCalledWith("task_disposable_domains_1", expect.objectContaining({
      cron: DISPOSABLE_DOMAIN_CRON,
      path: DISPOSABLE_DOMAIN_CALLBACK_PATH,
      method: "POST",
      enable: true,
