...
describe("signup-risk privacy-minimized persistence", () => {
  ...
  it("stores a one-way normalized account fingerprint and bounded safe outcome fields only", async () => {
    ...
    await recordSignupRiskEvent({
      ...
      provider: "google",
      outcome: "verified",
      reasonCode: "human_proof_verified_with_more_detail_than_the_persisted_bound_allows",
      humanVerified: true,
    ...
    expect(serialized).not.toMatch(/ip(?:address)?|remoteip|device|useragent|fingerprintRaw|password|token/i);
