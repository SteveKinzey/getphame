...
export type AdminEmailPreviewTemplate = (typeof ADMIN_EMAIL_PREVIEW_TEMPLATES)[number];

export const ADMIN_EMAIL_PREVIEW_LABELS: Record<AdminEmailPreviewTemplate, string> = {
...
export function buildAdminEmailPreviewTemplate({
  ...
  if (template === "magic-link") {
    ...
    return wrapEmail("Your secure sign-in link", body, footer("<br/>You received this because a sign-in was requested for this email address."));
  }
  ...
  if (template === "welcome") {
    ...
    const stepsHtml = steps.map(([text, number]) => `<p style="margin:0 0 12px;font-size:14px;color:#1a2744;line-height:1.6;"><span style="display:inline-block;background:#C9A84C;color:#0F1B2D;font-weight:800;font-size:12px;border-radius:50%;width:22px;height:22px;text-align:center;line-height:22px;margin-right:8px;">${number}</span>${text}</p>`).join("");
    ...
    return wrapEmail("Welcome aboard", body, footer("<br/><a href='https://getphame.app/settings' style='color:#888;text-decoration:none;'>Manage your settings</a>"));
  }
  ...
  if (template in tiers) {
    ...
    const perksHtml = perks.map(perk => `<li style="margin:0 0 10px;font-size:14px;color:#1a2744;line-height:1.6;"><span style="display:inline-block;background:#C9A84C;color:#0F1B2D;font-weight:800;font-size:11px;border-radius:50%;width:20px;height:20px;text-align:center;line-height:20px;margin-right:8px;">✓</span>${perk}</li>`).join("");
    ...
    return wrapEmail(`You're on ${label}!`, body, footer("<br/>Questions? Reply to this email or visit <a href='https://getphame.app/settings' style='color:#888;text-decoration:none;'>your settings</a>."));
  }
