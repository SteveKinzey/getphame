...
export function buildDuplicateAuthHealthHistoryPresetName(
  sourceName: string,
  existingNames: string[]
) {
  const normalizedNames = new Set(
    existingNames.map(normalizeAuthHealthHistoryPresetName)
  );
  for (
    let copyNumber = 1;
    copyNumber <= MAX_AUTH_HEALTH_HISTORY_PRESETS + 1;
    copyNumber += 1
  ) {
    const suffix = copyNumber === 1 ? " copy" : ` copy ${copyNumber}`;
    const maxBaseLength =
      MAX_AUTH_HEALTH_HISTORY_PRESET_NAME_CHARS - suffix.length;
    const baseName =
      sourceName
        .replace(/\s+/g, " ")
        .trim()
    ...
    if (!normalizedNames.has(normalizeAuthHealthHistoryPresetName(candidate)))
      return candidate;
  }
...
export function validateCompleteAuthHealthHistoryPresetOrder(
  orderedIds: number[],
  ownedIds: number[]
) {
  ...
  if (orderedIds.some(id => !ownedIdSet.has(id))) {
    throw new Error(
      "Preset order can only contain the current owner's preset IDs."
    );
  }
  return orderedIds;
}
...
export async function listAuthHealthHistoryPresets(
  ownerUserId: number,
  database?: Database
) {
  ...
  return db
    ...
    .orderBy(
      asc(authHealthHistoryPresets.sortOrder),
      asc(authHealthHistoryPresets.id)
    )
...
export async function saveAuthHealthHistoryPreset(
  ...
  const [sameName] = await db
    .select({ id: authHealthHistoryPresets.id })
    .from(authHealthHistoryPresets)
    .where(
      and(
  ...
  if (input.id) {
    ...
    await db
      .update(authHealthHistoryPresets)
      .set(fields)
      .where(
        and(
          eq(authHealthHistoryPresets.id, input.id),
  ...
  const [ownedCount] = await db
    .select({ value: count(authHealthHistoryPresets.id) })
    .from(authHealthHistoryPresets)
    .where(eq(authHealthHistoryPresets.ownerUserId, ownerUserId));
  ...
  const [highestPosition] = await db
    .select({ value: max(authHealthHistoryPresets.sortOrder) })
    .from(authHealthHistoryPresets)
    .where(eq(authHealthHistoryPresets.ownerUserId, ownerUserId));
  const sortOrder = Number(highestPosition?.value ?? -1) + 1;

  const [inserted] = await db
    .insert(authHealthHistoryPresets)
...
export async function deleteAuthHealthHistoryPreset(
  ownerUserId: number,
  id: number,
  database?: Database
) {
...
export async function duplicateAuthHealthHistoryPreset(
  ownerUserId: number,
  id: number,
  database?: Database
) {
  ...
  const existing = await db
    .select({
      name: authHealthHistoryPresets.name,
      sortOrder: authHealthHistoryPresets.sortOrder,
    })
    .from(authHealthHistoryPresets)
    .where(eq(authHealthHistoryPresets.ownerUserId, ownerUserId))
    .limit(MAX_AUTH_HEALTH_HISTORY_PRESETS);
  if (existing.length >= MAX_AUTH_HEALTH_HISTORY_PRESETS)
    return { outcome: "limit_reached" as const };

  const name = buildDuplicateAuthHealthHistoryPresetName(
    source.name,
    existing.map(preset => preset.name)
  );
  const normalizedName = normalizeAuthHealthHistoryPresetName(name);
  const sortOrder =
    Math.max(-1, ...existing.map(preset => Number(preset.sortOrder ?? 0))) + 1;
  const [inserted] = await db
    .insert(authHealthHistoryPresets)
    .values({
      ownerUserId,
...
export async function reorderAuthHealthHistoryPresets(
  ...
  if (
    orderedIds.length < 1 ||
    orderedIds.length > MAX_AUTH_HEALTH_HISTORY_PRESETS
  ) {
