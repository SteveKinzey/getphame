...
const bulkSenderSource = readFileSync(
  resolve(process.cwd(), "server/bulkSender.ts"),
  "utf8"
);
const sendgridSource = readFileSync(
  resolve(process.cwd(), "server/sendgrid.ts"),
  "utf8"
);
...
describe("platform SendGrid isolation", () => {
  it("keeps the platform relay server-managed while allowing only verified user-owned SendGrid SMTP", () => {
    ...
    expect(sendgridSource).toContain(
      "customer outreach must never receive a SendGrid credential"
    );
    expect(bulkSenderSource).toMatch(
      /credentials\.provider\s*===\s*["']sendgrid["']\s*&&\s*!\(\s*credentials\.smtpHost\s*&&\s*credentials\.smtpPort\s*&&\s*credentials\.smtpUsername\s*\)/
    );
    expect(bulkSenderSource).toContain(
