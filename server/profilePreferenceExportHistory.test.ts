...
describe("profile preference export history", () => {
  ...
  it("stores only tenant ID, format, and timestamp with a tenant-time index", () => {
    ...
    const migration = read(
      "drizzle/manual-pending/20260826_add_profile_preference_export_history.sql"
    );
    ...
    const fields = [...(tableDefinition ?? "").matchAll(/^    (\w+):/gm)].map(
      match => match[1]
    );
    ...
    expect(schema).toMatch(/table\.userId,\s*table\.exportedAt/);
    expect(schema).toContain(
  ...
  it("records and lists history strictly through authenticated tenant ownership with a bounded newest-first list", () => {
    ...
    expect(helper).toMatch(
      /gte\(\s*profilePreferenceExportHistory\.exportedAt/
    );
    expect(helper).toMatch(
      /lte\(\s*profilePreferenceExportHistory\.exportedAt/
    );
    expect(helper).toContain(
    ...
    expect(router).toMatch(
      /recordProfilePreferenceExport\(\s*\{\s*userId:\s*ctx\.user\.id,\s*format:\s*input\.format\s*,?\s*\}\s*\)/
    );
  ...
  it("surfaces unavailable history storage as an explicit failure instead of an empty list", async () => {
    ...
    await expect(listProfilePreferenceExportHistory(1)).rejects.toThrow(
