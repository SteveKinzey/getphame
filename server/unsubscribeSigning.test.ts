...
const PRIMARY_SECRET = "unsubscribe-primary-key-with-more-than-thirty-two-characters";
const LEGACY_SECRET = "legacy-session-key-with-more-than-thirty-two-characters";

function buildLegacyToken(contactType: "contact" | "woo", id: number, userId: number) {
  const payload = `${contactType}:${id}:${userId}`;
  const signature = crypto.createHmac("sha256", LEGACY_SECRET).update(payload).digest("hex").slice(0, 16);
...
describe("unsubscribe signing-key separation", () => {
  ...
  it("issues and verifies a full-length signature using the dedicated key", () => {
    ...
    expect(verifyUnsubToken(token)).toEqual({ contactType: "contact", id: 17, userId: 8 });
  });
  ...
  it("keeps new tokens valid when JWT_SECRET rotates", () => {
    ...
    expect(verifyUnsubToken(token)).toEqual({ contactType: "woo", id: 19, userId: 8 });
  });
  ...
  it("temporarily verifies historical 16-character JWT signatures without issuing them", () => {
    ...
    expect(verifyUnsubToken(token)).toEqual({ contactType: "contact", id: 21, userId: 8 });
  });

  it("rejects tampering and refuses issuance without the dedicated key", () => {
    const token = buildUnsubToken("contact", 23, 8);
    const raw = Buffer.from(token, "base64url").toString("utf8");
    const tampered = Buffer.from(raw.replace("contact:23:8", "contact:24:8")).toString("base64url");
    expect(verifyUnsubToken(tampered)).toBeNull();

    vi.stubEnv("UNSUBSCRIBE_SIGNING_SECRET", "");
    expect(() => buildUnsubToken("contact", 23, 8)).toThrow("UNSUBSCRIBE_SIGNING_SECRET");
  });
});
