...
export function getAuthHealthAlertTransition(
  ...
  if (current.overallStatus === "fail" && previous?.overallStatus !== "fail") return "failure" as const;
  if (current.overallStatus === "ok" && previous?.overallStatus === "fail") return "recovery" as const;
  return null;
}
...
async function deliverAuthHealthTransitionAlert(
  ...
  if (!transition) return { attempted: false, delivered: true, transition: null } as const;
...
export async function authHealthHandler(req: Request, res: Response) {
  ...
  try {
    ...
    const duplicate = history[0]?.checkedAt >= Date.now() - RETRY_DEDUP_WINDOW_MS
      ? history[0]
      : null;
    if (duplicate) {
