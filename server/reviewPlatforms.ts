...
export async function listReviewPlatforms(userId: number): Promise<ReviewPlatform[]> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return db.select().from(reviewPlatforms).where(eq(reviewPlatforms.userId, userId));
}

/** Get the default review platform for a user (falls back to first in list) */
export async function getDefaultReviewPlatform(userId: number): Promise<ReviewPlatform | null> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const all = await db.select().from(reviewPlatforms).where(eq(reviewPlatforms.userId, userId));
...
export async function updateReviewPlatform(
  ...
  await db
    ...
    .where(and(eq(reviewPlatforms.id, platformId), eq(reviewPlatforms.userId, userId)));
}

/** Delete a review platform */
export async function deleteReviewPlatform(userId: number, platformId: number): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  if (target?.isDefault === 1) {
    ...
    if (remaining.length > 0) {
      await db
        .update(reviewPlatforms)
        .set({ isDefault: 1 })
        .where(and(eq(reviewPlatforms.id, remaining[0].id), eq(reviewPlatforms.userId, userId)));
    }
  }
}

/** Set a platform as the default (clears other defaults for this user) */
export async function setDefaultReviewPlatform(userId: number, platformId: number): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
