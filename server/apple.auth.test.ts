...
const hasAppleEnv = !!(process.env.APPLE_CLIENT_ID && process.env.APPLE_TEAM_ID && process.env.APPLE_KEY_ID && process.env.APPLE_PRIVATE_KEY);

describe("Apple Sign In credentials", () => {
  it.skipIf(!hasAppleEnv)("should have all 4 required env vars set", () => {
  ...
  it.skipIf(!hasAppleEnv)("APPLE_CLIENT_ID should be the Services ID format", () => {
  ...
  it.skipIf(!hasAppleEnv)("APPLE_TEAM_ID should be 10 characters", () => {
    const teamId = process.env.APPLE_TEAM_ID ?? "";
    expect(teamId).toHaveLength(10);
  });

  it.skipIf(!hasAppleEnv)("APPLE_KEY_ID should be 10 characters", () => {
    const keyId = process.env.APPLE_KEY_ID ?? "";
    expect(keyId).toHaveLength(10);
  });

  it.skipIf(!hasAppleEnv)("should successfully generate a client secret JWT from the private key", () => {
