...
describe("Apple Sign In credentials", () => {
  it("does not mistake non-empty build placeholders for Apple signing credentials", () => {
    ...
    expect(
      hasPemPrivateKeyEnvelope(
        "-----BEGIN PRIVATE KEY-----\\nredacted\\n-----END PRIVATE KEY-----"
      )
    ).toBe(true);
  });

  it.skipIf(!hasAppleSigningCredentials)(
    "should have all 4 required env vars set",
    () => {
  ...
  it.skipIf(!hasAppleSigningCredentials)(
    "APPLE_CLIENT_ID should be the Services ID format",
    () => {
  ...
  it.skipIf(!hasAppleSigningCredentials)(
    "APPLE_TEAM_ID should be 10 characters",
    () => {
  ...
  it.skipIf(!hasAppleSigningCredentials)(
    "APPLE_KEY_ID should be 10 characters",
    () => {
  ...
  it.skipIf(!hasAppleSigningCredentials)(
    "should successfully generate a client secret JWT from the private key",
    () => {
