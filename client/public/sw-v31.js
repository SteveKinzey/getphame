...
async function rememberOfflineLanguage(language) {
  ...
  await cache.put(
    LANGUAGE_CACHE_KEY,
    new Response(normalizeOfflineLanguage(language))
  );
}
...
async function getOfflinePage() {
  ...
  const language = normalizeOfflineLanguage(
    response ? await response.text() : "en"
  );
