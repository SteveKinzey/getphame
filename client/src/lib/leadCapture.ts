...
export const GUIDE_SHARE_TEXT =
  "Get the free Get Phame 30-Day Review Growth Playbook.";
...
export function validateLeadEmail(value: string): {
  normalized: string;
...
export function buildGuideShareUrls(guideUrl = GUIDE_LANDING_URL) {
  ...
  twitter.search = new URLSearchParams({
    text: GUIDE_SHARE_TEXT,
    url: guideUrl,
  }).toString();
