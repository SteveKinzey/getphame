...
export function getMagicLinkRecoveryKind(code: string): MagicLinkRecoveryKind {
  if (code === "link_expired" || code === "magic_link_expired")
    return "expired";
  if (code === "invalid_link" || code === "invalid_magic_link")
    return "invalid";
...
const AUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  google_missing_code:
    "Google did not return the information needed to sign you in. Please try again.",
  google_state_mismatch:
    "Google sign-in security check failed. Please try again.",
  google_no_id:
    "Google sign-in could not verify your account. Please try again.",
  ...
  apple_missing_code:
    "Apple did not return the information needed to sign you in. Please try again.",
  ...
  disposable_email:
    "Please use a non-disposable email address to create your account.",
};
