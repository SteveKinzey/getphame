...
export async function createContactExportPdfBlob(input: {
  ...
  const unicodeFont = detectTranscriptPdfUnicodeFont([labels.title, ...Object.values(labels), ...allText]);
  const { jsPDF } = await import("jspdf");
  const pdf = new jsPDF({ orientation: "landscape", format: "a4", unit: "pt", compress: true });
  ...
  const headers = [labels.contact, labels.sourceAndTags, labels.consent, labels.sends, labels.status, labels.created];
  let cursorY = 36;
  ...
  const drawDocumentHeader = () => {
    ...
    const generated = new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(generatedAt);
  ...
  const drawTableHeader = () => {
    ...
    headers.forEach((header, index) => {
      pdf.text(normalizePdfText(header), cursorX + 5, cursorY + 14, { maxWidth: columnWidths[index] - 10 });
  ...
  snapshot.rows.forEach((row, rowIndex) => {
    const cells = [
      ...
      [String(row.totalSent), formatExportDate(row.lastSentAt, locale)].filter(Boolean).join("\n"),
    ...
    setFont();
    pdf.setFontSize(7.8);
    const splitCells = cells.map((cell, index) => pdf.splitTextToSize(cell || "—", columnWidths[index] - 10) as string[]);
    ...
    splitCells.forEach((lines, index) => {
      pdf.text(lines.slice(0, 5), cursorX + 5, cursorY + 11, { lineHeightFactor: 1.15 });
      cursorX += columnWidths[index];
    });
    cursorY += rowHeight;
  });
  ...
  for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
    ...
    pdf.text(
      normalizePdfText(labels.page.replace("{{current}}", String(pageNumber)).replace("{{total}}", String(pageCount))),
      pageWidth - margin,
      footerY,
      { align: "right" },
    );
