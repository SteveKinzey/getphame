...
export function matchesTypedEmail(
  confirmation: string,
...
export function parseAdminUserDirectoryParams(searchString: string): {
  ...
  const smtpStatus =
    ...
    requestedStatus === "failing" ||
    requestedStatus === "unconnected"
      ? requestedStatus
      : "all";
...
export default function AdminUsersPage() {
  ...
  const auditQueryInput = useMemo(
    () => ({
      ...
      dateFrom: auditDateFrom
        ? new Date(`${auditDateFrom}T00:00:00.000`).getTime()
        : undefined,
      dateTo: auditDateTo
        ? new Date(`${auditDateTo}T23:59:59.999`).getTime()
        : undefined,
  ...
  const auditExportInput = useMemo(
    ...
    [
      auditQueryInput.dateFrom,
      auditQueryInput.dateTo,
      auditQueryInput.adminId,
      auditQueryInput.outcome,
    ]
  );
  const smtpAuditLogs = trpc.admin.listSmtpAuditLogs.useQuery(auditQueryInput, {
    enabled: user?.role === "admin",
  });
  const smtpAuditActors = trpc.admin.listSmtpAuditActors.useQuery(undefined, {
    enabled: user?.role === "admin",
  });
  const smtpAuditExport = trpc.admin.exportSmtpAuditLogs.useQuery(
    auditExportInput,
    {
      enabled: false,
      retry: false,
  ...
  const [deleteAccount, setDeleteAccount] = useState<DirectoryAccount | null>(
    null
  );
  const [deleteConfirmation, setDeleteConfirmation] = useState("");
  const [combineSource, setCombineSource] = useState<DirectoryAccount | null>(
    null
  );
  ...
  const [accessAccount, setAccessAccount] = useState<DirectoryAccount | null>(
    null
  );
  const [accessKind, setAccessKind] = useState<"months" | "years" | "lifetime">(
    "months"
  );
  const [accessQuantity, setAccessQuantity] = useState("1");
  const [suspendAccount, setSuspendAccount] = useState<DirectoryAccount | null>(
    null
  );
  const [suspendDays, setSuspendDays] = useState("7");
  const [suspendConfirmation, setSuspendConfirmation] = useState("");
  const [emailAccount, setEmailAccount] = useState<DirectoryAccount | null>(
    null
  );
  const [emailTemplate, setEmailTemplate] = useState<
    "smtp_onboarding" | "custom"
  >("smtp_onboarding");
  const [emailSubject, setEmailSubject] = useState("");
  const [emailBodyText, setEmailBodyText] = useState("");
  const [outboxAccount, setOutboxAccount] = useState<DirectoryAccount | null>(
    null
  );
  ...
  const setRole = trpc.admin.setUserRole.useMutation({
    onSuccess: () => {
      toast.success(
        t("adminUsers.roleUpdated", {
          defaultValue: "Administrator access updated.",
        })
      );
      void refresh();
    },
  ...
  const setLifeAccess = trpc.admin.setLifeAccess.useMutation({
    onSuccess: (_data, variables) => {
      toast.success(
        variables.enabled
          ? t("adminUsers.lifeGranted", {
              defaultValue: "Life access granted.",
            })
          : t("adminUsers.lifeRemoved", {
              defaultValue: "Life access removed.",
            })
      );
      void refresh();
    },
  ...
  const createUser = trpc.admin.createUser.useMutation({
    onSuccess: data => {
      toast.success(
        t("adminUsers.userCreated", {
          defaultValue: "Passwordless account created for {{email}}.",
          email: data.email,
        })
      );
  ...
  const grantFlexibleAccess = trpc.admin.grantFlexibleAccess.useMutation({
    onSuccess: () => {
      toast.success(
        t("adminUsers.accessUpdated", { defaultValue: "Paid access updated." })
      );
  ...
  const suspendUser = trpc.admin.suspendUser.useMutation({
    onSuccess: () => {
      toast.success(
        t("adminUsers.userSuspended", {
          defaultValue: "Account suspended and active sessions revoked.",
        })
      );
  ...
  const restoreUser = trpc.admin.restoreUser.useMutation({
    onSuccess: () => {
      toast.success(
        t("adminUsers.userRestored", {
          defaultValue: "Account access restored.",
        })
      );
      void refresh();
    },
  ...
  const sendUserEmail = trpc.admin.sendUserEmail.useMutation({
    onSuccess: data => {
      if (data.ok)
        toast.success(
          t("adminUsers.emailSent", {
            defaultValue: "Email sent from hello@getphame.app.",
          })
        );
      else
        toast.error(
          t("adminUsers.emailNotConfirmed", {
            defaultValue:
              "Delivery was not confirmed. The outbox contains the recorded attempt.",
          })
        );
  ...
  const deleteUser = trpc.admin.deleteUser.useMutation({
    onSuccess: () => {
      toast.success(
        t("adminUsers.deleted", { defaultValue: "Account deleted." })
      );
      setDeleteAccount(null);
      setDeleteConfirmation("");
      if (directory.data?.users.length === 1 && page > 1)
  ...
  const combineAccounts = trpc.admin.combineAccounts.useMutation({
    onSuccess: data => {
      toast.success(
        t("adminUsers.combined", {
          defaultValue:
            "Accounts combined. {{name}} is now the surviving account.",
  ...
  const removeUserSmtp = trpc.admin.removeUserSmtp.useMutation({
    onSuccess: data => {
      toast.success(
        data.removed
          ? t("adminUsers.smtpRemoved", {
              defaultValue: "SMTP credentials removed.",
            })
          : t("adminUsers.smtpAlreadyAbsent", {
              defaultValue: "No SMTP credentials were connected.",
            })
      );
      setSmtpAccount(null);
      setSmtpConfirmation("");
    ...
    onSuccess: (data, variables) => {
      if (data.ok) {
        if (data.recovered) {
          ...
          toast.success(
            t("adminUsers.smtpRecovered", {
              defaultValue: "SMTP connection recovered and verified.",
            })
          );
        } else {
          setSmtpRecovery(current =>
            current?.userId === variables.userId ? null : current
          );
          toast.success(
            t("adminUsers.smtpRetestPassed", {
              defaultValue: "SMTP verification passed.",
            })
          );
        }
      } else {
        ...
        toast.error(
          data.error ||
            t("adminUsers.smtpRetestFailed", {
              defaultValue: "SMTP verification failed.",
            })
        );
      }
      void refresh();
  ...
  const exportAuditCsv = async () => {
    ...
    if (!result.data?.total) {
      toast.error(
        t("adminUsers.smtpAuditNoExportRows", {
          defaultValue: "No audit records match the current filters.",
        })
      );
      return;
    }
    ...
    toast.success(
      t("adminUsers.smtpAuditExported", {
  ...
  const combineTarget = mergeCandidates.data?.users.find(
    candidate => candidate.id === Number(combineTargetId)
  );
  ...
  return (
    <div className="min-h-screen pb-40 rr-bg-cream-warm">
      <header className="px-5 pt-10 pb-6 rr-bg-navy">
        ...
        >
          <ArrowLeft size={15} />{" "}
          {t("adminUsers.back", { defaultValue: "Admin Dashboard" })}
        </button>
        <div className="flex items-center gap-2 mb-1 rr-text-gold">
          <Users size={17} />
          <span className="text-xs font-black uppercase tracking-[0.18em]">
            {t("adminUsers.eyebrow", { defaultValue: "User Management" })}
          </span>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h1 className="text-3xl font-black text-white">
            {t("adminUsers.title", { defaultValue: "Manage users" })}
          </h1>
          <button
            type="button"
          ...
          >
            <Plus size={17} />{" "}
            {t("adminUsers.addUser", { defaultValue: "Add user" })}
          </button>
        </div>
        <p className="mt-1 text-sm font-semibold text-white/75">
          {t("adminUsers.subtitle", {
            defaultValue:
              "Search every account, combine duplicates, or control administrator and Life access.",
          })}
        </p>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-5">
        <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
          ...
          >
            {t("adminUsers.searchLabel", { defaultValue: "Search users" })}
          </label>
          <div className="mt-2 grid gap-3 sm:grid-cols-[minmax(0,1fr)_220px]">
            ...
            <div className="relative">
              <Filter
                size={17}
                className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 rr-text-navy-muted"
              />
              <label htmlFor="smtp-status-filter" className="sr-only">
                {t("adminUsers.smtpFilterLabel", {
                  defaultValue: "Filter by SMTP status",
                })}
              </label>
              <select
                ...
                value={smtpStatus}
                onChange={event =>
                  setSmtpStatus(event.target.value as SmtpStatusFilter)
                }
              ...
              >
                <option value="all">
                  {t("adminUsers.smtpFilterAll", {
                    defaultValue: "All SMTP statuses",
                  })}
                </option>
                <option value="verified">
                  {t("adminUsers.smtpFilterVerified", {
                    defaultValue: "SMTP verified",
                  })}
                </option>
                <option value="unverified">
                  {t("adminUsers.smtpFilterUnverified", {
                    defaultValue: "SMTP unverified",
                  })}
                </option>
                <option value="failing">
                  {t("adminUsers.smtpFilterFailing", {
                    defaultValue: "SMTP health check failed",
                  })}
                </option>
                <option value="unconnected">
                  {t("adminUsers.smtpFilterUnconnected", {
                    defaultValue: "No SMTP connected",
                  })}
                </option>
              </select>
            </div>
          ...
          <p className="mt-2 text-xs font-semibold rr-text-navy-muted">
            {directory.data
              ? t("adminUsers.accountCount", {
                  defaultValue: "{{count}} accounts",
                  count: directory.data.total,
                })
          ...
          <div className="flex flex-col gap-3">
            {directory.data?.users.map(account => {
              ...
              const isBusy =
                ...
                (setRole.isPending ||
                  setLifeAccess.isPending ||
                  grantFlexibleAccess.isPending ||
                  suspendUser.isPending ||
                  restoreUser.isPending ||
                  sendUserEmail.isPending ||
                  retestUserSmtp.isPending ||
                  removeUserSmtp.isPending ||
                  deleteUser.isPending ||
                  combineAccounts.isPending);
              return (
                <article
                  key={account.id}
                  className="rounded-2xl bg-white p-4 shadow-sm"
                  data-testid={`admin-user-${account.id}`}
                >
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 className="truncate text-base font-black rr-text-navy">
                          {account.name ||
                            t("adminUsers.unnamed", {
                              defaultValue: "Unnamed user",
                            })}
                        </h2>
                        {account.role === "admin" && (
                          <span className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-black rr-bg-navy rr-text-gold">
                            <ShieldCheck size={12} />{" "}
                            {t("adminUsers.admin", { defaultValue: "Admin" })}
                          </span>
                        )}
                        {account.lifeAccess && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 text-xs font-black text-amber-900">
                            <Crown size={12} />{" "}
                            {t("adminUsers.life", { defaultValue: "Life" })}
                          </span>
                        )}
                        {account.isSuspended && (
                      ...
                      <p className="truncate text-sm font-semibold rr-text-navy-mid">
                        {account.email ||
                          t("adminUsers.noEmail", { defaultValue: "No email" })}
                      </p>
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        {account.smtpConnected ? (
                          ...
                          >
                            ...
                            {account.smtpVerified
                              ? t("adminUsers.smtpVerified", {
                                  defaultValue: "SMTP verified",
                                })
                              : t("adminUsers.smtpUnverified", {
                                  defaultValue: "SMTP unverified",
                                })}
                          </span>
                        ) : (
                          ...
                          >
                            <Unplug size={13} />{" "}
                            {t("adminUsers.smtpNotConnected", {
                              defaultValue: "No SMTP",
                            })}
                          </span>
                        )}
                        {account.smtpFromEmail && (
                          <span className="truncate text-xs font-semibold rr-text-navy-muted">
                            {account.smtpFromEmail}
                          </span>
                        )}
                      </div>
                      {smtpRecovery?.userId === account.id && (
                        <p
                          data-testid={`smtp-recovery-${account.id}`}
                          className="mt-2 inline-flex items-center gap-1.5 rounded-xl bg-emerald-50 px-2.5 py-1.5 text-xs font-black text-emerald-800"
                        >
                          <MailCheck size={14} />
                          {t("adminUsers.smtpRecoveryConfirmed", {
                            defaultValue: "Recovery confirmed {{time}}",
                            time: new Date(
                              smtpRecovery.checkedAt
                            ).toLocaleString(),
                          })}
                        </p>
                      )}
                      ...
                      {account.isSuspended && account.suspendedUntil && (
                        <p className="mt-1 inline-flex items-center gap-1 text-xs font-bold text-red-700">
                          ...
                          {t("adminUsers.suspendedUntil", {
                            defaultValue:
                              "Access resumes {{date}} ({{days}} day(s) remaining).",
                            date: new Date(
                            ...
                            days: Math.max(
                              1,
                              Math.ceil(
                                (account.suspendedUntil - Date.now()) /
                                  86_400_000
                              )
                            ),
                          })}
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-end">
                      ...
                      >
                        ...
                        {t("adminUsers.outbox", { defaultValue: "Outbox" })}
                      </button>
                      {account.isSuspended ? (
                        <button
                          type="button"
                          disabled={isBusy || account.role === "admin"}
                          onClick={() =>
                            restoreUser.mutate({ userId: account.id })
                          }
                        ...
                        >
                          <ShieldCheck size={15} />{" "}
                          {t("adminUsers.restore", { defaultValue: "Restore" })}
                        </button>
                      ) : (
                        <button
                          type="button"
                          disabled={
                            isBusy || isSelf || account.role === "admin"
                          }
                          onClick={() => {
                        ...
                        >
                          <ShieldOff size={15} />{" "}
                          {t("adminUsers.suspend", { defaultValue: "Suspend" })}
                        </button>
                      )}
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        disabled={isBusy || account.role === "admin"}
                        onClick={() =>
                          setLifeAccess.mutate({
                            userId: account.id,
                            enabled: !account.lifeAccess,
                          })
                        }
                      ...
                      >
                        {account.lifeAccess
                          ? t("adminUsers.removeLife", {
                              defaultValue: "Remove Life",
                            })
                          : t("adminUsers.grantLife", {
                              defaultValue: "Grant Life",
                            })}
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <Merge size={15} />{" "}
                        {t("adminUsers.combine", { defaultValue: "Combine" })}
                      </button>
                      <button
                        type="button"
                        ...
                        onClick={() =>
                          retestUserSmtp.mutate({ userId: account.id })
                        }
                        className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-sm font-black text-emerald-800 transition active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-45"
                      >
                        {retestUserSmtp.isPending &&
                        retestUserSmtp.variables?.userId === account.id ? (
                          <Loader2 className="animate-spin" size={15} />
                        ) : (
                          <RefreshCw size={15} />
                        )}
                        {t("adminUsers.retestSmtp", {
                          defaultValue: "Re-test SMTP",
                        })}
                      </button>
                      <button
                        type="button"
                        disabled={
                          isBusy || !account.smtpConnected || !account.email
                        }
                        onClick={() => {
                      ...
                      >
                        <Unplug size={15} />{" "}
                        {t("adminUsers.removeSmtp", {
                          defaultValue: "Remove SMTP",
                        })}
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <Trash2 size={15} />{" "}
                        {t("adminUsers.delete", { defaultValue: "Delete" })}
                      </button>
                    </div>
                  </div>
                </article>
            ...
            {directory.data?.users.length === 0 && (
              <div className="rounded-2xl bg-white p-8 text-center font-semibold rr-text-navy-muted">
                {t("adminUsers.empty", {
                  defaultValue: "No users match that search.",
                })}
              </div>
            )}
          </div>
        ...
        {directory.data && directory.data.pageCount > 1 && (
          <div className="mt-5 flex items-center justify-between rounded-2xl bg-white p-3 shadow-sm">
            <button
              ...
              className="flex min-h-11 items-center gap-1 rounded-xl px-3 font-black rr-text-navy disabled:opacity-40"
            >
              <ChevronLeft size={18} />{" "}
              {t("adminUsers.previous", { defaultValue: "Previous" })}
            </button>
            <span className="text-sm font-black rr-text-navy">
              {t("adminUsers.page", {
                defaultValue: "Page {{page}} of {{count}}",
                page,
                count: directory.data.pageCount,
              })}
            </span>
            <button
              disabled={page >= directory.data.pageCount}
              onClick={() => setPage(value => value + 1)}
              className="flex min-h-11 items-center gap-1 rounded-xl px-3 font-black rr-text-navy disabled:opacity-40"
            >
              {t("adminUsers.next", { defaultValue: "Next" })}{" "}
        ...
        <section
          className="mt-7 rounded-2xl bg-white p-4 shadow-sm"
          aria-labelledby="smtp-audit-title"
        >
          <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex items-start gap-3">
              <div className="flex size-10 shrink-0 items-center justify-center rounded-xl rr-bg-navy rr-text-gold">
              ...
              <div>
                ...
                >
                  {t("adminUsers.smtpAuditTitle", {
                    defaultValue: "SMTP removal audit log",
                  })}
                </h2>
                <p className="text-sm font-semibold rr-text-navy-muted">
                  {t("adminUsers.smtpAuditSubtitle", {
                    defaultValue:
                      "A durable record of who permanently removed user SMTP credentials and when.",
                  })}
                </p>
              </div>
            </div>
            <button
              ...
              disabled={
                !smtpAuditLogs.data?.total || smtpAuditExport.isFetching
              }
            ...
            >
              {smtpAuditExport.isFetching ? (
                <Loader2 className="animate-spin" size={16} />
              ) : (
                <Download size={16} />
              )}
              {t("adminUsers.smtpAuditExport", {
          ...
          <div
            className="mt-4 grid gap-3 rounded-2xl bg-slate-50 p-3 sm:grid-cols-2 lg:grid-cols-4"
            data-testid="smtp-audit-filters"
          >
            ...
            <label className="text-xs font-black uppercase tracking-wide rr-text-navy-muted">
              {t("adminUsers.smtpAuditAdministrator", {
              ...
              >
                <option value="all">
                  {t("adminUsers.smtpAuditAllAdministrators", {
                    defaultValue: "All administrators",
                  })}
                </option>
                {smtpAuditActors.data?.map(actor => (
                  <option key={actor.id} value={String(actor.id)}>
                    {actor.name || actor.email || `Admin #${actor.id}`}
                  </option>
                ))}
              </select>
            </label>
            <label className="text-xs font-black uppercase tracking-wide rr-text-navy-muted">
              ...
              <select
                value={auditOutcome}
                onChange={event =>
                  setAuditOutcome(event.target.value as SmtpAuditOutcomeFilter)
                }
                className="mt-1 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold normal-case tracking-normal rr-text-navy outline-none focus:ring-2 focus:ring-amber-400"
                name="rr-pages-admin-users-audit-outcome-666"
              >
                <option value="all">
                  {t("adminUsers.smtpAuditAllOutcomes", {
                    defaultValue: "All outcomes",
                  })}
                </option>
                <option value="removed">
                  {t("adminUsers.smtpAuditOutcomeRemoved", {
          ...
          {(auditDateFrom ||
            ...
            >
              {t("adminUsers.smtpAuditClearFilters", {
                defaultValue: "Clear audit filters",
              })}
            </button>
          )}
          ...
          {smtpAuditLogs.isLoading ? (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-black rr-text-navy">
                      {entry.actorName ||
                        entry.actorEmail ||
                        `Admin #${entry.actorUserId}`}
                      <span className="font-semibold rr-text-navy-muted">
                        {" "}
                        {t("adminUsers.smtpAuditRemoved", {
                          defaultValue: "removed SMTP credentials for",
                        })}{" "}
                      </span>
                      {entry.targetName ||
                        entry.targetEmail ||
                    ...
                    <p className="mt-1 break-all text-xs font-semibold rr-text-navy-muted">
                      {entry.actorEmail || `Admin #${entry.actorUserId}`} →{" "}
                      {entry.targetEmail || `User #${entry.targetUserId}`} ·{" "}
                      {entry.smtpUser}
                    </p>
                  </div>
                  <time
                    className="text-xs font-black rr-text-navy-mid"
            ...
            <p className="py-8 text-center text-sm font-semibold rr-text-navy-muted">
              {t("adminUsers.smtpAuditEmpty", {
                defaultValue: "No SMTP credential removals have been recorded.",
              })}
            </p>
          )}
          {smtpAuditLogs.data && smtpAuditLogs.data.pageCount > 1 && (
            <nav
              ...
              aria-label={t("adminUsers.smtpAuditPagination", {
                defaultValue: "SMTP audit pages",
              })}
              data-testid="smtp-audit-pagination"
            >
              <button
                type="button"
                disabled={smtpAuditLogs.data.page <= 1}
                onClick={() => setAuditPage(value => Math.max(1, value - 1))}
                className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl px-4 font-black rr-text-navy transition active:scale-[0.97] disabled:opacity-40"
              >
                <ChevronLeft size={18} />{" "}
                {t("adminUsers.previous", { defaultValue: "Previous" })}
              </button>
              <span className="text-center text-sm font-black rr-text-navy">
                {t("adminUsers.page", {
                  defaultValue: "Page {{page}} of {{count}}",
                  page: smtpAuditLogs.data.page,
                  count: smtpAuditLogs.data.pageCount,
                })}
              </span>
              <button
                type="button"
                disabled={
                  smtpAuditLogs.data.page >= smtpAuditLogs.data.pageCount
                }
                onClick={() =>
                  setAuditPage(value =>
                    Math.min(smtpAuditLogs.data?.pageCount ?? value, value + 1)
                  )
                }
                className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl px-4 font-black rr-text-navy transition active:scale-[0.97] disabled:opacity-40"
              >
      ...
      >
        <DialogContent className="max-w-lg border-0 bg-white text-slate-950">
          <DialogHeader>
            <div className="mb-1 flex size-11 items-center justify-center rounded-full bg-amber-100 text-amber-900">
              <Plus size={22} />
            </div>
            <DialogTitle className="text-xl font-black text-slate-950">
              {t("adminUsers.addUserTitle", {
                defaultValue: "Create a passwordless user",
              })}
            </DialogTitle>
            <DialogDescription className="text-sm font-medium text-slate-600">
              {t("adminUsers.addUserDescription", {
                defaultValue:
                  "The user will sign in with a magic link. Administrators never create or manage a user password.",
              })}
            </DialogDescription>
          </DialogHeader>
          <form
          ...
          >
            ...
            >
              ...
              <input
                ...
                autoComplete="name"
                className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 px-3 text-base font-semibold text-slate-950 outline-none focus:ring-2 focus:ring-amber-400"
              />
            </label>
            <label
              className="block text-sm font-black text-slate-900"
            ...
            <DialogFooter>
              <button
                ...
                className="min-h-10 rounded-md px-4 text-sm font-black text-slate-700 disabled:opacity-45"
              >
                {t("common.cancel", { defaultValue: "Cancel" })}
              </button>
              <button
                type="submit"
                disabled={!newUserEmail.trim() || createUser.isPending}
              ...
              >
                ...
                {t("adminUsers.createPasswordless", {
      ...
      >
        <DialogContent className="max-w-lg border-0 bg-white text-slate-950">
          <DialogHeader>
            ...
            <DialogTitle className="text-xl font-black text-slate-950">
              {t("adminUsers.manageAccessTitle", {
                defaultValue: "Manage paid access",
              })}
            </DialogTitle>
            <DialogDescription className="text-sm font-medium text-slate-600">
              {t("adminUsers.manageAccessDescription", {
                defaultValue:
                  "New time is added after any unused paid time. Calendar months and years preserve the matching calendar date where possible.",
              })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <p className="rounded-xl bg-slate-50 p-3 text-sm font-bold rr-text-navy">
              {accessAccount?.name ||
                accessAccount?.email ||
                t("adminUsers.user", { defaultValue: "User" })}
            </p>
            <label
              className="block text-sm font-black text-slate-900"
              htmlFor="access-kind"
            >
              ...
              <select
                ...
                onChange={event =>
                  setAccessKind(
                    event.target.value as "months" | "years" | "lifetime"
                  )
                }
                className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-base font-bold text-slate-950 outline-none focus:ring-2 focus:ring-amber-400"
              >
                <option value="months">
                  {t("adminUsers.accessMonths", {
                    defaultValue: "Grant months",
                  })}
                </option>
                <option value="years">
                  {t("adminUsers.accessYears", { defaultValue: "Grant years" })}
                </option>
                <option value="lifetime">
                  {t("adminUsers.accessLifetime", {
                    defaultValue: "Grant Life access",
                  })}
                </option>
              </select>
            </label>
            {accessKind !== "lifetime" && (
              <label
                className="block text-sm font-black text-slate-900"
                htmlFor="access-quantity"
              >
                {accessKind === "months"
                  ? t("adminUsers.months", { defaultValue: "Months (1–120)" })
                  : t("adminUsers.years", { defaultValue: "Years (1–10)" })}
                <input
                  id="access-quantity"
                  type="number"
                  min="1"
                  max={accessKind === "months" ? 120 : 10}
                  value={accessQuantity}
                  onChange={event => setAccessQuantity(event.target.value)}
                  inputMode="numeric"
                  className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 px-3 text-base font-semibold text-slate-950 outline-none focus:ring-2 focus:ring-amber-400"
                />
              </label>
            ...
            {accessKind === "lifetime" && (
              <p className="rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-950">
                {t("adminUsers.lifeAccessNote", {
                  defaultValue:
                    "Life access does not expire and replaces any existing paid expiry.",
                })}
              </p>
            )}
            <DialogFooter>
              <button
                type="button"
                disabled={grantFlexibleAccess.isPending}
              ...
              <button
                ...
                disabled={
                  ...
                  grantFlexibleAccess.isPending ||
                  (accessKind !== "lifetime" &&
                    (!Number.isInteger(Number(accessQuantity)) ||
                      Number(accessQuantity) < 1 ||
                      Number(accessQuantity) >
                        (accessKind === "months" ? 120 : 10)))
                }
                onClick={() =>
                  accessAccount &&
                  grantFlexibleAccess.mutate({
              ...
              >
                ...
                {t("adminUsers.applyAccess", { defaultValue: "Apply access" })}
              </button>
            </DialogFooter>
          </div>
        </DialogContent>
      ...
      >
        <AlertDialogContent className="border-0 bg-white text-slate-950">
          <AlertDialogHeader>
            <div className="mb-1 flex size-11 items-center justify-center rounded-full bg-red-100 text-red-700">
              <ShieldOff size={22} />
            </div>
            <AlertDialogTitle className="text-xl font-black text-slate-950">
              {t("adminUsers.suspendTitle", {
                defaultValue: "Suspend this account?",
              })}
            </AlertDialogTitle>
            <AlertDialogDescription className="space-y-3 text-sm font-medium text-slate-600">
              <span className="block">
                {t("adminUsers.suspendDescription", {
                  defaultValue:
                    "Suspension blocks protected activity immediately and revokes every active session for this user.",
                })}
              </span>
              <span className="block font-black text-red-700">
                {t("adminUsers.suspendRecovery", {
                  defaultValue:
                    "You can restore the account later from this directory.",
                })}
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          >
            {t("adminUsers.suspendDays", {
              defaultValue: "Suspend for days (1–3650)",
            })}
            <input
              id="suspend-days"
              type="number"
              ...
              className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 px-3 text-base font-semibold text-slate-950 outline-none focus:ring-2 focus:ring-red-400"
            />
          </label>
          <label
            className="block text-sm font-black text-slate-900"
          ...
          >
            {t("adminUsers.typeSuspend", {
              defaultValue: "Type SUSPEND to confirm",
            })}
            <input
              id="suspend-confirmation"
              value={suspendConfirmation}
              onChange={event => setSuspendConfirmation(event.target.value)}
              autoComplete="off"
              className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 px-3 text-base font-black text-slate-950 outline-none focus:ring-2 focus:ring-red-400"
            />
          </label>
          <AlertDialogFooter>
            ...
            <button
              ...
              disabled={
                !suspendAccount ||
                suspendConfirmation !== "SUSPEND" ||
                !Number.isInteger(Number(suspendDays)) ||
                Number(suspendDays) < 1 ||
                Number(suspendDays) > 3650 ||
                suspendUser.isPending
              }
              onClick={() =>
                suspendAccount &&
                suspendUser.mutate({
                  userId: suspendAccount.id,
                  days: Number(suspendDays),
                  confirmation: "SUSPEND",
                })
              }
              className="inline-flex min-h-10 items-center justify-center rounded-md bg-red-700 px-4 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-45"
            >
              {suspendUser.isPending && (
                <Loader2 className="mr-2 animate-spin" size={16} />
              )}
              {t("adminUsers.suspendNow", { defaultValue: "Suspend account" })}
            </button>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      >
        <DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto border-0 bg-white text-slate-950">
          <DialogHeader>
            <div className="mb-1 flex size-11 items-center justify-center rounded-full bg-blue-100 text-blue-800">
              <Send size={22} />
            </div>
            <DialogTitle className="text-xl font-black text-slate-950">
              {t("adminUsers.emailTitle", { defaultValue: "Email user" })}
            </DialogTitle>
            <DialogDescription className="text-sm font-medium text-slate-600">
              {t("adminUsers.emailDescription", {
                defaultValue:
                  "This general communication is sent from hello@getphame.app. Magic links are always sent from no-reply@getphame.app.",
              })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <label
              className="block text-sm font-black text-slate-900"
              htmlFor="admin-email-template"
            >
              {t("adminUsers.emailTemplate", {
                defaultValue: "Message starting point",
              })}
              <select
              ...
              >
                <option value="smtp_onboarding">
                  {t("adminUsers.smtpTemplate", {
                    defaultValue: "SMTP setup guide",
                  })}
                </option>
                <option value="custom">
                  {t("adminUsers.customTemplate", {
            ...
            >
              ...
              <input
                ...
                className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 px-3 text-base font-semibold text-slate-950 outline-none focus:ring-2 focus:ring-blue-400"
              />
            </label>
            <label
              className="block text-sm font-black text-slate-900"
            ...
            >
              ...
              <textarea
                ...
                rows={14}
                className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm font-medium text-slate-950 outline-none focus:ring-2 focus:ring-blue-400"
              />
            </label>
            <p className="text-xs font-semibold rr-text-navy-muted">
              {t("adminUsers.emailSecurityHint", {
                defaultValue:
                  "Never ask a user to send passwords, app passwords, or verification codes by email. Sent mail is retained for 180 days in the user outbox.",
              })}
            </p>
            <DialogFooter>
              <button
              ...
              >
                ...
                {t("adminUsers.sendFromHello", {
                  defaultValue: "Send from hello@getphame.app",
                })}
              </button>
            </DialogFooter>
          </div>
      ...
      <Dialog
        open={Boolean(outboxAccount)}
        onOpenChange={open => {
          if (!open) setOutboxAccount(null);
        }}
      >
        <DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto border-0 bg-white text-slate-950">
          <DialogHeader>
            <div className="mb-1 flex size-11 items-center justify-center rounded-full bg-slate-100 rr-text-navy">
              <ClipboardList size={22} />
            </div>
            <DialogTitle className="text-xl font-black text-slate-950">
              {t("adminUsers.outboxTitle", {
                defaultValue: "Sent-mail outbox",
              })}
            </DialogTitle>
            <DialogDescription className="text-sm font-medium text-slate-600">
              {t("adminUsers.outboxDescription", {
                defaultValue:
                  "Administrator emails sent to this user from hello@getphame.app. Records automatically expire after 180 days.",
              })}
            </DialogDescription>
          </DialogHeader>
          {outbox.isLoading ? (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs font-semibold rr-text-navy-muted">
                        {entry.fromEmail} → {entry.recipientEmail} ·{" "}
                        {new Date(entry.createdAt).toLocaleString()}
                      </p>
                    </div>
                    <span
                      className={`rounded-full px-2 py-1 text-xs font-black ${entry.status === "sent" ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"}`}
                    >
                      {entry.status === "sent"
                        ? t("adminUsers.sent", { defaultValue: "Sent" })
                        : t("adminUsers.failed", {
                            defaultValue: "Not confirmed",
                          })}
                    </span>
                  </div>
                  {entry.status !== "sent" && entry.failureCode && (
                    <p className="mt-2 text-xs font-bold text-red-700">
                      {t("adminUsers.deliveryFailure", {
                        defaultValue: "Delivery status: {{code}}",
                        code: entry.failureCode,
                      })}
                    </p>
                  )}
                  <pre className="mt-3 whitespace-pre-wrap rounded-lg bg-slate-50 p-3 text-xs font-medium leading-5 rr-text-navy">
                    {entry.bodyText}
                  </pre>
                </article>
              ))}
            ...
            <p className="py-8 text-center text-sm font-semibold rr-text-navy-muted">
              {t("adminUsers.outboxEmpty", {
                defaultValue:
                  "No administrator email has been recorded for this user.",
              })}
            </p>
          )}
          <DialogFooter>
            <button
              type="button"
              onClick={() => setOutboxAccount(null)}
              className="min-h-10 rounded-md px-4 text-sm font-black rr-text-navy"
            >
              {t("common.cancel", { defaultValue: "Cancel" })}
            </button>
          </DialogFooter>
            ...
            <AlertDialogDescription className="space-y-3 text-sm font-medium text-slate-600">
              <span className="block">
                {t("adminUsers.deleteDescription", {
                  defaultValue:
                    "This permanently deletes {{name}} and all linked contacts, requests, integrations, settings, analytics, and history.",
                  name:
                    deleteAccount?.name ||
                    deleteAccount?.email ||
                    "this account",
              ...
              <span className="block font-black text-red-700">
                {t("adminUsers.deleteIrreversible", {
                  defaultValue:
                    "This cannot be undone. Combine duplicate accounts instead if data should be preserved.",
                })}
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      >
        <AlertDialogContent className="border-0 bg-white text-slate-950">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-xl font-black text-slate-950">
              {t("adminUsers.removeSmtpTitle", {
                defaultValue: "Remove SMTP credentials?",
              })}
            </AlertDialogTitle>
            <AlertDialogDescription className="space-y-3 text-sm font-medium text-slate-600">
              <span className="block">
                {t("adminUsers.removeSmtpDescription", {
                  defaultValue:
                    "This permanently removes the SMTP host, username, encrypted password, sender details, and verification state for {{email}}.",
                  email: smtpAccount?.email || "this user",
                })}
              </span>
              <span className="block font-black text-amber-900">
                {t("adminUsers.removeSmtpPreservesAccount", {
                  defaultValue:
                    "The user account, contacts, requests, plan, and history are not deleted.",
                })}
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <label
            className="text-sm font-black text-slate-900"
            htmlFor="remove-smtp-confirmation"
          >
            {t("adminUsers.typeEmailToRemoveSmtp", {
              defaultValue: "Type {{email}} to confirm",
              email: smtpAccount?.email || "the user's email",
            })}
          </label>
          <input
          ...
          />
          <AlertDialogFooter>
            <AlertDialogCancel disabled={removeUserSmtp.isPending}>
              {t("common.cancel", { defaultValue: "Cancel" })}
            </AlertDialogCancel>
            <button
              type="button"
              disabled={
                !smtpAccount ||
                !matchesTypedEmail(smtpConfirmation, smtpAccount.email) ||
                removeUserSmtp.isPending
              }
              onClick={() =>
                smtpAccount &&
                removeUserSmtp.mutate({
                  userId: smtpAccount.id,
                  confirmationEmail: smtpConfirmation.trim(),
                })
              }
            ...
            >
              ...
              {t("adminUsers.removeSmtpPermanently", {
                defaultValue: "Remove SMTP permanently",
              })}
            </button>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      >
        <AlertDialogContent className="border-0 bg-white text-slate-950">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-xl font-black text-slate-950">
              {t("adminUsers.combineTitle", {
                defaultValue: "Combine duplicate accounts",
              })}
            </AlertDialogTitle>
            <AlertDialogDescription className="text-sm font-medium text-slate-600">
              {t("adminUsers.combineDescription", {
                defaultValue:
                  "Everything owned by {{source}} will move to the surviving account. The source account is deleted after the transfer succeeds.",
                source:
                  combineSource?.name ||
                  combineSource?.email ||
                  "the source account",
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">
            <p className="font-black">
              {t("adminUsers.sourceDeleted", {
                defaultValue: "Source account — deleted",
              })}
            </p>
            <p className="truncate font-semibold">
              {combineSource?.name || "Unnamed user"} ·{" "}
              {combineSource?.email || "No email"}
            </p>
          </div>

          <label
            className="text-sm font-black text-slate-900"
            htmlFor="combine-target"
          >
            {t("adminUsers.chooseSurvivor", {
              defaultValue: "Surviving account",
            })}
          </label>
          <select
            id="combine-target"
          ...
          >
            <option value="">
              {mergeCandidates.isLoading
                ? "Loading accounts…"
                : "Choose the account that remains"}
            </option>
            {mergeCandidates.data?.users
              ...
              .map(candidate => (
                <option key={candidate.id} value={candidate.id}>
                  ...
                  {candidate.email || `Account #${candidate.id}`}
          ...
          {combineSource && combineTarget && (
            ...
            >
              <p className="font-black">
                {t("adminUsers.mergeDirection", {
                  defaultValue: "Merge direction",
                })}
              </p>
              <p className="mt-2 truncate font-semibold">
                {combineSource.name ||
                  combineSource.email ||
                  `Account #${combineSource.id}`}
                <span className="ml-1 font-black text-red-700">
                  (
                  {t("adminUsers.deletedAfterMerge", {
              ...
              <ArrowDown
                aria-hidden="true"
                className="mx-auto my-1 text-blue-800"
                size={20}
              />
              <p className="truncate font-semibold">
                {combineTarget.name ||
                  combineTarget.email ||
                  `Account #${combineTarget.id}`}
                <span className="ml-1 font-black text-emerald-700">
                  ({t("adminUsers.survivesMerge", { defaultValue: "survives" })}
                  )
                </span>
              </p>
            </div>
          ...
          <p className="rounded-xl bg-blue-50 p-3 text-xs font-semibold text-blue-950">
            {t("adminUsers.combineRules", {
              defaultValue:
                "The survivor keeps its login identity and strongest role/plan. Contacts, requests, templates, platforms, analytics, and history move over. When one SMTP connection is verified and the other is not, the verified connection is kept automatically. The merge still stops if both SMTP connections share the same verification state or if another billing or email-service conflict needs your choice.",
            })}
          </p>

          <label
            className="text-sm font-black text-slate-900"
            htmlFor="combine-account-confirmation"
          >
            {t("adminUsers.typeCombine", {
              defaultValue: "Type COMBINE to confirm",
            })}
          </label>
          <input
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={combineAccounts.isPending}>
              {t("common.cancel", { defaultValue: "Cancel" })}
            </AlertDialogCancel>
            <button
              type="button"
              disabled={
                !combineSource ||
                !combineTargetId ||
                combineConfirmation !== "COMBINE" ||
                combineAccounts.isPending
              }
              onClick={() =>
                combineSource &&
                combineAccounts.mutate({
