...
export default function ChurnSurveyPage() {
  ...
  const guarantee = trpc.stripe.guaranteeStatus.useQuery(undefined, {
    retry: false,
  });
  ...
  const amount = formatMoney(
    guarantee.data?.amount ?? null,
    guarantee.data?.currency ?? null
  );
  const deadline = formatDate(guarantee.data?.deadlineAt ?? null);
  const canRefund =
    guarantee.data?.eligible || guarantee.data?.reason === "already_refunded";
  ...
  const statusCopy = useMemo(() => {
    ...
    if (reason === "eligible") {
      return {
        badge: t("eligibleBadge"),
        body: t("eligibleText", { amount, deadline }),
        tone: "eligible" as const,
      };
    }
    if (reason === "already_refunded") {
      return {
        badge: t("refundedBadge"),
        body: t("refundedText"),
  ...
  const finalError =
    claimGuarantee.error?.message || cancelRenewal.error?.message;
      ...
      <header className="relative overflow-hidden bg-[#061a3a] text-white">
        ...
        <div className="relative mx-auto max-w-5xl px-5 pb-10 pt-8 sm:px-8 sm:pb-12 sm:pt-10">
          <div className="mb-7 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <img
                ...
                aria-hidden="true"
              />
              <span className="font-['Syne'] text-sm font-black tracking-[0.14em] text-white">
                GET <span className="text-[#D4A017]">PHAME</span>
              </span>
            </div>
            <button
              type="button"
          ...
          <div className="max-w-3xl">
            <p className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-[#D4A017]">
              {t("eyebrow")}
            </p>
      ...
      <section className="mx-auto -mt-5 grid max-w-5xl gap-5 px-5 sm:px-8 lg:grid-cols-[1.12fr_.88fr]">
        <div className="rounded-[1.75rem] border border-black/5 bg-white p-5 shadow-[0_18px_55px_rgba(6,26,58,.12)] sm:p-7">
          {guarantee.isLoading ? (
            ...
            <div className="flex min-h-56 flex-col items-center justify-center gap-4 text-center">
              ...
              <p className="max-w-sm font-bold text-[#31415f]">
                {t("loadError")}
              </p>
              <button
                type="button"
                onClick={() => guarantee.refetch()}
                className="rounded-xl bg-[#061a3a] px-5 py-3 text-sm font-black text-white"
              >
                {t("continue")}
              </button>
            </div>
            ...
            <>
              <div className="mb-6 flex items-start gap-4">
                <div className="mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#061a3a] text-[#D4A017]">
                  {statusCopy.tone === "eligible" ? (
                    <CircleDollarSign size={23} />
                  ) : statusCopy.tone === "refunded" ? (
                    <BadgeCheck size={23} />
                  ) : (
                    <Clock3 size={23} />
                  )}
                ...
                <div>
                  <span
                    className={`inline-flex rounded-full px-3 py-1 text-xs font-black ${statusCopy.tone === "eligible" ? "bg-emerald-100 text-emerald-800" : statusCopy.tone === "expired" ? "bg-amber-100 text-amber-900" : statusCopy.tone === "refunded" ? "bg-sky-100 text-sky-900" : "bg-slate-100 text-slate-700"}`}
                  >
                    {statusCopy.badge}
                  </span>
                  ...
                  <p className="mt-2 text-sm font-medium leading-relaxed text-[#52617b]">
                    {statusCopy.body}
                  </p>
                </div>
              </div>
              ...
              {stage === "survey" ? (
                ...
                <ConfirmationPanel
                  ...
                  alreadyRefunded={
                    guarantee.data?.reason === "already_refunded"
                  }
                  confirmed={confirmed}
                ...
                <button
                  type="button"
                  onClick={() => navigate("/settings")}
                  className="w-full rounded-2xl bg-[#061a3a] px-5 py-4 text-sm font-black text-white transition active:scale-[.98]"
                >
                  {t("settingsButton")}
                </button>
              )}
        ...
        <aside className="rounded-[1.75rem] bg-[#061a3a] p-5 text-white shadow-[0_18px_55px_rgba(6,26,58,.18)] sm:p-7 lg:self-start">
          <div className="mb-5 flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#D4A017] text-[#061a3a]">
              <ShieldCheck size={22} />
            </div>
          ...
          <div className="space-y-4">
            {[
              t("refundIncludesOne"),
              t("refundIncludesTwo"),
              t("refundIncludesThree"),
            ].map(item => (
              <div key={item} className="flex items-start gap-3">
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#D4A017] text-[#061a3a]">
          ...
          <div className="mt-7 flex items-center gap-2 border-t border-white/10 pt-5 text-xs font-bold text-white/60">
            <LockKeyhole size={14} className="text-[#D4A017]" />
            {t("stripeVerified")}
          </div>
        </aside>
      </section>
...
function SurveyPanel({
  ...
  return (
    <div className="border-t border-slate-100 pt-6">
      ...
      <label className="mt-5 block text-xs font-black uppercase tracking-wider text-[#52617b]">
        {t("commentLabel")}
      </label>
      <textarea
        value={comment}
        ...
        rows={3}
        className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-[#061a3a] outline-none transition focus:border-[#D4A017] focus:ring-2 focus:ring-[#D4A017]/20"
        name="rr-pages-churn-survey-comment-252"
      />
      <div className="mt-5 grid gap-2 sm:grid-cols-2">
        <button
          type="button"
          onClick={onContinue}
          className="rounded-xl bg-[#061a3a] px-5 py-3.5 text-sm font-black text-white transition active:scale-[.98]"
        >
          {t("continue")}
        </button>
        <button
          type="button"
          onClick={onContinue}
          className="rounded-xl px-5 py-3.5 text-sm font-bold text-[#52617b] transition hover:bg-slate-50"
        >
          {t("skipSurvey")}
        </button>
      </div>
...
function ConfirmationPanel({
  ...
  return (
    <div className="border-t border-slate-100 pt-6">
      <div
        className={`rounded-2xl border p-4 ${isRefund ? "border-[#D4A017]/45 bg-[#fff9e8]" : "border-amber-200 bg-amber-50"}`}
      >
        <div className="flex items-start gap-3">
          {isRefund ? (
            <CircleDollarSign
              size={22}
              className="mt-0.5 shrink-0 text-[#9a7000]"
            />
          ) : (
            <CalendarClock
              size={22}
          ...
          <div>
            <h3 className="font-black">
              {isRefund ? t("confirmRefundTitle") : t("renewalTitle")}
            </h3>
            <p className="mt-1 text-sm font-medium leading-relaxed text-[#52617b]">
              {isRefund ? t("confirmRefundNotice") : t("renewalText")}
            </p>
          </div>
        </div>
      </div>
      <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-4 transition hover:bg-slate-50">
        <input
          ...
          className="mt-0.5 h-5 w-5 accent-[#061a3a]"
          name="rr-pages-churn-survey-confirmed-274"
        />
        <span className="text-sm font-bold leading-relaxed text-[#31415f]">
          {isRefund ? t("confirmRefundCheckbox") : t("confirmRenewalCheckbox")}
        </span>
      </label>
      {error && (
        <p
          role="alert"
          className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-800"
        >
          {isRefund ? t("refundError") : t("cancelError")}{" "}
      ...
      <button
        ...
        onClick={onSubmit}
        className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#061a3a] px-5 py-4 text-sm font-black text-white transition enabled:active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-45"
      >
        {pending && <Loader2 size={17} className="animate-spin" />}
        {pending
          ? isRefund
            ? t("processingRefund")
            : t("processingCancellation")
          : isRefund
            ? alreadyRefunded
              ? t("finishCancellationButton")
              : t("refundButton")
            : t("cancelRenewalButton")}
      </button>
      <button
        type="button"
        onClick={onBack}
        disabled={pending}
        className="mt-2 w-full rounded-xl px-5 py-3 text-sm font-bold text-[#52617b] transition hover:bg-slate-50"
      >
        {t("stayButton")}
      </button>
    </div>
  ...
  return (
    <div className="flex min-h-[28rem] flex-col items-center justify-center text-center">
      ...
      <h2 className="mt-5 text-2xl font-black">
        {isRenewal ? t("renewalSuccessTitle") : t("refundSuccessTitle")}
      </h2>
      <p className="mt-3 max-w-md text-sm font-medium leading-relaxed text-[#52617b]">
        {isRenewal
          ? t("renewalSuccessBody", { date: accessEndsAt })
          : kind === "already_refunded"
            ? t("alreadyRefundedSuccessBody")
            : t("refundSuccessBody", { amount })}
      </p>
      <button
        type="button"
        onClick={onDone}
        className="mt-7 rounded-xl bg-[#061a3a] px-6 py-3.5 text-sm font-black text-white transition active:scale-[.98]"
      >
        {t("settingsButton")}
      </button>
    </div>
