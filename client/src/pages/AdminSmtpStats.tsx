...
function HealthBar({
  ...
  return (
    <div
      className="flex h-2 rounded-full overflow-hidden w-full"
      style={{ background: "oklch(0.93 0.02 260)" }}
    >
      {okPct > 0 && (
        <div
          style={{ width: `${okPct}%`, background: "oklch(0.55 0.18 145)" }}
        />
      )}
      {failPct > 0 && (
        <div
          style={{ width: `${failPct}%`, background: "oklch(0.60 0.20 27)" }}
        />
      )}
      {neverPct > 0 && (
        <div
          style={{ width: `${neverPct}%`, background: "oklch(0.80 0.02 260)" }}
        />
      )}
    </div>
  );
...
export default function AdminSmtpStatsPage() {
  ...
  const { data, isLoading, refetch, isFetching } =
    trpc.admin.smtpStats.useQuery(undefined, {
  ...
  const runHealthCheck = trpc.admin.runHealthCheck.useMutation({
    onSuccess: result => {
      toast.success(
        `Health check complete — ran at ${new Date(result.ranAt).toLocaleTimeString()}`
      );
      refetch();
    },
      ...
      <div className="px-5 pt-5 flex flex-col gap-4">
        ...
        {totals && (
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <p className="text-sm font-bold mb-1 rr-text-navy-mid">
                Connected Accounts
              </p>
              <p className="text-3xl font-black rr-text-navy">{totals.total}</p>
            </div>
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <p className="text-sm font-bold mb-1 rr-text-navy-mid">
                Failure Rate
          ...
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl rr-bg-surface">
            ...
            <p className="text-sm font-bold rr-text-navy-mid">
              ...
              <span className="font-bold">
                {new Date(totals.lastRunAt).toLocaleString()}
              </span>
            </p>
          </div>
        ) : (
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div
            ...
            style={{ borderColor: "oklch(0.93 0.02 260)" }}
          >
            <div className="flex items-center gap-2">
              <Server size={16} className="rr-text-navy-muted" />
              <h2 className="text-sm font-black rr-text-navy">By Provider</h2>
            </div>
          </div>
          ...
          {summary.length === 0 ? (
            ...
            <div
              className="divide-y"
              style={{ borderColor: "oklch(0.95 0.01 260)" }}
            >
              {summary.map(row => (
                  ...
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-0.5 text-sm font-bold rr-text-navy-mid">
                        <span>
                          {row.total} account{row.total !== 1 ? "s" : ""}
                        </span>
                        {row.ok > 0 && (
                        ...
                        {row.fail > 0 && (
                          <span style={{ color: "oklch(0.50 0.18 27)" }}>
                            ✗ {row.fail} failing
                          </span>
                        )}
                        {row.neverChecked > 0 && (
                          <span className="rr-text-navy-faint">
                            — {row.neverChecked} unchecked
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {row.fail === 0 && row.ok > 0 && (
                      <span
                        className="shrink-0 px-2 py-0.5 rounded-full text-xs font-bold rr-bg-green-pale"
        ...
        {totals && totals.neverChecked > 0 && (
          ...
          >
            ...
            <p>
              <span className="font-bold">
                {totals.neverChecked} account
                {totals.neverChecked !== 1 ? "s" : ""}
              </span>{" "}
              have never been checked — the daily cron runs at midnight and will
