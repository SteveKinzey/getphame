      ...
      <main className="mx-auto max-w-3xl px-4 pt-5 sm:px-5">
        <section className="rounded-2xl bg-white p-4 shadow-sm sm:p-5" aria-labelledby="retention-controls-title">
          ...
          {policy.isLoading ? <div className="flex min-h-32 items-center justify-center"><Loader2 size={22} className="animate-spin rr-text-navy-muted" /></div> : <div className="grid gap-4 sm:grid-cols-2">
            <label className="flex flex-col gap-2 rounded-xl border p-4" style={{ borderColor: "oklch(0.88 0.03 260)" }}><span className="text-sm rr-fw-black rr-text-navy">{t("adminAuditRetention.routeDays", { defaultValue: "Route audit history" })}</span><span className="text-xs font-bold rr-text-navy-muted">{t("adminAuditRetention.routeDaysHelp", { defaultValue: "Days to retain sanitized public-route audit outcomes." })}</span><div className="flex items-center gap-2"><input aria-label={t("adminAuditRetention.routeDays", { defaultValue: "Route audit history" })} type="number" min={MIN_RETENTION_DAYS} max={MAX_RETENTION_DAYS} value={routeDays} onChange={(event) => setRouteDays(Number(event.target.value))} className="h-11 w-28 rounded-lg border bg-white px-3 text-sm font-black rr-text-navy" style={{ borderColor: "oklch(0.84 0.04 260)" }}  name="rr-pages-admin-audit-retention-route-days-93" /><span className="text-sm font-bold rr-text-navy-muted">{t("adminAuditRetention.days", { defaultValue: "days" })}</span></div></label>
            <label className="flex flex-col gap-2 rounded-xl border p-4" style={{ borderColor: "oklch(0.88 0.03 260)" }}><span className="text-sm rr-fw-black rr-text-navy">{t("adminAuditRetention.rendererDays", { defaultValue: "Renderer-error history" })}</span><span className="text-xs font-bold rr-text-navy-muted">{t("adminAuditRetention.rendererDaysHelp", { defaultValue: "Days to retain sanitized preview-renderer failure signals." })}</span><div className="flex items-center gap-2"><input aria-label={t("adminAuditRetention.rendererDays", { defaultValue: "Renderer-error history" })} type="number" min={MIN_RETENTION_DAYS} max={MAX_RETENTION_DAYS} value={rendererDays} onChange={(event) => setRendererDays(Number(event.target.value))} className="h-11 w-28 rounded-lg border bg-white px-3 text-sm font-black rr-text-navy" style={{ borderColor: "oklch(0.84 0.04 260)" }}  name="rr-pages-admin-audit-retention-renderer-days-94" /><span className="text-sm font-bold rr-text-navy-muted">{t("adminAuditRetention.days", { defaultValue: "days" })}</span></div></label>
          </div>}
