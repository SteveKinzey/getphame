...
import {
  createContactExportPdfBlob,
  downloadContactExportBlob,
} from "@/lib/contactExportPdf";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function toContactSearchLocale(
  language: string
): "en" | "zh-CN" | "es" | "fr" | "it" | "th" | "zh-TW" {
...
export default function SavedContacts() {
  ...
  const [naturalSearchResult, setNaturalSearchResult] =
    useState<NaturalContactSearchResult | null>(null);
  const [exportingFormat, setExportingFormat] = useState<"csv" | "pdf" | null>(
    null
  );
  const [dormancyFilter, setDormancyFilter] = useState<
    "all" | "30" | "60" | "90"
  >("all");
  const [tagFilter, setTagFilter] = useState<string | null>(null);
  const [sourceFilter, setSourceFilter] = useState<
    "all" | "stripe" | "woocommerce" | "manual"
  >("all");
  const [optedOutFilter, setOptedOutFilter] = useState<"all" | "unsubscribed">(
    "all"
  );
  const [consentFilter, setConsentFilter] = useState<
    "all" | "consent" | "no_consent" | "opted_out"
  >(() => {
  ...
  const { data: wooSyncHistory = [] } = trpc.woo.syncHistory.useQuery(
    undefined,
    {
  ...
  const allComplianceChecked =
    complianceChecked.realCustomers &&
  ...
  const { data: sendHistory = [], isLoading: historyLoading } =
    trpc.contacts.sendHistory.useQuery(
  ...
  const syncFromStripeMutation = trpc.contacts.syncFromStripe.useMutation({
    onSuccess: result => {
      ...
      if (result.inserted > 0) {
        toast.success(
          `${result.inserted} new contact${result.inserted !== 1 ? "s" : ""} imported from Stripe.`
        );
      } else {
        toast.info(
          result.total === 0
            ? "No Stripe customers found."
            : "All Stripe customers are already in your contacts."
        );
      }
    },
    onError: e => toast.error(e.message),
  ...
  const [consentPreviewMode, setConsentPreviewMode] = useState<
    "desktop" | "mobile"
  >("desktop");
  const bulkConsentRequestMutation =
    trpc.contacts.bulkConsentRequest.useMutation({
      onSuccess: result => {
        toast.success(
          `Consent emails sent: ${result.sent} delivered${result.failed > 0 ? `, ${result.failed} failed — check SMTP settings` : ""}.`
        );
        utils.contacts.list.invalidate();
      },
      onError: err => {
  ...
  const sendConsentTestMutation =
    trpc.contacts.sendConsentTestEmail.useMutation({
      onSuccess: result => {
      ...
      if (result.sent + queued > 0 && result.failed === 0) {
        toast.success(
          `${result.sent} review request${result.sent !== 1 ? "s" : ""} sent` +
            (queued > 0
              ? `; ${queued} queued until local quiet hours end`
              : "!") +
            (result.skippedDueToLimit > 0
              ? ` (${result.skippedDueToLimit} skipped — free limit reached)`
              : "")
        );
      } else if (result.sent + queued > 0 && result.failed > 0) {
        toast.warning(
          `${result.sent} sent, ${queued} queued, ${result.failed} failed. Check your email connection in Settings.`
        );
      } else {
        toast.error(
          `All ${result.failed} sends failed. Check your email connection in Settings.`
        );
      }
    },
    onError: e => {
  ...
  const now = Date.now();
  const searchableContacts: Contact[] =
    naturalSearchResult?.results ?? contacts;
  const filtered = searchableContacts.filter(c => {
    ...
    if (consentFilter === "consent" && c.consentBasis !== "explicit_opt_in")
      return false;
    if (
      consentFilter === "no_consent" &&
      (c.consentBasis === "explicit_opt_in" || c.consentBasis === "opted_out")
    )
      return false;
    if (consentFilter === "opted_out" && c.consentBasis !== "opted_out")
      return false;
  ...
  const interpretedFilters = naturalSearchResult
    ? [
        naturalSearchResult.filters.text
          ? t("contactsTools.filters.text", {
              value: naturalSearchResult.filters.text,
              defaultValue: `Matches “${naturalSearchResult.filters.text}”`,
            })
          : null,
        naturalSearchResult.filters.source
          ? t("contactsTools.filters.source", {
              value: naturalSearchResult.filters.source,
              defaultValue: `Source: ${naturalSearchResult.filters.source}`,
            })
          : null,
        naturalSearchResult.filters.tag
          ? t("contactsTools.filters.tag", {
              value: naturalSearchResult.filters.tag,
              defaultValue: `Tag: ${naturalSearchResult.filters.tag}`,
            })
          : null,
        naturalSearchResult.filters.sentState === "never"
          ? t("contactsTools.filters.neverSent", "Never contacted")
          : null,
        naturalSearchResult.filters.sentState === "sent"
          ? t("contactsTools.filters.sent", "Contacted before")
          : null,
        naturalSearchResult.filters.sentState === "dormant"
          ? t("contactsTools.filters.dormant", {
              count: naturalSearchResult.filters.dormantDays ?? 0,
              defaultValue: `Not contacted in ${naturalSearchResult.filters.dormantDays} days`,
            })
          : null,
        naturalSearchResult.filters.consent === "recorded"
          ? t("contactsTools.filters.consentRecorded", "Consent recorded")
          : null,
        naturalSearchResult.filters.consent === "missing"
          ? t("contactsTools.filters.consentMissing", "Consent not recorded")
          : null,
        naturalSearchResult.filters.suppression === "active"
          ? t("contactsTools.filters.active", "Active contacts")
          : null,
        naturalSearchResult.filters.suppression === "opted_out"
          ? t("contactsTools.filters.optedOut", "Opted out")
          : null,
        naturalSearchResult.filters.createdFrom
          ? t("contactsTools.filters.createdFrom", {
              value: naturalSearchResult.filters.createdFrom,
              defaultValue: `Added after ${naturalSearchResult.filters.createdFrom}`,
            })
          : null,
        naturalSearchResult.filters.createdTo
          ? t("contactsTools.filters.createdTo", {
              value: naturalSearchResult.filters.createdTo,
              defaultValue: `Added before ${naturalSearchResult.filters.createdTo}`,
            })
          : null,
  ...
  function handleSubmit() {
    ...
    if (!editContact && !form.consentGiven) {
      toast.error(
        t(
          "contactsTools.addConsentRequired",
          "Consent confirmation is required to add a contact"
        )
      );
      return;
    }
    ...
    if (editContact) {
      ...
      createMutation.mutate({
        ...payload,
        consentGiven: true,
        consentSource: "manual_add_contact_form",
      });
    }
  }
  ...
  function runNaturalSearch(query = naturalQuery) {
    ...
    if (trimmed.length < 3) {
      toast.error(
        t(
          "contactsTools.search.minimum",
          "Describe what you want to find using at least 3 characters."
        )
      );
      return;
    }
  ...
  async function exportVisibleContacts(format: "csv" | "pdf") {
    if (filtered.length === 0) {
      toast.error(
        t(
          "contactsTools.export.empty",
          "There are no visible contacts to export."
        )
      );
      return;
    }
    ...
    try {
      const snapshot = await prepareExportMutation.mutateAsync({
        contactIds,
        format,
      });
      if (snapshot.exportedCount === 0) {
        toast.error(
          t(
            "contactsTools.export.unavailable",
            "These contacts are no longer available to export."
          )
        );
        return;
      }
      ...
      if (format === "csv") {
        const blob = new Blob([snapshot.csv ?? ""], {
        ...
        const blob = await createContactExportPdfBlob({
          ...
          labels: {
            ...
            truncated: t(
              "contactsTools.export.pdf.truncated",
              "Limited to the first 750 contacts"
            ),
            contact: t("contactsTools.export.pdf.contact", "Contact"),
            sourceAndTags: t(
              "contactsTools.export.pdf.sourceAndTags",
              "Source / tags"
            ),
      ...
      track("contact_export", {
        format,
        count: snapshot.exportedCount,
        truncated,
      });
      toast.success(
        truncated
          ? t("contactsTools.export.successTruncated", {
              count: snapshot.exportedCount,
              defaultValue: `Downloaded ${snapshot.exportedCount} contacts. The export limit was applied.`,
            })
          : t("contactsTools.export.success", {
              count: snapshot.exportedCount,
              defaultValue: `Downloaded ${snapshot.exportedCount} contacts.`,
            })
      );
    } catch (error) {
      toast.error(
    ...
    <div className="min-h-screen pb-40 rr-bg-cream-warm">
      ...
      <div className="px-5 pt-14 pb-6 rr-bg-navy">
        ...
        >
          {wooCreds && (
            <>
              <select
                value={wooDays}
                onChange={e =>
                  setWooDays(Number(e.target.value) as 30 | 60 | 90)
                }
              ...
              <Button
                ...
                title={
                  wooCreds.lastSyncedAt
                    ? `Last synced ${format(new Date(wooCreds.lastSyncedAt), "MMM d, h:mm a")}`
          ...
          <Button
            ...
            title={
              syncStatus?.stripeLastSyncedAt
                ? `Last synced ${format(new Date(syncStatus.stripeLastSyncedAt), "MMM d, h:mm a")}`
          ...
          >
            ...
            {t("pageHeader.importCsv", "Import CSV")}
          </Button>
          <Button
            onClick={() => exportVisibleContacts("csv")}
            disabled={
              filtered.length === 0 || exportingFormat !== null || isLoading
            }
            ...
            style={{ background: "oklch(0.32 0.07 260)" }}
            title={t(
              "contactsTools.export.csvTitle",
              "Download the currently visible contacts as a sanitized CSV"
            )}
          >
            {exportingFormat === "csv" ? (
              <Loader2
              ...
              <FileSpreadsheet size={14} className="mr-1" aria-hidden="true" />
            )}
            {exportingFormat === "csv"
              ? t("contactsTools.export.preparing", "Preparing…")
              : t("contactsTools.export.csv", "Export CSV")}
          </Button>
          <Button
            ...
            style={{ background: "oklch(0.32 0.07 260)" }}
            title={t(
              "contactsTools.export.pdfTitle",
              "Download the currently visible contacts as a PDF"
            )}
          >
            {exportingFormat === "pdf" ? (
              <Loader2
            ...
            {exportingFormat === "pdf"
              ? t("contactsTools.export.preparing", "Preparing…")
              : t("contactsTools.export.pdfButton", "Export PDF")}
          </Button>
        </div>
      </div>
      ...
      {wooCreds && (
        <div
          ...
          style={{
            ...
            borderBottom: "1px solid oklch(0.91 0.02 260)",
          }}
        >
          <ShoppingCart
            size={12}
          ...
          <span className="text-xs flex items-center gap-1 rr-text-navy-muted">
            ...
            {syncFromWooMutation.isPending ? (
              ...
              <>
                ...
                >
                  Last synced{" "}
                  {format(new Date(wooCreds.lastSyncedAt), "MMM d 'at' h:mm a")}
                </span>
                <button
              ...
              >
                Never synced — tap WooCommerce above to import
              </span>
            )}
          </span>
        </div>
      ...
      <div className="px-4 pt-4 space-y-3">
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h2
                id="contact-natural-search-heading"
                className="text-base font-black rr-text-navy"
              >
                {t(
                  "contactsTools.search.title",
                  "Find contacts in plain language"
                )}
              </h2>
              <p className="mt-0.5 text-sm rr-text-navy-muted">
                {t(
                  "contactsTools.search.description",
                  "Describe the customers you need. Contact data stays in Get Phame; only your query is interpreted."
                )}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <Input
              ...
              autoComplete="off"
              placeholder={t(
                "contactsTools.search.placeholder",
                "Example: Stripe contacts not contacted in 90 days"
              )}
            ...
            >
              ...
              {naturalSearchMutation.isPending
                ? t("contactsTools.search.searching", "Interpreting…")
                : t("contactsTools.search.action", "Find contacts")}
            </Button>
          </form>
          <p
            id="natural-contact-search-help"
            className="mt-2 text-xs rr-text-navy-muted"
          >
            {t(
              "contactsTools.search.help",
              "Try a source, consent status, tag, suppression state, date, or follow-up interval."
            )}
          </p>
          ...
          {naturalSearchMutation.isPending && (
            <div
              role="status"
              aria-live="polite"
              className="mt-3 rounded-xl bg-[oklch(0.98_0.01_80)] p-3"
            >
          ...
          {!naturalSearchResult && !naturalSearchMutation.isPending && (
            <div
              ...
              aria-label={t(
                "contactsTools.search.examplesLabel",
                "Search examples"
              )}
            >
              {[
                t(
                  "contactsTools.search.examples.dormant",
                  "Stripe contacts not contacted in 90 days"
                ),
                t(
                  "contactsTools.search.examples.consent",
                  "Contacts with recorded consent"
                ),
                t(
                  "contactsTools.search.examples.optedOut",
          ...
          {naturalSearchResult && (
            <div
              className="mt-3 rounded-xl bg-[oklch(0.96_0.025_160)] p-3"
              aria-live="polite"
            >
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs rr-text-navy-muted">
                    {naturalSearchResult.truncated
                      ? t("contactsTools.search.truncated", {
                          count: naturalSearchResult.results.length,
                          defaultValue: `Showing the first ${naturalSearchResult.results.length} results.`,
                        })
                      : t(
                          "contactsTools.search.complete",
                          "All matching contacts are shown."
                        )}
                    {naturalSearchResult.redacted
                      ? ` ${t("contactsTools.search.redacted", "A secret-like value was removed before interpretation.")}`
                      : ""}
                  </p>
                </div>
                <button
                  ...
                  aria-label={t(
                    "contactsTools.search.clear",
                    "Clear conversational search"
                  )}
                >
              ...
              {interpretedFilters.length > 0 && (
                <div
                  ...
                  aria-label={t(
                    "contactsTools.search.interpretedAs",
                    "Interpreted filters"
                  )}
                >
                  {interpretedFilters.map(filter => (
                    <span
                      key={filter}
                      className="rounded-full bg-white px-2.5 py-1 text-xs font-bold rr-text-navy"
                    >
                      {filter}
                    </span>
                  ))}
              ...
              {(
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      setSourceFilter(
                        opt as "all" | "stripe" | "woocommerce" | "manual"
                      )
                    }
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2 overflow-x-auto pb-1">
              ...
              <button
                ...
                style={{
                  background:
                    tagFilter === null ? "oklch(0.55 0.12 160)" : "white",
              ...
              {allTags.map(tag => (
                <button
                  ...
                  style={{
                    background:
                      tagFilter === tag ? "oklch(0.55 0.12 160)" : "white",
        ...
        {isLoading ? (
          <ContactListSkeleton
            label={t(
              "contactsTools.loading.contacts",
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 font-medium">
              {search || naturalSearchResult
                ? t("contactsTools.search.noMatches", "No matches found")
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {!!c.optedOut && (
                        <span
                          ...
                          style={{
                            background: "oklch(0.95 0.02 27)",
                            color: "oklch(0.50 0.15 27)",
                    ...
                    <div className="flex flex-wrap items-center gap-1 mt-2">
                      {parseTags(c.tags).map(tag => (
                        ...
                        >
                          ...
                          <button
                            onClick={e => {
                              e.stopPropagation();
                              const newTags = parseTags(c.tags).filter(
                                t => t !== tag
                              );
                      ...
                      {tagInputId === c.id ? (
                        ...
                        <button
                          ...
                          style={{
                            borderColor: "oklch(0.75 0.08 160)",
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <Button
                      ...
                      disabled={!!c.optedOut}
                      title={
                        c.optedOut ? "Contact has unsubscribed" : undefined
                      }
                    >
      ...
      {selectedCount > 0 && (
        ...
        >
          <div
            className="rounded-2xl px-4 py-3 flex items-center justify-between gap-3 shadow-xl rr-bg-navy"
            style={{ border: "1px solid oklch(0.35 0.07 260)" }}
          >
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shrink-0 rr-bg-gold rr-text-navy">
                {selectedCount}
              </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="e.g. Regular customer, prefers email"
                rows={2}
              />
            </div>
            {!editContact && (
              <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
                ...
                <label
                  ...
                  className="cursor-pointer text-xs leading-snug text-amber-900"
                >
                  {t("contactsTools.addConsentLabel", {
                    businessName:
                      profile?.consentLabelName ||
                      profile?.businessName ||
                      "your business",
                    defaultValue:
                      "I confirm this customer has consented to be contacted by {{businessName}} via email and/or text about their experience and purchases",
                  })}
                </label>
              </div>
            )}
      ...
      >
        <AlertDialogContent className="max-w-sm mx-4">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Send a review request email to <strong>{sendTarget?.name}</strong>{" "}
              ({sendTarget?.email})?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog
        ...
        onOpenChange={o => {
          if (!o) setBulkConfirmOpen(false);
        }}
      >
        <AlertDialogContent className="max-w-sm mx-4">
          <AlertDialogHeader>
            <AlertDialogTitle>
              Send to {selectedCount} contacts?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This will send a review request email to all {selectedCount}{" "}
              selected contact{selectedCount !== 1 ? "s" : ""} using your
            ...
            {platforms.some(p => p.platform === "yelp") && (
              <div
                className="mt-2 rounded-xl px-3 py-2 text-xs flex items-start gap-2 rr-bg-gold-pale"
                style={{ border: "1px solid oklch(0.85 0.12 80)" }}
              >
                <AlertTriangle
                  size={13}
                  className="mt-0.5 shrink-0"
                ...
                <span className="rr-text-gold-dim">
                  <strong>Yelp note:</strong> Your Yelp listing will appear as a
                  search suggestion rather than a direct link. Yelp discourages
                  direct solicitation.
                </span>
              </div>
            )}
            ...
            {selectedCount >= 20 && (
              ...
              >
                ...
                />
                <span className="rr-text-gold-dim">
                  <strong>Large send ({selectedCount} contacts):</strong> Sudden
                  spikes can look spammy to review platforms. Consider spreading
                  sends over multiple days.
                </span>
              </div>
            )}
            <div className="mt-3">
            ...
            <div className="mt-1">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full px-3 py-2 pr-8 rounded-xl text-sm outline-none appearance-none bg-white"
                  style={{
                ...
                <ChevronDown
                  size={12}
                  className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none rr-text-navy-mid"
                />
              </div>
            </div>
          )}
          {/* Reminder toggle */}
          <div
            className="mt-2 flex items-start gap-3 rounded-xl px-3 py-2.5 cursor-pointer rr-bg-white-card"
            style={{ border: "1px solid oklch(0.88 0.03 260)" }}
          ...
          >
            ...
            <div>
              <label
                htmlFor="bulk-reminder-checkbox"
                className="text-xs font-semibold cursor-pointer block rr-text-navy"
              >
                Schedule 3-day follow-up reminders
              </label>
              <p className="text-xs mt-0.5 rr-text-navy-mid">
                Automatically send a reminder to any contact who hasn't
                responded in 3 days.
              </p>
            </div>
          </div>
          ...
          <div
            className="mt-2 rounded-xl p-3 rr-bg-white-card"
            style={{ border: "1px solid oklch(0.88 0.03 260)" }}
          >
            ...
            {[
              {
                key: "realCustomers" as const,
                label: "These are real customers who transacted with me",
              },
              {
                key: "noIncentives" as const,
                label: "No incentives or discounts are being offered",
              },
              {
                key: "allCustomers" as const,
                label:
                  "I'm sending to all customers, not filtering by satisfaction",
              },
            ].map(({ key, label }) => (
              <div
                key={key}
                ...
                onClick={() =>
                  setComplianceChecked(v => ({ ...v, [key]: !v[key] }))
                }
              >
                {complianceChecked[key] ? (
                  <CheckCircle2
                    size={14}
                    className="mt-0.5 shrink-0 rr-text-green"
                  />
                ) : (
                  <div
                    className="w-3.5 h-3.5 mt-0.5 shrink-0 rounded-full border-2"
                    style={{ borderColor: "oklch(0.70 0.04 260)" }}
                  />
                )}
                <span
                  className="text-xs"
                  style={{
                    color: complianceChecked[key]
                      ? "oklch(0.35 0.05 260)"
                      : "oklch(0.50 0.04 260)",
                  }}
                >
          ...
          <AlertDialogFooter>
            ...
            >
              {bulkSendMutation.isPending ? (
                ...
                <>
                  <Star size={14} className="mr-1" /> Send {selectedCount}{" "}
                  Requests
                </>
              )}
            </AlertDialogAction>
      ...
      >
        <AlertDialogContent className="max-w-md mx-4">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Sending to <strong>{selectedCount}</strong> contact
              {selectedCount !== 1 ? "s" : ""} with no consent on file. Contacts
              with existing consent or who opted out are skipped automatically.
            </AlertDialogDescription>
          </AlertDialogHeader>
          {/* Template Editor */}
          <div className="flex flex-col gap-2 mt-1">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                style={{
                  background: consentPreviewOpen
                    ? "oklch(0.22 0.09 260)"
                    : "oklch(0.95 0.02 260)",
                  color: consentPreviewOpen ? "white" : "oklch(0.40 0.06 260)",
                }}
              >
                <Eye size={11} />{" "}
                {consentPreviewOpen ? "Hide preview" : "Preview"}
              </button>
            </div>
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {(
                [
                  ...
                  {
                    label: "{{currentDate}}",
                    desc: "Today's date (e.g. August 10, 2026)",
                  },
                ] as const
              ).map(({ label, desc }) => (
                <button
            ...
            <div>
              <label className="block text-xs font-semibold mb-1 rr-text-navy-mid">
                Subject
              </label>
              <input
                type="text"
            ...
            {(consentCustomSubject || consentCustomBody) && (
              <button
                onClick={() => {
                  setConsentCustomSubject("");
                  setConsentCustomBody("");
                }}
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          You received this email because you are a customer of{" "}
                          {biz}.{" "}
                          <span
                            style={{
                              color: "#9ca3af",
          ...
          <AlertDialogFooter className="mt-2">
            <button
              onClick={() =>
                sendConsentTestMutation.mutate({
            ...
            <AlertDialogCancel onClick={() => setConsentConfirmOpen(false)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
      ...
      <Dialog
        open={wooSyncHistoryOpen}
        onOpenChange={open => {
          setWooSyncHistoryOpen(open);
          if (!open) setWooHistorySearch("");
        }}
      >
          ...
          {wooSyncHistory.length > 0 && (
            <div className="relative">
              <Search
                ...
                style={{ color: "oklch(0.60 0.05 260)" }}
                aria-hidden="true"
              />
              <Input
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto">
            {(() => {
              ...
              return filtered.map(log => (
                <div
                  key={log.id}
                  className="rounded-lg px-3 py-2.5 flex items-start gap-3 rr-bg-white-card"
                    ...
                    <p className="text-xs mt-0.5 rr-text-navy-muted">
                      {log.added > 0 ? (
                        <>
                          ...
                          customer{log.added !== 1 ? "s" : ""} imported ·{" "}
                          {log.total} orders scanned ({log.daysWindow}d window)
                        </>
                      ) : (
                        <>
                          No new customers · {log.total} orders scanned (
                          {log.daysWindow}d window)
                        </>
                      )}
                    </p>
                  </div>
          ...
          {syncFromWooMutation.isPending && (
            <div className="px-1">
              ...
              <div
                ...
                style={{ background: "oklch(0.92 0.02 260)" }}
              >
                <div
                  className="h-full rounded-full animate-pulse rr-bg-gold"
                  style={{ width: "60%" }}
                />
              </div>
            </div>
          ...
          <DialogFooter className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5">
              <Button
                ...
                onClick={() =>
                  wooCreds && syncFromWooMutation.mutate({ days: wooDays })
                }
                className="rr-bg-navy rr-text-gold"
                aria-label={
                  syncFromWooMutation.isPending
                    ? "Syncing WooCommerce orders"
                    : `Sync WooCommerce orders (last ${wooDays} days)`
                }
              >
                {syncFromWooMutation.isPending ? (
                  <>
          ...
          {historyLoading ? (
            ...
            <div className="flex flex-col gap-2 max-h-80 overflow-y-auto pr-1">
              {sendHistory.map(row => (
                <div
                  key={row.id}
                  className="rounded-xl px-3 py-2.5 flex items-start justify-between gap-2 rr-bg-white-card"
                ...
                {historyContact.consentBasis === "explicit_opt_in" ? (
                  ...
                  >
                    ...
                    <div>
                      ...
                      {historyContact.consentCapturedAt && (
                        ...
                        >
                          {format(
                            new Date(historyContact.consentCapturedAt),
                            "MMM d, yyyy 'at' h:mm a"
                          )}
                        </p>
                      )}
                  ...
                  >
                    <ShieldOff
                      size={13}
                      className="mt-0.5 shrink-0"
                      style={{ color: "oklch(0.55 0.15 27)" }}
                    />
                    <div>
                      <p
                        className="text-xs font-semibold"
                      ...
                      {historyContact.optedOutAt && (
                        <p
                          className="text-xs mt-0.5"
                          style={{ color: "oklch(0.55 0.10 27)" }}
                        >
                          {format(
                            new Date(historyContact.optedOutAt),
                            "MMM d, yyyy 'at' h:mm a"
                          )}
                        </p>
                      )}
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        Select this contact and use the Consent button to send a
