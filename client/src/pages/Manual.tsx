...
function filterSections(sections: ManualSection[], rawQuery: string) {
  ...
  return sections.flatMap(section => {
    if (
      `${section.title} ${section.summary}`.toLocaleLowerCase().includes(query)
    ) {
      return [section];
    }
...
function ManualSearchInsightsPanel() {
  ...
  return (
    <section
      className="mt-6 border-t-4 border-[#e9b949] bg-white px-5 py-5 shadow-sm sm:px-6"
      aria-labelledby="manual-search-insights-title"
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl">
          <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-[#725200]">
            ...
            {t("manual.insights.eyebrow", {
              defaultValue: "Documentation intelligence",
            })}
          </div>
          <h2
            id="manual-search-insights-title"
            className="mt-2 text-xl font-black rr-text-navy"
            style={{ fontFamily: "'Poppins', sans-serif" }}
          >
            {t("manual.insights.title", {
              defaultValue: "Searches with no results",
            })}
          </h2>
          <p className="mt-2 text-sm leading-6 rr-text-navy-muted">
            {t("manual.insights.description", {
              defaultValue:
                "Use these aggregate terms to prioritize missing Manual topics. Repeated searches from the same account count once per UTC day.",
            })}
          </p>
          <p className="mt-2 text-xs leading-5 rr-text-navy-muted">
            {t("manual.insights.privacy", {
              defaultValue:
                "Private by design: reports exclude account IDs, customer records, referrers, devices, and raw event rows.",
            })}
          </p>
        </div>
        ...
        <div
          ...
          role="group"
          aria-label={t("manual.insights.periodLabel", {
            defaultValue: "Reporting period",
          })}
        >
          {(["30", "90", "365"] as const).map(period => (
            <Button
              ...
              className={
                periodDays === period
                  ? "rr-bg-navy font-bold text-white"
                  : "border-[#14213d] bg-white font-bold rr-text-navy"
              }
              aria-pressed={periodDays === period}
              onClick={() => setPeriodDays(period)}
            >
              {t("manual.insights.periodDays", {
                count: Number(period),
      ...
      {insights.isLoading ? (
        <p className="mt-5 text-sm rr-text-navy-muted" role="status">
          {t("manual.insights.loading", {
            defaultValue: "Loading missing-topic searches…",
          })}
        </p>
      ) : insights.isError ? (
        <div
          className="mt-5 border-l-4 border-[#b42318] bg-[#fff4f2] px-4 py-3"
          role="alert"
        >
          <p className="text-sm font-bold text-[#8a1c13]">
            {t("manual.insights.error", {
              defaultValue:
                "Missing-topic analytics could not be loaded. Try again.",
            })}
          </p>
          <Button
            type="button"
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <div className="border-t-2 border-[#14213d] bg-[#f7f5ef] px-4 py-3">
              <p className="text-2xl font-black rr-text-navy">
                {formatter.format(insights.data.summary.totalSearches)}
              </p>
              <p className="text-xs font-bold rr-text-navy-muted">
                {t("manual.insights.totalSearches", {
                  defaultValue: "Zero-result searches",
                })}
              </p>
            </div>
            <div className="border-t-2 border-[#14213d] bg-[#f7f5ef] px-4 py-3">
              <p className="text-2xl font-black rr-text-navy">
                {formatter.format(insights.data.summary.uniqueTerms)}
              </p>
              <p className="text-xs font-bold rr-text-navy-muted">
                {t("manual.insights.uniqueTerms", {
                  defaultValue: "Distinct terms",
                })}
              </p>
            </div>
            <div className="border-t-2 border-[#14213d] bg-[#f7f5ef] px-4 py-3">
              <p className="text-sm font-black rr-text-navy">
                {t("manual.insights.roleBreakdown", {
                  ...
                  admin: formatter.format(
                    insights.data.summary.roleCounts.admin
                  ),
                  defaultValue: "User {{user}} · Admin {{admin}}",
                })}
              </p>
              <p className="mt-1 text-xs font-bold rr-text-navy-muted">
                {t("manual.insights.manualScope", {
                  defaultValue: "Manual scope",
                })}
              </p>
            </div>
          </div>
          ...
          {insights.data.items.length ? (
              ...
              <table className="w-full min-w-[680px] border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b-2 border-[#14213d]">
                    <th
                      scope="col"
                      className="px-3 py-3 font-black rr-text-navy"
                    >
                      {t("manual.insights.term", {
                        defaultValue: "Missing topic search",
                      })}
                    </th>
                    <th
                      scope="col"
                      className="px-3 py-3 font-black rr-text-navy"
                    >
                      {t("manual.insights.role", { defaultValue: "Manual" })}
                    </th>
                    <th
                      scope="col"
                      className="px-3 py-3 font-black rr-text-navy"
                    >
                      {t("manual.insights.locale", { defaultValue: "Locale" })}
                    </th>
                    <th
                      scope="col"
                      className="px-3 py-3 text-right font-black rr-text-navy"
                    >
                      {t("manual.insights.count", { defaultValue: "Count" })}
                    </th>
                    <th
                      scope="col"
                      className="px-3 py-3 font-black rr-text-navy"
                    >
                      {t("manual.insights.lastSearched", {
                        defaultValue: "Last searched",
                      })}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {insights.data.items.map(item => (
                    <tr
                      key={`${item.query}-${item.manualRole}-${item.locale}`}
                      className="border-b border-[#e8e3d7] last:border-b-0"
                    >
                      <th
                        scope="row"
                        className="max-w-[320px] px-3 py-3 font-bold rr-text-navy"
                      >
                        {item.query}
                      </th>
                      <td className="px-3 py-3 rr-text-navy-mid">
                        {item.manualRole === "admin"
                          ? t("manual.insights.adminManual", {
                              defaultValue: "Admin Manual",
                            })
                          : t("manual.insights.userManual", {
                      ...
                      <td className="px-3 py-3 text-right font-black rr-text-navy">
                      ...
                      <td className="px-3 py-3 rr-text-navy-mid">
                        {new Intl.DateTimeFormat(
                          ...
                          { dateStyle: "medium" }
                        ).format(new Date(item.lastSearchedAt))}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="mt-5 bg-[#f7f5ef] px-5 py-7 text-center">
              <Search
                ...
                aria-hidden="true"
              />
              <h3 className="mt-3 text-base font-black rr-text-navy">
                {t("manual.insights.emptyTitle", {
                  defaultValue: "No missing-topic searches yet",
                })}
              </h3>
              <p className="mt-1 text-sm rr-text-navy-muted">
                {t("manual.insights.emptyBody", {
                  defaultValue:
                    "Zero-result searches will appear here after daily account-level deduplication.",
                })}
              </p>
            </div>
          )}
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <h3
          className="text-lg font-black leading-snug rr-text-navy"
          style={{ fontFamily: "'Poppins', sans-serif" }}
        >
          <a
            className="rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e9b949] focus-visible:ring-offset-2"
            href={`#manual-topic-${topic.id}`}
          >
            {topic.title}
          </a>
        </h3>
      ...
      <p className="mt-3 max-w-4xl text-sm leading-6 rr-text-navy-mid">
        {topic.body}
      </p>

      <ol
        className="mt-4 space-y-3"
        aria-label={t("manual.stepsLabel", {
          defaultValue: `Steps for ${topic.title}`,
        })}
      >
        {topic.steps.map((step, index) => (
          <li
            key={`${topic.id}-${index}`}
            className="flex gap-3 text-sm leading-6 rr-text-navy-mid"
          >
      ...
      {topic.notes?.length ? (
        <div className="mt-4 border-l-4 border-[#e9b949] bg-[#fff9e8] px-4 py-3">
          ...
          {topic.notes.map((note, index) => (
            <p
              key={`${topic.id}-note-${index}`}
              className="mt-1 text-sm leading-6 text-[#4d3b00]"
            >
              {note}
            </p>
          ))}
...
export default function ManualPage() {
  ...
  const { mutate: trackZeroResultSearch } =
    trpc.analytics.trackManualZeroResultSearch.useMutation();
  const role: ManualRole = user?.role === "admin" ? "admin" : "user";
  const manualLocale = normalizeManualLocale(
    i18n.resolvedLanguage ?? i18n.language
  ) as "en" | "es" | "fr" | "it" | "th" | "zh-CN" | "zh-TW";
  const manual = useMemo(
    () => getManualDocument(i18n.resolvedLanguage ?? i18n.language, role),
    [i18n.language, i18n.resolvedLanguage, role]
  );
  const filteredSections = useMemo(
    () => filterSections(manual.sections, query),
  ...
  const paidTopicCount = manual.sections.reduce(
    (total, section) =>
      total + section.topics.filter(topic => topic.access === "paid").length,
    0
  );
  const adminSectionCount = manual.sections.filter(
  ...
  const exportPdf = async (section?: ManualSection) => {
    ...
    try {
      const displayNames = new Intl.DisplayNames([manualLocale], {
        type: "language",
      });
      const languageName =
        displayNames.of(manualLocale.split("-")[0]) ?? manualLocale;
      const blob = await createManualPdfBlob(manual, {
        ...
        labels: {
          fullScope: t("manual.export.fullScope", {
            defaultValue: "Entire visible Manual",
          }),
          sectionScope: t("manual.export.sectionScope", {
            defaultValue: "Manual section",
          }),
          scope: t("manual.export.scope", { defaultValue: "Export scope" }),
          language: t("manual.export.language", { defaultValue: "Language" }),
          generated: t("manual.export.generated", {
            defaultValue: "Generated",
          }),
          lastUpdated: t("manual.export.lastUpdated", {
            defaultValue: "Manual last updated",
          }),
          version: t("manual.export.version", {
            defaultValue: "Manual version",
          }),
          access: {
            all: t("manual.badges.all", { defaultValue: "Free & paid" }),
            paid: t("manual.badges.paid", {
              defaultValue: "Paid subscription only",
            }),
            admin: t("manual.badges.admin", {
          ...
          featurePath: t("manual.export.featurePath", {
            defaultValue: "Feature path",
          }),
          page: t("manual.export.page", {
      ...
      toast.success(
        section
          ? t("manual.export.sectionSuccess", {
              section: section.title,
              defaultValue: "Downloaded {{section}} as a PDF.",
            })
          : t("manual.export.fullSuccess", {
              defaultValue: "Downloaded the complete Manual as a PDF.",
            })
      );
    } catch {
      toast.error(
        t("manual.export.error", {
          defaultValue:
            "The Manual PDF could not be created. Please try again.",
        })
      );
    } finally {
    ...
    <div className="min-h-screen pb-40 rr-bg-cream-warm">
      <header className="rr-bg-navy px-4 pb-8 pt-12 text-white sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-4 grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div>
              <h1
                className="max-w-4xl text-3xl font-black leading-tight sm:text-4xl"
                style={{ fontFamily: "'Poppins', sans-serif" }}
              >
                {manual.title}
              </h1>
              <p className="mt-3 max-w-4xl text-sm leading-6 text-white/80 sm:text-base">
                {manual.introduction}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-2 text-center sm:min-w-[290px]">
              <div className="border-t-2 border-[#e9b949] bg-white/5 px-4 py-3">
                ...
                <p className="text-xs font-bold text-white/70">
                  {t("manual.sectionCount", {
                    defaultValue: "Manual sections",
                  })}
                </p>
              </div>
              <div className="border-t-2 border-[#e9b949] bg-white/5 px-4 py-3">
                <p className="text-2xl font-black rr-text-gold">
                  {role === "admin" ? adminSectionCount : paidTopicCount}
                </p>
                <p className="text-xs font-bold text-white/70">
                  {role === "admin"
                    ? t("manual.adminSectionCount", {
                        defaultValue: "Admin-only sections",
                      })
                    : t("manual.paidTopicCount", {
                        defaultValue: "Paid-only topics",
                      })}
                </p>
              </div>
            </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="relative">
          <Search
            className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 rr-text-navy-muted"
            size={19}
            aria-hidden="true"
          />
          <Input
            ...
            placeholder={t("manual.searchPlaceholder", {
              defaultValue: "Search this manual",
            })}
        ...
        <p className="mt-2 text-xs rr-text-navy-muted" aria-live="polite">
          {query.trim()
            ? t("manual.searchResults", {
                count: resultCount,
                defaultValue: "{{count}} matching topics",
              })
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          >
            ...
            {exportingScope === "full"
              ...
              : t("manual.export.fullButton", {
                  defaultValue: "Download full Manual PDF",
                })}
          </Button>
          <p
            id="manual-export-help"
            className="text-xs leading-5 rr-text-navy-muted"
          >
            {t("manual.export.help", {
              defaultValue:
                "The PDF includes only the Manual available to your authenticated role and current language.",
            })}
          </p>
        </div>
        ...
        <nav
          ...
          aria-label={t("manual.sectionNavigation", {
            defaultValue: "Manual sections",
          })}
        >
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="hidden lg:block">
            <nav
              className="sticky top-6 border-t-4 border-[#e9b949] bg-white p-5 shadow-sm"
              aria-label={t("manual.sectionNavigation", {
                defaultValue: "Manual sections",
              })}
            >
                ...
                {manual.sections.map(section => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        size={17}
                        className="shrink-0 text-[#9a6b00]"
                        aria-hidden="true"
                      />
            ...
            {filteredSections.length ? (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <h2
                            className="text-xl font-black leading-tight rr-text-navy sm:text-2xl"
                            style={{ fontFamily: "'Poppins', sans-serif" }}
                          >
                            <a
                              href={`#manual-section-${section.id}`}
                        ...
                        <Button
                          ...
                          disabled={exportingScope !== null}
                          aria-label={t("manual.export.sectionAria", {
                            section: section.title,
                            defaultValue: "Download {{section}} as a PDF",
                          })}
                        ...
                        >
                          ...
                          {exportingScope === section.id
                            ? t("manual.export.preparing", {
                                defaultValue: "Preparing PDF…",
                              })
                            : t("manual.export.sectionButton", {
                                defaultValue: "Download section PDF",
                              })}
                        </Button>
                      </div>
                    </div>
              ...
              <section className="border-t-4 border-[#e9b949] bg-white px-6 py-14 text-center shadow-sm">
                <Search
                  ...
                  aria-hidden="true"
                />
                <h2 className="mt-4 text-xl font-black rr-text-navy">
                  {t("manual.noResultsTitle", {
                    defaultValue: "No matching topics",
                  })}
                </h2>
                <p className="mx-auto mt-2 max-w-lg text-sm leading-6 rr-text-navy-muted">
                  {t("manual.noResultsBody", {
                    defaultValue:
                      "Try a shorter search or browse the section links above.",
                  })}
                </p>
                <Button
                  type="button"
                  className="mt-5 rr-bg-navy font-bold text-white"
                  onClick={() => setQuery("")}
                >
