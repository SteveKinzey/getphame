    ...
    <div className="min-h-screen bg-[#0a1628] text-white flex flex-col">
          ...
          >
            <div>
              <h1 className="font-display font-extrabold text-4xl xl:text-5xl leading-tight mb-4">
                {t("onboarding.heroHeadline", { defaultValue: "Get More 5-Star" })}
                <br />
                <span style={{ color: "oklch(0.78 0.15 75)" }}>{t("onboarding.heroHighlight", { defaultValue: "Google Reviews" })}</span>
              </h1>
              <p className="text-xl font-bold leading-relaxed text-white/90">
                {t("onboarding.heroSubtitle", { defaultValue: "Send personalized review requests from your own email account. Your customers see it come from you — not a generic sender." })}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {[
                { icon: Shield, text: t("onboarding.trust1", { defaultValue: "AES-256 encrypted credentials — your data stays safe" }) },
                { icon: Lock, text: t("onboarding.trust2", { defaultValue: "Sends from YOUR email — maximum deliverability" }) },
                { icon: Zap, text: t("onboarding.trust3", { defaultValue: "Set up in under 2 minutes — no technical skills needed" }) },
              ].map((item) => (
            ...
            <div className="flex items-center gap-3 pt-4" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
              ...
              <span className="text-base font-bold text-white/70">
                {t("onboarding.socialProof", { defaultValue: "Trusted by 500+ local businesses" })}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 text-sm font-bold text-white/50">
              ...
              <span>{t("onboarding.compliance", { defaultValue: "Every email includes an unsubscribe link — fully compliant" })}</span>
            </div>
          </motion.div>
              ...
              <div className="relative rounded-2xl p-8 md:p-10 shadow-2xl" style={{ background: "oklch(0.18 0.03 250)", border: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                <div className="lg:hidden text-center mb-8">
                  ...
                  <h2 className="font-display font-extrabold text-2xl" style={{ color: "oklch(0.78 0.15 75)" }}>
                    {t("onboarding.heroHighlight", { defaultValue: "Google Reviews" })}
                  </h2>
                  <p className="text-base font-bold mt-3 leading-relaxed text-white/80">
                    {t("onboarding.heroSubtitleShort", { defaultValue: "Send personalized review requests from your own email account." })}
                  </p>
                </div>

                {/* Desktop heading */}
                <div className="hidden lg:block text-center mb-8">
                  <h2 className="font-display font-bold text-xl mb-1">{t("onboarding.cardHeading", { defaultValue: "Create your free account" })}</h2>
                  <p className="text-base font-bold text-white/70">{t("onboarding.cardSubheading", { defaultValue: "No credit card required" })}</p>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-3 my-1">
                    ...
                    <span className="text-sm font-black text-white/60">{t("onboarding.or", { defaultValue: "or" })}</span>
                    <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.12)" }} />
                  </div>
                  ...
                  {!showEmailForm ? (
                    ...
                    >
                      <Mail size={20} />
                      {t("onboarding.emailBtn", { defaultValue: "Continue with Email" })}
                    </button>
                  ) : magicState === "sent" ? (
                    <div
                    ...
                    >
                      ...
                      <div>
                        <p className="font-bold text-white text-sm mb-1">{t("onboarding.checkInbox", { defaultValue: "Check your inbox!" })}</p>
                        <p className="text-base font-bold text-white/70">
                          {t("onboarding.sentLink", { defaultValue: "We sent a sign-in link to" })} <strong className="text-white">{email}</strong>.
                          <br />{t("onboarding.expires", { defaultValue: "It expires in 15 minutes." })}
                        </p>
                      </div>
                      <button
                        onClick={handleRetry}
                      ...
                      >
                        {t("onboarding.useDifferentEmail", { defaultValue: "Use a different email" })}
                      </button>
                    </div>
                  ) : (
                    <form onSubmit={handleMagicLinkSend} className="flex flex-col gap-2">
                      ...
                      >
                        {magicState === "loading" ? (
                          <>
                            ...
                            {t("onboarding.sendingLink", { defaultValue: "Sending link…" })}
                          </>
                        ) : (
                          <>
                            <Mail size={18} />
                            {t("onboarding.sendSignInLink", { defaultValue: "Send Sign-In Link" })}
                          </>
                        )}
                      </button>
                      <button
                      ...
                      >
                        {t("onboarding.cancel", { defaultValue: "Cancel" })}
                      </button>
                    </form>
                  )}
                </div>
      ...
      <footer className="py-6 px-6 text-center">
        <div className="flex items-center justify-center gap-4 text-sm font-bold text-white/50">
          <a href="/privacy-policy" className="hover:text-white transition-colors">{t("onboarding.privacy", { defaultValue: "Privacy" })}</a>
          <span>·</span>
          <a href="/terms-of-service" className="hover:text-white transition-colors">{t("onboarding.terms", { defaultValue: "Terms" })}</a>
          <span>·</span>
          <a href="mailto:support@getphame.app" className="hover:text-white transition-colors">{t("onboarding.support", { defaultValue: "Support" })}</a>
        </div>
      </footer>
    </div>
  );
