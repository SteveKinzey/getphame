    ...
    return (
      <div className="flex min-h-screen items-center justify-center rr-bg-cream-warm px-5 text-center">
        <div className="max-w-sm">
          <ShieldAlert
            className="mx-auto mb-3 text-rose-700"
            size={46}
            aria-hidden="true"
          />
  ...
  if (release.error || !release.data) {
    return (
      <div className="min-h-screen rr-bg-cream-warm px-5 py-12">
        <section
          className="mx-auto max-w-xl rounded-2xl border border-rose-200 bg-white p-6 text-center shadow-sm"
          role="alert"
        >
          <AlertTriangle
  ...
  const auditMessage =
    auditStatus === "checking"
      ...
      : auditStatus === "unavailable"
        ? t("securityAuditReleaseVerification.reportUnavailable", {
            defaultValue:
        ...
        : auditStatus === "recorded"
          ...
          : t("securityAuditReleaseVerification.reportPending", {
              defaultValue:
                "The release is verified. The first normal-cadence scheduled report remains pending by design.",
            });

  return (
    <div
      className="min-h-screen overflow-x-hidden rr-bg-cream-warm pb-20"
    ...
    >
            ...
            <div className="max-w-3xl">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1.5 text-xs font-black uppercase tracking-[0.14em] text-white/80">
                  {t("securityAuditReleaseVerification.internalRecord", {
                    defaultValue: "Internal evidence record",
                  })}
                </span>
              </div>
              ...
              <p className="mt-3 rr-b2 rr-on-dark-secondary">
                {t("securityAuditReleaseVerification.description", {
                  defaultValue:
                    "A server-protected administrator dossier for the Security Audit History release, its exact validation gates, production controls, and pending normal-cadence confirmation.",
                })}
              </p>
            </div>
            <section
              className="border-t-2 border-[#D4A017] bg-white/5 p-5"
              aria-label={t(
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.14em] text-white/60">
                    {t("securityAuditReleaseVerification.statusEyebrow", {
                      defaultValue: "Determination",
                    })}
                  </p>
                  <p className="mt-1 text-xl font-extrabold text-white">
                    {t("securityAuditReleaseVerification.statusVerified", {
                      defaultValue: "Release verified",
                    })}
                  </p>
                  <p className="mt-2 text-sm leading-6 text-white/75">
                    {t("securityAuditReleaseVerification.statusBody", {
                      defaultValue:
                        "Reviewed source, exact-candidate gates, production boundaries, schema readiness, and protected-main lineage passed.",
                    })}
                  </p>
                  <p className="mt-3 border-t border-white/10 pt-3 text-sm font-bold text-amber-200">
                    {t("securityAuditReleaseVerification.pendingExpected", {
                      defaultValue:
                        "First scheduled report: pending — expected",
                    })}
                  </p>
                </div>
              </div>
      ...
      <main className="mx-auto max-w-7xl space-y-7 px-4 py-6 sm:px-6 sm:py-9">
        ...
        >
          <article className="overflow-hidden bg-white shadow-sm">
            <div className="border-b border-[#0b2a52]/10 p-5 sm:p-6">
              <p className="rr-h6 rr-text-navy-muted">
                {t("securityAuditReleaseVerification.accessEyebrow", {
              ...
              >
                {t("securityAuditReleaseVerification.accessTitle", {
                  defaultValue:
                    "Only administrators receive the dossier payload",
                })}
              </h2>
              <p className="mt-2 rr-b2 rr-text-navy-muted">
                {t("securityAuditReleaseVerification.accessDescription", {
                  defaultValue:
                    "The browser guard improves navigation; centralized server middleware remains the enforcement point.",
                })}
              </p>
            </div>
            <div className="divide-y divide-[#0b2a52]/10">
              {data.accessBoundary.map(item => {
                const Icon =
                  accessIcons[item.id as keyof typeof accessIcons] ??
                  ShieldCheck;
                const allowed = item.status === "allowed";
                return (
                  <div
                    key={item.id}
                    className="grid gap-3 p-5 sm:grid-cols-[13rem_minmax(0,1fr)_minmax(0,1fr)] sm:items-center sm:p-6"
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`flex h-10 w-10 shrink-0 items-center justify-center ${allowed ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"}`}
                      >
                      ...
                      <div>
                        <p className="text-sm font-extrabold rr-text-navy">
                          {t(
                            `securityAuditReleaseVerification.access.${item.id}.context`,
                            { defaultValue: item.id }
                          )}
                        </p>
                        <p
                          className={`mt-0.5 text-xs font-black uppercase tracking-[0.12em] ${allowed ? "text-emerald-700" : "text-rose-700"}`}
                        >
                          {allowed
                            ? t("securityAuditReleaseVerification.allowed", {
                                defaultValue: "Allowed",
                              })
                            : t("securityAuditReleaseVerification.blocked", {
                    ...
                    <div>
                      <p className="text-xs font-black uppercase tracking-[0.12em] rr-text-navy-muted">
                        {t("securityAuditReleaseVerification.pageOutcome", {
                          defaultValue: "Page",
                        })}
                      </p>
                      <p className="mt-1 text-sm font-bold rr-text-navy">
                        {t(
                          `securityAuditReleaseVerification.access.${item.id}.page`,
                          { defaultValue: item.pageOutcome }
                        )}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-black uppercase tracking-[0.12em] rr-text-navy-muted">
                        {t("securityAuditReleaseVerification.serverOutcome", {
                          defaultValue: "Server payload",
                        })}
                      </p>
                      <p className="mt-1 font-mono text-sm font-bold rr-text-navy">
                        {t(
                          `securityAuditReleaseVerification.access.${item.id}.server`,
                          {
                            defaultValue: `${item.apiOutcome} · ${item.payloadOutcome}`,
                          }
                        )}
                      </p>
                    </div>
          ...
          <aside
            className="rr-bg-navy p-5 text-white sm:p-6"
          ...
          >
            ...
            >
              {t("securityAuditReleaseVerification.scopeTitle", {
                defaultValue: data.release.title,
              })}
            </h2>
        ...
        <section
          ...
          aria-labelledby="validation-gates-title"
        >
          <div className="border-b border-[#0b2a52]/10 p-5 sm:p-6">
            <p className="rr-h6 rr-text-navy-muted">
              {t("securityAuditReleaseVerification.validationEyebrow", {
                defaultValue: "Exact-candidate validation",
              })}
            </p>
            <h2 id="validation-gates-title" className="rr-h3 mt-1 rr-text-navy">
              {t("securityAuditReleaseVerification.validationTitle", {
                defaultValue: "Every release gate passed",
              })}
            </h2>
            <p className="mt-2 rr-b2 rr-text-navy-muted">
              {t("securityAuditReleaseVerification.validationDescription", {
                defaultValue:
                  "The reviewed restoration merge was tested independently of later repository changes.",
              })}
            </p>
          </div>
          <div className="grid gap-px bg-[#0b2a52]/10 sm:grid-cols-2 xl:grid-cols-3">
            {data.validationGates.map(item => {
              const Icon =
                validationIcons[item.id as keyof typeof validationIcons] ??
                CheckCircle2;
              return (
                <div key={item.id} className="bg-white p-5 sm:p-6">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs font-black uppercase tracking-[0.12em] text-emerald-700">
                      {t("securityAuditReleaseVerification.passed", {
                        defaultValue: "Passed",
                      })}
                    </span>
                  </div>
                  <p className="mt-5 text-sm font-extrabold rr-text-navy">
                    {t(
                      `securityAuditReleaseVerification.validation.${item.id}.label`,
                      { defaultValue: item.id }
                    )}
                  </p>
                  <p className="mt-1 font-mono text-lg font-black rr-text-navy">
                    {t(
                      `securityAuditReleaseVerification.validation.${item.id}.result`,
        ...
        >
          <article className="bg-white shadow-sm">
            <div className="border-b border-[#0b2a52]/10 p-5 sm:p-6">
              <p className="rr-h6 rr-text-navy-muted">
                {t("securityAuditReleaseVerification.productionEyebrow", {
                  defaultValue: "Non-mutating production proof",
                })}
              </p>
              <h2
                ...
                className="rr-h3 mt-1 rr-text-navy"
              >
                {t("securityAuditReleaseVerification.productionTitle", {
                  defaultValue: "Production failed closed and retained privacy",
                })}
              </h2>
            </div>
            <div className="divide-y divide-[#0b2a52]/10">
              {data.productionControls.map(item => {
                const Icon =
                  productionIcons[item.id as keyof typeof productionIcons] ??
                  ShieldCheck;
                return (
                  <div key={item.id} className="flex gap-4 p-5 sm:p-6">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
                        <p className="text-sm font-extrabold rr-text-navy">
                          {t(
                            `securityAuditReleaseVerification.production.${item.id}.title`,
                            { defaultValue: item.id }
                          )}
                        </p>
                        <p className="font-mono text-sm font-black text-emerald-800">
                          {t(
                            `securityAuditReleaseVerification.production.${item.id}.result`,
                            { defaultValue: item.result }
                          )}
                        </p>
                      </div>
                      <p className="mt-1 text-sm leading-6 rr-text-navy-muted">
                        {t(
                          `securityAuditReleaseVerification.production.${item.id}.detail`,
                          { defaultValue: item.result }
                        )}
                      </p>
                    </div>
          ...
          <div className="grid gap-4">
            <article
              className="border-t-2 border-[#D4A017] bg-white p-5 shadow-sm sm:p-6"
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="rr-h6 rr-text-navy-muted">
                    {t("securityAuditReleaseVerification.lineageEyebrow", {
                      defaultValue: "Protected lineage",
                    })}
                  </p>
                  <h2 id="lineage-title" className="rr-h3 mt-1 rr-text-navy">
                    {t("securityAuditReleaseVerification.lineageTitle", {
                      defaultValue: "Reviewed release preserved in main",
                    })}
                  </h2>
                </div>
              </div>
              <dl className="mt-5 grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
                <div className="border-l-2 border-[#D4A017] pl-3">
                  <dt className="text-xs font-black uppercase tracking-[0.12em] rr-text-navy-muted">
                    {t("securityAuditReleaseVerification.pullRequest", {
                      defaultValue: "Pull request",
                    })}
                  </dt>
                  <dd className="mt-1 font-mono text-sm font-black rr-text-navy">
                    #{data.lineage.pullRequest}
                  </dd>
                </div>
                <div className="border-l-2 border-[#D4A017] pl-3">
                  <dt className="text-xs font-black uppercase tracking-[0.12em] rr-text-navy-muted">
                    {t("securityAuditReleaseVerification.mergeCommit", {
                      defaultValue: "Merge commit",
                    })}
                  </dt>
                  <dd className="mt-1 font-mono text-sm font-black rr-text-navy">
                    {data.lineage.mergeCommit}
                  </dd>
                </div>
                <div className="border-l-2 border-[#D4A017] pl-3">
                  <dt className="text-xs font-black uppercase tracking-[0.12em] rr-text-navy-muted">
                    {t("securityAuditReleaseVerification.qualityGateRun", {
                      defaultValue: "Quality-gate run",
                    })}
                  </dt>
                  <dd className="mt-1 font-mono text-sm font-black rr-text-navy">
                    {data.lineage.qualityGateRun}
                  </dd>
                </div>
              </dl>
            </article>
            <article
              className="bg-white p-5 shadow-sm sm:p-6"
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="rr-h6 rr-text-navy-muted">
                    {t("securityAuditReleaseVerification.schemaEyebrow", {
                      defaultValue: "Schema readiness",
                    })}
                  </p>
                  <h2 id="schema-title" className="rr-h3 mt-1 rr-text-navy">
                    {t("securityAuditReleaseVerification.schemaTitle", {
                      defaultValue: "Additive reporting storage present",
                    })}
                  </h2>
                </div>
              </div>
              <p className="mt-5 break-all font-mono text-sm font-black rr-text-navy">
                {data.schema.table}
              </p>
              <ul className="mt-3 space-y-2">
                {data.schema.indexes.map(index => (
                  <li
                    key={index}
                    className="break-all font-mono text-xs rr-text-navy-muted"
                  >
                    {index}
                  </li>
                ))}
              ...
              <p className="mt-4 text-sm font-bold text-emerald-800">
                {t("securityAuditReleaseVerification.noRowsOpened", {
                  defaultValue:
                    "No production row values were opened or copied.",
                })}
              </p>
            </article>
          </div>
        ...
        <section
          className="overflow-hidden border border-[#D4A017]/35 bg-white shadow-sm"
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p
                    className="mt-2 rr-b2 rr-text-navy-muted"
                    aria-live="polite"
                  >
                    {auditMessage}
                  </p>
              ...
              <span
                className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black ${auditStatus === "checking" ? "bg-sky-100 text-sky-900" : auditStatus === "unavailable" ? "bg-rose-100 text-rose-800" : auditStatus === "recorded" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-900"}`}
              >
                {auditStatus === "checking" ? (
                  <Loader2
        ...
        <section
          className="border border-[#0b2a52]/10 bg-white p-5 shadow-sm sm:p-6"
