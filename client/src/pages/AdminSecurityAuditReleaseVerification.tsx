    ...
    auditStatus === "checking"
      ...
      : auditStatus === "unavailable"
        ...
        : auditStatus === "recorded"
          ...
          : t("securityAuditReleaseVerification.reportPending", { defaultValue: "The release is ready. The first operational proof arrives when the next scheduled dependency-audit workflow records its sanitized summary." });

  return (
    <div className="min-h-screen overflow-x-hidden rr-bg-cream-warm pb-20" data-testid="security-audit-release-verification-page">
      <header className="rr-bg-navy px-5 pb-8 pt-8 text-white sm:px-6 sm:pt-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
            <div className="max-w-3xl">
              <BrandLockup tone="split" iconClassName="h-9 w-9" textClassName="text-base" />
              <p className="mt-5 inline-flex items-center gap-2 rounded-full border border-[#D4A017]/50 bg-[#D4A017]/10 px-3 py-1.5 text-xs font-black uppercase tracking-[0.14em] rr-text-gold">
                <LockKeyhole size={14} aria-hidden="true" />
                {t("securityAuditReleaseVerification.adminOnly", { defaultValue: "Administrator only" })}
              </p>
              <h1 className="rr-h1 mt-4 rr-on-dark">
                {t("securityAuditReleaseVerification.title", { defaultValue: "Security Audit release verification" })}
              </h1>
              <p className="mt-3 rr-b2 rr-on-dark-secondary">
                {t("securityAuditReleaseVerification.description", { defaultValue: "A protected evidence record for the Security Audit History release, its authorization controls, production schema, and the first scheduled-report confirmation." })}
              </p>
            </div>
            <section className="rounded-2xl border border-[#D4A017]/50 bg-[#D4A017]/10 p-5" aria-label={t("securityAuditReleaseVerification.releaseStatusLabel", { defaultValue: "Release status" })}>
              <div className="flex gap-3">
                <CheckCircle2 className="mt-0.5 shrink-0 rr-text-gold" size={25} aria-hidden="true" />
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.14em] text-white/65">{t("securityAuditReleaseVerification.statusEyebrow", { defaultValue: "Release status" })}</p>
                  <p className="mt-1 text-xl font-extrabold text-white">{t("securityAuditReleaseVerification.statusVerified", { defaultValue: "Verified" })}</p>
                  <p className="mt-2 text-sm leading-6 text-white/75">{t("securityAuditReleaseVerification.statusBody", { defaultValue: "Reviewed release, protected routes, schema readiness, and authorized interface checks are complete." })}</p>
                </div>
              </div>
            </section>
          </div>
      ...
      <main className="mx-auto max-w-7xl space-y-7 px-4 py-6 sm:px-6 sm:py-9">
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.55fr)]" aria-labelledby="release-evidence-title">
          <article className="rounded-2xl bg-white p-5 shadow-sm sm:p-6">
            <div className="flex items-start gap-3">
              <ClipboardCheck className="mt-1 shrink-0 rr-text-gold" size={25} aria-hidden="true" />
              <div>
                <p className="rr-h6 rr-text-navy-muted">{t("securityAuditReleaseVerification.evidenceEyebrow", { defaultValue: "Verified release evidence" })}</p>
                <h2 id="release-evidence-title" className="rr-h3 mt-1 rr-text-navy">{t("securityAuditReleaseVerification.evidenceTitle", { defaultValue: "Production controls are in place" })}</h2>
                <p className="mt-2 rr-b2 rr-text-navy-muted">{t("securityAuditReleaseVerification.evidenceDescription", { defaultValue: "This view holds a concise operational record. It deliberately excludes raw dependency logs, advisory details, customer data, and credentials." })}</p>
              </div>
            </div>
            <ul className="mt-6 grid gap-3 md:grid-cols-2" aria-label={t("securityAuditReleaseVerification.evidenceListLabel", { defaultValue: "Verified release controls" })}>
              {data.evidence.map(item => {
                const Icon = evidenceIcons[item.id as keyof typeof evidenceIcons] ?? ShieldCheck;
                return (
                  <li key={item.id} className="rounded-xl border border-[#0b2a52]/10 bg-[#fbfaf6] p-4">
                    <div className="flex gap-3">
                      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-800"><Icon size={19} aria-hidden="true" /></span>
                      <div>
                        <p className="text-sm font-extrabold rr-text-navy">{t(`securityAuditReleaseVerification.evidence.${item.id}.title`, { defaultValue: item.title })}</p>
                        <p className="mt-1 text-sm leading-6 rr-text-navy-muted">{t(`securityAuditReleaseVerification.evidence.${item.id}.detail`, { defaultValue: item.detail })}</p>
                      </div>
                    </div>
                  </li>
                );
          ...
          <aside className="rounded-2xl rr-bg-navy p-5 text-white sm:p-6" aria-labelledby="release-scope-title">
            ...
            <dl className="mt-6 space-y-4 border-t border-white/10 pt-5">
              ...
              <div><dt className="text-xs font-bold uppercase tracking-[0.12em] text-white/55">{t("securityAuditReleaseVerification.verifiedAt", { defaultValue: "Verified" })}</dt><dd className="mt-1 text-sm font-bold text-white">{data.verifiedAt}</dd></div>
            </dl>
          </aside>
        </section>

        <section className="overflow-hidden rounded-2xl border border-[#D4A017]/35 bg-white shadow-sm" aria-labelledby="next-confirmation-title">
          <div className="border-b border-[#0b2a52]/10 bg-[#fff9e8] p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="rr-h6 rr-text-navy-muted">{t("securityAuditReleaseVerification.nextEyebrow", { defaultValue: "Next operational confirmation" })}</p>
                  <h2 id="next-confirmation-title" className="rr-h3 mt-1 rr-text-navy">{t("securityAuditReleaseVerification.nextTitle", { defaultValue: data.nextOperationalConfirmation.title })}</h2>
                  <p className="mt-2 rr-b2 rr-text-navy-muted" aria-live="polite">{auditMessage}</p>
                </div>
              </div>
        ...
        <section className="rounded-2xl border border-[#0b2a52]/10 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="privacy-title">
