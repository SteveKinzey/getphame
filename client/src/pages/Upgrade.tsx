...
const COMPARISON_ROWS: Array<{
  ...
  {
    featureKey: "reviewRequests",
    free: "__FREE_ALLOWANCE__",
    pro: "__UNLIMITED__",
    lifetime: "__UNLIMITED__",
  },
  ...
  {
    featureKey: "multiPlatformPhames",
    free: "__ONE__",
    pro: "__UNLIMITED__",
    lifetime: "__UNLIMITED__",
  },
  {
    featureKey: "price",
    free: "__FREE__",
    pro: "__MONTHLY_PRICE__",
    lifetime: "__LIFETIME_PRICE__",
  },
];
...
const PRO_FEATURES = [
  {
    ...
    key: "pricingCard.unlimitedReviewRequests",
    fallback: "Unlimited review requests",
  },
  {
    icon: <Zap size={14} />,
    key: "pricingCard.prioritySending",
    fallback: "Priority sending & follow-ups",
  },
  {
    icon: <BarChart2 size={14} />,
    key: "pricingCard.advancedAnalytics",
    fallback: "Advanced analytics",
  },
  {
    icon: <Star size={14} />,
    key: "pricingCard.customEmailTemplates",
    fallback: "Custom email templates",
  },
  {
    icon: <Crown size={14} />,
    key: "pricingCard.woocommerceSync",
    fallback: "WooCommerce sync",
  },
  {
    icon: <Shield size={14} />,
    key: "pricingCard.prioritySupport",
...
export default function UpgradePage() {
  ...
  const { data: subscriptionStatus } = trpc.stripe.subscriptionStatus.useQuery(
    undefined,
    {
  ...
  const [campaignPromotionCode] = useState(() => {
    ...
    const rawCode =
      new URLSearchParams(window.location.search).get("promo")?.trim() ?? "";
  ...
  const [pricingFeature] = useState<PremiumFeatureKey>(() => {
    ...
    return normalizePremiumFeatureKey(
  ...
  useEffect(() => {
    fetch("/api/paypal/status")
      .then(
        r => r.json() as Promise<{ enabled: boolean; clientId: string | null }>
      )
  ...
  const createPortal = trpc.stripe.createPortal.useMutation({
    onSuccess: ({ url }) => {
      ...
      toast.success(
        t("paidUser.portalOpened", {
          defaultValue: "Secure billing opened in a new tab.",
        })
      );
    },
  ...
  const renewalDate = subscriptionStatus?.currentPeriodEnd
    ? new Intl.DateTimeFormat(undefined, {
        ...
        day: "numeric",
        year: "numeric",
      }).format(new Date(subscriptionStatus.currentPeriodEnd))
    : null;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 pb-40 rr-bg-navy">
        ...
        <h2 className="text-3xl font-black text-center mb-2 text-white">
          {t("paidUser.currentStatus", {
            defaultValue: "Your status: {{tierLabel}}",
            tierLabel,
          })}
        </h2>
        <p className="text-center mb-8 text-white font-bold text-lg">
          {isLife
            ? t("paidUser.lifeMessage", {
                defaultValue:
                  "Life access is active. There is nothing to upgrade or renew.",
              })
            : t("paidUser.manageMessage", {
                defaultValue:
                  "Change your billing cycle or end your subscription below.",
              })}
        </p>
        {canManage && (
          <div
            className="w-full max-w-md rounded-2xl p-5 mb-6 rr-bg-navy-mid"
            style={{ border: "1px solid oklch(0.80 0.18 80 / 0.35)" }}
          >
            ...
            >
              ...
              {effectivePlan === "annual"
                ? t("paidUser.switchMonthly", {
                    defaultValue: "Switch to Monthly or manage billing",
                  })
                : t("paidUser.changePlan", {
                    defaultValue: "Change plan or manage billing",
                  })}
            </button>
            <button
            ...
            >
              {t("paidUser.endSubscription", {
                defaultValue: "End subscription",
              })}
            </button>
            <p className="text-xs text-white/60 text-center mt-3">
              {t("paidUser.endExplanation", {
                defaultValue:
                  "Ending your subscription returns the account to Free at the end of the paid period.",
              })}
            </p>
            <PlanSwitchDialog
              open={planSwitchOpen}
    ...
    <div className="min-h-screen pb-40 rr-bg-navy">
      ...
      <div className="px-5 pt-14 pb-5 max-w-6xl mx-auto">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.78fr_1.22fr] lg:items-center">
          <div>
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl leading-[0.95] text-white rr-fw-black">
              {t("header.title")}
              <br />
            ...
            <p className="mt-4 max-w-lg text-sm sm:text-base leading-relaxed text-white/75">
              {t(`premiumConversion.features.${pricingFeature}.description`, {
                defaultValue:
                  "Compare the free plan with paid access before deciding. Your current work stays in place while you review the options.",
              })}
            </p>
          </div>
            ...
            {upgradeImageFailed ? (
              ...
              <img
                ...
                alt={t("header.heroImageAlt", {
                  defaultValue: "Get Phame dashboard preview",
                })}
      ...
      <div className="px-4 flex flex-col gap-6 max-w-7xl mx-auto">
        ...
        {false && (
          <>
            {/* Plan selector tabs */}
            <div className="flex rounded-2xl p-1 gap-1 rr-bg-navy-mid">
              {(["monthly", "annual", "lifetime"] as Plan[]).map(plan => {
                ...
                const planBadge =
                  plan === "annual"
                    ? t("planSelector.mostPopularBadge")
                    : plan === "lifetime"
                      ? t("planSelector.bestValueBadge")
                      : undefined;
                return (
                  <button
                    key={plan}
                  ...
                  >
                    ...
                    {planBadge && selectedPlan !== plan && (
                      <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[9px] px-1.5 py-0.5 rounded-full font-black whitespace-nowrap rr-bg-gold rr-text-navy">
                        {planBadge}
                      </span>
                    )}
                  </button>
            ...
            {/* Pricing card */}
            <div
              className="rounded-2xl p-6 rr-bg-navy-mid"
            ...
            >
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-black text-white">
                      {t("promotionLink.applied", {
                        defaultValue:
                          "Your campaign code will be applied at checkout",
                      })}
                    </p>
                    <p className="mt-0.5 text-xs font-bold rr-text-gold">
                      {campaignPromotionCode}
                    </p>
                    <p className="mt-1 text-xs leading-5 text-white/70">
                      {t("promotionLink.verified", {
                        defaultValue:
                          "Eligibility is verified securely before Stripe Checkout opens.",
                      })}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex items-end gap-2 mb-1">
                <span className="text-5xl font-black rr-text-gold">
                  {PLANS[selectedPlan].price}
                </span>
                <span className="text-xl mb-2 text-white font-bold">
                  {selectedPlan === "monthly"
                    ? t("pricingCard.monthlySub")
                    : selectedPlan === "annual"
                      ? t("pricingCard.annualSub")
                      : t("pricingCard.lifetimeSub")}
                </span>
              </div>
              ...
              {PLANS[selectedPlan].savings && (
                <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold mb-4 rr-bg-navy rr-text-gold">
                  {selectedPlan === "annual" && <Calendar size={11} />}
                  {selectedPlan === "lifetime" && <Shield size={11} />}
                  {selectedPlan === "annual"
                    ? t("pricingCard.annualSavings")
                    : t("pricingCard.lifetimeSavings")}
                </div>
              )}
              ...
              {selectedPlan === "monthly" && (
                <p className="text-sm mb-4 text-white/80 font-bold">
                  {t(
                    "pricingCard.monthlyDescription",
              ...
              {selectedPlan === "lifetime" && (
                <>
                  <p className="text-sm mb-3 text-white/80 font-bold">
                    {t(
                      "pricingCard.lifetimeDescription",
                  ...
                  >
                    <p
                      ...
                      style={{ color: "oklch(0.20 0.08 80)" }}
                    >
                      {t("pricingCard.lifetimeRoiCalloutTitle")}
                    </p>
                    <p
                      className="text-xs"
                      style={{ color: "oklch(0.25 0.06 80)" }}
                    >
              ...
              <div className="flex flex-col gap-3 mb-6">
                {PRO_FEATURES.map(f => (
                  <div key={f.key} className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 rr-bg-gold rr-text-navy">
                      {f.icon}
                    </div>
              ...
              <div
                className="flex items-center justify-center gap-1.5 mb-3 px-3 py-2 rounded-xl"
                style={{
                  background: "oklch(0.19 0.08 260)",
                  border: "1px solid oklch(0.28 0.07 260)",
                }}
              >
                <Shield size={13} className="rr-text-gold flex-shrink-0" />
                <span className="text-xs font-bold rr-text-gold">
                  {t("pricingCard.guarantee", "7-day money-back guarantee")}
                </span>
              </div>
              ...
              {showPromptPay ? (
                <div className="mt-3">
                  <button
                    ...
                    className="w-full py-3 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 transition-opacity disabled:opacity-50 text-white"
                    style={{
                  ...
                  >
                    ...
                    {createThbCheckout.isPending
                      ...
                      : t("pricingCard.payWithPromptPay", {
                          thb: PLANS[selectedPlan].thb,
                        })}
                  </button>
                  <p
                    className="text-center text-xs mt-1 mb-1"
                    style={{ color: "var(--text-on-dark-muted)" }}
                  >
        ...
        >
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl text-sm font-mono tracking-wider outline-none rr-bg-navy text-white"
        ...
        >
          ...
          >
            <div
              ...
              style={{ color: "var(--text-on-dark-secondary)" }}
            >
              {t("comparisonTable.feature", "Feature")}
            </div>
            <div style={{ color: "var(--text-on-dark-secondary)" }}>
              {t("comparisonTable.free", "Free")}
            </div>
            <div className="rr-text-gold">
              {t("comparisonTable.pro", "Pro")}
            </div>
            <div style={{ color: "oklch(0.90 0.14 80)" }}>
              {t("comparisonTable.lifetime", "Lifetime")}
            </div>
          </div>
          ...
          {COMPARISON_ROWS.map((row, i) => {
            ...
            const renderCell = (val: string | boolean) => {
              ...
              if (val === false)
                return (
                  <span style={{ color: "var(--text-on-dark-disabled)" }}>
                    —
                  </span>
                );
              if (val === "__FREE_ALLOWANCE__") {
                return (
                  <span>
                    {t("comparisonTable.freeRequestAllowance", {
                      defaultValue: "10 first, then 5 / rolling 30 days",
                    })}
                  </span>
                );
              }
              if (val === "__UNLIMITED__")
                return (
                  <span>
                    {t("comparisonTable.unlimited", {
              ...
              if (val === "__MONTHLY_PRICE__") {
                return (
                  <span>
                    {USD_DISPLAY.monthly}{" "}
                    {t("pricingCard.monthlySub", { defaultValue: "/ month" })}
                  </span>
                );
              }
              if (val === "__LIFETIME_PRICE__")
                return <span>{USD_DISPLAY.lifetime}</span>;
          ...
          {[
            {
              ...
              a: t(
                "upgradeFaq.a1",
                "Yes — you pay $349 once and Get Phame is yours forever. No monthly fees, no renewals, no surprises. You also get all future updates included."
              ),
            },
            {
              q: t(
                "upgradeFaq.q2",
                "What happens if I cancel a monthly or annual plan?"
              ),
              a: t(
                "upgradeFaq.a2",
                "You keep access until the end of your current billing period. After that your account returns to Free: 10 initial requests, then 5 every rolling 30 days. Your contacts and history are never deleted."
              ),
            },
            {
              q: t(
              ...
              a: t(
                "upgradeFaq.a3",
                "Yes. You can upgrade from monthly to annual or lifetime at any time from Settings. The unused portion of your current plan is not refunded, but the new plan starts immediately."
              ),
            },
            {
              ...
              a: t(
                "upgradeFaq.a4",
                "We offer a 7-day refund on all plans, no questions asked. Contact support@getphame.app within 7 days of purchase and we will process the refund within 24 hours."
              ),
            },
            {
              q: t(
                "upgradeFaq.q5",
                "Does the lifetime plan cover multiple locations?"
              ),
              a: t(
                "upgradeFaq.a5",
                "The lifetime plan covers one business/location. If you manage multiple locations, you will need a separate account for each. Contact us for agency or multi-location pricing."
              ),
            },
            {
              ...
              a: t(
                "upgradeFaq.a6",
                "All major credit and debit cards via Stripe. Thai users can also pay via PromptPay — tap the PromptPay option below the main checkout button."
              ),
            },
          ].map((faq, i) => (
...
function FreeVsPremiumOverview({
  featureKey,
}: {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h2
          id="free-vs-premium-heading"
          className="mt-2 text-2xl font-black text-white sm:text-3xl"
        >
          {t(`${featurePath}.title`, {
            defaultValue: "Choose the plan that fits your workflow",
          })}
        </h2>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <article className="rounded-3xl border border-white/10 bg-[#061a3a]/55 p-5 sm:p-6">
          ...
          <ul
            ...
            aria-label={t("comparisonTable.free", { defaultValue: "Free" })}
          >
            {(["followUpReminders", "savedContacts", "csvImport"] as const).map(
              key => (
                <li
                  key={key}
                  className="flex items-start gap-3 text-sm font-bold text-white/75"
                >
                  <Check
                    size={16}
                    className="mt-0.5 shrink-0 text-[oklch(0.72_0.18_145)]"
                    aria-hidden="true"
                  />
                  <span>
                    {t(`comparisonTable.${key}`, {
                      defaultValue: COMPARISON_FEATURE_DEFAULTS[key],
                    })}
                  </span>
                </li>
              )
        ...
        <article className="relative rounded-3xl border border-[oklch(0.80_0.18_80/0.72)] bg-[linear-gradient(145deg,oklch(0.31_0.11_260),oklch(0.22_0.07_260))] p-5 shadow-[0_18px_50px_oklch(0.80_0.18_80/0.14)] sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] rr-text-gold">
                {t("premiumConversion.marker.label", {
            ...
            <span
              className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-amber-200/10 rr-text-gold"
              aria-hidden="true"
            >
              <Crown size={21} />
            </span>
          ...
          <ul
            ...
            aria-label={t("premiumConversion.modal.benefitsLabel", {
              defaultValue: "Paid plan benefits",
            })}
          >
          ...
          >
            {t("pricingGrid.title", {
              defaultValue: "Choose the plan that fits your growth",
            })}
          </button>
        </article>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative grid w-full max-w-sm gap-3">
        {[
          [
            t("pricingGrid.heroSendRequests", {
              defaultValue: "Send requests",
            }),
            <Zap key="zap" size={16} />,
          ],
          [
            t("pricingGrid.heroAutomateFollowUps", {
              defaultValue: "Automate follow-ups",
            }),
            <Check key="check" size={16} />,
          ],
          [
            t("pricingGrid.heroTrackGrowth", {
              defaultValue: "Track your growth",
            }),
            <BarChart2 key="chart" size={16} />,
          ],
        ].map(([label, icon]) => (
          <div
            key={label as string}
            className="flex items-center justify-between rounded-2xl border border-white/10 bg-[#061a3a]/80 px-4 py-3 shadow-lg backdrop-blur-sm"
          >
            <span className="text-sm font-black text-white">{label}</span>
            <span className="grid h-8 w-8 place-items-center rounded-full rr-bg-gold rr-text-navy">
              {icon}
            </span>
          </div>
        ))}
...
function PricingPlanGrid({
  ...
  return (
    <section aria-labelledby="upgrade-plan-grid-heading">
      ...
      <div
        data-testid="upgrade-plan-guidance"
        className="mb-5 grid grid-cols-1 gap-2 text-left sm:grid-cols-3"
      >
        {PLAN_ORDER.map(plan => (
          <button
            key={plan}
      ...
      {campaignPromotionCode && (
        ...
        >
          ...
          <div className="text-left">
            <p className="text-sm font-black text-white">
              {t("promotionLink.applied", {
                defaultValue: "Your campaign code will be applied at checkout",
              })}
            </p>
            <p className="mt-0.5 text-xs font-bold rr-text-gold">
              {campaignPromotionCode}
      ...
      >
        {PLAN_ORDER.map(plan => {
          ...
          const planDescription =
            plan === "monthly"
              ? t("pricingCard.monthlyDescription", {
                  defaultValue: "Cancel anytime. No contracts.",
                })
              : isAnnual
                ? t("pricingCard.annualDescription", {
                    defaultValue:
                      "Billed once per year. Equivalent to $24.17/mo.",
                  })
                : t("pricingCard.lifetimeDescription", {
                    defaultValue: "One-time payment. No renewals, ever.",
                  });
            ...
            >
              ...
              >
                <div className="flex items-end gap-2">
                  <span className="text-4xl font-black rr-text-gold">
                    {planConfig.price}
                  </span>
              ...
              >
                {PRO_FEATURES.map(feature => (
                  ...
                  >
                    <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full rr-bg-gold rr-text-navy">
                      {feature.icon}
                    </span>
                    <span>
      ...
      <div className="flex flex-col gap-1 text-left sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.16em] rr-text-gold">
            {t("pricingGrid.savingsEyebrow", {
              defaultValue: "Savings calculator",
            })}
          </p>
          <h3
            id="upgrade-savings-heading"
            className="mt-1 text-lg font-black text-white"
          >
            {t("pricingGrid.savingsTitle", {
              defaultValue: "See what each commitment saves",
            })}
          </h3>
        </div>
        <p className="text-xs font-semibold text-white/60">
          {t("pricingGrid.savingsBasis", {
            defaultValue: "Compared with paying monthly",
          })}
        </p>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-white/10 bg-[#061a3a]/55 p-3">
          ...
          <p className="mt-1 text-lg font-black text-white">
            {formatUsd(USD_PRICES.monthly * 12, locale)}
          </p>
          <p className="mt-1 text-xs font-semibold text-white/60">
            {t("pricingGrid.monthlyAnnualCost", {
              defaultValue: "for 12 months",
            })}
          </p>
        </div>
        <div className="rounded-xl border border-[oklch(0.80_0.18_80/0.58)] bg-[oklch(0.25_0.09_260)] p-3 shadow-[0_0_20px_oklch(0.80_0.18_80/0.12)]">
          ...
          <p className="mt-1 text-lg font-black rr-text-gold">
            {t("pricingGrid.annualSave", {
              defaultValue: "Save {{amount}}",
              amount: formatUsd(USD_ANNUAL_SAVINGS, locale),
            })}
          </p>
          <p className="mt-1 text-xs font-semibold text-white/70">
            {t("pricingGrid.annualSavingsDetail", {
              defaultValue: "{{price}} instead of {{monthly}} for year one",
              price: formatUsd(USD_PRICES.annual, locale),
              monthly: formatUsd(USD_PRICES.monthly * 12, locale),
            })}
          </p>
        </div>
        <div className="rounded-xl border border-white/10 bg-[#061a3a]/55 p-3">
          ...
          <p className="mt-1 text-lg font-black text-[oklch(0.72_0.18_145)]">
            {t("pricingGrid.lifetimeSave", {
              defaultValue: "Save {{amount}} by year two",
              amount: formatUsd(USD_LIFETIME_SAVINGS_BY_YEAR_TWO, locale),
            })}
          </p>
          <p className="mt-1 text-xs font-semibold text-white/60">
            {t("pricingGrid.lifetimePayback", {
              defaultValue: "Pays for itself in about {{months}} months",
              months: USD_LIFETIME_PAYBACK_MONTHS,
            })}
          </p>
        </div>
      </div>
    ...
    if (value === true)
      return (
        <Check
          ...
          aria-label={t("pricingGrid.includedLabel", {
            defaultValue: "Included",
          })}
        />
      );
    if (value === false) return <span className="text-white/35">—</span>;
    if (value === "__FREE_ALLOWANCE__")
      return (
        <span>
          {t("comparisonTable.freeRequestAllowance", {
            defaultValue: "10 + 5 / 30d",
          })}
        </span>
      );
    if (value === "__UNLIMITED__")
      return (
        <span>
          {t("comparisonTable.unlimited", { defaultValue: "Unlimited" })}
        </span>
      );
    if (value === "__ONE__") return <span>1</span>;
    if (value === "__FREE__")
      return <span>{t("comparisonTable.free", { defaultValue: "Free" })}</span>;
    if (value === "__MONTHLY_PRICE__")
      return <span>{USD_DISPLAY.monthly}</span>;
    if (value === "__LIFETIME_PRICE__")
      return <span>{USD_DISPLAY.lifetime}</span>;
            ...
            <span>
              <span className="block text-sm font-black text-white">
                {t("comparisonTable.mobileTitle", {
                  defaultValue: "Compare plan features",
                })}
              </span>
              <span className="mt-0.5 block text-xs font-semibold text-white/60">
                {t("comparisonTable.mobileSubtitle", {
                  defaultValue: "Open the compact feature comparison",
                })}
              </span>
            </span>
        ...
        <DrawerContent className="max-h-[82vh] overflow-y-auto border-white/15 rr-bg-navy text-white">
          <DrawerHeader className="text-left">
            <p className="text-[10px] font-black uppercase tracking-[0.14em] rr-text-gold">
              {t("comparisonTable.mobileEyebrow", {
            ...
            <DrawerDescription className="text-white/65">
              {t("comparisonTable.mobileDescription", {
                defaultValue:
          ...
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-[oklch(0.20_0.07_260)] mx-4">
            <div className="grid grid-cols-[minmax(0,1.42fr)_repeat(3,minmax(0,0.78fr))] border-b border-white/10 bg-[#061a3a] px-2 py-2 text-center text-[10px] font-black">
              <div className="text-left text-white/55">
                {t("comparisonTable.feature", { defaultValue: "Feature" })}
              </div>
              <div className="text-white/55">
                {t("comparisonTable.free", { defaultValue: "Free" })}
              </div>
              <div className="rr-text-gold">
                {t("comparisonTable.pro", { defaultValue: "Pro" })}
              </div>
              <div className="text-[oklch(0.90_0.14_80)]">
                {t("comparisonTable.lifetime", { defaultValue: "Lifetime" })}
              </div>
            </div>
          ...
          <DrawerFooter>
            <DrawerClose asChild>
              <button
                type="button"
                className="w-full rounded-xl border border-white/15 py-3 text-sm font-black text-white"
              >
                {t("pricingGrid.closeComparison", {
...
function AlternativePaymentOptions({
  ...
  return (
    <section
      className="rounded-2xl border border-white/10 rr-bg-navy-mid"
      aria-labelledby="alternative-payment-heading"
    >
      <div className="border-b border-white/10 px-5 py-4">
        <p className="text-xs font-black uppercase tracking-[0.14em] rr-text-gold">
          {t("pricingCard.otherPaymentMethods", {
            defaultValue: "Other payment methods",
          })}
        </p>
        <h2
          id="alternative-payment-heading"
          className="mt-1 text-lg font-black text-white"
        >
          {t("pricingCard.checkoutForPlan", {
            defaultValue: "Checkout for {{plan}}",
            ...
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-xs font-black transition-colors ${
                selectedPlan === plan
                  ...
                  : "border border-white/15 text-white/75 hover:border-[oklch(0.80_0.18_80/0.55)]"
              }`}
            >
              {PLANS[plan].label}
            </button>
        ...
        {showPromptPay ? (
          <div className="rounded-xl border border-white/10 bg-[#061a3a]/50 p-4">
            ...
            {!isThai && (
              <button
                type="button"
                onClick={onHidePromptPay}
                className="mt-2 text-xs font-bold text-white/60 underline"
              >
                {t("pricingCard.hidePromptPay")}
              </button>
            )}
          ...
          <div className="rounded-xl border border-white/10 bg-[#061a3a]/50 p-4">
            ...
            <button
              type="button"
              onClick={onRevealPromptPay}
              className="mt-3 text-xs font-black underline rr-text-gold"
            >
