...
const TrackingSummaryCard = lazy(
  () => import("@/components/dashboard/TrackingSummaryCard")
);
const PlatformBreakdownChart = lazy(
  () => import("@/components/dashboard/PlatformBreakdownChart")
);
      ...
      <div className="p-4">
          ...
          {months > 0 && (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg rr-bg-gold">
              ...
              <span className="text-xs font-black rr-text-navy">
                {t("referralRewards.monthsShort", { count: months })}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="flex flex-col items-center rounded-xl py-2.5 px-2 rr-bg-white-card">
            ...
            <span className="text-sm font-semibold rr-text-navy-mid text-center leading-tight">
              {t("referralRewards.joined")}
            </span>
          </div>
          <div
            className="flex flex-col items-center rounded-xl py-2.5 px-2"
          ...
          >
            <Gift
              size={13}
              style={{ color: "oklch(0.45 0.18 145)", marginBottom: 4 }}
            />
            <span className="text-lg font-black rr-text-navy">
              {isLoading ? "—" : months}
            </span>
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          ...
          >
            ...
            >
              <path
                ...
                fill="oklch(0.35 0.18 145)"
              />
            </svg>
            <span
              className="text-xs font-bold"
              style={{ color: "oklch(0.35 0.18 145)" }}
            >
              {t("referralRewards.whatsApp")}
            </span>
          </button>
          {/* iMessage / SMS */}
          <button
          ...
          >
            ...
            <span
              className="text-xs font-bold"
              style={{ color: "oklch(0.35 0.15 220)" }}
            >
              {t("referralRewards.message")}
            </span>
          </button>
          {/* Copy link */}
          <button
          ...
          >
            ...
            >
              {copied
                ? t("referralRewards.copied")
                : t("referralRewards.copyLink")}
            </span>
          </button>
        </div>
        ...
        >
          <span className="flex-1 text-sm font-semibold rr-text-navy-mid truncate">
            {shareUrl}
          </span>
        </div>
...
function ShareReferralCard() {
  ...
  const handleShare = async () => {
    ...
    try {
      await navigator.clipboard.writeText(
        `${t("shareReferralCard.shareText")} ${shareUrl}`
      );
      setCopied(true);
  ...
  return (
    <div className="rounded-2xl overflow-hidden rr-bg-navy">
      {/* Gold accent bar */}
      <div className="h-1 w-full rr-bg-gold" />
      ...
      <div className="p-4">
        ...
        >
          ...
          {copied
            ? t("shareReferralCard.copiedToClipboard")
            : t("shareReferralCard.sharePhame")}
        </button>
        ...
        {referralStats &&
          (referralStats.totalReferrals > 0 ||
            ...
            >
              <span>
                <span className="rr-text-gold font-black">
...
export default function HomePage() {
  ...
  const recheckMailServer = trpc.smtp.test.useMutation({
    onSuccess: result => {
      ...
      if (result.ok)
        toast.success(
          t("smtp.connectionTestPassed", {
            defaultValue: "Mail server connection verified.",
          })
        );
      else
        toast.error(
          result.error ||
            t("smtp.connectionTestFailed", {
              defaultValue: "Mail server still needs attention.",
            })
        );
    },
  ...
  const handleShare = async () => {
    ...
    if (outcome === "shared") {
      ...
      setShareStatus(
        t("pwaInstallBanner.shareSuccess", {
          defaultValue: "Get Phame shared successfully.",
        })
      );
      return;
    }
    if (outcome === "copied") {
      ...
      const message = t("pwaInstallBanner.shareCopied", {
    ...
    if (outcome === "cancelled") {
      ...
      setShareStatus(
        t("pwaInstallBanner.shareCancelled", {
    ...
    const message = t("pwaInstallBanner.shareError", {
      defaultValue: "Unable to share Get Phame right now.",
    });
    setShareStatus(message);
    toast.error(message);
  };
  ...
  const smtpHealthFailed =
    smtpConnected && smtpStatus?.lastHealthStatus === "fail";
  ...
  const tierLabel =
    effectivePlan === "monthly"
      ? t("homePage.monthlyPro", { defaultValue: "Monthly Pro" })
      : effectivePlan === "annual"
        ? t("homePage.annualPro", { defaultValue: "Annual Pro" })
        : effectivePlan === "life"
          ? t("homePage.lifePlan", { defaultValue: "Life Plan" })
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 mb-6">
          ...
          >
            ...
            >
              <button
                onClick={handleShare}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-colors rr-text-gold"
                style={{ background: "oklch(0.32 0.08 260)" }}
                title={t("nav.shareHint", {
          ...
          {/* Greeting */}
          <h1
            className="text-2xl leading-tight rr-fw-black"
            style={{ paddingTop: "10px", color: "#f9ae00" }}
          >
            {(() => {
              const firstName = user?.name ? user.name.split(" ")[0] : null;
              return firstName
                ? t("homePage.greeting", {
                    defaultValue: `Hey, ${firstName}!`,
                    name: firstName,
                  })
        ...
        <div className="grid grid-cols-3 gap-3 relative z-10">
          {[
            {
              label: t("homePage.thisMonth", { defaultValue: "This Month" }),
              value: stats?.thisMonth ?? 0,
              icon: <Send size={14} />,
            },
            {
              label: t("homePage.allTime", { defaultValue: "All Time" }),
              value: stats?.total ?? 0,
        ...
        <MailServerHealthBadge
          ...
          translate={t}
          onTestConnection={() => recheckMailServer.mutate()}
          isTesting={recheckMailServer.isPending}
          testResult={recheckMailServer.data ?? null}
        />
        ...
        <button
          ...
          title={t("nav.complianceHint", {
            defaultValue: "View Compliance Guide",
          })}
        >
          ...
          >
            {t("nav.complianceActive", { defaultValue: "Compliance: Active" })}
          </span>
          <span
            className="text-xs"
            style={{ color: "var(--text-on-dark-secondary)" }}
          >
            {t("nav.complianceViewGuide", { defaultValue: "— view guide" })}
          </span>
        </button>
      </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            {smtpHealthFailed && (
              <div
                ...
                style={{
                  background: "oklch(0.98 0.04 30)",
                  border: "1.5px solid oklch(0.75 0.18 30)",
                }}
              >
                <div className="flex items-start gap-3">
                  <WifiOff
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    >
                      ...
                      {smtpStatus?.lastHealthCheck
                        ? ` ${t("homePage.lastChecked", { defaultValue: "Last checked" })} ${formatRelativeTime(new Date(smtpStatus.lastHealthCheck), t)}.`
                        : ""}
                    </p>
                    {smtpStatus?.lastHealthError && (
                      ...
                      >
                        {t("homePage.errorPrefix", { defaultValue: "Error:" })}{" "}
                        {smtpStatus.lastHealthError}
                      </p>
                    )}
                  </div>
                ...
                >
                  {t("homePage.fixInSettings", {
                    defaultValue: "Fix in Settings →",
                  })}
                </button>
              </div>
            )}
            ...
            {allDone && !setupBannerDismissed && (
              <div
                ...
                style={{
                  ...
                  border: "1.5px solid oklch(0.80 0.14 145)",
                }}
              >
                <CheckCircle2
                  size={22}
            ...
            {isExpiringSoon && !expiryBannerDismissed && (
              <div
                className="rounded-2xl p-4 shadow-sm flex items-start gap-3 rr-bg-gold-pale"
                style={{ border: "1.5px solid oklch(0.82 0.14 80)" }}
              >
                <AlertTriangle
                  size={22}
                  className="mt-0.5 shrink-0"
                  style={{ color: "oklch(0.60 0.18 60)" }}
                />
                <div className="flex-1 min-w-0">
                  <p
                    className="text-sm font-black"
                  ...
                  <button
                    onClick={() => navigate("/settings")}
                    className="mt-2 flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg transition-opacity active:opacity-70 text-white"
                    style={{ background: "oklch(0.60 0.18 60)" }}
                  >
                    <CreditCard size={12} />
                    {t("homePage.manageBilling", {
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <h2 className="text-sm font-black mb-2 rr-text-navy">
                {t("homePage.seoTitle", {
                  defaultValue: "Get More Business Reviews",
                })}
              </h2>
              <p className="text-xs leading-relaxed rr-text-navy-muted">
                {t("homePage.seoDescription", {
                  defaultValue:
                    "Get Phame makes it easy to send personalized review requests to your customers via email — for Google, Yelp, TripAdvisor, Bing, Facebook, and more. Build your online reputation across every platform, increase star ratings, and attract new customers — all from one simple dashboard.",
                })}
              </p>
            </div>
                ...
                <div className="flex flex-col gap-2">
                  {stats.recent.map(req => (
                    <div
                      ...
                      style={{ borderColor: "oklch(0.94 0.01 260)" }}
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-black rr-bg-navy rr-text-gold">
                      ...
                      <div className="text-right">
                        <div
                          className="text-xs px-2 py-0.5 rounded-full font-bold rr-bg-green-pale"
                          style={{ color: "oklch(0.45 0.12 145)" }}
                        >
                          {req.sentAt
                            ? t("homePage.sent")
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                ...
                {!editingGoal ? (
                  <button
                    onClick={() => {
                      setGoalInput(String(reviewGoal || ""));
                      setEditingGoal(true);
                    }}
                    className="flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-lg rr-bg-surface"
                    style={{ color: "oklch(0.50 0.10 260)" }}
                  >
                    <Pencil size={11} />
                    {reviewGoal > 0
                      ? t("homePage.editGoal")
                      : t("homePage.setYourGoal")}
                  </button>
                ) : (
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg rr-bg-surface"
              ...
              {editingGoal ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    style={{
                      border: "2px solid oklch(0.80 0.10 260)",
                      fontSize: "16px",
                    }}
                    autoFocus
                    onKeyDown={e => {
                  ...
                  <span className="text-sm font-semibold rr-text-navy-mid">
                    {t("homePage.reviewsPerMonth", {
                      defaultValue: "reviews / month",
                    })}
                  </span>
                </div>
              ) : reviewGoal > 0 ? (
                <div>
                  <div className="flex items-end justify-between mb-2">
                    <div>
                      <span className="text-3xl font-black rr-text-navy">
                        {stats?.respondedThisMonth ?? 0}
                      </span>
                      <span className="text-sm ml-1 rr-text-navy-muted">
                        / {reviewGoal}{" "}
                        {t("homePage.goal", { defaultValue: "goal" })}
                      </span>
                    </div>
                    {(stats?.respondedThisMonth ?? 0) >= reviewGoal ? (
                      ...
                      >
                        {t("homePage.goalReached", {
                          defaultValue: "Goal reached! 🎉",
                        })}
                      </span>
                    ) : (
                      <span className="text-sm font-semibold rr-text-navy-mid">
                        {reviewGoal - (stats?.respondedThisMonth ?? 0)}{" "}
                        {t("homePage.toGo", { defaultValue: "to go" })}
                      </span>
                    )}
                  </div>
                  <div
                  ...
                  <p className="text-xs mt-1.5 rr-text-navy-faint">
                    {Math.min(
                      100,
                      Math.round(
                        ((stats?.respondedThisMonth ?? 0) / reviewGoal) * 100
                      )
                    )}
                    %{" "}
                    {t("homePage.ofMonthlyGoal", {
                      defaultValue: "of monthly goal",
                    })}
                  </p>
                </div>
              ) : (
            ...
            {stats?.platformBreakdown &&
              stats.platformBreakdown.filter(p => p.platform !== "unknown")
                .length > 0 && (
                <DeferredDashboardSection
                  loadingLabel={t("homePage.loadingAnalytics")}
                  minHeightClassName="min-h-[184px]"
                >
