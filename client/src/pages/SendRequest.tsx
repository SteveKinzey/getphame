...
import {
  getChangedToneLineSegments,
  getToneTextDiff,
  type ToneDiffSegment,
} from "@/lib/toneDraftDiff";
import {
...
const SUCCESS_IMG = "https://assets.getphame.app/rr-send-success.webp";

function ToneDiffText({
  segments,
  mode,
}: {
    ...
    <span className="whitespace-pre-wrap">
      {segments.map((segment, index) => (
        <span
          ...
          className={segment.kind === changedKind ? changedClass : undefined}
        >
          {segment.text}
        </span>
      ))}
...
export default function SendRequestPage() {
  ...
  const [selectedTone, setSelectedTone] = useState<
    "warmer" | "professional" | "concise"
  >("warmer");
  const [loadedDraftSourceKey, setLoadedDraftSourceKey] = useState("");
  const [tonePreviewDraft, setTonePreviewDraft] = useState<{
    subject: string;
    body: string;
  } | null>(null);
  const [tonePreviewSourceDraft, setTonePreviewSourceDraft] = useState<{
    subject: string;
    body: string;
  } | null>(null);
  const [tonePreviewRationales, setTonePreviewRationales] = useState<
    TonePreviewRationale[]
  >([]);
    ...
    onSuccess: () => {
      ...
      toast.success(
        t("successScreen.scheduleFollowUp") +
          " " +
          t("toasts.scheduled", { defaultValue: "scheduled!" })
      );
    },
  ...
  const sendRequest = trpc.requests.send.useMutation({
    onSuccess: data => {
      ...
      track("send_request", {
        platform: activePlatform?.platform ?? "unknown",
      });
      toast.success(
        t("toasts.reviewRequestSent", { defaultValue: "Review request sent!" })
      );
    },
    onError: err => {
      setSending(false);
  ...
  const tonePreviewComparison = useMemo(() => {
    ...
    return {
      subject: getToneTextDiff(
        tonePreviewSourceDraft.subject,
        tonePreviewDraft.subject
      ),
  ...
  const renderedTonePreviewComparison = useMemo(() => {
    ...
    const project = (
      ...
      tonePreviewChangedOnly
        ? getChangedToneLineSegments(segments, changedKind)
        : segments;
    return {
      subject: {
  ...
  function handleToneAdjustment() {
    if (!profile?.businessName) {
      toast.error(
        t("profile.addBusinessNameAndLink", {
          defaultValue:
            "Add your business name and review link in Settings first.",
        })
      );
      return;
    }
  ...
  function handleApplyTonePreview() {
    ...
    setTonePreviewChangedOnly(false);
    toast.success(
      t("mainForm.toneApplied", {
        defaultValue: "Tone applied. Review the message before sending.",
      })
    );
  }
  ...
  async function handleCopyTonePreviewDraft() {
    ...
    try {
      ...
      toast.success(
        t("mainForm.tonePreviewCopySuccess", {
          defaultValue: "Revised draft copied to your clipboard.",
        })
      );
    } catch {
      toast.error(
        t("mainForm.tonePreviewCopyError", {
          defaultValue:
            "Could not copy the revised draft. Please select and copy it manually.",
        })
      );
    }
  }
  ...
  const draftContext = useMemo(
    () => ({
      customerName:
        customerName ||
        t("mainForm.previewCustomer", { defaultValue: "Customer" }),
      businessName: profile?.businessName ?? "",
  ...
  const previewSubject = useMemo(() => {
    return renderReviewRequestDraft(draftSubject, draftContext).replace(
      /\s*[\r\n]+\s*/g,
      " "
    );
  ...
  const hasUnsavedSendDraft =
    ...
    (customerName.trim().length > 0 ||
      ...
      (loadedDraftSourceKey === draftSourceKey &&
        (draftSubject !== sourceDraft.subject ||
          draftBody !== sourceDraft.body)) ||
      tonePreviewOpen ||
      finalPreviewOpen ||
  ...
  function validate() {
    ...
    if (!customerName.trim())
      errs.name = t("validationErrors.customerNameRequired");
    ...
    if (!draftSubject.trim()) {
      errs.subject = t("mainForm.subjectRequired", {
        defaultValue: "Enter an email subject.",
      });
      ...
      errs.subject = t("mainForm.subjectSingleLine", {
        defaultValue: "Keep the subject on one line.",
      });
    }
    if (!draftBody.trim()) {
      errs.body = t("mainForm.bodyRequired", {
        defaultValue: "Enter an email message.",
      });
  ...
  function handleReviewBeforeSend() {
    ...
    if (!activeReviewValue) {
      toast.custom(
        toastId => (
          <div
            className="flex flex-col gap-2 px-4 py-3 rounded-xl shadow-lg bg-white"
            style={{
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-sm font-bold rr-text-navy-mid"
  ...
  if (sent) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 pb-40 rr-bg-navy">
        <div className="w-40 h-40 mb-6">
          <img
            src={SUCCESS_IMG}
            alt={t("successScreen.requestSent")}
        ...
        {isMilestone && (
          <div
            className="w-full max-w-xs rounded-2xl p-4 mb-6 rr-bg-navy-mid"
            style={{ border: "1.5px solid oklch(0.80 0.18 80)" }}
          >
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 rr-bg-gold">
        ...
        {lastRequestId && !reminderSettings?.followUpEnabled && (
          <div
            className="w-full max-w-xs rounded-2xl p-4 mb-4 rr-bg-navy-mid"
            style={{ border: "1.5px solid oklch(0.45 0.08 260)" }}
          >
            {reminderScheduled ? (
              ...
              <>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {scheduleFollowUpNow.isPending ? (
                      <Loader2 size={12} className="animate-spin" />
                    ) : (
                      <Bell size={12} />
                    )}
                    {t("successScreen.yesRemindMe", {
                      defaultValue: "Yes, remind me",
                    })}
                  </button>
                  <button
  ...
  return (
    <>
      <div className="min-h-screen pb-40 rr-bg-cream-warm">
        ...
        <div className="px-5 pt-14 md:pt-6 pb-6 rr-bg-navy animate-scale-in">
          ...
          {profile && (
            <p className="text-base font-bold mt-1 text-white/90">
              {t("mainForm.from")}{" "}
              {smtpStatus?.email ??
                t("smtp.noEmailConnected", {
                  defaultValue: "No email connected",
                })}
            </p>
          )}
        </div>
        ...
        <div
          className="px-4 py-4 lg:px-8 lg:py-6 animate-fade-up"
          ...
          <div className="max-w-5xl mx-auto lg:grid lg:grid-cols-[1fr_380px] lg:gap-8 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold mb-1 rr-text-gold-dim">
                      {t("smtp.emailNotConnectedTitle", {
                        defaultValue: "Email not connected",
                      })}
                    </p>
                    <p
                      className="text-xs mb-2"
                      style={{ color: "oklch(0.50 0.08 80)" }}
                    >
                      {t("smtp.emailNotConnectedDesc", {
                        defaultValue:
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-bold mb-1 rr-text-navy">
                            {t("page.freeLimitReached", {
                              defaultValue: "Free limit reached",
                            })}
                          </p>
                          <p
                            className="text-xs mb-2"
                          ...
                          >
                            {quota.nextAvailableAt
                              ? t("page.freeRollingLimitReachedDesc", {
                                  defaultValue:
                                    "You've used all 5 requests in your current rolling 30-day allowance. Your next request becomes available on {{date}}, or upgrade to keep sending now.",
                                  date: new Date(
                                    quota.nextAvailableAt
                                  ).toLocaleDateString(),
                                })
                              : t("page.freeInitialLimitReachedDesc", {
                                  defaultValue:
                                    "You've used your 10 initial requests. Your recurring allowance is 5 requests every rolling 30 days, or upgrade for unlimited requests.",
                                })}
                          </p>
                          <button
                        ...
                        >
                          {quota.phase === "initial"
                            ? t("page.freeInitialRemaining", {
                                defaultValue:
                                  "Free plan: {{remaining}} of 10 initial requests remaining — then 5 every rolling 30 days",
                                remaining: quota.remaining,
                              })
                            : t("page.freeRollingRemaining", {
                                defaultValue:
                                  "Free plan: {{remaining}} of 5 requests remaining in your rolling 30-day allowance",
                                remaining: quota.remaining,
                              })}
                        </span>
                      </div>
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                <h2 className="text-base font-black mb-4 rr-text-navy">
                  {t("page.customerDetails", {
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-3 py-3 pr-8 rounded-xl text-sm outline-none appearance-none bg-white rr-text-navy"
                        style={{
                      ...
                      >
                        <option value="default">
                          {defaultTemplate
                            ? t("mainForm.defaultTemplateWithName", {
                                templateName: defaultTemplate.name,
                              })
                            : t("mainForm.defaultTemplate")}
                        </option>
                        {templates
                          ?.filter(tmpl => !tmpl.isDefault)
                          .map(tmpl => (
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <PencilLine
                            size={15}
                            className="rr-text-gold-dim"
                            aria-hidden="true"
                          />
                          <h3 className="text-sm font-black rr-text-navy">
                            {t("mainForm.editMessageTitle", {
                              defaultValue: "Edit this message",
                            })}
                          </h3>
                        </div>
                        <p className="text-xs mt-1 rr-text-navy-muted">
                          {t("mainForm.editMessageDescription", {
                            defaultValue:
                      ...
                      <div>
                        <p className="flex items-center gap-1.5 text-xs font-black rr-text-navy">
                          <Sparkles
                            ...
                            aria-hidden="true"
                          />
                          {t("mainForm.aiToneTitle", {
                            defaultValue: "AI tone adjustment",
                          })}
                          {!hasPaidAiAccess && (
                            <ProBadge variant="locked" size="sm" />
                          )}
                        ...
                        <p className="mt-0.5 text-[11px] rr-text-navy-muted">
                          {t("mainForm.aiToneDescription", {
                            defaultValue:
                              "Refines your draft while keeping placeholders and review links intact.",
                          })}
                        </p>
                      </div>
                      <div className="flex min-w-0 items-center gap-2">
                        <label className="sr-only" htmlFor="send-request-tone">
                          {t("mainForm.aiToneLabel", {
                            defaultValue: "Email tone",
                          })}
                        </label>
                        <select
                          ...
                          onChange={event =>
                            setSelectedTone(
                              event.target.value as typeof selectedTone
                            )
                          }
                        ...
                        >
                          <option value="warmer">
                            {t("mainForm.aiToneWarmer", {
                              defaultValue: "Warmer",
                            })}
                          </option>
                          <option value="professional">
                            {t("mainForm.aiToneProfessional", {
                              defaultValue: "Professional",
                            })}
                          </option>
                          <option value="concise">
                            {t("mainForm.aiToneConcise", {
                        ...
                        <button
                          ...
                          aria-label={
                            hasPaidAiAccess
                              ? t("mainForm.applyAiTone", {
                                  defaultValue: "Apply AI tone adjustment",
                                })
                              : t("mainForm.unlockAiTone", {
                                  defaultValue: "Unlock AI tone adjustment",
                                })
                          }
                        >
                          {adjustTone.isPending ? (
                    ...
                    <div className="flex flex-col gap-4">
                      <div>
                        <div className="mb-1 flex items-center justify-between gap-3">
                          ...
                          >
                            {t("mainForm.subjectLabel", {
                              defaultValue: "Subject",
                            })}
                          </label>
                          <span
                            className="text-[11px] rr-text-navy-muted"
                            aria-live="polite"
                          >
                            {draftSubject.length}/
                            {MAX_REVIEW_REQUEST_SUBJECT_CHARS}
                        ...
                        <input
                          ...
                          aria-describedby={
                            errors.subject
                              ? "send-request-subject-error"
                              : undefined
                          }
                        />
                        {errors.subject && (
                          <p
                            id="send-request-subject-error"
                            className="text-xs mt-1 rr-text-red"
                            role="alert"
                          >
                            {errors.subject}
                          </p>
                      ...
                      <div>
                        <div className="mb-1 flex items-center justify-between gap-3">
                          <label
                            htmlFor="send-request-body"
                            className="text-xs font-bold rr-text-navy-mid"
                          >
                            {t("mainForm.messageLabel", {
                        ...
                        <textarea
                          ...
                          aria-describedby={
                            errors.body
                              ? "send-request-body-error"
                              : "send-request-placeholder-help"
                          }
                        />
                        {errors.body ? (
                          <p
                            id="send-request-body-error"
                            className="text-xs mt-1 rr-text-red"
                            role="alert"
                          >
                            {errors.body}
                          ...
                          <p
                            id="send-request-placeholder-help"
                            className="text-xs mt-1 rr-text-navy-muted"
                          >
                            {t("mainForm.placeholderHelp", {
                              defaultValue:
                  ...
                  {profile?.businessName && (
                    <div className="px-4 py-3 rounded-xl rr-bg-white-card lg:hidden">
                      <p className="text-xs font-bold mb-2 rr-text-navy-mid">
                        {t("mainForm.livePreview", {
                          defaultValue: "Live email preview",
                        })}
                      </p>
                      <p
                      ...
                      <p
                        ...
                        style={{ color: "oklch(0.50 0.03 260)" }}
                      >
                        <strong>{t("mainForm.subject")}</strong>{" "}
                        {previewSubject}
                      </p>
                      <p
                        className="text-xs"
                ...
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                  ...
                  <div className="px-5 py-4 flex flex-col gap-3">
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-bold rr-text-navy-mid">
                        {t("mainForm.from")}
                      </span>
                      <span className="text-sm rr-text-navy">
                        {smtpStatus?.email ?? "your@email.com"}
                      </span>
                    </div>
                    <div className="flex flex-col gap-1">
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-xs font-bold rr-text-gold">
                      {t("mainForm.complianceReminderTitle", {
                        defaultValue: "Send responsibly",
                      })}
                    </p>
                  </div>
                  <p className="text-xs text-white/80">
                    {t("mainForm.complianceReminderBody", {
                      defaultValue:
      ...
      >
        <DialogContent
          ...
          data-testid="send-final-preview-dialog"
        >
          <DialogHeader>
            <DialogTitle className="rr-text-navy">
              {t("mainForm.finalPreviewTitle", {
                defaultValue: "Review the final email",
              })}
            </DialogTitle>
            <DialogDescription>
              {t("mainForm.finalPreviewDescription", {
                defaultValue:
          ...
          <div className="flex flex-col gap-4">
            <div
              className="grid gap-3 rounded-2xl p-4 rr-bg-cream-warm sm:grid-cols-2"
              style={{ border: "1px solid oklch(0.90 0.02 260)" }}
            >
              <div>
                ...
                <p className="break-all text-sm rr-text-navy">
                  {smtpStatus?.email ?? "your@email.com"}
                </p>
              </div>
              <div>
                <p className="text-xs font-bold rr-text-navy-mid">
                  {t("mainForm.to", { defaultValue: "To:" })}
                </p>
                <p className="break-all text-sm rr-text-navy">
            ...
            >
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm font-bold rr-text-navy">
                  {previewSubject}
                </p>
              </div>
              <div>
                ...
                <div className="mt-1 whitespace-pre-wrap text-sm leading-relaxed rr-text-navy">
                  {previewBody}
                </div>
              </div>
            </div>
            ...
            {platforms?.some(platform => platform.platform === "yelp") && (
              <div
                className="flex items-start gap-2 rounded-xl px-3 py-2.5 rr-bg-gold-pale"
                style={{ border: "1px solid oklch(0.85 0.12 80)" }}
              >
                <AlertTriangle
                  size={15}
                  className="mt-0.5 shrink-0 rr-text-gold-dim"
                  aria-hidden="true"
                />
            ...
            >
              <div className="mb-2 flex items-center gap-2">
                ...
                <p className="text-sm font-black rr-text-navy">
                  {t("bulkSendDialog.complianceChecklistTitle")}
                </p>
              </div>
              <div className="flex flex-col gap-2">
                {(
                  [
                    {
                      key: "realCustomers",
                      label: t("bulkSendDialog.realCustomersCheck"),
                    },
                    {
                      key: "noIncentives",
                      label: t("bulkSendDialog.noIncentivesCheck"),
                    },
                    {
                      key: "allCustomers",
                      label: t("bulkSendDialog.allCustomersCheck"),
                    },
                  ] as const
                ).map(item => (
                  <label
                    key={item.key}
                    className="flex min-h-10 cursor-pointer items-start gap-3 rounded-xl px-2 py-2 hover:bg-white/70"
                  >
                    <input
                      type="checkbox"
                      checked={complianceChecked[item.key]}
                      onChange={event =>
                        setComplianceChecked(current => ({
                          ...current,
                    ...
                    />
                    <span className="text-xs leading-relaxed rr-text-navy-mid">
                      {item.label}
                    </span>
                  </label>
                ))}
                <label className="flex min-h-10 cursor-pointer items-start gap-3 rounded-xl px-2 py-2 hover:bg-white/70">
                  ...
                  <span className="text-xs leading-relaxed rr-text-navy-mid">
                    {t(
                      "bulkSendDialog.consentCheck",
                      "I confirm customers have consented to be contacted by my business via email and/or text"
                    )}
                  </span>
                </label>
              </div>
          ...
          <DialogFooter>
            ...
            >
              ...
              {sending
                ? t("mainForm.sending")
                : t("mainForm.confirmAndSend", {
                    defaultValue: "Confirm & send",
                  })}
            </button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={tonePreviewOpen} onOpenChange={handleTonePreviewOpenChange}>
        <DialogContent
          className="max-h-[92vh] overflow-y-auto sm:max-w-3xl lg:max-w-4xl"
          data-testid="ai-tone-preview-dialog"
        >
          <DialogHeader>
            <DialogTitle className="rr-text-navy">
              {t("mainForm.tonePreviewTitle", {
                defaultValue: "Review the AI-adjusted email",
              })}
            </DialogTitle>
            <DialogDescription>
              {t("mainForm.tonePreviewDescription", {
                defaultValue:
                  "Your current draft will not change until you choose Apply adjusted tone.",
              })}
            </DialogDescription>
          </DialogHeader>

          {tonePreviewSourceDraft &&
            tonePreviewDraft &&
            tonePreviewComparison &&
            renderedTonePreviewComparison && (
              <section
                className="overflow-hidden rounded-2xl bg-white"
                style={{ border: "1px solid oklch(0.90 0.02 260)" }}
                data-testid="ai-tone-preview-comparison"
                aria-label={t("mainForm.tonePreviewComparisonTitle", {
                  defaultValue: "Compare the changes",
                })}
              >
                <div
                ...
                >
                  ...
                  />
                  <p className="text-sm font-black rr-text-navy">
                    {t("mainForm.tonePreviewComparisonTitle", {
                      defaultValue: "Compare the changes",
                    })}
                  </p>
                  <button
                    ...
                    onClick={() =>
                      setTonePreviewChangedOnly(current => !current)
                    }
                    ...
                    style={{
                      background: tonePreviewChangedOnly
                        ? "oklch(0.74 0.15 83)"
                        : "oklch(0.93 0.02 260)",
                  ...
                  >
                    ...
                    {tonePreviewChangedOnly
                      ? t("mainForm.tonePreviewShowFullDraft", {
                          defaultValue: "Show full drafts",
                        })
                      : t("mainForm.tonePreviewChangedOnly", {
                          defaultValue: "Changed lines only",
                        })}
                  </button>
                  <p
                    id="tone-preview-filter-description"
                    className="basis-full text-xs leading-relaxed rr-text-navy-mid"
                  >
                    {t("mainForm.tonePreviewChangedOnlyDescription", {
                      defaultValue:
                        "Filters both drafts to the full lines that contain a highlighted change.",
                    })}
                  </p>
                  <p className="basis-full text-xs leading-relaxed rr-text-navy-mid">
                    {t("mainForm.tonePreviewDiffLegend", {
                      defaultValue:
                        "Removed text is highlighted in red. Added text is highlighted in green.",
                    })}
                  </p>
                </div>
                ...
                <div className="grid gap-px bg-[oklch(0.90_0.02_260)] md:grid-cols-2">
                  <section
                    ...
                    data-testid="ai-tone-preview-original"
                    aria-label={t("mainForm.tonePreviewOriginalDraft", {
                      defaultValue: "Original draft",
                    })}
                  >
                    <p className="mb-4 text-xs font-black uppercase tracking-wide rr-text-navy-mid">
                      {t("mainForm.tonePreviewOriginalDraft", {
                        defaultValue: "Original draft",
                      })}
                    </p>
                    <div className="mb-4">
                      ...
                      <p className="mt-1 break-words text-sm font-bold leading-relaxed rr-text-navy">
                        {renderedTonePreviewComparison.subject.before.length >
                        0 ? (
                          <ToneDiffText
                            segments={
                              renderedTonePreviewComparison.subject.before
                            }
                            mode="before"
                          />
                        ) : (
                          <span className="italic rr-text-navy-mid">
                            {t("mainForm.tonePreviewNoChangedLines", {
                              defaultValue: "No changed lines in this section.",
                            })}
                          </span>
                        )}
                      </p>
                    ...
                    <div>
                      ...
                      <div className="mt-1 break-words text-sm leading-relaxed rr-text-navy">
                        {renderedTonePreviewComparison.body.before.length >
                        0 ? (
                          <ToneDiffText
                            segments={renderedTonePreviewComparison.body.before}
                            mode="before"
                          />
                        ) : (
                          <span className="italic rr-text-navy-mid">
                  ...
                  <section
                    className="min-w-0 bg-white p-4"
                    data-testid="ai-tone-preview-adjusted"
                    aria-label={t("mainForm.tonePreviewAdjustedDraft", {
                      defaultValue: "AI-adjusted draft",
                    })}
                  >
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                      <p className="text-xs font-black uppercase tracking-wide rr-text-navy-mid">
                        {t("mainForm.tonePreviewAdjustedDraft", {
                          defaultValue: "AI-adjusted draft",
                        })}
                      </p>
                      <button
                        ...
                        onClick={() => {
                          buttonPressHaptic();
                          void handleCopyTonePreviewDraft();
                        }}
                      ...
                      >
                        ...
                        {t("mainForm.tonePreviewCopyRevised", {
                          defaultValue: "Copy revised draft",
                        })}
                      </button>
                    </div>
                    <div className="mb-4">
                      ...
                      <p className="mt-1 break-words text-sm font-bold leading-relaxed rr-text-navy">
                        {renderedTonePreviewComparison.subject.after.length >
                        0 ? (
                          <ToneDiffText
                            segments={
                              renderedTonePreviewComparison.subject.after
                            }
                            mode="after"
                          />
                        ) : (
                    ...
                    <div>
                      ...
                      <div className="mt-1 break-words text-sm leading-relaxed rr-text-navy">
                        {renderedTonePreviewComparison.body.after.length > 0 ? (
                          <ToneDiffText
                            segments={renderedTonePreviewComparison.body.after}
                            mode="after"
                          />
                        ) : (
                          <span className="italic rr-text-navy-mid">
                ...
                {tonePreviewRationales.length > 0 && (
                  <section
                    ...
                    style={{ borderColor: "oklch(0.90 0.02 260)" }}
                    aria-labelledby="tone-preview-rationales-title"
                    data-testid="ai-tone-preview-rationales"
                  >
                    <div className="flex items-start gap-2">
                      <Lightbulb
                        size={17}
                        className="mt-0.5 rr-text-gold"
                        aria-hidden="true"
                      />
                      <div>
                        <h3
                          id="tone-preview-rationales-title"
                          className="text-sm font-black rr-text-navy"
                        >
                          {t("mainForm.tonePreviewRationaleTitle", {
                            defaultValue: "Why AI suggested these adjustments",
                          })}
                        </h3>
                        <p className="mt-1 text-xs leading-relaxed rr-text-navy-mid">
                          {t("mainForm.tonePreviewRationaleDescription", {
                            defaultValue:
                              "A brief explanation for each adjusted field. AI keeps request details, placeholders, and links intact.",
                          })}
                        </p>
                      </div>
                    </div>
                    <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                      {tonePreviewRationales.map(entry => (
                        <li
                          key={entry.field}
                          className="rounded-xl p-3 rr-bg-cream-warm"
                          style={{ border: "1px solid oklch(0.90 0.03 83)" }}
                        >
                          <p className="text-xs font-black uppercase tracking-wide rr-text-navy-mid">
                            {entry.field === "subject"
                              ? t("mainForm.subject")
                          ...
                          <p className="mt-1 text-sm leading-relaxed rr-text-navy">
                            {entry.rationale}
                          </p>
                        </li>
                      ))}
          ...
          <DialogFooter>
            ...
            >
              ...
              {t("mainForm.applyAdjustedTone", {
                defaultValue: "Apply adjusted tone",
              })}
            </button>
          </DialogFooter>
        </DialogContent>
      ...
      <ContactPickerModal
        ...
        onImport={contacts => {
          if (contacts.length === 1) {
            ...
            setErrors({});
            toast.success(
              `${t("page.imported", { defaultValue: "Imported" })} ${contacts[0].name}`
            );
          } else {
            ...
            setErrors({});
            toast.success(
              t("page.importedMultiple", {
                defaultValue: `Imported ${contacts.length} contacts - sending to first contact. Use Bulk Send for multiple.`,
