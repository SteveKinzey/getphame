...
export default function AdminReferralRewardsPage() {
  ...
  const { data: rows, isLoading } = trpc.adminReferrals.listDeferred.useQuery(
    undefined,
    {
  ...
  const processAll = trpc.adminReferrals.processAll.useMutation({
    onSuccess: ({ processed, skipped }) => {
      toast.success(
        `Done — ${processed} rewarded, ${skipped} skipped (not on paid plan).`
      );
      utils.adminReferrals.listDeferred.invalidate();
    },
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: "oklch(0.70 0.05 260)" }}>
                Referrers who converted a paid user but were on free tier at the
                time
              </p>
            </div>
          </div>
      ...
      <div className="px-5 pt-5 space-y-4">
        ...
        {eligibleCount > 0 && (
          <button
            ...
            style={{
              background: processAll.isPending
                ? "oklch(0.75 0.12 80)"
                : "oklch(0.80 0.18 80)",
              color: "oklch(0.22 0.09 260)",
            }}
        ...
        {!isLoading && rows?.length === 0 && (
          <div
            ...
            style={{
              background: "white",
              border: "1px solid oklch(0.91 0.02 260)",
            }}
          >
            <CheckCircle2
              size={32}
          ...
          <div className="space-y-3">
            {rows.map(row => (
              <div
                ...
                style={{
                  border: `1.5px solid ${row.isNowEligible ? "oklch(0.72 0.16 150)" : "oklch(0.91 0.02 260)"}`,
                }}
              >
                {/* Eligibility banner */}
                {row.isNowEligible ? (
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {/* Referrer */}
                  <div>
                    <p className="text-xs rr-text-navy-muted mb-0.5">
                      Referrer
                    </p>
                    ...
                    <div className="mt-1 flex items-center gap-1.5">
                      ...
                      {row.referrerPlanExpiresAt && (
                        <span className="text-xs rr-text-navy-muted">
                          exp{" "}
                          {format(
                            new Date(row.referrerPlanExpiresAt),
                            "MMM d, yyyy"
                          )}
                        </span>
                      )}
                ...
                >
                  ...
                  {row.isNowEligible
                    ...
                    : "Cannot reward — not on paid plan"}
                </button>
              </div>
            ))}
          </div>
        ...
        >
          ...
          <p className="text-xs" style={{ color: "oklch(0.45 0.06 260)" }}>
            Rewards extend the referrer's <strong>planExpiresAt</strong> by 30
            days from their current expiry. Lifetime subscribers are
            automatically excluded. Processing is idempotent — already-rewarded
            rows are filtered out before this page loads.
          </p>
        </div>
      </div>
    </div>
