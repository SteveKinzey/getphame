...
export const WALKTHROUGH_CAPTION_LANGUAGES = [
  "en",
  "es",
  "fr",
  "it",
...
const CAPTION_FONT_SIZE_PREFERENCE_KEY =
  "getphame-walkthrough-caption-font-size";
const CAPTION_BACKGROUND_PREFERENCE_KEY =
  "getphame-walkthrough-caption-background";
const CAPTION_FONT_FAMILY_PREFERENCE_KEY =
  "getphame-walkthrough-caption-font-family";
const CAPTION_TEXT_COLOR_PREFERENCE_KEY =
  "getphame-walkthrough-caption-text-color";
const CAPTION_TEXT_OPACITY_PREFERENCE_KEY =
  "getphame-walkthrough-caption-text-opacity";
const CAPTION_LINE_SPACING_PREFERENCE_KEY =
  "getphame-walkthrough-caption-line-spacing";
const CAPTION_TEXT_EDGE_PREFERENCE_KEY =
  ...
  return source
    ...
    .flatMap((block, blockIndex) => {
      ...
      if (
        ...
        return [];
      return [
        { id: `cue-${blockIndex}-${startTime}`, startTime, endTime, text },
      ];
    });
}

export function findActiveTranscriptCueIndex(
...
export function isWalkthroughCaptionLanguage(
  value: string | null
): value is CaptionLanguage {
...
export default function VideoDemo() {
  ...
  const trackCaptionLanguage =
    trpc.analytics.trackCaptionLanguage.useMutation();
    ...
    () => {
      ...
      try {
        storedLanguage = window.localStorage.getItem(
          CAPTION_LANGUAGE_PREFERENCE_KEY
        );
      } catch {
        // Browser negotiation still works when storage is restricted or unavailable.
      }
      return resolveWalkthroughCaptionLanguage(
        storedLanguage,
        getOrderedBrowserLanguages()
      );
    }
  );
    ...
    () => {
      ...
      try {
        const storedSize = window.localStorage.getItem(
          CAPTION_FONT_SIZE_PREFERENCE_KEY
        );
        return storedSize === "small" || storedSize === "large"
          ? storedSize
          : "medium";
      } catch {
        return "medium";
    ...
    () => {
      ...
      try {
        const storedBackground = window.localStorage.getItem(
          CAPTION_BACKGROUND_PREFERENCE_KEY
        );
        return storedBackground === "black" ||
          storedBackground === "translucent"
          ? storedBackground
          : "navy";
      } catch {
        return "navy";
  ...
  const [captionFontFamily, setCaptionFontFamily] = useState<CaptionFontFamily>(
    () => {
      ...
      try {
        const storedFamily = window.localStorage.getItem(
          CAPTION_FONT_FAMILY_PREFERENCE_KEY
        );
        return storedFamily === "serif" || storedFamily === "mono"
          ? storedFamily
          : "sans";
      } catch {
        return "sans";
    ...
    () => {
      ...
      try {
        const storedColor = window.localStorage.getItem(
          CAPTION_TEXT_COLOR_PREFERENCE_KEY
        );
        return storedColor === "gold" || storedColor === "cyan"
          ? storedColor
          : "white";
      } catch {
        return "white";
  ...
  const [captionTextOpacity, setCaptionTextOpacity] =
    useState<CaptionTextOpacity>(() => {
      if (typeof window === "undefined") return "solid";
      try {
        const storedOpacity = window.localStorage.getItem(
          CAPTION_TEXT_OPACITY_PREFERENCE_KEY
        );
        return storedOpacity === "high" || storedOpacity === "soft"
          ? storedOpacity
          : "solid";
      } catch {
        return "solid";
  ...
  const [captionLineSpacing, setCaptionLineSpacing] =
    useState<CaptionLineSpacing>(() => {
      if (typeof window === "undefined") return "standard";
      try {
        const storedSpacing = window.localStorage.getItem(
          CAPTION_LINE_SPACING_PREFERENCE_KEY
        );
    ...
    () => {
      ...
      try {
        const storedEdge = window.localStorage.getItem(
          CAPTION_TEXT_EDGE_PREFERENCE_KEY
        );
        return storedEdge === "none" || storedEdge === "outline"
          ? storedEdge
          : "shadow";
      } catch {
        return "shadow";
  ...
  const captionLanguageOptions = [
    {
      value: "en" as const,
      label: t("landing.modal.captionLanguageEnglish", {
        defaultValue: "English",
      }),
      trackLabel: t("landing.modal.captionTrackEnglish", {
        defaultValue: "English captions",
      }),
    },
    {
      value: "es" as const,
      label: t("landing.modal.captionLanguageSpanish", {
        defaultValue: "Spanish",
      }),
      trackLabel: t("landing.modal.captionTrackSpanish", {
        defaultValue: "Spanish captions",
      }),
    },
    {
      value: "fr" as const,
      label: t("landing.modal.captionLanguageFrench", {
        defaultValue: "French",
      }),
      trackLabel: t("landing.modal.captionTrackFrench", {
        defaultValue: "French captions",
      }),
    },
    {
      value: "it" as const,
      label: t("landing.modal.captionLanguageItalian", {
        defaultValue: "Italian",
      }),
      trackLabel: t("landing.modal.captionTrackItalian", {
        defaultValue: "Italian captions",
      }),
    },
    {
      value: "de" as const,
      label: t("landing.modal.captionLanguageGerman", {
        defaultValue: "German",
      }),
      trackLabel: t("landing.modal.captionTrackGerman", {
        defaultValue: "German captions",
      }),
    },
    {
      value: "pt" as const,
      label: t("landing.modal.captionLanguagePortuguese", {
        defaultValue: "Portuguese",
      }),
      trackLabel: t("landing.modal.captionTrackPortuguese", {
        defaultValue: "Portuguese captions",
      }),
    },
  ];
  const selectedCaptionLanguageLabel =
    captionLanguageOptions.find(option => option.value === captionLanguage)
      ?.label ?? captionLanguageOptions[0].label;
  const captionFontSizeOptions = [
    ...
    },
    {
      ...
      label: t("landing.modal.captionFontSizeMedium", {
        defaultValue: "Medium",
      }),
    },
    {
  ...
  const captionBackgroundOptions = [
    {
      value: "navy" as const,
      label: t("landing.modal.captionBackgroundNavy", {
        defaultValue: "Dark navy",
      }),
    },
    {
      value: "black" as const,
      label: t("landing.modal.captionBackgroundBlack", {
        defaultValue: "Black",
      }),
    },
    {
      value: "translucent" as const,
      label: t("landing.modal.captionBackgroundClear", {
  ...
  const captionFontFamilyOptions = [
    {
      value: "sans" as const,
      label: t("landing.modal.captionFontFamilySans", {
        defaultValue: "Sans serif",
      }),
    },
    {
      value: "serif" as const,
      label: t("landing.modal.captionFontFamilySerif", {
        defaultValue: "Serif",
      }),
    },
    {
      value: "mono" as const,
      label: t("landing.modal.captionFontFamilyMono", {
  ...
  const captionTextColorOptions = [
    {
      ...
      label: t("landing.modal.captionTextColorWhite", {
        defaultValue: "White",
      }),
    },
    {
  ...
  const captionTextOpacityOptions = [
    {
      ...
      label: t("landing.modal.captionTextOpacitySolid", {
        defaultValue: "100%",
      }),
    },
    {
  ...
  const captionLineSpacingOptions = [
    {
      ...
      label: t("landing.modal.captionLineSpacingCompact", {
        defaultValue: "Compact",
      }),
    },
    {
      value: "standard" as const,
      label: t("landing.modal.captionLineSpacingStandard", {
        defaultValue: "Standard",
      }),
    },
    {
      value: "spacious" as const,
      label: t("landing.modal.captionLineSpacingSpacious", {
  ...
  const captionTextEdgeOptions = [
    ...
    },
    {
      ...
      label: t("landing.modal.captionTextEdgeShadow", {
        defaultValue: "Shadow",
      }),
    },
    {
      value: "outline" as const,
      label: t("landing.modal.captionTextEdgeOutline", {
        defaultValue: "Outline",
      }),
    },
  ];
  const selectedCaptionFontSizeLabel =
    captionFontSizeOptions.find(option => option.value === captionFontSize)
      ?.label ?? captionFontSizeOptions[1].label;
  const selectedCaptionBackgroundLabel =
    captionBackgroundOptions.find(option => option.value === captionBackground)
      ?.label ?? captionBackgroundOptions[0].label;
  const selectedCaptionFontFamilyLabel =
    captionFontFamilyOptions.find(option => option.value === captionFontFamily)
      ?.label ?? captionFontFamilyOptions[0].label;
  const selectedCaptionTextColorLabel =
    captionTextColorOptions.find(option => option.value === captionTextColor)
      ?.label ?? captionTextColorOptions[0].label;
  const selectedCaptionTextOpacityLabel =
    captionTextOpacityOptions.find(
      option => option.value === captionTextOpacity
    )?.label ?? captionTextOpacityOptions[0].label;
  const selectedCaptionLineSpacingLabel =
    captionLineSpacingOptions.find(
      option => option.value === captionLineSpacing
    )?.label ?? captionLineSpacingOptions[1].label;
  const selectedCaptionTextEdgeLabel =
    captionTextEdgeOptions.find(option => option.value === captionTextEdge)
      ?.label ?? captionTextEdgeOptions[1].label;
  ...
  const syncCaptionMode = useCallback(() => {
    ...
    for (let index = 0; index < tracks.length; index += 1) {
      ...
      track.mode =
        captionsEnabled && track.language === captionLanguage
          ? "showing"
          : "disabled";
    }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") {
        if (captionSettingsMenuOpen || captionLanguageMenuOpen || isMenuTarget)
          return;
        setOpen(false);
        return;
      }
  ...
  useEffect(() => {
    ...
    void fetch(WALKTHROUGH_CAPTION_TRACKS[captionLanguage], {
      ...
      .then(source => {
        ...
        if (cues.length === 0)
          throw new Error("Transcript contains no usable cues");
  ...
  const downloadTranscript = async (format: TranscriptExportFormat) => {
    ...
    const generatedValue = new Intl.DateTimeFormat(undefined, {
      dateStyle: "long",
    }).format(new Date());
    const metadata = {
      brand: "GET PHAME",
      ...
      generatedValue,
      sourceLabel: t("landing.modal.transcriptExportSourceLabel", {
        defaultValue: "Source",
      }),
    ...
    try {
      ...
      setTranscriptExportMessage(
        t("landing.modal.transcriptExportError", {
          defaultValue:
            "The transcript download could not be created. Please try again.",
        })
      );
    }
  };
          ...
          <FadeUp delay={0.1}>
            <div className="max-w-3xl mx-auto">
              ...
              <button
                type="button"
                onClick={openVideo}
                aria-label={t("landing.thumbnail.playButtonAriaLabel", {
                  defaultValue: "Play product walkthrough video",
                })}
              ...
              >
                ...
                <div className="relative aspect-video bg-[oklch(0.14_0.03_250)]">
                  <img
                    ...
                    alt={t("landing.thumbnail.altText", {
                      defaultValue:
                        "Get Phame platform walkthrough video thumbnail",
                    })}
                ...
                <div className="flex items-center gap-3 px-5 py-3.5 bg-[#0f1d32] border-t border-[#1e3050]">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-semibold text-white">
                      {t("landing.bottomBar.title", {
                        defaultValue: "Get Phame — Platform Walkthrough",
                      })}
                    </p>
                    <p className="text-xs text-slate-400 font-medium">
                      {t("landing.bottomBar.description", {
                        defaultValue:
                          "Individual outreach · Connected inbox · Limited reminders · Stop on completion",
                      })}
                    </p>
                  </div>
                  <div className="ml-auto shrink-0 text-xs font-medium text-primary group-hover:underline">
                    {t("landing.bottomBar.watchNow", {
          ...
          >
            ...
            >
              ...
              <button
                ref={closeButtonRef}
                onClick={() => setOpen(false)}
                aria-label={t("landing.modal.closeButtonAriaLabel", {
                  defaultValue: "Close video",
                })}
              ...
              <h3 id="get-phame-video-title" className="sr-only">
                {t("landing.modal.videoTitle", {
                  defaultValue: "Get Phame platform walkthrough",
                })}
              </h3>
              <p id="get-phame-video-description" className="sr-only">
                {t("landing.modal.captionNotice", {
                  defaultValue:
                    "English narration with optional English, Spanish, French, Italian, German, and Portuguese captions.",
                })}
              </p>
                  ...
                  <div className="min-w-0">
                    <div className="relative aspect-video max-h-[calc(100dvh-10rem)] overflow-hidden rounded-xl bg-black">
                      <video
                        ...
                        aria-label={t("landing.modal.videoTitle", {
                          defaultValue: "Get Phame platform walkthrough",
                        })}
                        ...
                        >
                          <div className="max-w-md">
                            <p className="text-base font-semibold text-white">
                              {t("landing.modal.videoError", {
                                defaultValue:
                                  "The walkthrough could not load in this browser.",
                              })}
                            </p>
                            <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
                              ...
                              >
                                {t("landing.modal.videoOpenDirect", {
                                  defaultValue: "Open video directly",
                                })}
                              </a>
                            </div>
                          </div>
                    ...
                    <div className="mt-2 flex flex-col gap-2">
                      <p className="px-1 text-center text-xs leading-relaxed text-slate-300 sm:text-left sm:text-sm">
                        {t("landing.modal.captionsHelp", {
                          defaultValue:
                            "Use the quick CC button or press C to toggle captions. Customize their appearance, or select any transcript line to jump to that moment.",
                        })}
                      </p>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button
                              ...
                              aria-label={t(
                                captionsEnabled
                                  ? "landing.modal.captionsDisable"
                                  : "landing.modal.captionsEnable",
                                {
                                  defaultValue: captionsEnabled
                                    ? "Disable captions"
                                    : "Enable captions",
                                }
                              )}
                            ...
                            >
                              ...
                              <kbd
                                className="hidden rounded border border-current/25 px-1.5 py-0.5 text-[10px] font-black leading-none opacity-75 sm:inline-flex"
                                aria-hidden="true"
                              >
                                C
                              </kbd>
                              ...
                              >
                                {t(
                                  captionsEnabled
                                    ? "landing.modal.captionsOn"
                                    : "landing.modal.captionsOff",
                                  {
                                    defaultValue: captionsEnabled
                                      ? "Captions on"
                                      : "Captions off",
                        ...
                        >
                          <DropdownMenuTrigger asChild>
                            <button
                              ...
                              aria-controls="getphame-walkthrough-player"
                              aria-label={t(
                                "landing.modal.captionSettingsAriaLabel",
                                {
                                  defaultValue:
                                    "Open caption appearance settings",
                                }
                              )}
                            ...
                            >
                              ...
                              />
                              <span>
                                {t("landing.modal.captionSettings", {
                                  defaultValue: "Caption settings",
                                })}
                              </span>
                            </button>
                          </DropdownMenuTrigger>
                          ...
                          >
                            ...
                            >
                              <p className="mb-1 text-xs font-bold uppercase tracking-wider text-primary">
                                {t("landing.modal.captionPreview", {
                                  ...
                                  <span className="getphame-caption-preview-text">
                                    {transcriptCues[activeCueIndex]?.text ??
                                      t("landing.modal.captionPreviewSample", {
                                        defaultValue:
                                          "Your captions will look like this.",
                                      })}
                                  </span>
                                </span>
                              </div>
                            ...
                            >
                              <RotateCcw
                                ...
                                aria-hidden="true"
                              />
                              {t("landing.modal.captionSettingsReset", {
                            ...
                            <DropdownMenuLabel className="inline-flex min-h-10 w-[6.5rem] items-center px-1 py-1 align-middle text-xs font-bold uppercase tracking-wider text-primary">
                              {t("landing.modal.captionFontSize", {
                            ...
                            <DropdownMenuSeparator className="bg-white/15" />
                            <DropdownMenuLabel className="inline-flex min-h-10 w-[6.5rem] items-center px-1 py-1 align-middle text-xs font-bold uppercase tracking-wider text-primary">
                              {t("landing.modal.captionBackground", {
                            ...
                            <DropdownMenuSeparator className="bg-white/15" />
                            <DropdownMenuLabel className="inline-flex min-h-10 w-[6.5rem] items-center px-1 py-1 align-middle text-xs font-bold uppercase tracking-wider text-primary">
                              {t("landing.modal.captionFontFamily", {
                            ...
                            <DropdownMenuSeparator className="bg-white/15" />
                            <DropdownMenuLabel className="inline-flex min-h-10 w-[6.5rem] items-center px-1 py-1 align-middle text-xs font-bold uppercase tracking-wider text-primary">
                              {t("landing.modal.captionTextColor", {
                            ...
                            <DropdownMenuSeparator className="bg-white/15" />
                            <DropdownMenuLabel className="inline-flex min-h-10 w-[6.5rem] items-center px-1 py-1 align-middle text-xs font-bold uppercase tracking-wider text-primary">
                              {t("landing.modal.captionTextOpacity", {
                                defaultValue: "Text opacity",
                              })}
                            </DropdownMenuLabel>
                            <DropdownMenuRadioGroup
                            ...
                            <DropdownMenuSeparator className="bg-white/15" />
                            <DropdownMenuLabel className="inline-flex min-h-10 w-[6.5rem] items-center px-1 py-1 align-middle text-xs font-bold uppercase tracking-wider text-primary">
                              {t("landing.modal.captionLineSpacing", {
                            ...
                            <DropdownMenuSeparator className="bg-white/15" />
                            <DropdownMenuLabel className="inline-flex min-h-10 w-[6.5rem] items-center px-1 py-1 align-middle text-xs font-bold uppercase tracking-wider text-primary">
                              {t("landing.modal.captionTextEdge", {
                                defaultValue: "Text edge",
                              })}
                            </DropdownMenuLabel>
                            <DropdownMenuRadioGroup
                        ...
                        >
                          <DropdownMenuTrigger asChild>
                            <button
                              ...
                              aria-controls="getphame-walkthrough-player"
                              aria-label={t(
                                "landing.modal.captionLanguageAriaLabel",
                                {
                                  defaultValue:
                            ...
                            >
                              ...
                              />
                              <span>
                                {t("landing.modal.captionLanguage", {
                                  defaultValue: "Language",
                                })}
                              </span>
                          ...
                          >
                            ...
                            <DropdownMenuRadioGroup
                              ...
                              onValueChange={value => {
                                if (!isWalkthroughCaptionLanguage(value))
                                  return;
                  ...
                  >
                    <div className="border-b border-white/10 px-1 pb-3">
                      <div className="flex items-center justify-between gap-3">
                        <h4
                          id="getphame-transcript-title"
                          className="font-display text-base font-bold text-white"
                        >
                          {t("landing.modal.transcriptTitle", {
                            defaultValue: "Interactive transcript",
                          })}
                        </h4>
                      ...
                      <p className="mt-1 text-xs leading-relaxed text-slate-300">
                        {t("landing.modal.transcriptHelp", {
                          defaultValue:
                            "Select a line to jump to that moment in the video.",
                        })}
                      </p>
                      <div
                        role="group"
                        aria-label={t(
                          "landing.modal.transcriptExportGroupLabel",
                          {
                    ...
                    >
                      {transcriptStatus === "loading" && (
                        <p
                          role="status"
                          className="px-2 py-6 text-center text-sm font-semibold text-slate-300"
                        >
                          {t("landing.modal.transcriptLoading", {
                      ...
                      {transcriptStatus === "error" && (
                        ...
                        >
                          {t("landing.modal.transcriptError", {
                            defaultValue:
                              "The transcript is unavailable right now.",
                          })}
                        </p>
                      )}
                      {transcriptStatus === "ready" && (
                        <ol
                          ...
                          aria-label={t("landing.modal.transcriptCueList", {
                            defaultValue: "Transcript cues",
                          })}
                        >
                          {transcriptCues.map((cue, index) => {
                            const isActive = index === activeCueIndex;
                            const timeLabel = formatTranscriptTime(
                              cue.startTime
                            );
                            return (
                              <li key={cue.id}>
                                <button
                                  ...
                                  aria-label={t(
                                    "landing.modal.transcriptJumpTo",
                                    {
                                      defaultValue:
                                        "Jump to {{time}}: {{text}}",
                                      time: timeLabel,
                                ...
                                >
                                  <span className="flex items-start gap-3">
                                    <span
                                      className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 font-mono text-[11px] font-bold ${isActive ? "bg-primary text-primary-foreground" : "bg-white/10 text-slate-300"}`}
                                    >
                                      {timeLabel}
                                    </span>
                                  ...
                                  {isActive && (
                                    <span className="mt-1.5 block pl-[3.15rem] text-[10px] font-black uppercase tracking-wider text-primary">
                                      {t("landing.modal.transcriptCurrent", {
