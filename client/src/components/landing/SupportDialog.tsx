...
export default function SupportDialog() {
  ...
  const chooseAttachment = (file: File | null) => {
    ...
    if (!(["image/jpeg", "image/png", "image/webp"] as const).includes(file.type as UploadedAttachment["mimeType"])) {
      setSelectedFile(null);
      setAttachmentError(t("landing.support.attachmentType", { defaultValue: "Attach a JPG, PNG, or WebP screenshot." }));
      return;
    }
    if (file.size > MAX_ATTACHMENT_BYTES) {
      setSelectedFile(null);
      setAttachmentError(t("landing.support.attachmentSize", { defaultValue: "Keep screenshots to 8 MB or less." }));
      return;
    }
      ...
      >
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              <label htmlFor={`${formId}-topic`} className="mb-1.5 block text-sm font-semibold text-white">
                {t("landing.support.topic", { defaultValue: "What can we help with?" })}
              </label>
              <select
                id={`${formId}-topic`}
                value={values.topic}
              ...
              >
                <option value="billing">{t("landing.support.topicBilling", { defaultValue: "Billing" })}</option>
                <option value="onboarding">{t("landing.support.topicOnboarding", { defaultValue: "Onboarding" })}</option>
                <option value="technical">{t("landing.support.topicTechnical", { defaultValue: "Technical issue" })}</option>
              </select>
            </div>
            <div>
            ...
            <div>
              <label htmlFor={`${formId}-attachment`} className="mb-1.5 block text-sm font-semibold text-white">
                {t("landing.support.attachment", { defaultValue: "Screenshot" })} <span className="text-slate-300">({t("landing.support.optional", { defaultValue: "optional" })})</span>
              </label>
              <input
                ref={fileInputRef}
              ...
              />
              <p id={`${formId}-attachment-hint`} className="mt-1.5 text-sm text-slate-200">
                {t("landing.support.attachmentHint", { defaultValue: "JPG, PNG, or WebP only. Maximum 8 MB." })}
              </p>
              {selectedFile && !attachmentError && (
                <div className="mt-2 flex items-center justify-between gap-3 rounded-lg border border-[#496087] bg-[#07152d] px-3 py-2 text-sm text-slate-100">
                  <span className="flex min-w-0 items-center gap-2"><Paperclip size={15} aria-hidden="true" /><span className="truncate">{selectedFile.name}</span></span>
                  <button
                    type="button"
                    ...
                    aria-label={t("landing.support.removeAttachment", { defaultValue: "Remove screenshot" })}
                  >
                    <X size={16} aria-hidden="true" />
                  </button>
                </div>
              )}
              {attachmentError && <p id={`${formId}-attachment-error`} className="mt-1.5 text-sm font-medium text-red-200" role="alert">{attachmentError}</p>}
            </div>
            ...
            >
              {submitSupport.isPending || uploadScreenshot.isPending ? <><Loader2 size={16} className="animate-spin" aria-hidden="true" />{uploadScreenshot.isPending ? t("landing.support.uploading", { defaultValue: "Uploading screenshot…" }) : t("landing.support.sending", { defaultValue: "Sending…" })}</> : <><Send size={16} aria-hidden="true" />{t("landing.support.submit", { defaultValue: "Send support request" })}</>}
            </button>
          </form>
        )}
      </DialogContent>
