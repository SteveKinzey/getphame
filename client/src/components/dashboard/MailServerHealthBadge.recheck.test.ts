...
const source = readFileSync(resolve(process.cwd(), "client/src/components/dashboard/MailServerHealthBadge.tsx"), "utf8");
...
describe("Mail server health recheck", () => {
  it("offers an in-place saved-server test only for needs-attention state", () => {
    ...
    expect(source).toContain('data-testid="dashboard-mail-health-test-connection"');
  ...
  it("keeps the troubleshooting deep link alongside the recheck control", () => {
    ...
    expect(source).toContain('data-testid="dashboard-mail-health-troubleshoot"');
  });
});
