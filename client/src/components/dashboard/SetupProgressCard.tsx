...
import {
  ...
  type OnboardingChecklistTelemetryEvent,
} from "@/lib/onboardingChecklistTelemetry";
...
export default function SetupProgressCard({
  ...
  const { mutate: submitChecklistEvent } =
    trpc.analytics.trackOnboardingChecklistEvent.useMutation();
  ...
  useEffect(() => {
    ...
    for (const stepId of incompleteStepIds.split(",").filter(Boolean)) {
      trackChecklistEvent(
        `${stepId}_step_viewed` as OnboardingChecklistTelemetryEvent
      );
    }
  ...
  return (
    <section className="rr-card p-4" aria-labelledby="setup-progress-title">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="rr-h6 rr-text-gold">
            {t("homePage.setupProgressEyebrow")}
          </p>
          <h2 id="setup-progress-title" className="rr-h4 mt-1 rr-text-navy">
            {t("homePage.completeSetup")}
          </h2>
          <p className="rr-b2 mt-1 rr-text-navy-mid">
            {t("homePage.setupProgressDescription")}
          </p>
        </div>
        <span className="shrink-0 rounded-full px-2.5 py-1 text-xs font-black rr-bg-gold rr-text-navy">
          {t("homePage.setupProgressCount", {
      ...
      <div
        ...
        aria-valuetext={t("homePage.setupProgressCount", {
          completed: completedCount,
          total: steps.length,
        })}
      >
      ...
      <ol
        ...
        style={{ borderColor: "oklch(0.92 0.005 100)" }}
      >
        {steps.map((step, index) => {
          ...
          return (
            ...
            >
              <div
                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${step.complete ? "rr-bg-green-pale" : "rr-bg-surface"}`}
              >
                {step.complete ? (
                  <CheckCircle2
                    size={19}
                    className="rr-text-green"
                    aria-label={t("homePage.setupStepComplete")}
                  />
                ) : (
              ...
              <div className="min-w-0 flex-1">
                <p
                  className={`rr-h5 ${step.complete ? "rr-text-green" : "rr-text-navy"}`}
                >
                  {step.title}
                </p>
                <p className="rr-l2 mt-0.5 rr-text-navy-mid">
                  {step.complete
                    ? t("homePage.setupStepComplete")
                    : step.description}
                </p>
              </div>
              {!step.complete && (
                <button
                  ...
                  onClick={() => {
                    trackChecklistEvent(
                      `${step.id}_step_actioned` as OnboardingChecklistTelemetryEvent
                    );
              ...
              {!step.complete && (
                <span className="sr-only">
                  {t("homePage.setupStepNumber", { step: index + 1 })}
                </span>
              )}
            </li>
          );
      ...
      {consentStats && consentStats.total >= 1 && (
        <div
          className="mt-4 pt-4"
          style={{ borderTop: "1px solid oklch(0.92 0.005 100)" }}
        >
          <div className="flex items-center justify-between mb-1.5">
            <div className="flex items-center gap-1.5">
              <ShieldCheck size={14} className="rr-text-green" />
              <span className="text-xs font-bold rr-text-navy">
                Contact consent
              </span>
            </div>
