...
import MailServerHealthBadge, {
  resolveMailServerHealth,
} from "./MailServerHealthBadge";
...
describe("MailServerHealthBadge", () => {
  it("resolves personal, bulk, attention, and disconnected tenant states without provider secrets", () => {
    expect(
      resolveMailServerHealth(
        { connected: true, verified: true, activeDeliveryChannel: "personal" },
        undefined
      )
    ).toBe("healthy");
    expect(
      resolveMailServerHealth(
        { connected: true, verified: false, activeDeliveryChannel: "personal" },
        undefined
      )
    ).toBe("attention");
    expect(
      resolveMailServerHealth(
        {
          ...
          activeDeliveryChannel: "personal",
        },
        undefined
      )
    ).toBe("attention");
    expect(
      resolveMailServerHealth(
        ...
        { connected: true, selectedForOutreach: true }
      )
    ).toBe("bulk_active");
    expect(
      resolveMailServerHealth(
        { connected: false, activeDeliveryChannel: null },
        undefined
      )
    ).toBe("disconnected");
  });
  ...
  it("renders an accessible Settings path with health state copy", () => {
    const markup = renderToStaticMarkup(
      createElement(MailServerHealthBadge, {
        smtp: {
          connected: true,
          verified: true,
          activeDeliveryChannel: "personal",
