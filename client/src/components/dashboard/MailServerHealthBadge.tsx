import { AlertCircle, CheckCircle2, CircleHelp, Mail, Settings2 } from "lucide-react";
...
export default function MailServerHealthBadge({
  ...
  const details: Record<MailServerHealth, { label: string; message: string; tooltip: string; icon: typeof CheckCircle2; tone: string; background: string }> = {
    healthy: {
      ...
      tooltip: translate("dashboard.mailHealth.healthyTooltip", { defaultValue: "A verified personal mail server is selected, and its latest health signal does not require action." }),
      icon: CheckCircle2,
    ...
    attention: {
      ...
      tooltip: translate("dashboard.mailHealth.attentionTooltip", { defaultValue: "Your saved connection needs attention. Re-test it or update the provider settings before sending customer outreach." }),
      icon: AlertCircle,
    ...
    disconnected: {
      ...
      tooltip: translate("dashboard.mailHealth.disconnectedTooltip", { defaultValue: "No verified tenant-owned delivery channel is selected. Customer outreach stays paused until you connect one in Settings." }),
      icon: Mail,
    ...
    bulk_active: {
      ...
      tooltip: translate("dashboard.mailHealth.bulkActiveTooltip", { defaultValue: "A verified tenant-owned bulk provider is selected for customer outreach. Manage or replace it in Settings." }),
      icon: CheckCircle2,
      tone: "oklch(0.30 0.12 145)",
      background: "oklch(0.96 0.04 145)",
    },
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <a href="/settings" data-testid="dashboard-mail-server-health" className="mt-4 flex items-center gap-3 rounded-xl px-4 py-3 transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60" style={{ background: current.background, color: current.tone }}>
          ...
          <CircleHelp size={15} aria-label={translate("dashboard.mailHealth.details", { defaultValue: "Mail server health details" })} />
