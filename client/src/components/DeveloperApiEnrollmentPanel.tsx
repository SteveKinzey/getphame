              ...
              {!status?.termsAccepted ? (
                ...
                <form className="mt-4 grid gap-3" onSubmit={(event) => {
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label><span className="mb-1 block text-xs font-bold rr-text-navy">{t("developerEnrollment.send.businessName", { defaultValue: "Business name" })}</span><input value={businessName} onChange={(event) => setBusinessName(event.target.value)} required minLength={2} maxLength={160} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm rr-text-navy outline-none focus-visible:ring-2 focus-visible:ring-[oklch(0.66_0.16_80)]"  name="rr-components-developer-api-enrollment-panel-business-name-132" /></label>
                    <label><span className="mb-1 block text-xs font-bold rr-text-navy">{t("developerEnrollment.send.website", { defaultValue: "Website (optional)" })}</span><input type="url" value={websiteUrl} onChange={(event) => setWebsiteUrl(event.target.value)} maxLength={512} placeholder="https://" className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm rr-text-navy outline-none focus-visible:ring-2 focus-visible:ring-[oklch(0.66_0.16_80)]"  name="rr-components-developer-api-enrollment-panel-website-url-133" /></label>
                  </div>
                  <label><span className="mb-1 block text-xs font-bold rr-text-navy">{t("developerEnrollment.send.useCase", { defaultValue: "How will the integration use sending access?" })}</span><textarea value={useCase} onChange={(event) => setUseCase(event.target.value)} required minLength={20} maxLength={1500} rows={3} className="w-full rounded-xl border border-slate-200 bg-white p-3 text-sm rr-text-navy outline-none focus-visible:ring-2 focus-visible:ring-[oklch(0.66_0.16_80)]" placeholder={t("developerEnrollment.send.useCasePlaceholder", { defaultValue: "Example: After a completed appointment, our server imports the customer and sends one review request." })}  name="rr-components-developer-api-enrollment-panel-use-case-135" /></label>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label><span className="mb-1 block text-xs font-bold rr-text-navy">{t("developerEnrollment.send.monthlyVolume", { defaultValue: "Expected sends per month" })}</span><input type="number" min={1} max={1_000_000} value={monthlyVolume} onChange={(event) => setMonthlyVolume(event.target.value)} required className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm rr-text-navy outline-none focus-visible:ring-2 focus-visible:ring-[oklch(0.66_0.16_80)]"  name="rr-components-developer-api-enrollment-panel-monthly-volume-137" /></label>
