...
export default function HomeInstallBanner() {
  ...
  const dismiss = () => {
    ...
    setDismissed(true);
    trackPwaEvent.mutate({
      event: "install_banner_dismissed",
      platform: analyticsPlatform,
    });
  };
  ...
  const remindLater = () => {
    ...
    setDismissed(true);
    trackPwaEvent.mutate({
      event: "install_banner_remind_later",
      platform: analyticsPlatform,
    });
  };
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(135deg, oklch(0.22 0.09 260), oklch(0.30 0.11 260))",
        borderColor: "oklch(0.80 0.18 80 / 0.55)",
      }}
      data-testid="home-install-banner"
    >
      <div
        className="absolute -right-8 -top-10 h-28 w-28 rounded-full"
        style={{ background: "oklch(0.80 0.18 80 / 0.10)" }}
        aria-hidden="true"
      />
      <div className="relative flex items-start gap-3 sm:items-center">
        <div
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl rr-bg-gold rr-text-navy"
          aria-hidden="true"
        >
          <Smartphone size={22} />
        </div>
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs font-semibold leading-relaxed text-white/75 sm:text-sm">
            {t("pwaInstallBanner.description", {
              defaultValue:
                "Open your review dashboard faster and use Get Phame like an app.",
            })}
          </p>
          <div className="mt-3 flex flex-col items-start gap-2 sm:flex-row sm:items-center">
            ...
            >
              ...
              {opening
                ...
                : installState.platform === "ios"
                  ? t("pwaInstallBanner.iosCta", {
                      defaultValue: "Show iPhone steps",
                    })
            ...
            >
              ...
              {t("pwaInstallBanner.remindLater", {
        ...
        <button
          ...
          aria-label={t("pwaInstallBanner.dismiss", {
            defaultValue: "Dismiss install banner",
          })}
      ...
      <p className="sr-only" aria-live="polite">
        {opening
          ? t("pwaInstallBanner.openingStatus", {
              defaultValue: "Opening the Get Phame installation options.",
            })
          : ""}
      </p>
    </section>
