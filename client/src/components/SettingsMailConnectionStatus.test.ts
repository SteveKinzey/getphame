...
const translate = vi.fn(
  (_key: string, { defaultValue }: { defaultValue: string }) => defaultValue
);
...
describe("Settings mail-connection card status", () => {
  it("renders the actual Settings personal-connection status for active, inactive, and recovery scenarios", () => {
    const active = renderToStaticMarkup(
      createElement(SettingsPersonalMailConnectionStatus, {
        status: { verified: true, selectedForOutreach: true },
        translate,
      })
    );
    ...
    const needsAttention = renderToStaticMarkup(
      createElement(SettingsPersonalMailConnectionStatus, {
        status: { verified: true, lastHealthStatus: "failed" },
        translate,
      })
    );
    ...
    expect(needsAttention).toContain(
      'data-mail-delivery-state="needs_attention"'
    );
  ...
  it("renders the actual Settings bulk-connection legacy block with recovery guidance", () => {
    const legacy = renderToStaticMarkup(
      createElement(SettingsBulkMailConnectionStatus, {
    ...
    expect(legacy).toContain(
      "Connect a bulk-mail provider account that you own"
    );
  });
});
