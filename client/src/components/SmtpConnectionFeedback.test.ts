...
describe("Settings SMTP connection feedback controls", () => {
  it("renders the actual pre-save test action, testing spinner, and unsaved success state", () => {
    const idle = renderToStaticMarkup(
      createElement(SmtpCandidateConnectionActions, {
        result: null,
        testing: false,
        connecting: false,
        onTest: noOp,
  ...
  it("renders connect progress and saved success feedback for SMTP and bulk mail credentials", () => {
    ...
    const smtpSaved = renderToStaticMarkup(
      createElement(ConnectionSavedNotice, {
        message:
          "Your email server is connected and ready for customer outreach.",
      })
    );
    const bulkSaved = renderToStaticMarkup(
      createElement(ConnectionSavedNotice, {
  ...
  it("renders provider-specific Gmail and Workspace App Password help beside the input label", () => {
    const gmail = renderToStaticMarkup(
      createElement(SmtpAppPasswordHelpTooltip, {
        provider: "gmail",
        translate,
      })
    );
    ...
    expect(workspaceCopy.content).toContain(
      "Workspace administrator must allow App Passwords"
    );
  });
});
