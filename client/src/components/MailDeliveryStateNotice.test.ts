...
describe("MailDeliveryStateNotice", () => {
  const expectations: Array<[MailDeliveryNoticeState, string]> = [
    ...
    [
      "not_selected",
      "Connected, but not selected for customer review requests",
    ],
    [
      "needs_attention",
  ...
  it.each(expectations)(
    "renders the %s state with clear visible guidance",
    (state, text) => {
      const html = renderToStaticMarkup(
        createElement(MailDeliveryStateNotice, { state })
      );
  ...
  it("renders translated recovery text when a locale resolver is supplied", () => {
    const html = renderToStaticMarkup(
      createElement(MailDeliveryStateNotice, {
        state: "needs_attention",
        translate: () => "Localized recovery guidance",
      })
    );
