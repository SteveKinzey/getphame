...
function PasskeyRow({ passkey, onChanged }: { passkey: Passkey; onChanged: () => Promise<void> | void }) {
  ...
  const dateFormatter = new Intl.DateTimeFormat(i18n.resolvedLanguage || "en", { year: "numeric", month: "short", day: "numeric" });

  async function saveName() {
    const trimmed = name.trim();
    if (!trimmed) return;
    try {
      await rename.mutateAsync({ credentialId: passkey.id, displayName: trimmed });
      await onChanged();
      setEditing(false);
      toast.success(t("passkeys.security.renameSuccess", { defaultValue: "Passkey renamed." }));
    } catch {
      toast.error(t("passkeys.security.renameError", { defaultValue: "Passkey could not be renamed." }));
    }
  }
  ...
  async function remove() {
    try {
      ...
      await onChanged();
      toast.success(t("passkeys.security.removeSuccess", { defaultValue: "Passkey removed." }));
    } catch {
      toast.error(t("passkeys.security.removeError", { defaultValue: "Passkey could not be removed." }));
    }
  }
  ...
  return (
    <li className="border-t border-border py-4 first:border-t-0 first:pt-0 last:pb-0">
      <div className="flex items-start gap-3">
        <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/15 text-foreground"><KeyRound size={17} aria-hidden="true" /></div>
        <div className="min-w-0 flex-1">
          {editing ? (
            <div className="flex items-center gap-2">
              <label htmlFor={`passkey-name-${passkey.id}`} className="sr-only">{t("passkeys.security.nameLabel", { defaultValue: "Passkey name" })}</label>
              <input id={`passkey-name-${passkey.id}`} value={name} maxLength={80} onChange={(event) => setName(event.target.value)} className="min-h-10 min-w-0 flex-1 rounded-lg border border-border bg-background px-3 text-sm font-semibold text-foreground outline-none focus:ring-2 focus:ring-ring/40" />
              <button type="button" onClick={saveName} disabled={pending || !name.trim()} className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary text-primary-foreground disabled:opacity-50" aria-label={t("passkeys.security.saveName", { defaultValue: "Save passkey name" })}>{rename.isPending ? <Loader2 size={16} className="animate-spin" aria-hidden="true" /> : <Check size={16} aria-hidden="true" />}</button>
              <button type="button" onClick={() => { setName(passkey.displayName); setEditing(false); }} disabled={pending} className="flex h-10 w-10 items-center justify-center rounded-lg bg-muted text-foreground" aria-label={t("passkeys.security.cancelRename", { defaultValue: "Cancel rename" })}><X size={16} aria-hidden="true" /></button>
            </div>
          ) : (
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="truncate text-sm font-black text-foreground">{passkey.displayName}</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{t("passkeys.security.created", { defaultValue: "Created {{date}}", date: dateFormatter.format(new Date(passkey.createdAt)) })}{passkey.lastUsedAt ? ` · ${t("passkeys.security.lastUsed", { defaultValue: "Last used {{date}}", date: dateFormatter.format(new Date(passkey.lastUsedAt)) })}` : ""}</p>
                {passkey.backedUp && <span className="mt-2 inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-1 text-xs font-bold text-emerald-700"><ShieldCheck size={13} aria-hidden="true" />{t("passkeys.security.synced", { defaultValue: "Synced by your device provider" })}</span>}
              </div>
              <button type="button" onClick={() => setEditing(true)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground" aria-label={t("passkeys.security.rename", { defaultValue: "Rename {{name}}", name: passkey.displayName })}><Pencil size={15} aria-hidden="true" /></button>
            </div>
          )}
          {!editing && <div className="mt-3">{confirmingRemove ? (
            <div className="flex flex-wrap items-center gap-2 rounded-xl bg-destructive/10 p-3">
              <p className="w-full text-xs font-bold text-destructive">{t("passkeys.security.removeConfirm", { defaultValue: "Remove this passkey? You will not be able to use it to sign in." })}</p>
              <button type="button" onClick={remove} disabled={pending} className="flex min-h-9 items-center gap-1.5 rounded-lg bg-destructive px-3 text-xs font-bold text-white disabled:opacity-50">{revoke.isPending ? <Loader2 size={14} className="animate-spin" aria-hidden="true" /> : <Trash2 size={14} aria-hidden="true" />}{t("passkeys.security.confirmRemove", { defaultValue: "Remove passkey" })}</button>
              <button type="button" onClick={() => setConfirmingRemove(false)} disabled={pending} className="min-h-9 rounded-lg bg-muted px-3 text-xs font-bold text-foreground">{t("passkeys.security.cancel", { defaultValue: "Cancel" })}</button>
            </div>
          ) : <button type="button" onClick={() => setConfirmingRemove(true)} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg px-2 text-xs font-bold text-destructive hover:bg-destructive/10"><Trash2 size={14} aria-hidden="true" />{t("passkeys.security.remove", { defaultValue: "Remove" })}</button>}</div>}
        </div>
      </div>
    </li>
  );
...
export default function PasskeySecurityCard() {
  ...
  async function registerPasskey(isVerifiedResume = false) {
    ...
    try {
      ...
      setStatus(t("passkeys.security.verifying", { defaultValue: "Verifying your new passkey…" }));
      await finishRegistration.mutateAsync({ ceremonyId: ceremony.ceremonyId, displayName: displayName.trim() || t("passkeys.security.defaultName", { defaultValue: "My passkey" }), response });
      setDisplayName("");
      setStatus(null);
