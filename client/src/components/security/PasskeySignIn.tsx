...
import { isPasskeyEnrollmentRequiredError, isPasskeyEnrollmentReturnError, readPasskeyEnrollmentEmail, rememberPasskeyEnrollmentEmail, type PasskeyEnrollmentReturnError } from "@/lib/passkeyEnrollment";

const EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export default function PasskeySignIn({ redirectTo = "/" }: { redirectTo?: string }) {
  const { t } = useTranslation();
  const [email, setEmail] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [status, setStatus] = useState<string | null>(null);
  const [enrollmentEmail, setEnrollmentEmail] = useState(() => readPasskeyEnrollmentEmail());
  const [enrollmentOpen, setEnrollmentOpen] = useState(() => {
    const params = new URLSearchParams(window.location.search);
    return params.get("passkey_enroll") === "1" && Boolean(readPasskeyEnrollmentEmail());
  });
  ...
  const pending = beginAuthentication.isPending || finishAuthentication.isPending;
  ...
  function handleEnrollmentOpenChange(nextOpen: boolean) {
    ...
    if (!nextOpen) window.requestAnimationFrame(() => submitButtonRef.current?.focus());
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!EMAIL_PATTERN.test(normalizedEmail)) {
      setError(t("passkeys.signIn.invalidEmail", { defaultValue: "Enter a valid email address." }));
      return;
    }
    try {
      setStatus(t("passkeys.signIn.preparing", { defaultValue: "Preparing your secure passkey prompt…" }));
      const ceremony = await beginAuthentication.mutateAsync({ email: normalizedEmail });
      ...
      setStatus(t("passkeys.signIn.verifying", { defaultValue: "Verifying your passkey…" }));
      await finishAuthentication.mutateAsync({ ceremonyId: ceremony.ceremonyId, response });
  ...
  return (
    <>
      <AddPasskeyModal open={enrollmentOpen} email={enrollmentEmail} onOpenChange={handleEnrollmentOpenChange} verificationError={verificationError} />
      {!supported ? <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">{t("passkeys.signIn.unsupported", { defaultValue: "Passkeys are not available in this browser. You can still sign in with a secure email link." })}</div> : <section aria-labelledby="passkey-sign-in-title">
      ...
      <form onSubmit={handleSubmit} className="space-y-3" noValidate>
        ...
        <button ref={submitButtonRef} type="submit" disabled={pending} aria-busy={pending} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground transition-[filter,transform] duration-150 hover:brightness-110 active:scale-[0.97] disabled:cursor-wait disabled:opacity-60">
