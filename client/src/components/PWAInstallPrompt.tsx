...
export default function PWAInstallPrompt() {
  ...
  useEffect(() => {
    ...
    const handleInstalled = () => {
      ...
      trackPwaEvent.mutate({
        event: "app_installed",
        platform: analyticsPlatform(detectedPlatform),
      });
    };
    ...
    const showWhenOverlaysClose = () => {
      ...
      unsubscribeWelcome = subscribeToPwaInstall(() => {
        ...
        if (
          currentOverlayState.welcomeVisible ||
          currentOverlayState.upgradeVisible
        )
          return;
        unsubscribeWelcome?.();
    ...
    const timer =
      detectedPlatform === "other" || localStorage.getItem(STORAGE_KEY)
        ? undefined
  ...
  const record = (
    event: Parameters<typeof trackPwaEvent.mutate>[0]["event"]
  ) => {
  ...
  useEffect(() =>
    registerPwaInstallRequest(async () => {
      if (installPrompt) {
        await install();
        return;
      }
  ...
  const shareGetPhame = async () => {
    setSharing(true);
    setShareStatus("idle");
    let localizedShareText =
      "Turn happy customers into more trusted reviews with Get Phame.";
    try {
    ...
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-base font-bold mt-1 text-white/90">
              Install Get Phame for the best experience — it works like a native
              app.
            </p>
          </div>
          <button
            ...
            <>
              <Step
                ...
                text={
                  <>
                    Tap the{" "}
                    <span className="font-bold rr-text-gold">Share</span> button
              ...
              <Step
                number={3}
                icon={
                  <span className="text-sm font-black rr-text-gold">✓</span>
                }
                text={
                  <>
                    Tap <span className="font-bold rr-text-gold">Add</span> —
            ...
            <>
              ...
              <Step
                ...
                text={
                  <>
                    Tap{" "}
                    <span className="font-bold rr-text-gold">Install app</span>{" "}
                    or{" "}
                    <span className="font-bold rr-text-gold">
                      Add to Home screen
                    </span>
              ...
              <Step
                ...
                text={
                  <>
                    Confirm installation — Get Phame will appear on your home
                    screen
                  </>
                }
              />
        ...
        <p className="sr-only" aria-live="polite">
          {shareStatus === "shared"
            ? "Get Phame shared successfully."
            : shareStatus === "copied"
              ? "Get Phame link copied to the clipboard."
