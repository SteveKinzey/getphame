...
export default function PWAInstallPrompt({ onOpenGuide }: { onOpenGuide?: () => void }) {
      ...
      >
        ...
        {onOpenGuide && (
          <button
            ...
            className="w-full flex items-center justify-center gap-2 mt-4 py-3 rounded-2xl text-sm font-bold transition-all active:scale-95"
            style={{ background: "rgba(255,255,255,0.08)", color: "oklch(0.80 0.18 80)" }}
          >
            <BookOpen size={14} />
        ...
        <button
          onClick={dismiss}
          className="w-full text-center text-sm font-bold mt-3 text-white/60"
        >
          Maybe later
        </button>
      </div>
