...
> = {
  send_limit: {
    ...
    description:
      "Your free allowance protects a simple starting workflow. A paid plan removes the Get Phame plan limit when you are ready to continue.",
    benefits: [
  ...
  koalendar: {
    ...
    description:
      "Connect Koalendar with a private webhook and automatically add eligible invitees after each meeting ends.",
    benefits: [
  ...
  bulk_sender: {
    ...
    description:
      "Connect a supported transactional SMTP relay while Get Phame keeps provider-aware safety controls in place.",
    benefits: [
        ...
        <div className="relative overflow-hidden px-5 pb-5 pt-6 sm:px-7 sm:pb-7 sm:pt-7">
          ...
          <button
            ...
            aria-label={t("premiumConversion.modal.closeLabel", {
              defaultValue: "Close upgrade details",
            })}
          ...
          <DialogHeader className="gap-3 text-left">
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-amber-200/25 bg-amber-200/10 px-3 py-1.5 text-xs font-black uppercase tracking-[0.08em] rr-text-gold">
              ...
              {t("premiumConversion.modal.eyebrow", {
                defaultValue: "Premium feature",
              })}
            </div>
            <div className="flex items-start gap-3">
              <span
                className="mt-0.5 grid h-11 w-11 flex-none place-items-center rounded-2xl bg-amber-200/10 rr-text-gold"
                aria-hidden="true"
              >
                <Icon size={21} />
              </span>
          ...
          >
            {(["one", "two", "three"] as const).map((benefitKey, index) => (
              <li
                key={benefitKey}
                className="flex items-start gap-3 rounded-2xl bg-white/[0.07] px-3.5 py-3 text-sm font-bold leading-snug text-white"
              >
                <span
                  className="mt-0.5 grid h-6 w-6 flex-none place-items-center rounded-full bg-amber-200/10 rr-text-gold"
                  aria-hidden="true"
                >
                  <ArrowRight size={14} />
                </span>
                <span>
                  {t(`${featurePath}.benefits.${benefitKey}`, {
                    defaultValue: defaults.benefits[index],
                  })}
                </span>
              </li>
            ))}
          ...
          <p className="mt-2 text-center text-xs font-semibold leading-relaxed text-white/55">
            {t("premiumConversion.modal.contextHint", {
              defaultValue:
                "You can compare plans without losing the page you were working on.",
            })}
          </p>
        </div>
      </DialogContent>
