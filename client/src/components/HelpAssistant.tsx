...
const SOURCE_TITLE_KEYS: Record<string, { key: string; defaultValue: string }> =
  {
    "getting-started": {
      key: "helpAssistant.sources.gettingStarted",
      defaultValue: "Get Phame setup",
    },
    "email-connection": {
      key: "helpAssistant.sources.emailConnection",
      defaultValue: "Connect your sending email",
    },
    "sending-requests": {
      key: "helpAssistant.sources.sendingRequests",
      defaultValue: "Send an individual request",
    },
    "platform-compliance": {
      key: "helpAssistant.sources.compliance",
      defaultValue: "Review outreach compliance guide",
    },
    "contacts-import": {
      key: "helpAssistant.sources.contacts",
      defaultValue: "Contacts and consented imports",
    },
    reminders: {
      key: "helpAssistant.sources.reminders",
      defaultValue: "Follow-up reminders",
    },
    "developer-integrations": {
      key: "helpAssistant.sources.developer",
      defaultValue: "Developer Integrations",
    },
    "account-billing": {
      key: "helpAssistant.sources.billing",
      defaultValue: "Account and plan management",
    },
    "privacy-support": {
      key: "helpAssistant.sources.privacy",
      defaultValue: "Privacy and human support",
    },
  };
...
export default function HelpAssistant() {
  ...
  const language = useMemo<SupportedLang>(() => {
    ...
    return SUPPORTED_LANGS.includes(resolved as SupportedLang)
      ? (resolved as SupportedLang)
      : "en";
  }, [i18n.language, i18n.resolvedLanguage]);

  const quickQuestions = [
    t("helpAssistant.quick.connectEmail", {
      defaultValue: "How do I connect my sending email?",
    }),
    t("helpAssistant.quick.googleLink", {
      defaultValue: "Where do I add my Google review link?",
    }),
    t("helpAssistant.quick.yelp", {
      defaultValue: "What should I know before using Yelp?",
    }),
  ];
    ...
    onError: () => {
      setMessages(current => [
        ...current,
        {
          id: createId(),
          role: "assistant",
          text: t("helpAssistant.unavailable", {
            defaultValue:
              "The help assistant is unavailable right now. Open a support request and our team can help.",
          }),
          shouldEscalate: true,
          source: "fallback",
        },
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <button
            ...
            aria-label={t("helpAssistant.open", {
              defaultValue: "Open Get Phame help",
            })}
          >
        ...
        <DialogContent className="flex h-[min(760px,calc(100dvh-1.5rem))] w-[calc(100vw-1.5rem)] max-w-2xl flex-col gap-0 overflow-hidden border-border bg-background p-0 text-foreground shadow-2xl sm:w-full">
            ...
            <div className="flex items-center gap-3 pr-8">
              ...
              <div>
                ...
                <DialogDescription className="rr-l2 rr-on-dark-muted">
                  {t("helpAssistant.description", {
                    defaultValue:
                      "Grounded answers from approved product guidance",
                  })}
                </DialogDescription>
              </div>
            </div>
          ...
          <div
            ...
            role="log"
            aria-live="polite"
            aria-label={t("helpAssistant.conversation", {
              defaultValue: "Help conversation",
            })}
          >
            {messages.length === 0 ? (
              <div className="mx-auto max-w-lg space-y-5">
                <div className="rounded-2xl border border-primary/25 bg-primary/10 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="rr-h5">
                        {t("helpAssistant.privateTitle", {
                          defaultValue: "Keep private data out of chat",
                        })}
                      </p>
                      <p className="rr-b2 mt-1 text-muted-foreground">
                        {t("helpAssistant.privateBody", {
                          defaultValue:
                            "Do not enter passwords, API keys, card details, customer lists, or private review content. Questions are not saved as a durable chat transcript.",
                        })}
                      </p>
                    </div>
                  </div>
                ...
                <div>
                  <p className="rr-h5 mb-3">
                    {t("helpAssistant.tryQuestion", {
                      defaultValue: "Try a common question",
                    })}
                  </p>
              ...
              <div className="space-y-4">
                {messages.map(message => (
                  ...
                  >
                    <span
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${message.role === "assistant" ? "bg-[var(--navy)] text-white" : "bg-primary text-primary-foreground"}`}
                    >
                    ...
                    <div
                      className={`max-w-[85%] rounded-2xl px-4 py-3 ${message.role === "assistant" ? "border border-border bg-card text-card-foreground" : "bg-primary text-primary-foreground"}`}
                    >
                      ...
                      {message.redacted && (
                        <p className="mt-3 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-foreground">
                          <AlertTriangle
                            ...
                            aria-hidden="true"
                          />
                          {t("helpAssistant.redacted", {
                            defaultValue:
                              "A possible private value was removed before your question was processed.",
                          })}
                        </p>
                      )}
                      ...
                      {message.citations && message.citations.length > 0 && (
                        <div className="mt-3 border-t border-border pt-3">
                          <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
                            {t("helpAssistant.sourcesLabel", {
                              defaultValue: "Approved sources",
                            })}
                          </p>
                            ...
                            {message.citations.map(citation => {
                              ...
                              return (
                                ...
                                >
                                  {localized
                                    ? t(localized.key, {
                                        defaultValue: localized.defaultValue,
                                      })
                                    : citation.title}
                      ...
                      {message.role === "assistant" &&
                        (message.shouldEscalate ||
                          ...
                          <Button
                            ...
                            onClick={openSupport}
                          >
                            {t("helpAssistant.humanSupport", {
                              defaultValue: "Open a support request",
                            })}
                          </Button>
                        )}
                    </div>
                  ...
                  <div className="flex items-center gap-2.5" role="status">
                    ...
                    <span className="inline-flex items-center gap-2 rounded-2xl border border-border bg-card px-4 py-3 text-sm text-muted-foreground">
                      ...
                      {t("helpAssistant.thinking", {
                        defaultValue: "Checking approved guidance…",
                      })}
                    </span>
                  </div>
                )}
            ...
            <form onSubmit={handleSubmit} className="flex items-end gap-2">
              <div className="flex-1">
                <label htmlFor="help-assistant-question" className="sr-only">
                  {t("helpAssistant.questionLabel", {
                    defaultValue: "Ask a Get Phame question",
                  })}
                </label>
                <Textarea
                  ...
                  placeholder={t("helpAssistant.placeholder", {
                    defaultValue: "Ask how something works…",
                  })}
              ...
              <Button
                ...
                aria-label={t("helpAssistant.send", {
                  defaultValue: "Send help question",
                })}
              >
                {ask.isPending ? (
                  <Loader2
            ...
            <div className="mt-2 flex items-center justify-between gap-3 text-xs text-muted-foreground">
              <span>
                {t("helpAssistant.disclaimer", {
                  defaultValue:
                    "Answers use approved Get Phame guidance and may need human review.",
                })}
              </span>
              <button
                type="button"
                className="shrink-0 font-bold text-foreground underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                onClick={openSupport}
              >
                {t("helpAssistant.supportLink", {
                  defaultValue: "Human support",
                })}
              </button>
            </div>
          </div>
