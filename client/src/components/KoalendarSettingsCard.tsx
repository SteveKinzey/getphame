...
export default function KoalendarSettingsCard({
  hasPaidAccess,
}: KoalendarSettingsCardProps) {
  ...
  const disconnect = trpc.koalendar.disconnect.useMutation({
    onSuccess: () => {
      refresh();
      toast.success("Koalendar disconnected.");
    },
    onError: error => toast.error(error.message),
  });
  ...
  return (
    <section
      ...
      aria-labelledby="koalendar-heading"
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3">
          <div
            className="w-9 h-9 rounded-xl flex items-center justify-center rr-bg-gold"
            aria-hidden="true"
          >
          ...
          <div>
            ...
            <p className="text-xs rr-text-navy-muted">
              Import completed bookings as contacts after each meeting ends.
            </p>
          </div>
        </div>
      ...
      {!hasPaidAccess ? (
        <div className="rounded-xl p-4 rr-bg-surface-darker">
          ...
          <p className="text-xs mt-1 mb-3 rr-text-navy-muted">
            Upgrade to connect Koalendar and automatically add eligible invitees
            to Saved Contacts.
          </p>
          <button
            type="button"
        ...
        <div className="rounded-xl p-4 rr-bg-surface-darker">
          <p className="text-xs rr-text-navy-muted mb-3">
            Create a private per-account webhook URL, then add it to Koalendar
            for booking created, rescheduled, and canceled events.
          </p>
          <button
            type="button"
        ...
        <div className="space-y-4">
          <div className="rounded-xl p-4 rr-bg-surface-darker">
            <label
              className="block text-xs font-black rr-text-navy mb-1"
              htmlFor="koalendar-webhook-url"
            >
              Your private webhook URL
            </label>
            <p className="text-[11px] rr-text-navy-muted mb-2">
              Keep this URL private. Rotating it immediately invalidates the old
              URL.
            </p>
            ...
            <ol className="mt-3 space-y-1 text-xs rr-text-navy-muted list-decimal pl-4">
              ...
              <li>
                Paste this URL and enable created, rescheduled, and canceled
                events.
              </li>
              <li>
                Get Phame imports non-canceled invitees after the scheduled end
                time.
              </li>
            </ol>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs font-black uppercase tracking-wider rr-text-navy-muted">
                Recent bookings
              </h3>
              <span className="text-[11px] rr-text-navy-muted">
              ...
              <div className="space-y-2">
                {status.recentBookings.map(booking => {
                  ...
                  return (
                    <div
                      key={booking.id}
                      className="rounded-xl p-3 rr-bg-surface-darker"
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span
                          className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black ${badge.className}`}
                        >
                          {badge.label}
                        </span>
                      </div>
                      <p className="mt-2 flex items-center gap-1 text-[11px] rr-text-navy-muted">
                        <Clock size={11} aria-hidden="true" /> Ends{" "}
