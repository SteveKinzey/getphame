...
export default function MagicLinkForm({
  ...
  const requestMagicLink = useCallback(
    async (candidateEmail: string): Promise<MagicLinkRequestResult> => {
      try {
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      if (!normalizedEmail) {
        setError(
          t("login.emailRequired", { defaultValue: "Email is required." })
        );
        return;
      }
  ...
  const handleResend = useCallback(async () => {
    ...
    setResendNotice(
      t("login.resendSuccess", {
        defaultValue: "A fresh sign-in link is on its way.",
      })
    );
    setRequestState("sent");
    ...
    return (
      ...
      >
        <div role="status" aria-live="polite" aria-atomic="true">
          ...
          <p className="text-sm font-semibold text-[#C9A84C] break-all mb-5">
            {sentTo}
          </p>
        ...
        <p
          ...
          role="timer"
          aria-atomic="true"
          className="text-xs font-medium text-white/60 mb-3"
        >
          {resendSeconds > 0
            ...
            : t("login.resendReady", {
                defaultValue: "You can request another link now.",
              })}
        </p>
        ...
        {error && (
          <div
            role="alert"
            className="mb-3 rounded-xl border border-red-400/25 bg-red-500/10 px-4 py-3 text-sm text-red-300"
          >
            {error}
          </div>
        )}
    ...
    >
      <div>
        ...
        <input
          ...
          aria-describedby={
            emailValidation === "idle" ? undefined : emailFeedbackId
          }
      ...
      {error && (
        <div
          role="alert"
          className="rounded-xl border border-red-400/25 bg-red-500/10 px-4 py-3 text-sm text-red-300"
        >
          {error}
        </div>
      )}
