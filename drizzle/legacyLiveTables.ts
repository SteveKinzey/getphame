...
export const authDiagnosticEventsLegacy20260712 = mysqlTable(
  "auth_diagnostic_events_legacy_20260712",
  {
...
export const authHealthChecksLegacy20260712 = mysqlTable(
  "auth_health_checks_legacy_20260712",
  {
    id: int().autoincrement().notNull(),
...
export const contactSourceDeliveries = mysqlTable(
  "contact_source_deliveries",
  {
    id: int().autoincrement().notNull(),
  ...
  table => [
    index("contact_source_delivery_event_uq").on(
      table.sourceId,
      table.eventFingerprint
    ),
    index("contact_source_delivery_user_received_idx").on(
      table.userId,
      table.receivedAt
    ),
...
export const contactSourceRateWindows = mysqlTable(
  "contact_source_rate_windows",
  {
    id: int().autoincrement().notNull(),
    sourceId: int().notNull(),
    userId: int().notNull(),
  ...
  table => [
    index("contact_source_rate_window_uq").on(
      table.sourceId,
...
export const contactSources = mysqlTable(
  "contact_sources",
  {
    id: int().autoincrement().notNull(),
  ...
  table => [
    ...
    index("contact_sources_endpointTokenHash_unique").on(
      table.endpointTokenHash
    ),
...
export const releaseCheckpoints = mysqlTable(
  "release_checkpoints",
  {
    id: int().autoincrement().notNull(),
  ...
  table => [index("release_checkpoints_published_idx").on(table.publishedAt)]
);

export const securityServiceIdentities = mysqlTable(
  "security_service_identities",
  {
    id: varchar({ length: 36 }).notNull(),
    identityClass: mysqlEnum("identity_class", [
      "scheduled_job",
      "integration_webhook",
    ]).notNull(),
  ...
  table => [
    index("security_service_identities_credential_hash_unique").on(
      table.credentialHash
    ),
    index("security_service_identity_owner_idx").on(table.ownerUserId),
    index("security_service_identity_scope_idx").on(
...
export const securitySupportAccessGrants = mysqlTable(
  "security_support_access_grants",
  {
    ...
    consentEvidenceReference: varchar("consent_evidence_reference", {
      length: 500,
    }),
  ...
  table => [
    index("security_support_grants_actor_idx").on(
      table.actorUserId,
      table.expiresAt
    ),
    index("security_support_grants_org_idx").on(
      table.organizationId,
      table.expiresAt
    ),
  ]
...
export const signupRateLimitWindows = mysqlTable(
  "signup_rate_limit_windows",
  {
    id: int().autoincrement().notNull(),
  ...
  table => [
    index("signup_rate_limit_dimension_window_unique").on(
      table.dimension,
      table.dimensionHash,
      table.windowStartedAt
    ),
...
export const signupRiskEvents = mysqlTable(
  "signup_risk_events",
  {
    id: int().autoincrement().notNull(),
    userId: int(),
  ...
  table => [
    index("signup_risk_event_user_occurred_idx").on(
      table.userId,
      table.occurredAt
    ),
    index("signup_risk_event_email_occurred_idx").on(
      table.emailFingerprint,
      table.occurredAt
    ),
    index("signup_risk_event_ip_occurred_idx").on(
      table.ipHash,
      table.occurredAt
    ),
    index("signup_risk_event_device_occurred_idx").on(
      table.deviceHash,
      table.occurredAt
    ),
  ]
...
export const signupRiskProfiles = mysqlTable(
  "signup_risk_profiles",
  {
    id: int().autoincrement().notNull(),
    userId: int().notNull(),
    emailFingerprint: varchar("email_fingerprint", { length: 64 }).notNull(),
    ...
    riskReasonsJson: text("risk_reasons_json").notNull(),
    status: mysqlEnum(["clear", "restricted", "high_risk", "blocked"])
      .default("restricted")
      .notNull(),
  ...
  table => [
    ...
    index("signup_risk_profile_status_score_idx").on(
      table.status,
      table.riskScore
    ),
...
export const signupRiskRateWindows = mysqlTable(
  "signup_risk_rate_windows",
  {
    id: int().autoincrement().notNull(),
  ...
  table => [
    index("signup_risk_rate_window_unique").on(
      table.fingerprint,
      table.action,
      table.windowStartedAt
    ),
...
export const smtpHealthSnapshots = mysqlTable(
  "smtp_health_snapshots",
  {
    id: int().autoincrement().notNull(),
    triggerSource: mysqlEnum("trigger_source", [
      "scheduled",
      "manual",
    ]).notNull(),
...
export const webauthnChallenges = mysqlTable(
  ...
  table => [
    ...
    index("webauthn_challenges_expiry_idx").on(
      table.expiresAt,
      table.consumedAt
    ),
  ]
