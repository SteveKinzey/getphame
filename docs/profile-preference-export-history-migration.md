...
The reviewed non-destructive SQL in `drizzle/manual-pending/20260826_add_profile_preference_export_history.sql` was therefore applied through the managed schema executor. `SHOW CREATE TABLE profile_preference_export_history` confirmed the intended four columns and the `(user_id, exported_at)` index.
