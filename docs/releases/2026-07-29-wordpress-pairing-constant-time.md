...
PR 42 review then identified two additional hardening changes: malformed public claim secrets must use that same generic 404 contract, and the per-IP start limiter must cap and prune tracked clients. A replacement checkpoint and production verification for those review remediations remain pending. Protected-main required check results, final merge, and exact-tree parity evidence also remain pending. Connector pull request 2 remains intentionally unmerged pending its PHP CI gate.
