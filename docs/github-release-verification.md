...
The replacement candidate passed 20 focused tests, strict TypeScript, 151 full-suite files with 893 passing tests and 7 explicit integration skips, an OSV audit across 703 production package versions, client and server production builds, targeted formatting, whitespace validation, and an added-line credential-pattern scan. All review conversations were resolved only after the fixes reached the reviewed head. The pull request reached a clean protected state and merged normally without administrator bypass or force-push; its release branch was deleted afterward.
