...
export const BULK_SENDER_PRESETS: Record<BulkSenderProvider, BulkSenderPreset> =
  {
    sendgrid: {
      ...
      description:
        "Your own SendGrid SMTP relay; never the Get Phame platform account",
      docsUrl:
      ...
      secretHelp:
        "Use an API key from your own SendGrid account with Mail Send permission. Get Phame never shares or uses its platform SendGrid account here.",
    },
    amazon_ses: {
      label: "Amazon SES",
      ...
      secretHelp:
        "Use IAM-derived SES SMTP credentials, not your normal AWS access keys.",
    ...
    mailgun: {
      ...
      secretHelp:
        "Use the SMTP credentials for the sending domain, not the Mailgun API key.",
      defaultRegion: "us",
      regions: [
    ...
    mailersend: {
      ...
      secretHelp:
        "Copy the generated SMTP credentials from your MailerSend domain settings.",
    },
    smtp2go: {
      label: "SMTP2GO",
      ...
      regions: [
        ...
        {
          id: "eu",
          label: "European Union only",
          host: "mail-eu2.smtp2go.com",
        },
    ...
    elastic_email: {
      ...
      secretPlaceholder: "Generated SMTP password",
      secretHelp:
        "Use the generated SMTP password from your Elastic Email account.",
    },
    zoho_zeptomail: {
    ...
    socketlabs: {
      ...
      secretHelp:
        "Find these credentials under Configuration → SMTP Credentials.",
    },
    custom_smtp: {
      label: "Custom SMTP",
      ...
      secretHelp:
        "Enter the exact connection details supplied by your email provider.",
    },
  };

export function getBulkSenderPreset(
  provider: BulkSenderProvider
): BulkSenderPreset {
  return BULK_SENDER_PRESETS[provider];
}

export function resolveBulkSenderHost(
  provider: BulkSenderProvider,
  region?: string | null
): string {
  ...
  return (
    preset.regions.find(option => option.id === selectedRegion)?.host ??
    preset.defaultHost
  );
}
