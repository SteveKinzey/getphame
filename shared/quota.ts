...
export function buildFreeQuotaSummary(input: {
  ...
  const phase: FreeQuotaPhase =
    input.totalSent < FREE_INITIAL_REQUESTS ? "initial" : "rolling";
  const limit =
    phase === "initial" ? FREE_INITIAL_REQUESTS : FREE_ROLLING_REQUESTS;
  const used =
    phase === "initial" ? input.totalSent : Math.min(input.rollingUsed, limit);
  const remaining = Math.max(limit - used, 0);
  const blocked = remaining === 0;
  const windowMs = FREE_ROLLING_WINDOW_DAYS * 24 * 60 * 60 * 1000;
  const nextAvailableAt =
    phase === "rolling" && blocked && input.oldestRollingSentAt
